...
export function useTradioLiveRoom(opts: {
  active: boolean;
  role: Role;
...
const handleGoLive = async () => {
  ...
  if (!isLive) {
    ...
    if (liveSessionId)
      await endLive({ sessionId: liveSessionId, showId: null, peakListeners: live.listenerCount });
    live.leave();
    ...
    <div className="mx-4 mt-3 rounded-2xl border border-pink-500/30 bg-gradient-to-r from-pink-500/10 via-fuchsia-500/5 to-transparent p-3 sm:mx-6 lg:mx-10">
      <div className="flex items-center gap-3">
        ...
        {tunedSessionId ? (
          ...
          <button
            onClick={() => top && setTunedSessionId(top.id)}
