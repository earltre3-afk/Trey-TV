...
const ScreenSwitch: React.FC = () => {
  ...
  switch (screen) {
    ...
    case 'my-list': return <MyListScreen />;
    case 'music': return <MusicScreen />;
    case 'premium': return <PremiumScreen />;
    case 'watch-parties': return <WatchPartiesScreen />;
    case 'source-hub': return <SourceHubScreen />;
...
class TvShellErrorBoundary extends React.Component<{ children: React.ReactNode }, ErrorBoundaryState> {
    ...
    return (
      <div className="dark grid min-h-screen place-items-center bg-[#05050A] p-10 text-white">
        <GlassPanel className="max-w-2xl p-8 text-center">
          ...
          <FocusButton variant="primary" className="mt-6" autoFocus onClick={() => this.setState({ error: null })}>
            Return to Trey TV
          </FocusButton>
        </GlassPanel>
      </div>
