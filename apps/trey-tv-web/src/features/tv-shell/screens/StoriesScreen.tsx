...
import { Heart, Crown, Star, Shield, Handshake, Users, Scale, Footprints, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <TVFrame activeRail="Browse" showTopNav>
      <section className="relative -mx-8 h-[calc(100vh-130px)] overflow-hidden">
        ...
        <div className="relative grid grid-cols-[1fr_400px] gap-10 px-12 pt-8 h-full">
          {/* Left content */}
          <div>
            <div className="text-fuchsia-300 font-bold tracking-[0.3em] text-sm">BUILT DIFFERENT: CHAPTER 4</div>
            <h1 className="text-8xl font-black tracking-tight mt-2 bg-gradient-to-r from-white to-fuchsia-200 bg-clip-text text-transparent">CROSSROADS</h1>
            <p className="mt-5 text-xl text-white/85 max-w-xl leading-relaxed">
              The city is watching your every move.<br />
              One decision can elevate your legacy...<br />or end it.
            </p>

            <div className="mt-10">
              <div className="text-xs tracking-[0.25em] font-bold text-fuchsia-300 mb-1">STORY OBJECTIVE</div>
              <div className="text-base text-white/80 max-w-md">Secure the deal without compromising your crew.</div>
            </div>

            <div className="mt-12">
              <div className="text-xs tracking-[0.25em] font-bold text-fuchsia-300 mb-4">WHAT WILL TREY DO?</div>
              <div className="grid grid-cols-4 gap-3">
                {decisions.map((d, i) => {
                  ...
                  return (
                    ...
                    >
                      <div className="absolute top-2 left-2 w-7 h-7 rounded-md border border-fuchsia-400/60 flex items-center justify-center text-xs font-black text-fuchsia-300">{d.id}</div>
            ...
            <div className="mt-8"><RemoteHints hints={[{label:'Navigate',icon:'⊕'},{label:'Select',icon:'OK'},{label:'Back',icon:'↩'}]} /></div>
          </div>
          ...
          <div className="space-y-4">
            <GlassPanel className="p-5">
              <div className="flex justify-between items-center mb-3">
                <div className="text-xs tracking-widest font-bold text-fuchsia-300">STORY PROGRESS</div>
                  ...
                  return (
                    <div key={n} className="flex-1 flex flex-col items-center">
                      <div className={
                        ...
                        (done ? 'bg-fuchsia-500 border-fuchsia-300' : active ? 'border-fuchsia-400 shadow-[0_0_18px_rgba(255,43,214,0.7)]' : 'border-white/20')
                      }>
            ...
            <GlassPanel className="p-5">
              <div className="text-xs tracking-widest font-bold text-fuchsia-300 mb-4">RELATIONSHIPS &amp; STATS</div>
                ...
                {meters.map((m) => (
                  <div key={m.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 rounded-full bg-white/10 overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-fuchsia-400 to-purple-500" style={{ width: `${m.value}%` }} />
                    </div>
        ...
        {/* Side arrows */}
        <button className="absolute left-2 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 border border-white/20 flex items-center justify-center outline-none focus:border-fuchsia-400"><ChevronLeft /></button>
        <button className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 border border-white/20 flex items-center justify-center outline-none focus:border-fuchsia-400"><ChevronRight /></button>
      </section>
    </TVFrame>
  );
};
