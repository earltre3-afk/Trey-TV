  ...
  return (
    <TVFrame activeRail="Search">
      ...
      <p className="text-white/60 mb-6">
        Find Trey TV originals, games, music videos, and creator channels.
      </p>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {["Kingmaker", "Spades", "Late Night Gaming", "After Hours"].map((term) => (
          <FocusCard
            key={term}
            className="p-5 h-24"
            onClick={() => (term === "Spades" ? navigate("spades") : navigate("detail"))}
          >
      ...
      <ContentRow
        ...
        items={[...featured.slice(0, 3), ...interactiveStories.slice(0, 2)]}
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {categories.map((category, index) => (
          <FocusCard
            key={category}
            className="h-32 p-5"
            glow={index % 3 === 0 ? "gold" : "magenta"}
            onClick={() => navigate(index === 4 ? "games" : "detail")}
          >
...
export const MusicScreen: React.FC = () => {
  ...
  return (
    <TVFrame activeRail="Music">
      <section className="relative mb-8 h-[320px] overflow-hidden rounded-3xl border border-fuchsia-500/20">
        ...
        <div className="relative flex h-full max-w-2xl flex-col justify-center px-10">
          ...
          <p className="mt-3 text-white/75">
            Music videos, sessions, late-night performances, and creator soundtracks.
          </p>
          <div className="mt-5 flex gap-3">
            ...
            <FocusButton variant="ghost" icon={<Music2 className="h-5 w-5" />}>
              Music Channels
            </FocusButton>
          </div>
        </div>
      ...
      <ContentRow
        title="Late Night Sound"
        items={featured.filter(
          (item) => item.title.includes("After") || item.title.includes("Life"),
        )}
        onSelect={() => navigate("detail")}
      />
    </TVFrame>
      ...
      <div className="flex items-start gap-5">
        ...
        <div>
          ...
          <p className="mt-3 max-w-3xl text-white/70">
            Membership features, early access, and premium creator experiences will appear here. No
            payment flow is enabled in this debug build.
          </p>
        </div>
      </div>
  ...
  <TVFrame activeRail="Parties">
    <GlassPanel className="p-8">
      ...
      <p className="mt-3 max-w-2xl text-white/70">
        Coming Soon. Group watch, synced chat, and creator-hosted premieres will live here.
      </p>
      <FocusButton variant="primary" className="mt-6" autoFocus>
        Create Party
      </FocusButton>
    </GlassPanel>
  </TVFrame>
...
export const SourceHubScreen: React.FC = () => (
  <TVFrame activeRail="Sources">
    <GlassPanel className="p-8">
      ...
      <p className="mt-3 max-w-2xl text-white/70">
        Coming Soon. Connected content sources and import status will be shown here when available.
      </p>
      <FocusButton variant="ghost" className="mt-6" autoFocus>
        Check Sources
      </FocusButton>
    </GlassPanel>
  </TVFrame>
