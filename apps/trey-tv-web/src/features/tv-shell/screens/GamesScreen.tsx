  ...
  return (
    <TVFrame activeRail="Games">
      ...
      <section className="relative h-[400px] w-full overflow-hidden rounded-3xl border border-fuchsia-500/20 mb-6 shadow-[0_0_60px_rgba(168,85,247,0.15)]">
        <img
          src={TV_ARTWORK.lateNightGamingGuide}
          alt=""
        ...
        <div className="relative px-12 py-10 max-w-2xl">
          ...
          <h1 className="text-7xl font-black leading-none mt-2">
            ...
            <span className="bg-gradient-to-r from-fuchsia-400 to-purple-500 bg-clip-text text-transparent italic">
              LOUNGE
            </span>
          </h1>
          <p className="mt-4 text-white/80 text-lg">
            ...
            <br />
            Welcome to the world of Trey Trizzy Gaming.
          </p>
          <div className="mt-6 flex gap-3">
            <FocusButton variant="primary" icon={<Gamepad2 className="w-5 h-5" />} autoFocus>
              Browse Games
            </FocusButton>
            <FocusButton variant="ghost" icon={<Zap className="w-5 h-5 text-fuchsia-300" />}>
              Quick Match
            </FocusButton>
          </div>
        </div>
        <span className="absolute top-6 right-6 px-3 py-1.5 rounded-md bg-amber-400 text-black text-xs font-black">
          4K ULTRA HD
        </span>
      </section>
      ...
      {selectedGame &&
        selectedGame.id !== "spades" &&
        selectedGame.id !== "interactive-stories" && (
          <section className="mb-8 rounded-2xl border border-amber-400/25 bg-amber-400/10 p-5">
            <div className="text-xs font-black tracking-[0.28em] text-amber-300">
              {selectedGame.status}
            </div>
            <div className="mt-1 text-2xl font-black">{selectedGame.title}</div>
            <p className="mt-1 text-sm text-white/70">
              This game tile is wired for remote selection. The full game flow is not enabled in
              this debug build yet.
            </p>
          </section>
        )}
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            <button
              key={it.t}
              className="flex items-center gap-3 px-5 py-5 rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/[0.02] outline-none focus:border-fuchsia-400 focus:shadow-[0_0_28px_rgba(255,43,214,0.55)] focus:scale-[1.03] transition-all"
            >
