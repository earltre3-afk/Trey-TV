...
export const GuideScreen: React.FC = () => {
  ...
  const selectedProgramIndex = Math.min(
    selected.prog,
    Math.max((channel?.programs.length ?? 1) - 1, 0),
  );
  ...
  const previewTags =
    program?.genres && program.genres.length > 0
      ? program.genres.slice(0, 3)
      : ["Gaming", "Live", "Multiplayer"];
  const previewImage =
    selectCardArtwork({ title: program?.title, image: program?.imageUrl || channel?.logoUrl }) ||
    IMG(2);
    ...
    <TVFrame activeRail="Live TV">
      <div className="flex items-end justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <FocusButton variant="ghost" icon={<Filter className="w-4 h-4" />}>
            Filter
          </FocusButton>
          <FocusButton variant="ghost" icon={<Calendar className="w-4 h-4" />}>
            Today
          </FocusButton>
        </div>
      </div>
      ...
      {error && (
        <div className="mb-3 rounded-2xl border border-amber-400/25 bg-amber-400/10 px-4 py-2 text-xs text-amber-200">
          Free TV provider is not available in this shell yet, so Trey TV is showing fallback guide
          data.
        </div>
      )}
      ...
      <div className="grid grid-cols-[1fr_360px] gap-4">
        <GlassPanel className="p-4 overflow-hidden">
          ...
          <div className="grid grid-cols-[80px_repeat(7,1fr)] items-center text-xs text-white/60 pb-3 border-b border-white/10 relative">
            <div>
              Today
              <br />
              <span className="text-white/40">{todayLabel}</span>
            </div>
            {guideTimes.map((t, i) => (
              <div
                key={t}
                className={"text-center font-semibold " + (i === 2 ? "text-fuchsia-300" : "")}
              >
                {i === 2 && (
                  <div className="inline-block px-2 py-0.5 rounded-md bg-fuchsia-500 text-white">
                    9:17 PM
                  </div>
                )}
                {i !== 2 && t}
          ...
          <div className="relative">
            ...
            {channels.map((ch, ci) => (
              <div
                key={ch.id}
                className="grid grid-cols-[80px_repeat(3,1fr)] gap-2 py-3 border-b border-white/5 items-center"
              >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {p.live && (
                          <span className="ml-auto text-[9px] font-black bg-fuchsia-500 px-1.5 rounded">
                            LIVE
                          </span>
                        )}
                      </div>
        ...
        <GlassPanel className="overflow-hidden flex flex-col">
          <div className="relative h-[200px]">
            ...
            <span className="absolute top-3 left-3 text-[10px] font-black bg-fuchsia-500 px-2 py-0.5 rounded">
              LIVE
            </span>
          </div>
          <div className="p-5 flex flex-col gap-3 flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              {previewTags.map((t) => (
                <span
                  key={t}
                  className="px-2 py-1 rounded-md text-[11px] font-semibold bg-white/5 border border-white/10"
                >
                  {t}
                </span>
              ))}
            ...
            <p className="text-sm text-white/70 leading-relaxed">
              {program?.description ||
                "Join Trizzy and the squad for ranked matches, chill vibes, and fan interactions."}
            </p>
