...
import {
  featured, continueWatching, newEpisodes, creators, musicVideos, trendingGames, heroFeature,
} from '../mockData';
  ...
  return (
    <TVFrame activeRail="Home">
      ...
      <section className="relative h-[460px] w-full overflow-hidden rounded-3xl border border-fuchsia-500/20 mb-8 shadow-[0_0_80px_rgba(168,85,247,0.15)]">
        ...
        {heroArtwork && <img src={heroArtwork} alt="" className="absolute inset-0 w-full h-full object-cover" />}
        <div className="absolute inset-0 bg-gradient-to-r from-black via-black/70 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />

        <div className="relative h-full flex flex-col justify-center px-12 max-w-3xl">
          <div className="text-amber-300 font-bold tracking-[0.3em] text-sm mb-3">{heroFeature.meta}</div>
          ...
          <div className="mt-6 flex items-center gap-3">
            <FocusButton variant="gold" icon={<Play className="w-5 h-5 fill-black" />} onClick={() => navigate('player')} autoFocus>
              Watch Now
            </FocusButton>
      ...
      <GameRow title="Trending Games" items={trendingGames} onSelect={(g) => g.title === 'Spades' ? navigate('spades') : navigate('games')} />
    </TVFrame>
  );
};
