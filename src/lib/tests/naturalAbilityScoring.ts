...
const ALL_ABILITIES: NaturalAbility[] = [
  ...
  'Seer', 'Shapeshifter', 'Healer', 'Dreamer', 'Prophet', 'Ungifted',
  'Manifestor', 'Creative',
];

function emptyScores(): Record<NaturalAbility, number> {
  return ALL_ABILITIES.reduce((acc, a) => {
    acc[a] = 0;
    return acc;
...
export function classifyCustomAnswer(
  text: string
): { primary: NaturalAbility; secondary: NaturalAbility; confidence: number } {
  const t = text.toLowerCase();

  const buckets: { ability: NaturalAbility; keywords: string[] }[] = [
    { ability: 'Reaper', keywords: ['block', 'leav', 'end', 'cut off', 'cut them', 'remove', 'delete', 'walk away', 'done', 'over', 'no more', 'goodbye'] },
    { ability: 'Healer', keywords: ['comfort', 'listen', 'hold space', 'support', 'be there', 'soft', 'gentle', 'safe', 'forgive', 'peace'] },
    { ability: 'Empath', keywords: ['feel', 'feelings', 'energy', 'sense', 'emotional', 'absorb', 'heart', 'their pain'] },
    { ability: 'Diviner', keywords: ['pattern', 'sign', 'notice', 'read between', 'hidden', 'meaning', 'clue', 'watch', 'observe', 'clock'] },
    { ability: 'Seer', keywords: ['future', 'predict', 'next step', 'foresight', 'coming', 'ahead', 'prepare', 'forecast'] },
    { ability: 'Charmer', keywords: ['charm', 'joke', 'laugh', 'smile', 'flirt', 'win the room', 'social', 'work the room', 'crowd'] },
    { ability: 'Manifestor', keywords: ['plan', 'action', 'make it happen', 'build', 'execute', 'do it', 'move', 'push through', 'go for it'] },
    { ability: 'Creative', keywords: ['art', 'song', 'story', 'visual', 'idea', 'concept', 'write', 'design', 'create', 'paint', 'make something'] },
    { ability: 'Alchemist', keywords: ['transform', 'turn pain', 'power', 'comeback', 'rise', 'flip it', 'use it', 'learn from'] },
    { ability: 'Herbalist', keywords: ['food', 'rest', 'body', 'nature', 'clean', 'ground', 'tea', 'water', 'sleep', 'walk', 'breath'] },
    { ability: 'Shapeshifter', keywords: ['adapt', 'switch', 'become', 'shift', 'match', 'blend', 'change my', 'pivot', 'mode'] },
    { ability: 'Prophet', keywords: ['truth', 'call it out', 'say it', 'tell them', 'warn', 'direct', 'real talk', 'honest'] },
    { ability: 'Ungifted', keywords: ['silent', 'silence', 'private', 'unread', 'no label', 'nothing', 'i dont know', "i don't know", 'skip', 'mind my'] },
    { ability: 'Dreamer', keywords: ['imagine', 'fantasy', 'dream', 'inner world', 'vision', 'wonder', 'what if'] },
  ];
...
export function calculateResult(
  ...
  const ranked = ALL_ABILITIES
    .map((a) => ({ ability: a, score: scores[a] }))
