...
const validatePreview = (input: PreviewInput): PreviewInput => ({
  ...
  birthLatitude: typeof input?.birthLatitude === "number" && Number.isFinite(input.birthLatitude) ? input.birthLatitude : undefined,
  birthLongitude: typeof input?.birthLongitude === "number" && Number.isFinite(input.birthLongitude) ? input.birthLongitude : undefined,
  birthTimePrecision: (["unknown", "morning", "afternoon", "evening", "night", "exact"] as const).includes(input?.birthTimePrecision as BirthTimePrecision)
...
async function ensureGroupMembership(userId: string, profile: any, identity: NonNullable<ReturnType<typeof calculateZodiacIdentity>>) {
  ...
  for (const entry of entries) {
    ...
    await (service as any)
      ...
      .upsert({ user_id: userId, group_thread_id: data.id, source, left_at: null }, { onConflict: "user_id,group_thread_id" });
  }
}
...
export const confirmZodiacIdentity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: profile } = await (supabase as any)
      .from("profiles")
      .select("zodiac_locked_at, date_of_birth, location, favorite_categories, favorite_moods, fan_type")
      .eq("id", user.id)
      .maybeSingle();
    ...
    const { data: saved, error } = await (supabase as any)
      ...
      .update(patch)
      .eq("id", user.id)
      .select("id, date_of_birth, location, birth_location_label, favorite_categories, favorite_moods, fan_type")
      .single();
    if (error) throw new Error(error.message);
...
export const getDailyZodiacReading = createServerFn({ method: "POST" })
  .inputValidator((input: { zodiacSign?: string; cuspLabel?: string | null; isCusp?: boolean }) => ({
  ...
  .handler(async ({ data }) => {
    ...
    const { data: saved, error } = await (service as any)
      ...
      .select("*")
      .single();
    if (error) return { ...fallback, zodiac_sign: sign, cusp_label: data.cuspLabel, reading_date: today };
    return saved;
  });
