...
export const TIER_META: Record<Tier, { label: string; price: number; weight: number; color: string; ring: string }> = {
...
function seed(): Submission[] {
  const now = Date.now();
  return [
    { id: "demo-1", userUid: "demo", userName: "KaiWave", userEmail: "kai@trey.tv", title: "Midnight Bloom", artist: "KaiWave", genre: "R&B", notes: "Late-night cruise vibe", source: "drive", driveLink: "https://drive.google.com/file/d/x", tier: "turbo", paymentStatus: "verified", status: "now_playing", createdAt: now - 60000, aiFirstImpression: { vibe: "Velvety after-hours R&B", strengths: ["Atmosphere", "Vocal texture"], hook: "Listen for the chorus drop at 1:12.", hypeScore: 8, predictedMood: "Reflective" } },
    { id: "demo-2", userUid: "demo", userName: "808 Saint", userEmail: "saint@trey.tv", title: "Pressure Cooker", artist: "808 Saint", genre: "Hip-Hop", notes: "Heavy 808s", source: "file", fileName: "pressure.mp3", tier: "super", paymentStatus: "verified", status: "queued", createdAt: now - 30000 },
    { id: "demo-3", userUid: "demo", userName: "Lyric", userEmail: "lyric@trey.tv", title: "Glass House", artist: "Lyric", genre: "Pop", notes: "Anthemic", source: "file", fileName: "glass.wav", tier: "skip", paymentStatus: "verified", status: "queued", createdAt: now - 20000 },
    { id: "demo-4", userUid: "demo", userName: "Verse", userEmail: "verse@trey.tv", title: "First Light", artist: "Verse", genre: "Indie", notes: "", source: "file", fileName: "first.mp3", tier: "regular", paymentStatus: "none", status: "queued", createdAt: now - 10000 },
  ];
}
...
export function MusicReviewProvider({ children }: { children: ReactNode }) {
  const [submissions, setSubmissions] = useState<Submission[]>(() => (typeof window !== "undefined" ? load() : []));
  ...
  const value = useMemo<Ctx>(() => ({
    submissions,
    add: (s) => {
      const sub: Submission = { ...s, id: crypto.randomUUID(), createdAt: Date.now(), status: "queued" };
...
const VIBES = [
  "Velvety after-hours R&B", "Cinematic neon-soul", "Anthemic stadium-pop", "Late-night freeway hip-hop",
  "Dreamy alt-bedroom-pop", "High-octane trap energy", "Warm sunset indie", "Glittery hyperpop euphoria",
  "Smoky lounge jazz-rap", "Crisp Afro-fusion bounce",
];
const STRENGTHS = ["Atmosphere", "Hook potential", "Vocal texture", "Rhythmic pocket", "Production polish", "Lyric imagery", "Dynamic range", "Genre crossover appeal"];
...
export function generateAIInsight(input: { title: string; artist: string; genre: string; notes: string }): AIInsight {
  ...
  const strengths = [pick(STRENGTHS, 1), pick(STRENGTHS, 7), pick(STRENGTHS, 13)].filter((s, i, a) => a.indexOf(s) === i);
