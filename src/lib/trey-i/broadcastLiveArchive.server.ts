...
/**
 * Publish a clip with publishing gate validation (server-side enforcement)
 * This MUST be called before any clip can become public
 * ENFORCES: ownership, rendering status, audio existence, review status, rights, consent, visibility
 *
 * SECURITY:
 * - Client passes access token (from verified Supabase session)
 * - Server verifies token using supabaseAdmin.auth.getUser(accessToken)
 * - Server derives verifiedUserId from token
 * - Server compares clip ownership to verifiedUserId
...
export const publishHighlightClipWithGatesServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: input }) => {
    ...
    // - Check rendering status (Gate 2)
    // - Validate audio exists (Gate 3)
    // - Verify review approval (Gate 4)
    // - Check rights status (Gate 5)
    // - Validate consent (Gate 6)
