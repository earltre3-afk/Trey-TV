...
import { Play, Grid3x3, Heart, BookmarkCheck, Sparkles, CheckCircle2, Tag } from "lucide-react";
...
export function NormalUserProfileModules({ profile, isOwner, viewerRole, topThree = [] }: Props) {
  ...
  return (
    <div className="space-y-5 min-w-0">
        ...
        <div className="rounded-3xl liquid-glass border border-primary/25 p-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-muted-foreground">
              Add a bio to help others discover you.
            </div>
          </div>
          <Link
      ...
      {/* ── Tab content ──────────────────────────────────────── */}
      {tab === "Posts" && <PostsGrid isOwner={isOwner} isGuest={isGuest} uid={profile.uid} />}
      {tab === "Liked" &&
        (isGuest ? (
        ...
        {items.map((p, i) => (
          ...
          >
            ...
            <span className="absolute bottom-1.5 left-1.5 text-[10px] flex items-center gap-1 text-white/90">
              ...
              {(p as any).duration ?? (p as any).viewers ?? "8.7K"}
            </span>
...
function AboutTab({ profile, isOwner }: Pick<ProfileContext, "profile"> & { isOwner: boolean }) {
  const pillClass = "rounded-xl bg-white/5 ring-1 ring-white/10 p-2.5";
  return (
    <div
      className={`rounded-3xl ${isOwner ? "owner-neon owner-glass" : "glass neon-border"} p-5 space-y-4`}
    >
      <div>
        <div className="text-[10px] tracking-[0.3em] text-primary mb-1">BIO</div>
        <p className="text-sm">{profile.bio || "No bio set."}</p>
        {profile.tagline && (
          <p className="mt-2 text-sm text-primary font-semibold">{profile.tagline}</p>
        )}
      </div>
      <div className="grid grid-cols-2 gap-2">
        ...
        <InfoPill
          label="Visibility"
          value={profile.profileVisibility?.replace("_", " ") ?? "public"}
        />
      ...
      {(profile.favoriteGenres ||
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {profile.favoriteCreators && profile.favoriteCreators !== "-" && (
            <InfoPill label="Creators" value={profile.favoriteCreators} />
          )}
          {profile.socialInstagram && profile.socialInstagram !== "-" && (
            <InfoPill label="Instagram" value={profile.socialInstagram} />
          )}
          {profile.socialTikTok && profile.socialTikTok !== "-" && (
            <InfoPill label="TikTok" value={profile.socialTikTok} />
          )}
          {profile.socialYouTube && profile.socialYouTube !== "-" && (
