  ...
  return (
    <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 z-30 w-[260px] xl:w-[280px] flex-col glass-strong border-r border-white/10 overflow-y-auto no-scrollbar">
      <Link to="/" className="relative flex items-center justify-center pt-6 pb-4 group">
        <span
          aria-hidden
          className="absolute inset-x-6 top-2 h-24 rounded-full blur-3xl opacity-70 bg-[conic-gradient(from_0deg,oklch(0.82_0.16_85_/_0.55),oklch(0.7_0.25_340_/_0.45),oklch(0.65_0.22_300_/_0.5),oklch(0.82_0.15_215_/_0.45),oklch(0.82_0.16_85_/_0.55))] animate-conic-spin"
        />
        <Logo className="relative h-24 transition-transform duration-500 group-hover:scale-105" />
      </Link>

      <div className="px-3 mb-2">
        <CreatorGoldNavButton className="w-full justify-center" />
      </div>
      ...
      <div className="mt-auto p-3 space-y-3">
        <div className="p-3 rounded-2xl border border-[oklch(0.65_0.22_300_/_0.4)] bg-[linear-gradient(135deg,oklch(0.25_0.1_300_/_0.6),oklch(0.18_0.05_270_/_0.6))] glow-purple">
          ...
          <div className="text-[11px] text-muted-foreground mt-1">
            Unlock pro analytics & monetization.
          </div>
          <Link
            to="/premium"
            className="mt-2 w-full inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-xs font-semibold border border-[oklch(0.7_0.25_340)] text-[oklch(0.7_0.25_340)] hover:bg-[oklch(0.7_0.25_340_/_0.1)]"
          >
            Upgrade
          </Link>
        </div>
        ...
        <Link
          ...
          className="flex items-center gap-3 p-2 rounded-2xl glass neon-border hover-lift"
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold flex items-center gap-1 truncate">
              {profileName}
              <VerifiedBadge
                kind={authIsAdmin ? "admin" : authIsCreator ? "creator" : "user"}
                className="!size-3.5"
              />
            </div>
...
function Section({
  ...
  return (
    <div className="px-3">
      <div className="px-3 pb-1 text-[10px] tracking-[0.22em] text-muted-foreground">
        {label.toUpperCase()}
      </div>
      <div className="space-y-0.5">
        {items.map((i, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`size-8 rounded-lg grid place-items-center transition ${active ? "bg-primary/15 text-primary" : "bg-white/5 text-muted-foreground group-hover:text-foreground"}`}
              >
                <i.icon className="size-4" />
              </div>
              <span
                className={`text-sm flex-1 ${active ? "font-semibold text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}
              >
                {i.label}
              </span>
              {i.badge ? (
              ...
              {active && (
                <span className="absolute right-2 size-1.5 rounded-full bg-primary animate-glow-pulse" />
              )}
            </Link>
          );
        })}
