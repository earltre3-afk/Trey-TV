...
const SignalPrivacyControls: React.FC<Props> = ({ value, onChange }) => {
  const options: {
    id: PrivacyMode;
    icon: LucideIcon;
    title: string;
  ...
  const tintMap: Record<string, { border: string; glow: string; text: string; bg: string }> = {
    fuchsia: {
      ...
      glow: "shadow-[0_0_25px_-5px_rgba(217,70,239,0.6)]",
      text: "text-fuchsia-300",
      bg: "bg-fuchsia-500/15",
    },
    cyan: {
      border: "border-cyan-400",
      glow: "shadow-[0_0_25px_-5px_rgba(34,211,238,0.6)]",
      text: "text-cyan-300",
      bg: "bg-cyan-500/15",
    },
    amber: {
      border: "border-amber-400",
      glow: "shadow-[0_0_25px_-5px_rgba(251,191,36,0.6)]",
      text: "text-amber-300",
      bg: "bg-amber-500/15",
    },
  };
        ...
        {options.map((o) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`text-[11px] font-bold tracking-wide leading-tight ${active ? t.text : "text-slate-300"}`}
              >
                {o.title}
              </div>
