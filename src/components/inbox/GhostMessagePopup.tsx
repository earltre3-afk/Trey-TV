...
const GHOST_TIMERS = [
  {
    label: "30s",
    seconds: 30,
    description: "Vanishes in 30 seconds",
    color: "oklch(0.82 0.15 215)",
  },
  {
    label: "5 min",
    seconds: 300,
    description: "Fades after 5 minutes",
...
export function GhostMessagePopup({ onSelect, onClose }: GhostMessagePopupProps) {
  return (
    <div className="ghost-popup-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="ghost-popup-panel animate-ghost-rise">
        ...
        <div
          ...
          style={{
            background: "radial-gradient(circle, oklch(0.7 0.25 340 / 0.5), transparent 70%)",
            filter: "blur(24px)",
          }}
        />
        ...
        <div className="relative flex items-center gap-3 mb-5">
          <div className="ghost-icon-ring">
            <Ghost
              className="size-5 text-[oklch(0.82_0.16_85)]"
              style={{ filter: "drop-shadow(0 0 8px oklch(0.82 0.16 85 / 0.9))" }}
            />
          </div>
          <div className="flex-1">
            <div className="text-xs tracking-[0.3em] text-[oklch(0.82_0.16_85)] font-semibold uppercase">
              Ghost Message
            </div>
            ...
            >
              ...
              <div className="relative flex items-center gap-3 p-3.5">
                <div className="ghost-timer-icon-wrap">
                  <Timer
                    className="size-4"
                    style={{ color: opt.color, filter: `drop-shadow(0 0 6px ${opt.color})` }}
                  />
                </div>
                <div className="flex-1 text-left">
                  <div
                    className="text-sm font-bold"
                    style={{ color: opt.color, textShadow: `0 0 12px ${opt.color}` }}
                  >
                    {opt.label}
                  </div>
