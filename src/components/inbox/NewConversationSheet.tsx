...
const QUICK_PROMPTS = [
  { icon: Sparkles, text: "Loved your latest drop — open to a collab idea?", tone: "gold" as const },
  { icon: Users, text: "Quick intro: I think you and I should connect on this.", tone: "violet" as const },
      ...
      >
        ...
        <div className="relative flex items-center gap-3 p-4 pb-3 sm:gap-4 sm:p-5 sm:pb-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[9px] font-black uppercase tracking-[0.3em] text-primary sm:text-[10px]">New Conversation</div>
            ...
            {!picked && <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground sm:text-xs">Find creators and conversations that move you forward.</p>}
          </div>
          <button onClick={onClose} className="grid size-9 shrink-0 place-items-center rounded-full border border-white/10 bg-white/[0.055] text-foreground hover:bg-white/10 sm:size-10" aria-label="Close">
            <X className="size-4" />
          </button>
        </div>

        {!picked ? (
          <ConnectStep inputRef={inputRef} query={query} setQuery={setQuery} results={results} startConvo={startConvo} />
        ) : (
          <ComposeStep picked={picked} draft={draft} setDraft={setDraft} setPicked={setPicked} confirm={confirm} />
        )}
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="relative px-4 sm:px-5">
        <div className="flex h-11 items-center gap-2 rounded-2xl border border-primary/30 bg-white/[0.05] px-3 shadow-[inset_0_1px_0_rgba(255,255,255,.06)] focus-within:border-[oklch(0.7_0.25_340_/_0.55)] sm:h-12">
          ...
          />
          <button onClick={() => setQuery("")} className="grid size-8 place-items-center rounded-full border border-[oklch(0.7_0.25_340_/_0.3)] bg-[oklch(0.7_0.25_340_/_0.1)] text-[oklch(0.85_0.2_340)]" aria-label="AI search">
        ...
        <div className="mt-2.5 flex gap-1.5 overflow-x-auto pb-1 no-scrollbar sm:mt-3 sm:gap-2">
          {["AI Match", "Online", "Collab Ready", "Same Vibes", "Verified"].map((label, i) => (
            <button key={label} className={`h-7 shrink-0 rounded-full border px-3 text-[11px] font-bold sm:h-8 sm:text-xs ${i === 0 ? "border-[oklch(0.7_0.25_340_/_0.6)] bg-[oklch(0.7_0.25_340_/_0.14)] text-white shadow-[0_0_18px_-10px_oklch(0.7_0.25_340)]" : "border-white/10 bg-white/[0.04] text-muted-foreground"}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="relative mt-3 flex min-h-0 flex-1 flex-col px-4 pb-4 sm:mt-4 sm:px-5 sm:pb-5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-white/10 bg-white/[0.03] no-scrollbar sm:rounded-[22px]">
          {results.length === 0 ? (
            ...
            results.map((p, i) => <CreatorRow key={p.id} peer={p} index={i} onClick={() => startConvo(p)} />)
          )}
          {/* Smart match footer */}
          <div className="flex items-center gap-2.5 border-t border-primary/20 bg-primary/[0.06] px-3 py-2.5 sm:gap-3 sm:px-4 sm:py-3">
            ...
            <button className="h-8 rounded-xl border border-primary/30 bg-primary/10 px-2.5 text-[11px] font-black text-primary sm:h-9 sm:px-3">View</button>
          </div>
        </div>
      </div>
    </>
...
function CreatorRow({ peer, index, onClick }: { peer: Peer; index: number; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="flex w-full items-center gap-2.5 border-b border-white/[0.07] px-3 py-2.5 text-left transition hover:bg-white/[0.04] sm:gap-3 sm:px-4 sm:py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-[10px] text-muted-foreground">@{peer.handle} · {meta.role} · {meta.followers}</div>
      </div>
      <span className={`hidden rounded-full border px-2 py-0.5 text-[9px] font-bold sm:inline-flex ${meta.tone === "blue" ? "border-[oklch(0.82_0.15_215_/_0.35)] bg-[oklch(0.82_0.15_215_/_0.08)] text-[oklch(0.82_0.15_215)]" : "border-[oklch(0.7_0.25_340_/_0.35)] bg-[oklch(0.7_0.25_340_/_0.08)] text-[oklch(0.85_0.2_340)]"}`}>
        {meta.tag}
      </span>
    ...
    <div className="relative flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 pb-4 no-scrollbar sm:gap-5 sm:px-5 sm:pb-5">
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-primary/35 bg-[linear-gradient(120deg,oklch(0.7_0.25_340_/_0.12),rgba(255,255,255,.04),oklch(0.82_0.16_85_/_0.08))] p-3 shadow-[0_0_28px_-20px_oklch(0.82_0.16_85)]">
        <div className="relative size-11 shrink-0 rounded-full conic-ring sm:size-14">
          <img src={picked.avatar} className="size-11 rounded-full object-cover sm:size-14" alt="" />
        </div>
        <div className="min-w-0 flex-1">
        ...
        <button onClick={() => setPicked(null)} className="h-8 shrink-0 rounded-xl border border-primary/30 bg-primary/10 px-2.5 text-[11px] font-black text-primary sm:h-9 sm:px-3">Change</button>
      </div>
        ...
        <div className="grid gap-2">
          {QUICK_PROMPTS.map((prompt) => {
            ...
            return (
              <button
                key={prompt.text}
                onClick={() => { haptic("light"); setDraft(prompt.text); }}
              ...
              >
                <span className={`grid size-9 shrink-0 place-items-center rounded-full border ${prompt.tone === "blue" ? "border-[oklch(0.82_0.15_215_/_0.4)] text-[oklch(0.82_0.15_215)]" : prompt.tone === "violet" ? "border-[oklch(0.65_0.22_300_/_0.4)] text-[oklch(0.78_0.22_300)]" : "border-primary/40 text-primary"}`}>
                  <Icon className="size-4" />
                </span>
                <span className="flex-1 text-xs font-medium leading-relaxed sm:text-sm">{prompt.text}</span>
      ...
      {/* Message box */}
      <div>
        <div className="mb-2 text-[10px] font-black uppercase tracking-[0.28em] text-muted-foreground">First message (optional)</div>
        <div className="rounded-2xl border border-[oklch(0.7_0.25_340_/_0.35)] bg-white/[0.04] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,.06)]">
          ...
          <div className="mt-2 flex items-center gap-2">
            {[ImageIcon, Mic, Calendar, Wand2].map((Icon, i) => (
              <button key={i} className="grid size-8 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-muted-foreground hover:text-primary sm:size-9" aria-label="Composer tool">
                <Icon className="size-3.5 sm:size-4" />
              </button>
            ))}
            <button onClick={confirm} className="ml-auto inline-flex h-10 items-center gap-2 rounded-xl border border-primary/45 bg-[linear-gradient(135deg,rgba(255,200,87,.14),rgba(255,176,0,.06))] px-4 text-sm font-black text-foreground shadow-[0_0_22px_-10px_oklch(0.82_0.16_85)] sm:h-11">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar">
        {[
          ...
          <button key={label} className={`inline-flex h-8 shrink-0 items-center gap-1.5 rounded-full border px-3 text-[11px] font-bold sm:h-9 ${tone === "blue" ? "border-[oklch(0.82_0.15_215_/_0.35)] text-[oklch(0.82_0.15_215)]" : tone === "gold" ? "border-primary/35 text-primary" : "border-[oklch(0.7_0.25_340_/_0.35)] text-[oklch(0.85_0.2_340)]"}`}>
