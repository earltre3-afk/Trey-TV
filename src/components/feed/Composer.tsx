  ...
  return (
    <div className="rounded-3xl p-4 glass border border-primary/20 shadow-[0_0_30px_-10px_oklch(0.82_0.16_85_/_0.4)] relative overflow-hidden">
      ...
      <div className="relative mt-3 flex items-center gap-2 flex-wrap">
        <button className="px-3 py-2 rounded-xl glass text-xs flex items-center gap-1.5 hover:bg-white/5">
        ...
        <button className="ml-auto px-4 py-2 rounded-xl text-sm font-semibold border border-primary text-primary glow-gold hover:bg-primary/10">
          Post
        </button>
      </div>
      ...
      <div className="relative mt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((t) => (
            <button key={t.label} className={`px-3 py-1 rounded-full text-xs border bg-transparent ${ringMap[t.color]} hover:bg-white/5`}>
              {t.label}
            </button>
          ))}
          <button className="size-7 grid place-items-center rounded-full border border-white/15 text-muted-foreground hover:bg-white/5">
