  ...
  return (
    <article className="rounded-3xl glass border border-white/8 overflow-hidden shadow-[0_10px_40px_-15px_rgba(0,0,0,0.7)]">
      <div className="flex items-center gap-3 p-4">
        ...
        <button onClick={() => toast("Post options")} className="size-8 grid place-items-center text-muted-foreground hover:text-foreground" aria-label="more">
      ...
      <div className="px-3">
        <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-[0_0_28px_-12px_oklch(0.65_0.22_300_/_0.6)]">
          ...
          <button onClick={() => setPlaying((p) => !p)} className="absolute inset-0 grid place-items-center bg-black/10 hover:bg-black/20 transition" aria-label="Play">
      ...
      <div className="flex items-center justify-between px-5 py-3 text-sm">
        <button onClick={() => setLiked((v) => !v)} className={`flex items-center gap-1.5 transition ${liked ? "text-[oklch(0.65_0.24_15)]" : "text-muted-foreground hover:text-foreground"}`}>
          <Heart className={`size-5 ${liked ? "fill-current" : ""}`} /> {fmt(likeCount)}
        </button>
        <button onClick={() => toast("Comments coming soon")} className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground">
          <MessageCircle className="size-5" /> {post.comments}
        </button>
        <button onClick={() => { setReshared((v) => !v); toast(reshared ? "Unshared" : "Reshared to your channel"); }} className={`flex items-center gap-1.5 transition ${reshared ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}>
          <Repeat2 className="size-5" /> {reshareCount}
        </button>
        <button onClick={() => setSaved((v) => !v)} className={`flex items-center gap-1.5 transition ${saved ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}>
          <Bookmark className={`size-5 ${saved ? "fill-current" : ""}`} /> {fmt(saveCount)}
        </button>
        <button
          onClick={async () => {
            try {
