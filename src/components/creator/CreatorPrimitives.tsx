...
export function CreatorMetricCard({
  label, value, delta, icon: Icon, tone = "gold", trend = "up", sub,
}: {
  label: string; value: string; delta?: string; icon: LucideIcon;
  tone?: Tone; trend?: "up" | "down" | "flat"; sub?: string;
}) {
  const TrendIcon = trend === "down" ? TrendingDown : TrendingUp;
  return (
    <div className={`relative rounded-2xl p-4 glass ring-1 ${TONE[tone]} bg-gradient-to-br hover-lift overflow-hidden`}>
      <div className="shimmer-sweep absolute inset-0 rounded-2xl pointer-events-none" />
      <div className="relative flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="text-[10px] tracking-[0.2em] text-muted-foreground uppercase truncate">{label}</div>
          <div className="mt-1 text-2xl md:text-3xl font-bold tabular-nums">{value}</div>
          {delta && (
            <div className={`mt-1 text-xs flex items-center gap-1 ${trend === "down" ? "text-[oklch(0.78_0.24_15)]" : "text-[oklch(0.82_0.18_150)]"}`}>
...
export function CreatorActionButton({
  icon: Icon, label, desc, to, onClick, accent = false,
}: {
  icon: LucideIcon; label: string; desc?: string;
  to?: string; onClick?: () => void; accent?: boolean;
}) {
...
export function CreatorStatusBadge({ status, label }: { status: string; label?: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] tracking-[0.18em] px-2 py-1 rounded-full border font-semibold uppercase ${tone}`}>
...
}: { icon?: LucideIcon; title: string; action?: ReactNode }) {
  return (
