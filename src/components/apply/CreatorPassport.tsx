...
export function CreatorPassport({
  ...
  const accentRing = isGold
    ? "shadow-[inset_0_0_0_1px_oklch(0.92_0.18_88/0.6),0_0_24px_oklch(0.85_0.2_85/0.35)]"
  ...
  return (
    <div className={`relative ${isGold ? "neon-gold" : "neon-blue"} p-5 md:p-6`}>
      <div className="liquid-sheen" />
      <div className="relative">
        <div className={`flex items-center gap-2 text-sm font-semibold ${accent}`}>
          <ShieldCheck className="h-4 w-4" />
          <span>{isGold ? "Verification Pass" : "Creator Passport"}</span>
        </div>
        ...
        <div className={`mx-auto mt-4 h-44 w-full max-w-[220px] overflow-hidden rounded-2xl bg-[radial-gradient(ellipse_at_50%_30%,oklch(0.2_0.08_260),oklch(0.08_0.02_260))] ${accentRing}`}>
          {avatarUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center">
              <span className={`font-display text-7xl tracking-tight ${isGold ? "title-split-gold" : "title-split-blue"}`}>{monogram}</span>
            </div>
          )}
        </div>
        ...
        <div className="text-center">
          <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">{isGold ? "Request UID" : "Creator UID"}</p>
