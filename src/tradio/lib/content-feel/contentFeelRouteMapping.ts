...
type Mapping = {
  question_key: PrescribeMeQuestionKey;
  answer_key: string;
  score: number;
  reason: string;
};
...
const push = (out: Mapping[], m: Mapping) => {
  const existing = out.find(
    (x) => x.question_key === m.question_key && x.answer_key === m.answer_key,
  );
 ...
 */
export const buildRouteMappingsFromProfile = (
  ...
  profile.behavioral_need_tags.forEach((need) => {
    ...
    if (answer)
      push(out, {
        ...
        answer_key: answer,
        score: Math.min(0.99, strength + 0.1),
        reason: `Content supports the "${need}" need.`,
      });
  });
  ...
  profile.mood_tags.forEach((mood) => {
    ...
    if (shift)
      push(out, {
        ...
        answer_key: shift,
        score: Math.min(0.95, strength),
        reason: `Mood "${mood}" maps to this shift.`,
      });
  });
  ...
  profile.energy_tags.forEach((energy) => {
    ...
    if (state)
      push(out, {
        question_key: "current_energy",
        answer_key: state,
        score: Math.min(0.9, strength - 0.05),
        reason: `Energy "${energy}" matches this state.`,
      });
  });

  // Platform lane is always routable.
  push(out, {
    question_key: "platform_lane",
    answer_key: profile.source_platform === "shared_universe" ? "any" : profile.source_platform,
    score: 0.8,
    reason: "Native platform lane.",
  });
  ...
  return out
    ...
    .map((m) => ({
      ...
      route_score: Number(m.score.toFixed(2)),
      reason: m.reason,
    }));
};
...
export const buildQuietContexts = (profile: ContentFeelProfile): string[] => {
  ...
  if (profile.energy_tags.includes("low") || profile.energy_tags.includes("intimate"))
    contexts.add("late_night");
  if (profile.mood_tags.includes("hype") || profile.energy_tags.includes("explosive"))
    contexts.add("hype_rotation");
...
export const profileToRouteIndexEntries = (
  profile: ContentFeelProfile,
): PrescribeMeRouteIndexEntry[] =>
...
export const profileToQuietIndexEntry = (
  profile: ContentFeelProfile,
): QuietRecommendationIndexEntry => ({
