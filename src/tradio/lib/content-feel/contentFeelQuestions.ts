...
export const PRESCRIBE_ME_QUESTIONS: PrescribeMeQuestion[] = [
  ...
  },
  {
    ...
    prompt: "How intense?",
    answers: [a("gentle", "Gentle"), a("balanced", "Balanced"), a("intense", "Intense")],
  },
  {
    key: "time_available",
    prompt: "How much time do you have?",
    answers: [a("quick", "A few minutes"), a("medium", "A little while"), a("long", "Settling in")],
  },
  {
    key: "social_preference",
    prompt: "Solo or social?",
    answers: [a("solo", "Just me"), a("shared", "With others"), a("community", "Community / live")],
  },
];

export const PRESCRIBE_ME_QUESTION_MAP: Record<PrescribeMeQuestionKey, PrescribeMeQuestion> =
  PRESCRIBE_ME_QUESTIONS.reduce(
    (acc, q) => {
      acc[q.key] = q;
      return acc;
    },
...
export const getAnswerLabel = (questionKey: PrescribeMeQuestionKey, answerKey: string): string =>
  PRESCRIBE_ME_QUESTION_MAP[questionKey]?.answers.find((ans) => ans.key === answerKey)?.label ??
  answerKey;
