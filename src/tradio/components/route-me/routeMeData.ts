...
import {
  analyzeContentFeelMock,
  MOCK_SCAN_SAMPLES,
} from "@/tradio/lib/content-feel/contentFeelMockAnalysis";
...
export const ROUTE_ME_SURFACES: RouteMeUniverseSurface[] = [
  {
    id: "trey_tv",
    label: "Trey TV",
    eyebrow: "Video, rooms, creators",
    summary: "Creator episodes, rooms, story arcs, and universe video moments.",
    accent: "from-fuchsia-400 to-rose-400",
    examples: ["Creator room", "Episode", "Live drop"],
  },
  {
    id: "tradio",
    label: "Tradio",
    eyebrow: "Music and radio",
    summary: "Stations, tracks, Song Wars, DJ mixes, beats, and hosted shows.",
    accent: "from-purple-400 to-cyan-300",
    examples: ["Station", "Beat", "Battle"],
  },
  {
    id: "fwd",
    label: "FWD",
    eyebrow: "GIFs and reactions",
    summary: "Reaction packs for the energy of the conversation.",
    accent: "from-cyan-300 to-emerald-300",
    examples: ["GIF pack", "Meme set", "Reply mood"],
  },
  {
    id: "storybook",
    label: "Storybook",
    eyebrow: "Interactive stories",
    summary: "Branching story paths with mood, choice, mystery, and romance arcs.",
    accent: "from-amber-300 to-fuchsia-300",
    examples: ["Mystery path", "Romance arc", "Choice scene"],
  },
  {
    id: "games",
    label: "Games",
    eyebrow: "Challenges",
    summary: "Quick games, social rooms, focus challenges, and competition loops.",
    accent: "from-lime-300 to-cyan-300",
    examples: ["Quick challenge", "Party room", "Score run"],
  },
  {
    id: "trance",
    label: "Future Trance",
    eyebrow: "Dance layer",
    summary: "Placeholder lane for future dance trends, tutorials, and challenges.",
    accent: "from-sky-300 to-violet-300",
    examples: ["Dance trend", "Tutorial", "Challenge"],
  },
];

const extraInputs: ContentFeelScanInput[] = [
  {
    content_id: "route-trey-room",
    content_type: "episode",
    source_platform: "trey_tv",
    title: "Trey TV After Hours Room",
    description: "A reflective creator room about choices, comedy, and community energy.",
  },
  {
    content_id: "route-fwd-pack",
    content_type: "gif",
    source_platform: "fwd",
    title: "Soft Launch Reaction Pack",
    description: "Funny supportive GIF reactions for group chat moments.",
  },
  {
    content_id: "route-game-pulse",
    content_type: "game",
    source_platform: "games",
    title: "Signal Sprint Challenge",
    description: "Fast social game challenge with bouncy competitive energy.",
  },
  {
    content_id: "route-trance-placeholder",
    content_type: "dance_video",
    source_platform: "trance",
    title: "Neon Step Starter",
    description: "Future dance trend placeholder with confident tutorial energy.",
  },
  {
    content_id: "route-tradio-songwars",
    content_type: "song_war_battle",
    source_platform: "tradio",
    title: "Trey Trizzy vs Kiana Lane",
    description: "Live Song Wars battle with explosive crowd energy.",
  },
];
...
const answerFor = (answers: PrescribeMeAnswer[], key: string) =>
  answers.find((answer) => answer.questionKey === key);
...
const titleFor = (
  lane: RouteMePlatformLane,
  destination: PrescribeMeRouteDestination,
): { title: string; summary: string; imageUrl?: string } => {
  if (lane === "tradio") {
    if (destination === "beat")
      return {
        title: BEATS[1]?.title ?? "Soul Connection",
        summary: "A producer beat pocket for creating without leaving the music lane.",
        imageUrl: BEATS[1]?.artwork,
      };
    if (destination === "dj_mix")
      return {
        title: DJ_MIXES[0]?.title ?? "Late Night Vibes",
        summary: "A DJ mix with a clear energy arc for settling into the session.",
        imageUrl: DJ_MIXES[0]?.artwork,
      };
    if (destination === "show")
      return {
        title: RADIO_SHOWS[0]?.title ?? "Midnight Therapy",
        summary: "A hosted show route with music blocks, talk breaks, and community energy.",
        imageUrl: IMG.midnightVelvet,
      };
    if (destination === "song_war_battle")
      return {
        title: "Trey Trizzy vs Kiana Lane",
        summary: "A Song Wars battle route for competitive live energy.",
        imageUrl: IMG.treyTrizzy,
      };
    if (destination === "track")
      return {
        title: TRACKS.afterHours.title,
        summary: "A direct track route for a focused listen.",
        imageUrl: TRACKS.afterHours.art,
      };
    return {
      title: ALL_STATIONS[0]?.title ?? "Midnight Velvet",
      summary: "A Tradio station route shaped around the answers from this session.",
      imageUrl: ALL_STATIONS[0]?.image,
    };
  }
  if (lane === "fwd")
    return {
      title: "FWD Soft Signal Pack",
      summary: "A reaction pack for sending the right tone into the conversation.",
      imageUrl: IMG.flowers,
    };
  if (lane === "storybook")
    return {
      title: "The Midnight Mansion Path",
      summary: "An interactive story route with mystery, romance, and reflective choices.",
      imageUrl: IMG.midnightDrive,
    };
  if (lane === "games")
    return {
      title: "Signal Sprint Challenge",
      summary: "A quick playable challenge with social energy and a clean session loop.",
      imageUrl: IMG.outOfOrbit,
    };
  if (lane === "trance")
    return {
      title: "Neon Step Starter",
      summary: "A future Trance placeholder for dance trends and movement challenges.",
      imageUrl: IMG.aiSphere,
    };
  return {
    title: "Trey TV After Hours Room",
    summary: "A creator room and video route for moving through the universe with intention.",
    imageUrl: IMG.treyTrizzy,
  };
};
  ...
  const scored = MOCK_ROUTE_PROFILES.map((profile) => {
    ...
    profile.prescribe_me.recommended_question_answers.forEach((mapping) => {
      if (
        answers.some(
          (answer) =>
            answer.questionKey === mapping.question_key && answer.answerKey === mapping.answer_key,
        )
      ) {
  ...
  return {
    ...
    primaryCta:
      destinationType === "station"
        ? "Start Station"
        : destinationType === "song_war_battle"
          ? "Enter Battle"
          : "Open Route",
...
export const QUIET_ROUTE_SUGGESTIONS: QuietRouteSuggestion[] = [
  {
    ...
    reason: safeRecommendationReason(
      "This route fits the energy of your current lane.",
      "current_route",
    ),
    cta: "Preview",
  },
  {
    ...
    reason: safeRecommendationReason(
      "This route fits broad Content Feel tags for the current lane.",
      "chosen_mood",
    ),
    cta: "Open",
  },
  {
    ...
    reason: safeRecommendationReason(
      "Because this lane has matching Content Feel tags.",
      "selected_answer",
    ),
    cta: "View Pack",
  },
