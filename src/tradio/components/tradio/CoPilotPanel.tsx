...
export const CoPilotPanel: React.FC<CoPilotPanelProps> = ({
  ...
  const handleReadRoom = async () => {
    setReading(true);
    try {
      const messages = (interaction?.chat ?? [])
        .slice(-20)
        .map((c: any) => `${c.authorName || "Listener"}: ${c.body}`);
      ...
      setReadResult({
        ...
        mood: "neutral",
        highlights: [],
        suggestedTopic: "Ask the audience what they are into",
      });
    } finally {
      setReading(false);
    }
  ...
  const handleSuggestLine = async () => {
    ...
    try {
      ...
      const res = await coPilotSuggestLine({
        data: { showTitle, segmentTitle, hostTone: "casual", recentChat },
      });
  ...
  const handleSuggestSongs = async () => {
    ...
    try {
      const recentRequests = (interaction?.requests ?? [])
        .slice(-10)
        .map((r: any) => `${r.songTitle} — ${r.artist || ""}`);
  ...
  return (
    <GlassCard className="p-4">
      ...
      <div className="mt-3 flex gap-2">
        <PrimaryButton onClick={handleReadRoom} disabled={reading}>
          {reading ? "Reading…" : "Read the room"}
        </PrimaryButton>
        <SecondaryButton onClick={handleSuggestLine} disabled={suggesting}>
          {suggesting ? "Suggesting…" : "Suggest a line"}
        </SecondaryButton>
        <SecondaryButton onClick={handleSuggestSongs} disabled={suggestSongsLoading}>
          {suggestSongsLoading ? "Picking…" : "Suggest songs"}
        </SecondaryButton>
      </div>
          ...
          <div>
            <div className="font-semibold">
              ...
              <span className="text-white/60">
                {readResult.energy} · {readResult.mood}
              </span>
            </div>
            <div className="text-xs text-white/60">
              Highlights: {readResult.highlights.join(" · ") || "None"}
            </div>
        ...
        {suggestLine && (
          <div className="rounded-xl border border-white/8 bg-white/[0.02] p-3">
            ...
            <div className="flex gap-2">
              ...
              <SecondaryButton onClick={() => navigator.clipboard?.writeText(suggestLine)}>
                Copy
              </SecondaryButton>
            </div>
          </div>
            ...
            {songPicks.map((p, i) => (
              <div key={i} className="rounded-xl border border-white/8 bg-white/[0.02] p-2.5">
                <div className="font-semibold text-white">
                  {p.title} <span className="text-white/60">— {p.artist}</span>
                </div>
