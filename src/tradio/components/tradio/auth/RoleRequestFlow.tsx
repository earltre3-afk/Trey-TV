...
const FLOW_CONFIG: Record<RoleRequestType, FlowConfig> = {
  artist: {
    ...
    subtitle: 'Tell us about your sound. Artist access unlocks releases, an artist station, playlists, and premieres.',
    benefit: 'Releases · Artist station · Playlists · Premieres · Song Wars',
    fields: [
      { field: 'artist_name', label: 'Artist / stage name', type: 'text', placeholder: 'e.g. Mila Rain' },
      { field: 'genres', label: 'Genres', type: 'text', placeholder: 'R&B, Soul, Alt-Pop' },
      { field: 'music_links', label: 'Existing music links', type: 'textarea', placeholder: 'Spotify, YouTube, SoundCloud…', optional: true },
      { field: 'station_goal', label: 'Station goal', type: 'textarea', placeholder: 'What kind of station do you want to build?' },
      { field: 'release_goals', label: 'Release goals', type: 'textarea', placeholder: 'How often do you plan to drop music?' },
      { field: 'fanbase', label: 'Fanbase description', type: 'textarea', placeholder: 'Tell us about your current audience.' },
      { field: 'verification_interest', label: 'Interested in verification?', type: 'select', options: ['Yes', 'Not yet'] },
    ],
  },
  producer: {
    title: 'Request Producer Access',
    subtitle: 'Producer access unlocks your beat catalog, packs, collabs, artist pitches, and DJ opportunities.',
    benefit: 'Beat catalog · Packs · Collabs · Artist pitches · Beat battles',
    fields: [
      { field: 'producer_name', label: 'Producer name', type: 'text', placeholder: 'e.g. Darius Cole' },
      { field: 'beat_genres', label: 'Beat genres', type: 'text', placeholder: 'Trap Soul, Lo-Fi, Drill' },
      { field: 'bpm_key', label: 'BPM / key preferences', type: 'text', placeholder: 'e.g. 120–150 BPM, minor keys', optional: true },
      { field: 'catalog_intent', label: 'Beat catalog intent', type: 'textarea', placeholder: 'What will you upload and how often?' },
      { field: 'collab_availability', label: 'Collab availability', type: 'select', options: ['Open to collabs', 'Selective', 'Solo for now'] },
      { field: 'licensing_intent', label: 'Licensing intent', type: 'select', options: ['Lease + exclusive', 'Exclusive only', 'Free / promo', 'Undecided'] },
    ],
  },
  dj: {
    title: 'Request DJ / Host Access',
    subtitle: 'Host access unlocks live shows, mixes, the Broadcast Studio, listener requests, replays, and Song Wars hosting.',
    benefit: 'Live shows · Mixes · Broadcast Studio · Requests · Song Wars hosting',
    fields: [
      { field: 'dj_name', label: 'Host / DJ name', type: 'text', placeholder: 'e.g. DJ Midnight Spin' },
      { field: 'show_type', label: 'Show type', type: 'text', placeholder: 'Live mix, talk + music, request hour…' },
      { field: 'preferred_genres', label: 'Preferred genres', type: 'text', placeholder: 'Hip-Hop, House, R&B' },
      { field: 'schedule_goal', label: 'Live schedule goal', type: 'text', placeholder: 'e.g. Weekly Friday nights' },
      { field: 'broadcast_experience', label: 'Broadcast experience', type: 'textarea', placeholder: 'Any hosting / DJ experience?', optional: true },
      { field: 'equipment', label: 'Equipment / readiness', type: 'textarea', placeholder: 'What is your setup?', optional: true },
      { field: 'show_concept', label: 'Show concept', type: 'textarea', placeholder: 'Describe your dream show.' },
    ],
  },
  verification: {
    ...
    fields: [
      { field: 'verify_role', label: 'Role being verified', type: 'select', options: ['Artist', 'Producer', 'DJ / Host'] },
      { field: 'public_identity', label: 'Public identity', type: 'text', placeholder: 'Your public Trey TV identity / name' },
      { field: 'links', label: 'Links / social proof', type: 'textarea', placeholder: 'Socials, press, streaming profiles…', optional: true },
      { field: 'reason', label: 'Reason for verification', type: 'textarea', placeholder: 'Why should you be verified?' },
    ],
  },
  broadcast: {
    ...
    fields: [
      { field: 'role', label: 'Your creator role', type: 'select', options: ['Artist', 'Producer', 'DJ / Host'] },
      { field: 'show_type', label: 'Show type', type: 'text', placeholder: 'DJ mix, premiere, producer spotlight…' },
      { field: 'show_concept', label: 'Station / show concept', type: 'textarea', placeholder: 'Describe your broadcast concept.' },
      { field: 'schedule_intent', label: 'Schedule intent', type: 'text', placeholder: 'e.g. Weekly, 2 hours, evenings' },
      { field: 'moderation_ack', label: 'I agree to the Tradio broadcast & moderation guidelines', type: 'checkbox' },
    ],
  },
};
...
export const RoleRequestFlow: React.FC<{
  ...
  const [legalValues, setLegalValues] = useState<LegalAcceptanceValues>(() => createLegalAcceptanceValues('role_access_request'));
  ...
  const canSubmit = legalAccepted && requiredFields.every((field) => {
  ...
  const handleSubmit = async () => {
    ...
    setLegalMessage(result.source === 'supabase' ? 'Access acknowledgement saved.' : result.warning);
        ...
        <GlassCard glow className="p-6 sm:p-8">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-300">{REQUEST_TYPE_LABEL[flowType]}</span>
          ...
          {submitted ? (
            <div className="mt-8 flex flex-col items-center gap-4 py-6 text-center">
              ...
              <p className="max-w-md text-sm leading-relaxed text-white/62">
                Your {REQUEST_TYPE_LABEL[flowType]} request is now <span className="font-bold text-amber-200">pending review</span>.
                Roles are granted by the Tradio team — you'll be notified once it's processed. (Mock flow: nothing is sent to a backend yet.)
              </p>
              <PrimaryButton className="mt-2 px-6 py-3 text-xs" onClick={onClose}>Done</PrimaryButton>
            </div>
          ) : (
            <>
                  ...
                  <div key={field.field}>
                    {field.type !== 'checkbox' && (
                      <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-white/55">
                        {field.label} {field.optional && <span className="text-white/30">(optional)</span>}
                      </label>
                    )}
                    {field.type === 'text' && (
                      <input
                    ...
                    {field.type === 'checkbox' && (
                      <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white/75">
                        <input
                          ...
                          onChange={(event) => setValue(field.field, event.target.checked ? 'acknowledged' : '')}
                          className="mt-0.5 h-4 w-4 accent-fuchsia-500"
                        />
                        <span className="flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-cyan-300" /> {field.label}</span>
                      </label>
                    )}
                  </div>
                ))}
              ...
              <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex gap-2">
                  <SecondaryButton className="px-4 py-3 text-xs" onClick={onClose}>Cancel</SecondaryButton>
                  <PrimaryButton
