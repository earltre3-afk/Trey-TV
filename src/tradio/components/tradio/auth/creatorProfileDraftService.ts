...
const DRAFT_STORAGE_KEY = (role: RoleProfileType, userId: string) => `tradio_draft_${role}_${userId}`;
 ...
 */
export function getCreatorProfileDraft(role: RoleProfileType, identity: TradioIdentity): CreatorProfileSpecificDraft {
 ...
 */
export function updateCreatorProfileDraftLocal(draft: CreatorProfileSpecificDraft, updates: Partial<CreatorProfileDraftPayload>): CreatorProfileSpecificDraft {
  const next = {
    ...draft,
  ...
  if (draft.role === 'artist') {
    const updatedDraft: ArtistProfileDraft = { ...draft, ...next, role: 'artist', data: { ...draft.data, ...updates, artistName: draft.data.artistName, genres: updates.genres ?? draft.data.genres } };
    return updatedDraft;
  }
  if (draft.role === 'producer') {
    const updatedDraft: ProducerProfileDraft = { ...draft, ...next, role: 'producer', data: { ...draft.data, ...updates, producerName: draft.data.producerName, genres: updates.genres ?? draft.data.genres } };
    return updatedDraft;
  }
  const updatedDraft: DjHostProfileDraft = { ...draft, ...next, role: 'dj', data: { ...draft.data, ...updates, djName: draft.data.djName, genres: updates.genres ?? draft.data.genres, showTypes: draft.data.showTypes } };
  return updatedDraft;
}
 ...
 */
export function saveCreatorProfileDraft(draft: CreatorProfileSpecificDraft): CreatorProfileSaveResult {
  try {
    const updatedDraft = {
      ...draft,
 ...
 */
export function validateCreatorProfileDraft(role: RoleProfileType, draft: CreatorProfileSpecificDraft): CreatorProfileValidationResult {
  ...
  const percentComplete = totalChecks > 0 ? Math.round(((totalChecks - required.length) / totalChecks) * 100) : 100;
 ...
 */
export function submitCreatorProfileForReview(role: RoleProfileType, identity: TradioIdentity, draft: CreatorProfileSpecificDraft): CreatorProfilePublishResult {
  ...
  const updatedDraft: CreatorProfileSpecificDraft = { ...draft, status: 'submitted', lastModified: Date.now() };
  storeDraft(updatedDraft);
 ...
 */
export function publishCreatorProfileMock(role: RoleProfileType, identity: TradioIdentity, draft: CreatorProfileSpecificDraft): CreatorProfilePublishResult {
  const validation = validateCreatorProfileDraft(role, draft);
  ...
  const updatedDraft: CreatorProfileSpecificDraft =
    draft.role === 'artist'
      ? { ...draft, status: 'published', lastModified, data: { ...draft.data, visibility: 'public' } }
      : draft.role === 'producer'
        ? { ...draft, status: 'published', lastModified, data: { ...draft.data, visibility: 'public' } }
        : { ...draft, status: 'published', lastModified, data: { ...draft.data, visibility: 'public' } };
  storeDraft(updatedDraft);
 ...
 */
export function getProfileDraftStatus(role: RoleProfileType, identity: TradioIdentity): { status: CreatorProfileDraftStatus; lastModified: number } {
  const draft = loadDraft(role, identity.user_id);
  if (!draft) {
