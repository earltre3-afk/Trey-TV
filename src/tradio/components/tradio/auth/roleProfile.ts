...
const PROFILE_ROLE: Record<RoleProfileType, TradioRole> = { artist: 'artist', producer: 'producer', dj: 'dj' };
...
export const ROLE_PROFILE_SECTIONS: Record<RoleProfileType, RoleProfileSection[]> = {
  artist: [
    ...
    { id: 'shows', title: 'Artist Shows', description: 'Broadcast Studio premieres.', requiresBroadcast: true },
    { id: 'analytics', title: 'Analytics', description: 'Plays, saves, and fan growth.', ownerOnly: true },
  ],
  producer: [
    ...
    { id: 'matches', title: 'Artist Match Suggestions', description: 'Artists that fit your sound.' },
    ...
    { id: 'spotlight', title: 'Producer Spotlight Shows', description: 'Broadcast Studio features.', requiresBroadcast: true },
    { id: 'analytics', title: 'Analytics', description: 'Leases, plays, and collabs.', ownerOnly: true },
  ],
  dj: [
    { id: 'stations', title: 'Hosted Stations', description: 'Channels you broadcast on.' },
    { id: 'shows', title: 'Live Shows', description: 'Your scheduled and live broadcasts.', requiresBroadcast: true },
    ...
    { id: 'broadcast', title: 'Broadcast Studio', description: 'Build and run premium shows.', requiresBroadcast: true },
    { id: 'analytics', title: 'Analytics', description: 'Listeners, fulfillment, tune-ins.', ownerOnly: true },
  ],
};
 ...
 */
export const getRoleProfileCompletion = (identity: TradioIdentity, role: RoleProfileType): RoleProfileCompletionChecklist => {
  const roleActive = hasRole(identity, PROFILE_ROLE[role]);
  const verified = identity.verification_status === 'verified';
  const base = (id: string, label: string, complete: boolean): RoleProfileChecklistItem => ({ id, label, complete });
  ...
  return { items, completed, total, percent: Math.round((completed / total) * 100), isComplete: completed === total };
};

export const getMissingProfileRequirements = (identity: TradioIdentity, role: RoleProfileType): string[] =>
  getRoleProfileCompletion(identity, role).items.filter((item) => !item.complete).map((item) => item.label);

export const getRoleProfileOwnerContext = (identity: TradioIdentity, role: RoleProfileType): RoleProfileOwnerContext => ({
...
export const getRoleProfilePublicUrl = (identity: TradioIdentity, role: RoleProfileType): string => {
...
export const getRoleProfileActivationState = (
  ...
  if (roleStatus === 'restricted') {
    ...
    status = !completion.isComplete
      ? 'approved_incomplete'
      : visibility === 'private' ? 'active_private' : 'active_public';
  } else {
