...
export const makeAccessEvent = (
  ...
  opts: {
    from?: RoleRequestStatus;
    to?: RoleRequestStatus;
    note?: string;
    actor?: string;
    created_at?: string;
  } = {},
...
export const MOCK_ROLE_ACCESS_REQUESTS: RoleAccessRequest[] = [
  {
    ...
    answers: [
      ...
      { field: "genres", label: "Genres", value: "R&B, Alt-Soul" },
      {
        field: "station_goal",
        label: "Station goal",
        value: "A late-night soul station with weekly premieres.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    answers: [{ field: "show_concept", label: "Show concept", value: "Midnight Soul Sessions" }],
  },
  {
    id: "req-6",
    request_type: "artist",
...
export const getRoleAccessRequestsMock = (): RoleAccessRequest[] =>
  MOCK_ROLE_ACCESS_REQUESTS.map((request) => ({
    ...request,
    answers: [...request.answers],
    events: deriveSeedEvents(request),
  }));
...
export const submitVerificationRequestMock = (
  answers: RoleApplicationAnswer[],
): RoleAccessRequest => submitRoleAccessRequestMock("verification", answers);

export const submitBroadcastAccessRequestMock = (
  answers: RoleApplicationAnswer[],
): RoleAccessRequest => submitRoleAccessRequestMock("broadcast", answers);
...
export const getCurrentUserAccessStateMock = (
  ...
  const approved_roles = identity.roles
    .filter((grant) => grant.role_status === "active" || grant.role_status === "approved")
    ...
    verification_status:
      identity.verification_status === "verified"
        ...
        : identity.verification_status === "pending"
          ? "pending"
          : "not_started",
    broadcast_access_status:
      identity.broadcast_access_status === "cleared"
        ? "approved"
        : ["submitted", "pending", "under_review"].includes(identity.broadcast_access_status)
          ? "pending"
          : "not_started",
  };
};
...
export const getNextAccessStep = (type: RoleRequestType, status: RoleRequestStatus): string => {
  if (status === "pending" || status === "submitted")
    return `Your ${REQUEST_TYPE_LABEL[type]} request is in review. We'll notify you once it's processed.`;
  if (status === "approved")
    return `${REQUEST_TYPE_LABEL[type]} is active. Switch modes to use the new tools.`;
  if (status === "rejected")
    return `Update your details and resubmit your ${REQUEST_TYPE_LABEL[type]} request.`;
  if (status === "needs_more_info")
    return `Add the requested details to continue your ${REQUEST_TYPE_LABEL[type]} request.`;
  if (status === "restricted")
    return `${REQUEST_TYPE_LABEL[type]} is restricted. Contact support to restore access.`;
