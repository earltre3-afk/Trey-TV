...
export const RoleIndicator: React.FC<{
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border bg-gradient-to-r font-semibold ${size === "sm" ? "px-2.5 py-1 text-[11px]" : "px-3 py-1.5 text-xs"} ${colors[tone] || colors.listener}`}
    >
...
export const ModeSwitcher: React.FC = () => {
  ...
  const statusCopy =
    isLoading && phaseCopy[bootstrapPhase]
    ...
    <GlassCard className="p-3">
        ...
        <div className="min-w-0 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-bold text-white">
              <span className="text-signature text-2xl text-fuchsia-300 font-normal drop-shadow-[0_0_10px_rgba(217,70,239,0.5)] hover:scale-105 transition-transform duration-300 select-none block origin-left">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-wider">
        ...
        {profileBridgeStatus === "profile_bridge_missing" && (
          <span className="rounded-full border border-amber-300/25 bg-amber-500/10 px-2.5 py-1 text-amber-200">
            Profile bridge not found
          </span>
        )}
        {!isLoading && bootstrapStatus === "profile_not_created" && (
          <span className="rounded-full border border-fuchsia-300/25 bg-fuchsia-500/10 px-2.5 py-1 text-fuchsia-200">
            Tradio profile not created yet
          </span>
        )}
        {!isLoading && bootstrapStatus === "database_not_ready" && (
          <span className="rounded-full border border-amber-300/25 bg-amber-500/10 px-2.5 py-1 text-amber-200">
            Profile setup paused: database not ready
          </span>
        )}
        {!isLoading && bootstrapStatus === "setup_incomplete" && (
          <button className="rounded-full border border-cyan-300/30 bg-cyan-500/10 px-2.5 py-1 text-cyan-200 hover:bg-cyan-500/20 transition">
            Finish Tradio setup
          </button>
        )}
        {identityWarnings[0] && (
          <span className="max-w-full truncate rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-white/45">
            {identityWarnings[0]}
          </span>
        )}
      </div>
...
export const BroadcastAccessBadge: React.FC<{ status: TradioBroadcastAccessState }> = ({
  status,
}) => (
  <span
    className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase ${
      status === "cleared"
        ...
        : ["pending", "under_review", "submitted"].includes(status)
...
export const RoleCapabilityCard: React.FC<{
  title: string;
  description: string;
...
export const ApplyForAccessCTA: React.FC<{ type?: RoleRequestType }> = ({ type = "broadcast" }) => {
  ...
  const copy = {
    artist: [
      "Artist access setup",
      "Connect your Trey TV creator profile and request artist tools.",
    ],
    ...
    broadcast: [
      ...
      "Request clearance for premium Tradio shows and scheduled broadcasts.",
    ],
  ...
  const showButton =
    canRequest || (request && ["rejected", "needs_more_info"].includes(request.status));
  const buttonLabel =
    request && ["rejected", "needs_more_info"].includes(request.status)
...
export const RoleLockedState: React.FC<{
  title: string;
  message: string;
  ctaType?: RoleRequestType;
}> = ({ title, message, ctaType }) => {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10">
      <GlassCard glow className="p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] font-bold">
              ...
              {ctaType && (
                <span className="rounded-full border border-fuchsia-300/25 bg-fuchsia-500/10 px-2.5 py-1 text-fuchsia-200">
                  Needs: {NEEDED_ACCESS_LABEL[ctaType]}
                </span>
              )}
              {request && <RequestStatusBadge status={request.status} />}
            </div>
        ...
        {ctaType && (
          <div className="mt-5">
            <ApplyForAccessCTA type={ctaType} />
          </div>
        )}
      </GlassCard>
    </div>
...
export const AccessGate: React.FC<{
  ...
  const { identity } = useTradioIdentity();
  if (!can(identity, capability))
    return <RoleLockedState title={title} message={message} ctaType={ctaType} />;
  return <>{children}</>;
};
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h4 className="text-sm font-mono font-black uppercase tracking-[0.2em] text-white">
                {title}
              </h4>
