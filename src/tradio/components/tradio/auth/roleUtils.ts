...
export const isGrantActive = (grant: TradioRoleGrant) =>
...
export const availableModesFor = (identity: TradioIdentity | null | undefined): TradioMode[] => {
  ...
  const modes = (Object.keys(modeRoleMap) as TradioMode[]).filter((mode) =>
    hasAnyRole(identity, modeRoleMap[mode]),
  );
...
export const canCreateArtistStation = (identity: TradioIdentity | null | undefined) =>
  hasAnyRole(identity, ["artist", "admin", "owner"]) &&
  (isCurrentMode(identity, "artist") || isCurrentMode(identity, "admin"));
...
export const canUploadBeat = (identity: TradioIdentity | null | undefined) =>
  hasAnyRole(identity, ["producer", "admin", "owner"]) &&
  (isCurrentMode(identity, "producer") || isCurrentMode(identity, "admin"));
...
export const canHostSongWar = (identity: TradioIdentity | null | undefined) => {
  ...
  return (
    hasAnyRole(identity, ["dj", "moderator"]) &&
    (isCurrentMode(identity, "dj") || isCurrentMode(identity, "admin"))
  );
};
...
export const canRequestBroadcastAccess = (identity: TradioIdentity | null | undefined) => {
  ...
  return Boolean(
    identity &&
    !["cleared", "pending", "under_review", "submitted"].includes(identity.broadcast_access_status),
  );
};
...
export const canRequestVerification = (identity: TradioIdentity | null | undefined) =>
  Boolean(
    identity &&
    ["unverified", "rejected"].includes(identity.verification_status) &&
    hasAnyRole(identity, ["artist", "producer", "dj"]),
  );
