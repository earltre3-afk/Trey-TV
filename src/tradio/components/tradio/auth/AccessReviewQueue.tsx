...
const ReviewCard: React.FC<{ request: RoleAccessRequest }> = ({ request }) => {
  ...
  const applicant =
    request.answers.find((a) => /name|identity/.test(a.field))?.value ?? "Applicant";
  const isTerminal =
  ...
  return (
    <GlassCard className="p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          onClick={() => setOpen((v) => !v)}
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/12 bg-white/[0.04] text-white/70 hover:border-white/25"
        >
            ...
            <div>
              ...
              <dl className="space-y-1">
                {request.evidence.map((item) => (
                  <div key={item.field} className="flex gap-2 text-[11px]">
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {reviewBtn(
              ...
              "restricted",
              ShieldX,
              "border-orange-300/30 bg-orange-500/10 text-orange-200 hover:bg-orange-500/20",
            )}
            {reviewBtn(
              "Archive",
              "cancelled",
  ...
  return (
    <div className="rounded-2xl border border-red-300/20 bg-red-500/[0.05] p-4 sm:p-5">
      ...
      <p className="mt-1.5 text-xs leading-relaxed text-white/55">
        Reviewer prototype. Approvals/rejections call protected SECURITY DEFINER RPC when Supabase
        is wired; in demo mode they update local mock state and simulate the grant. Admin/owner is
        never self-assignable.
      </p>

      <div className="mt-4 space-y-3">
        <div className="text-[10px] font-bold uppercase tracking-wider text-white/45">
          Needs review ({actionable.length})
        </div>
        {actionable.length === 0 && (
          <p className="text-[11px] italic text-white/40">No requests awaiting review.</p>
        )}
        ...
        {decided.length > 0 && (
          <>
            <div className="pt-2 text-[10px] font-bold uppercase tracking-wider text-white/45">
