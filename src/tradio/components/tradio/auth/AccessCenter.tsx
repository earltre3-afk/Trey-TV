...
const LANES: Lane[] = [
  {
    type: "artist",
    title: "Artist",
    benefit: "Releases · Artist station · Playlists · Premieres",
    Icon: Music2,
  },
  {
    type: "producer",
    title: "Producer",
    benefit: "Beat catalog · Packs · Collabs · Artist pitches",
    Icon: Disc3,
  },
  {
    type: "dj",
    title: "DJ / Host",
    benefit: "Live shows · Mixes · Requests · Song Wars hosting",
    Icon: Mic2,
  },
  {
    type: "verification",
    title: "Verification",
    benefit: "Creator trust badge · Premiere eligibility",
    Icon: ShieldCheck,
  },
  {
    type: "broadcast",
    title: "Broadcast Access",
    benefit: "Premium shows · Scheduled broadcasts",
    Icon: Radio,
  },
];
...
const LaneCard: React.FC<{
  lane: Lane;
  onOpenRoleProfile?: (role: RoleProfileType, ownerView?: boolean) => void;
}> = ({ lane, onOpenRoleProfile }) => {
    ...
    <GlassCard className="flex h-full flex-col justify-between p-4">
      <div>
        ...
        <p className="mt-2 text-[11px] leading-relaxed text-cyan-200/70">
          {getNextAccessStep(lane.type, status)}
        </p>
      </div>
      <div className="mt-4">
        {isApproved ? (
          profileRole && onOpenRoleProfile ? (
            <SecondaryButton
              ...
              onClick={() => onOpenRoleProfile(profileRole, true)}
            >
 ...
 */
export const AccessCenter: React.FC<{
  onOpenRoleProfile?: (role: RoleProfileType, ownerView?: boolean) => void;
}> = ({ onOpenRoleProfile }) => {
  const { identity, currentMode, currentRoleLabel } = useTradioIdentity();
  const access = useAccessRequests();
  const approvedRoles = identity.roles.filter(
    (g) => g.role_status === "active" || g.role_status === "approved",
  );
  ...
  return (
    <GlassCard glow className="p-5 sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-xl text-sm leading-relaxed text-white/62">
            You're operating as {currentRoleLabel}. Everyone starts as a listener — request access
            to unlock artist, producer, and DJ/host tools. Roles are reviewed before activation,
            never self-granted.
          </p>
        </div>
        <div className="shrink-0 rounded-2xl border border-white/8 bg-white/[0.03] p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {approvedRoles.map((grant) => (
              <span
                key={grant.id}
                className="rounded-full border border-emerald-300/25 bg-emerald-500/10 px-2.5 py-1 text-[10px] font-bold text-emerald-200"
              >
      ...
      {suggestions.length > 0 && (
        <div className="mt-5">
          <div className="mb-2 text-xs font-bold uppercase tracking-wider text-white/45">
            Suggested for {currentRoleLabel}
          </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-[11px] text-white/45">
                      {getNextAccessStep(request.request_type, request.status)}
                    </div>
                  </div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {LANES.map((lane) => (
          <LaneCard key={lane.type} lane={lane} onOpenRoleProfile={onOpenRoleProfile} />
        ))}
      </div>
