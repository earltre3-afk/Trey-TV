...
export const LiveShowConsole: React.FC<{
  ...
  const callerAction = useCallback(
    async (requestId: string, action: "take" | "disconnect" | "decline") => {
      try {
        ...
        if (!res.ok) {
          ...
          const label =
            action === "take" ? "ON AIR" : action === "disconnect" ? "Disconnected" : "Declined";
          setNotices((prev) => [
    ...
    <div className="space-y-6 px-4 sm:px-6 lg:px-10 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-red-500/10 border border-red-500/20 rounded-3xl p-5 shadow-[0_0_40px_rgba(239,68,68,0.15)] backdrop-blur-md gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="block text-[9px] font-mono text-red-400 font-bold uppercase tracking-[0.3em] animate-pulse">
              BROADCAST ACTIVE
            </span>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Chip
            label={`${live.listenerCount} listeners`}
        ...
        <div className="space-y-6">
          ...
          <GlassCard glow className="p-6 border border-white/10 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start pb-4 border-b border-white/5">
              <div>
                <span className="text-[10px] font-mono text-cyan-300 font-bold uppercase tracking-widest">
                  SEGMENT {activeSegmentIdx + 1} OF {show.segments.length} •{" "}
                  {currentSegment.type.toUpperCase()}
                </span>
                <h2 className="text-2xl font-black text-white mt-1">{currentSegment.title}</h2>
                {currentSegment.description && (
                  <p className="text-xs text-white/50 mt-1">{currentSegment.description}</p>
                )}
              </div>
            ...
            <div className="mt-3 flex items-center justify-between text-[11px] font-mono">
              <span className="text-white/50">
                −{Math.floor(rundown.remainingInSegment / 60)}:
                {String(rundown.remainingInSegment % 60).padStart(2, "0")} left in segment
              </span>
              <span
                className={
                  rundown.pacing.status === "behind"
                    ? "text-red-300"
                    : rundown.pacing.status === "ahead"
                      ? "text-cyan-300"
                      : "text-emerald-300"
                }
              >
                {rundown.pacing.status === "on-time"
                  ? "ON TIME"
                  : rundown.pacing.status === "behind"
                    ? `${rundown.pacing.deltaSeconds}s BEHIND`
            ...
            <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
              <div
                ...
                style={{
                  width: `${Math.min(100, (rundown.elapsedInSegment / Math.max(1, currentSegment.duration)) * 100)}%`,
                }}
              />
            </div>
              ...
              <div className="mt-5 bg-[#100B1A]/80 border border-purple-500/10 rounded-2xl p-4 relative overflow-hidden">
                ...
                <div className="mt-3 flex items-center gap-2">
                  <button
                    onClick={() =>
                      live.aiSpeak(
                        currentSegment.script ?? currentSegment.hostNotes ?? "",
                  ...
                  {live.aiSpeaking && live.aiSegmentLabel && (
                    <span className="text-[9px] font-mono text-cyan-300 animate-pulse">
                      {live.aiSegmentLabel}
                    </span>
                  )}
                </div>
                <Waveform
                  className="h-4 w-full opacity-35 mt-4"
                  bars={36}
            ...
            <div className="mt-5 space-y-3">
              <div className="text-[10px] font-mono text-cyan-300 font-bold uppercase tracking-widest">
                SEGMENT CUES & DIRECTIVES
              </div>
              <div className="space-y-2.5">
                {segmentCues.map((cue, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-3 bg-white/[0.02] border border-white/5 p-3 rounded-xl"
                  >
                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-cyan-500/10 text-cyan-300 text-[10px] font-black">
                      {idx + 1}
                    </span>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <GlassCard className="p-5 border-white/5 space-y-4">
              <div className="text-[10px] font-mono text-purple-300 font-bold uppercase tracking-widest">
                Console VU Meters
              </div>
                    ...
                    {Array.from({ length: 16 }).map((_, idx) => {
                      ...
                      if (active) {
                        if (levelVal > 80) bg = "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]";
                          ...
                          bg = "bg-yellow-400 shadow-[0_0_8px_rgba(250,204,21,0.5)]";
                        else bg = "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]";
                      }
                      return (
                        <div
                          key={idx}
                          className={`w-[5px] h-10 rounded-[1px] ${bg} transition-all duration-150`}
                        />
                      );
                    })}
                  </div>
            ...
            <GlassCard className="p-5 border-white/5 space-y-4">
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs text-white/50 mb-1">
                    <span className="flex items-center gap-1">
                      <Mic2 className="h-3.5 w-3.5 text-cyan-400" /> Host Mic
                    </span>
                    <button
                      onClick={() => live.toggleMic()}
                      className={`text-[10px] font-black rounded px-1.5 py-0.5 ${!live.micOn ? "bg-red-500/10 text-red-400" : "bg-emerald-500/10 text-emerald-400"}`}
                    >
                ...
                <div>
                  <div className="flex justify-between text-xs text-white/50 mb-1">
                    <span className="flex items-center gap-1">
                      <Music className="h-3.5 w-3.5 text-purple-400" /> Audio Bed
                    </span>
                  ...
                  <input
                    ...
                    max="100"
                    value={bedVol}
                    onChange={(e) => handleBedVol(Number(e.target.value))}
                    className="h-1.5 w-full cursor-pointer bg-white/10 rounded-full accent-purple-400"
                  />
                </div>
              </div>
            </GlassCard>
          ...
          <GlassCard className="p-5 border-white/10 relative">
                  ...
                  <div>
                    <div className="flex items-baseline gap-1.5">
                      <span className="font-bold text-white text-sm">
                        {onAirCaller.callerName ?? "Caller"}
                      </span>
                      <span className="rounded bg-emerald-500/15 text-[8px] font-mono text-emerald-300 px-1 py-0.5 font-bold uppercase tracking-wider">
                        ON AIR SIGNAL
                      </span>
                    </div>
                    {onAirCaller.lineNote && (
                      <p className="text-xs text-emerald-200 mt-1">{onAirCaller.lineNote}</p>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-2">
              {pendingCallers.length === 0 && (
                <div className="text-center text-xs text-white/25 py-3">
                  No pending call-in requests.
                </div>
              )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-xs text-white">
                        {c.callerName ?? "Listener"}
                      </div>
                      {c.lineNote && (
                        <div className="text-[10px] text-white/55 mt-0.5">{c.lineNote}</div>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5">
