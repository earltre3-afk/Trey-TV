...
import { BroadcastAccessBadge, PrescriptionRail, VerificationBadge } from "../auth/components";
import {
  NeedsMoreInfoState,
  OnboardingState,
...
const ROLE_REQUEST: Record<RoleProfileType, RoleRequestType> = {
  artist: "artist",
  producer: "producer",
  dj: "dj",
};
const ROLE_MESSAGE_CTA: Record<RoleProfileType, string> = {
  artist: "Message Artist",
  producer: "Contact Producer",
  dj: "Message Host",
};
const ROLE_MESSAGE_SURFACE: Record<RoleProfileType, MessageSourceSurface> = {
  artist: "artist_profile",
  producer: "producer_profile",
  dj: "dj_profile",
};
const ROLE_ENTITY_TYPE: Record<RoleProfileType, SourceEntityType> = {
  artist: "artist",
  producer: "producer",
  dj: "dj",
};
...
export const RoleProfilePage: React.FC<{
  ...
  const visibility = ownerView
    ? profileResult?.visibility || "private"
    : serviceResult?.visibility || "private";
  ...
  const Header = (
    <div className="flex items-center justify-between px-4 pt-[max(2rem,env(safe-area-inset-top))] pb-3 sm:px-6 lg:px-10">
      <button
        onClick={onBack}
        aria-label="Back"
        className="flex h-10 w-10 items-center justify-center rounded-full border border-white/12 bg-white/[0.05] text-white hover:border-white/25 transition"
      >
        ...
        <div className="px-4 sm:px-6 lg:px-10">
          <GlassCard className="mt-8 p-8 text-center">
            ...
            <p className="mt-2 text-white/50">
              We couldn't find a {ROLE_TITLE[role].toLowerCase()} with{" "}
              {lookup?.handle ? `@${lookup.handle}` : lookup?.publicProfileUid}.
            </p>
            <p className="mt-1 text-xs text-white/40">
              It may not be public yet or the URL might be incorrect.
            </p>
            <SecondaryButton className="mt-6 px-6 py-2.5 text-sm" onClick={onBack}>
              Back
            </SecondaryButton>
          </GlassCard>
        </div>
  ...
  // ── Non-active states ──────────────────────────────────────────────────────
  if (
    activation.status !== "active_public" &&
    activation.status !== "active_private" &&
    activation.status !== "approved_incomplete"
  ) {
    return (
      <div className="min-h-screen pb-16">
        {Header}
        <div className="px-4 sm:px-6 lg:px-10">
          ...
          {activation.status === "rejected" && (
            <RequestRejectedState
              reason={activation.reviewerNote}
              onResubmit={canRequest ? openFlow : undefined}
            />
          )}
          ...
          {(activation.status === "locked" || activation.status === "request_available") && (
            <OnboardingState
              ...
              primaryLabel={
                canRequest ? `Request ${REQUEST_TYPE_LABEL[ROLE_REQUEST[role]]}` : undefined
              }
    ...
    <div className="min-h-screen pb-16">
            ...
            <div className="-mt-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex items-end gap-4">
                <img
                  ...
                  className="h-20 w-20 rounded-2xl border-2 border-[#0A0A0F] object-cover shadow-lg"
                />
                    ...
                    <BadgeRow
                      ...
                      badges={
                        [
                          ...
                          ...(role === "dj" && owner.verified
                            ? [{ kind: "verified_creator" as const }]
                            : []),
              ...
              <div className="flex flex-wrap items-center gap-2">
                <SecondaryButton className="px-4 py-2.5 text-[11px]">
          ...
          <GlassCard className="p-4">
            <div className="mb-3 flex items-center gap-2 text-sm font-bold text-white">
              ...
              <span className="ml-auto inline-flex items-center gap-1 text-[11px] text-cyan-200/70">
                <a
                  href={owner.treyTvProfileHref}
                  className="inline-flex items-center gap-1 hover:text-cyan-200"
                >
                  Trey TV profile <ExternalLink className="h-3 w-3" />
                </a>
              </span>
            </div>
            <p className="mb-3 text-[11px] leading-relaxed text-white/50">
              This {owner.roleLabel} profile is an extension of your Trey TV identity — not a
              separate account.
            </p>
          ...
          <GlassCard glow className="p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-sm font-black text-white">
                  Finish your {owner.roleLabel} profile
                </div>
                <p className="mt-1 text-xs text-white/55">
                  Complete these to publish a public-ready profile.
                </p>
              </div>
              <div className="text-right">
                <div className="text-2xl font-black text-fuchsia-300">
                  {activation.completion.percent}%
                </div>
                <div className="text-[10px] uppercase tracking-wider text-white/40">
                  {activation.completion.completed}/{activation.completion.total} done
                </div>
              </div>
            </div>
            <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
              <div
                ...
                style={{ width: `${activation.completion.percent}%` }}
              />
            </div>
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {activation.completion.items.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-xs ${item.complete ? "border-emerald-300/20 bg-emerald-500/5 text-emerald-100" : "border-white/8 bg-white/[0.02] text-white/55"}`}
                >
                  <span
                    className={`flex h-4 w-4 items-center justify-center rounded-full text-[9px] ${item.complete ? "bg-emerald-400/30 text-emerald-100" : "border border-white/20 text-transparent"}`}
                  >
                    ✓
                  </span>
                  {item.label}
        ...
        <div className="mt-4 px-4 sm:px-6 lg:px-10">
          <GlassCard className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2 text-sm font-bold text-white">
              <Radio className="h-4 w-4 text-purple-300" /> Live broadcast features need Broadcast
              Access
            </div>
            <SecondaryButton
              className="px-4 py-2.5 text-[11px]"
              onClick={() => access?.openFlow("broadcast")}
            >
              Apply for Broadcast Access
            </SecondaryButton>
          </GlassCard>
        </div>
      )}
      ...
      {profileRecord?.artist_pick_title && (
        <div className="mt-6 px-4 sm:px-6 lg:px-10 animate-fade-in">
          <div className="mb-3 text-sm font-bold uppercase tracking-[0.15em] text-white/45 flex items-center gap-1.5">
            ...
            {role === "artist" ? "Artist" : role === "producer" ? "Producer" : "Host"}'s Featured
            Pick
          </div>
          <GlassCard
            glow
            className="p-5 flex flex-col md:flex-row items-start md:items-center gap-5 bg-gradient-to-r from-purple-950/10 via-black/40 to-cyan-950/10 border-white/10 relative overflow-hidden group"
          >
            {/* Soft backdrop glow */}
            ...
            <div className="relative h-24 w-24 rounded-2xl overflow-hidden border border-white/10 shadow-md shrink-0 bg-white/5">
              {profileRecord.artist_pick_image ? (
                <img
                  src={profileRecord.artist_pick_image}
                  alt="Featured Pick"
              ...
              <div className="absolute bottom-1.5 left-1.5 rounded-md bg-black/75 border border-white/10 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-widest text-primary">
                {profileRecord.artist_pick_type === "track"
                  ? "Release"
                  : profileRecord.artist_pick_type === "beat"
                    ? "Beat"
                    : profileRecord.artist_pick_type === "show"
                      ? "Show"
                      : "Album"}
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0 space-y-1.5">
              ...
              <div className="pt-2 md:pt-0">
                <h3 className="text-lg font-black tracking-tight text-white group-hover:text-primary transition-colors truncate">
                  {profileRecord.artist_pick_title}
                </h3>
                <p className="text-xs text-white/50 leading-relaxed mt-0.5">
                  Featured {profileRecord.artist_pick_type || "release"} curated directly by the
                  creator.
                </p>
              </div>
            </div>
      ...
      <div className="mt-6 px-4 sm:px-6 lg:px-10">
        <div className="mb-3 text-lg font-bold text-white">
          {isOwner ? "Your modules" : `${owner.display_name}'s profile`}
        </div>
            ...
            return (
              <GlassCard key={section.id} className="p-4">
                <div className="flex items-start justify-between gap-2">
                  ...
                  {section.ownerOnly && (
                    <span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-white/40">
                      Owner
                    </span>
                  )}
                ...
                {broadcastLocked && (
                  <p className="mt-2 text-[10px] font-bold uppercase tracking-wider text-amber-200/70">
      ...
      {isOwner && (
        <div className="mt-6 px-4 sm:px-6 lg:px-10">
          <div className="mb-3 flex items-center gap-2 text-lg font-bold text-white">
            <BarChart3 className="h-4 w-4 text-cyan-300" /> Analytics preview
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              ["Followers", "34.2K", "+6%"],
              [
                role === "producer" ? "Beat leases" : role === "dj" ? "Tune-ins" : "Saves",
                "18.9K",
                "+9%",
              ],
        ...
        <PrescriptionRail
          title={
            role === "artist"
              ...
              : role === "producer"
                ? "Prescribe artists for my beats"
      ...
      <div className="mt-6 px-4 sm:px-6 lg:px-10">
        <GlassCard className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-white/55">
              Open the polished, fan-facing {owner.roleLabel} page.
            </p>
          </div>
          <div className="flex gap-2">
            {role === "dj" && (
              <SecondaryButton className="px-4 py-2.5 text-[11px]" onClick={onOpenBroadcastStudio}>
                <Radio className="h-3.5 w-3.5" /> Broadcast Studio
              </SecondaryButton>
            )}
            <PrimaryButton
              className="px-4 py-2.5 text-[11px]"
              onClick={() => onOpenPublicShell?.(role)}
            >
