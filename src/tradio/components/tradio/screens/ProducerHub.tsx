...
import { Activity, ArrowRight, Disc3, Handshake, Package, Radio, Send, Upload, Users, Sparkles, Volume2 } from 'lucide-react';
import { TopBar, GlassCard, PrimaryButton, SecondaryButton, Chip, BeatCard, SegmentedTabs, VerifiedBadge } from '../ui';
import { ARTIST_MATCH_SUGGESTIONS, BEAT_PACKS, BEATS, COLLABORATION_REQUESTS, DJ_OPPORTUNITIES, PRODUCERS } from '../data';
...
export const ProducerHub: React.FC<{ onOpenBroadcastStudio?: (initialTab?: string) => void; onViewPublicProfile?: () => void; onEditProfile?: () => void }> = ({ onOpenBroadcastStudio, onViewPublicProfile, onEditProfile }) => {
  ...
  const [legalValues, setLegalValues] = useState<LegalAcceptanceValues>(() => createLegalAcceptanceValues('producer_beat_publish'));
  ...
  const handleBeatLegalAction = async (action: string) => {
    ...
    setLegalMessage(result.source === 'supabase' ? 'Beat rights confirmation saved.' : result.warning);
  };
    ...
    <div className="space-y-8 pb-4 lg:space-y-10">
      ...
      {(onEditProfile || onViewPublicProfile) && (
        <div className="flex flex-wrap gap-2 px-4 sm:px-6 lg:px-10">
          {onEditProfile && <SecondaryButton className="px-4 py-2.5 text-[11px]" onClick={onEditProfile}>Edit Producer Profile</SecondaryButton>}
          {onViewPublicProfile && <SecondaryButton className="px-4 py-2.5 text-[11px]" onClick={onViewPublicProfile}>View Public Producer Profile</SecondaryButton>}
        </div>
      )}
        ...
        <GlassCard glow className="overflow-hidden p-5">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex gap-4">
              <img src={currentProducer.avatar} alt={currentProducer.name} className="h-24 w-24 rounded-2xl border border-white/15 object-cover" />
              <div className="min-w-0">
                <Chip label="Producer Hub" selected icon={<Disc3 className="h-3.5 w-3.5" />} />
                <div className="mt-3 flex items-center gap-2">
                  <h1 className="text-3xl font-black tracking-tight text-white">{currentProducer.name}</h1>
                  {currentProducer.verified && <VerifiedBadge />}
                </div>
                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/70">{currentProducer.bio}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {currentProducer.specialties.map((tag) => <Chip key={tag} label={tag} />)}
                </div>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2 lg:w-[360px]">
              {[
                ...
                <div key={label} className="rounded-2xl border border-white/8 bg-white/[0.03] p-3 text-center">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <PrimaryButton disabled={!legalAccepted || legalStatus === 'saving'} onClick={() => handleBeatLegalAction('upload_beat')} className={legalAccepted ? '' : 'pointer-events-none opacity-40'}><Upload className="h-4 w-4" /> Upload Beat</PrimaryButton>
            <SecondaryButton disabled={!legalAccepted || legalStatus === 'saving'} onClick={() => handleBeatLegalAction('pitch_to_artist')} className={legalAccepted ? '' : 'pointer-events-none opacity-40'}><Send className="h-4 w-4" /> Pitch Beat to Artist</SecondaryButton>
            <SecondaryButton disabled={!legalAccepted || legalStatus === 'saving'} onClick={() => handleBeatLegalAction('send_to_dj_mix')} className={legalAccepted ? '' : 'pointer-events-none opacity-40'}><Radio className="h-4 w-4" /> Send to DJ Mix</SecondaryButton>
            <SecondaryButton><Activity className="h-4 w-4" /> Create Producer Station</SecondaryButton>
          </div>
        </GlassCard>
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-10 grid grid-cols-2 gap-3">
        <SecondaryButton onClick={() => onOpenBroadcastStudio?.('builder')} className="py-3 text-xs font-bold"><Sparkles className="h-3.5 w-3.5 text-purple-300" /> Create Producer Spotlight</SecondaryButton>
        <SecondaryButton onClick={() => onOpenBroadcastStudio?.('builder')} className="py-3 text-xs font-bold"><Volume2 className="h-3.5 w-3.5 text-cyan-300" /> Submit Beats To A Show</SecondaryButton>
      </div>
      ...
      {tab === 'beats' && (
        <div className="px-4 sm:px-6 lg:px-10">
          <ContentFeelAnalysisPanel profile={beatContentFeel.profile} status={beatContentFeel.status} onRun={beatContentFeel.run} />
        </div>
      )}
        ...
        <div className="grid gap-3 px-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-3 lg:px-10">
          {BEATS.map((beat) => (
            <BeatCard
              ...
              mood={beat.mood}
              plays={beat.plays}
              onClick={() => playItem(beatToPlaybackItem(beat), {
                source: {
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-white/50">{pack.beatCount} beats - {pack.theme}</div>
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between text-xs">
                    <span className="text-white/50">{pack.downloads.toLocaleString()} downloads</span>
          ...
          <div className="space-y-3">
            {COLLABORATION_REQUESTS.map((request) => (
              <GlassCard key={request.id} className="p-4">
                <div className="flex gap-3">
                  <img src={request.from.avatar} alt="" className="h-12 w-12 rounded-full object-cover" />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <div className="font-semibold text-white">{request.from.name}</div>
                      <span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-1 text-[10px] uppercase text-white/60">{request.status}</span>
                    </div>
                    <div className="text-xs capitalize text-purple-300">{request.type} request</div>
                    <p className="mt-2 text-sm leading-relaxed text-white/70">{request.message}</p>
                    <div className="mt-3 flex gap-2">
                      <SecondaryButton className="px-3 py-2 text-xs"><Handshake className="h-3.5 w-3.5" /> Accept</SecondaryButton>
                      <SecondaryButton className="px-3 py-2 text-xs"><Send className="h-3.5 w-3.5" /> Send Beat Pack</SecondaryButton>
                    </div>
                  </div>
                </div>
              </GlassCard>
          ...
          <GlassCard className="p-4">
            ...
            <div className="space-y-2">
              {DJ_OPPORTUNITIES.map((opportunity) => (
                <div key={opportunity.id} className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
                  <div className="text-sm font-semibold text-white">{opportunity.show}</div>
                  <div className="text-xs text-white/50">{opportunity.dj} - {opportunity.station}</div>
                  <div className="mt-2 flex items-center justify-between text-xs">
                    ...
                    <span className="text-white/50">{(opportunity.audience / 1000).toFixed(1)}K est.</span>
                  </div>
                </div>
              ))}
            </div>
