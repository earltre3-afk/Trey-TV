...
import type { CreatorProfileSpecificDraft, CreatorProfileValidationResult } from '../auth/creatorProfileTypes';

const GENRE_OPTIONS = ['Soul', 'Trap', 'Jazz Rap', 'R&B', 'Lo-Fi', 'Hip Hop', 'Phonk', 'Boom Bap', 'Cloud Rap'];
const MOOD_OPTIONS = ['Chill', 'Energetic', 'Focused', 'Emotional', 'Hype', 'Dark', 'Moody', 'Uplifting'];

const SPECIALTY_OPTIONS: Record<RoleProfileType, string[]> = {
  artist: ['Vocalist', 'Lyricist', 'Songwriter', 'Performer', 'Recording Artist'],
  producer: ['Mixing', 'Mastering', 'Sound Design', 'Sample Maker', 'Drum Programming', 'Arrangement'],
  dj: ['Radio Host', 'Turntablist', 'Club DJ', 'Curator', 'Live Host', 'Mix Engineer'],
};

const SHOW_TYPE_OPTIONS = ['Live Request Hour', 'Late Night Vibe', 'Weekly Countdown', 'Artist Spotlight', 'Producer Beat Clash', 'Underground Discovery'];
...
export const RoleProfileEditor: React.FC<RoleProfileEditorProps> = ({ role, identity, onClose }) => {
  // Fetch initial draft from persistence
  const [draft, setDraft] = useState<CreatorProfileSpecificDraft>(() => getCreatorProfileDraft(role, identity));
  const [validation, setValidation] = useState<CreatorProfileValidationResult>(() => validateCreatorProfileDraft(role, draft));
  ...
  const [legalValues, setLegalValues] = useState<LegalAcceptanceValues>(() => createLegalAcceptanceValues('creator_profile_publish'));
  ...
  const handlePublish = async () => {
    ...
    setLegalMessage(result.source === 'supabase' ? 'Creator profile acknowledgement saved.' : result.warning);
    ...
    return (
      <div className="fixed inset-0 z-50 bg-[#08060d] flex flex-col items-center justify-center text-center p-6 select-none">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center max-w-md">
          ...
          <h2 className="text-xl font-mono font-black uppercase tracking-[0.25em] text-white">Synthesizing Profile</h2>
  ...
  if (isPublishedSuccess) {
    return (
      <div className="fixed inset-0 z-50 bg-[#08060d] flex flex-col items-center justify-center p-6 text-center">
        ...
        <GlassCard glow className="max-w-md w-full p-8 space-y-6 relative border border-white/10 bg-black/80">
          <div className="flex flex-col items-center gap-3">
            ...
            <h2 className="text-2xl font-black text-white tracking-tight mt-2">Profile Published!</h2>
            <p className="text-xs text-white/65 max-w-sm leading-relaxed">
              Congratulations! Your official <strong className="text-purple-300 font-semibold">{roleLabel}</strong> blueprint is now active, indexed, and available for public discovery.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-white/8 bg-[#0d0914] p-4 text-left font-mono text-xs text-purple-200/90 space-y-2">
            <div className="flex items-center justify-between border-b border-white/5 pb-2">
              <span className="text-white/45 uppercase text-[9px] tracking-widest font-black">Role Active</span>
              <span className="text-emerald-400 font-bold uppercase text-[9px] tracking-wider">LIVE</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-white/45">Display Name:</span>
              <span className="text-white font-bold">{role === 'artist' ? draft.data.artistName : role === 'producer' ? draft.data.producerName : draft.data.djName}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-white/45">Public URL:</span>
              <span className="text-cyan-300 truncate pl-4">/tradio/{role}/{identity.public_profile_uid}</span>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#06040a] text-white flex flex-col pb-12 select-none relative">
      ...
      <div className="flex items-center justify-between px-4 pt-[max(1.5rem,env(safe-area-inset-top))] pb-4 border-b border-white/10 sm:px-6 lg:px-10 relative z-10 bg-black/40 backdrop-blur-xl">
        <button onClick={handleCloseAttempt} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/12 bg-white/5 text-white/70 hover:text-white hover:border-white/25 active:scale-95 transition-all">
          <ChevronLeft className="h-4 w-4" />
        </button>
        <div>
          <div className="text-sm font-mono font-black uppercase tracking-[0.2em] text-white">Setup {roleLabel} Profile</div>
          <div className="text-[10px] text-white/40 text-center mt-0.5">Tuning your creative blueprint</div>
        </div>
          ...
          <div className="flex rounded-xl bg-white/5 border border-white/10 p-1">
            <button
              ...
              className={`flex-1 py-2 text-center text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                activeTab === 'info' ? 'bg-white/10 text-white shadow-md' : 'text-white/55 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2 text-center text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                activeTab === 'sound' ? 'bg-white/10 text-white shadow-md' : 'text-white/55 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2 text-center text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                activeTab === 'role' ? 'bg-white/10 text-white shadow-md' : 'text-white/55 hover:text-white'
              }`}
            >
          ...
          <GlassCard className="p-6 space-y-5">
            ...
            {activeTab === 'info' && (
              <div className="space-y-4">
                <h3 className="text-xs font-mono font-black text-purple-300 uppercase tracking-widest pl-1 border-b border-white/5 pb-2">Profile Credentials</h3>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    value={role === 'artist' ? draft.data.artistName : role === 'producer' ? draft.data.producerName : draft.data.djName}
                    onChange={(e) => updateField(role === 'artist' ? 'artistName' : role === 'producer' ? 'producerName' : 'djName', e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">City</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Region / State</label>
                    <input
                      type="text"
                ...
                <div className="space-y-3.5 border-t border-white/5 pt-4">
                  <h4 className="text-[10px] font-mono font-black text-white/40 uppercase tracking-wider pl-1">Media Visuals</h4>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Profile Avatar Image Link</label>
                      <div className="relative">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Profile Banner Image Link</label>
                      <div className="relative">
                        <ImageIcon className="absolute left-3.5 top-2.5 h-3.5 w-3.5 text-white/30" />
                        <input
                          type="text"
            ...
            {activeTab === 'sound' && (
              <div className="space-y-5">
                <h3 className="text-xs font-mono font-black text-purple-300 uppercase tracking-widest pl-1 border-b border-white/5 pb-2">Acoustic Signatures</h3>
                
                {/* Genres selection */}
                <div className="space-y-2.5">
                  <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">
                    Core Sound Genres <span className="text-fuchsia-400">* (Select at least one)</span>
                  </label>
                ...
                {role === 'producer' && (
                  <div className="space-y-2.5 border-t border-white/5 pt-4">
                    <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">
                      Acoustic Mood Profile <span className="text-fuchsia-400">* (At least one mood or specialty required)</span>
                    </label>
                    <div className="flex flex-wrap gap-2">
                ...
                <div className="space-y-2.5 border-t border-white/5 pt-4">
                  <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">
                    Creative Specialties {role === 'producer' && <span className="text-fuchsia-400">*</span>}
                  </label>
                  <div className="flex flex-wrap gap-2">
            ...
            {activeTab === 'role' && (
              <div className="space-y-5">
                <h3 className="text-xs font-mono font-black text-purple-300 uppercase tracking-widest pl-1 border-b border-white/5 pb-2">Role Blueprint Focus</h3>

                {/* ARTIST SPECIFIC FIELDS */}
                {role === 'artist' && (
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Primary Release Goal</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-3.5 border-t border-white/5 pt-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-[10px] text-white/40 mt-0.5">Let systems know if your creations include explicit lyrics.</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => updateField('explicitComfort', !draft.data.explicitComfort)}
                          className={`w-11 h-6 rounded-full border transition-all flex items-center px-0.5 ${
                            draft.data.explicitComfort ? 'bg-purple-500/25 border-purple-500/50 justify-end' : 'bg-black/40 border-white/10 justify-start'
                          }`}
                        >
                          <span className={`w-4 h-4 rounded-full transition-all ${draft.data.explicitComfort ? 'bg-purple-300 shadow-[0_0_6px_#c084fc]' : 'bg-white/30'}`} />
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between border-t border-white/5 pt-3.5">
                        <div>
                          ...
                          <p className="text-[10px] text-white/40 mt-0.5">Publish a private social room directly on your public feed.</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => updateField('fanCommunityEnabled', !draft.data.fanCommunityEnabled)}
                          className={`w-11 h-6 rounded-full border transition-all flex items-center px-0.5 ${
                            draft.data.fanCommunityEnabled ? 'bg-purple-500/25 border-purple-500/50 justify-end' : 'bg-black/40 border-white/10 justify-start'
                          }`}
                        >
                          <span className={`w-4 h-4 rounded-full transition-all ${draft.data.fanCommunityEnabled ? 'bg-purple-300 shadow-[0_0_6px_#c084fc]' : 'bg-white/30'}`} />
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                {role === 'producer' && (
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Beat Licensing Default</label>
                      <select
                    ...
                    <div className="space-y-1.5 border-t border-white/5 pt-4">
                      <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Collaboration Availability</label>
                      <input
                        type="text"
                ...
                {role === 'dj' && (
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2.5">
                      <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">
                        Broadcast Show Concepts <span className="text-fuchsia-400">* (Select at least one)</span>
                      </label>
                      <div className="flex flex-wrap gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-4">
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Schedule Goal</label>
                        <input
                          type="text"
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest pl-1 block">Listener Request Rules</label>
                        <select
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="flex items-center justify-between border-t border-white/5 pt-3.5">
                        <div>
                          ...
                          <p className="text-[10px] text-white/40 mt-0.5">Automatically save live mixes into public archives.</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => updateField('replayArchiveEnabled', !draft.data.replayArchiveEnabled)}
                          className={`w-11 h-6 rounded-full border transition-all flex items-center px-0.5 ${
                            draft.data.replayArchiveEnabled ? 'bg-purple-500/25 border-purple-500/50 justify-end' : 'bg-black/40 border-white/10 justify-start'
                          }`}
                        >
                          <span className={`w-4 h-4 rounded-full transition-all ${draft.data.replayArchiveEnabled ? 'bg-purple-300 shadow-[0_0_6px_#c084fc]' : 'bg-white/30'}`} />
                        </button>
                      </div>

                      <div className="flex items-center justify-between border-t border-white/5 pt-3.5">
                        <div>
                          <h4 className="text-xs font-bold text-white">Broadcast Code of Conduct <span className="text-fuchsia-400">*</span></h4>
                          <p className="text-[10px] text-white/40 mt-0.5">I agree to follow the platform guidelines and host responsibly.</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => updateField('moderationAgreement', !draft.data.moderationAgreement)}
                          className={`w-11 h-6 rounded-full border transition-all flex items-center px-0.5 ${
                            draft.data.moderationAgreement ? 'bg-purple-500/25 border-purple-500/50 justify-end' : 'bg-black/40 border-white/10 justify-start'
                          }`}
                        >
                          <span className={`w-4 h-4 rounded-full transition-all ${draft.data.moderationAgreement ? 'bg-purple-300 shadow-[0_0_6px_#c084fc]' : 'bg-white/30'}`} />
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          <GlassCard glow className="p-5 border border-white/10 bg-black/60 relative">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div>
                ...
                <p className="text-[10px] text-white/45 mt-0.5">Pre-publish checklist diagnostics</p>
              </div>
              <div className="text-right">
                <span className="text-xl font-mono font-black text-purple-300">{validation.checklist.percentComplete}%</span>
                <span className="text-[9px] uppercase tracking-wider text-white/45 block mt-0.5">Ready</span>
              </div>
            </div>

            {/* Progress bar */}
            <div className="mt-3.5 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-gradient-to-r from-purple-500 via-fuchsia-500 to-cyan-400 transition-all duration-500" style={{ width: `${validation.checklist.percentComplete}%` }} />
            </div>

            {/* Checklist diagnostics */}
            <div className="mt-4.5 space-y-2.5">
              <span className="text-[9px] font-mono font-black uppercase tracking-wider text-white/30 block">Required Blueprint Fields:</span>
              
              {/* Required list */}
              <div className="space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                {validation.checklist.required.map((issue, idx) => (
                  <div key={idx} className="flex items-start gap-2.5 rounded-xl border border-rose-500/10 bg-rose-500/5 p-2.5 text-[11px] leading-relaxed text-rose-200">
                    <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-rose-500/25 text-[9px] font-mono">!</span>
                    <p className="flex-1">Missing <span className="font-bold underline">{issue.field}</span>: try {issue.fixedBy || 'entering details'}.</p>
                  </div>
                ))}
              ...
              {validation.checklist.optional.length > 0 && (
                <div className="space-y-1.5 mt-3.5 pt-3 border-t border-white/5">
                  <span className="text-[9px] font-mono font-black uppercase tracking-wider text-white/30 block">Optional Enhancements:</span>
                  <div className="space-y-1.5 max-h-[120px] overflow-y-auto pr-1">
                    {validation.checklist.optional.map((issue, idx) => (
                      <div key={idx} className="flex items-start gap-2.5 rounded-xl border border-amber-500/10 bg-amber-500/5 p-2.5 text-[10px] leading-relaxed text-amber-200">
                        <AlertCircle className="h-3.5 w-3.5 text-amber-300 shrink-0 mt-0.5" />
                        <p className="flex-1">{issue.message}. Fix by {issue.fixedBy || 'filling'}.</p>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {saveToast && (
        <div className="fixed bottom-6 right-6 z-50 rounded-2xl border border-emerald-500/30 bg-emerald-950/80 px-4 py-3 backdrop-blur-md flex items-center gap-2.5 text-emerald-100 shadow-[0_10px_30px_rgba(0,0,0,0.5)] animate-slide-up-modal font-mono text-xs">
          <CheckCircle className="h-4 w-4 text-emerald-400 animate-pulse" /> Draft saved successfully!
        </div>
      )}
      ...
      {showUnsavedWarning && (
        <>
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm cursor-default" onClick={() => setShowUnsavedWarning(false)} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-51 max-w-sm w-full p-6 rounded-3xl border border-white/12 bg-[#0c0a12] shadow-premium text-center space-y-4 animate-scale-up">
            ...
            <div>
              ...
              <p className="text-xs text-white/55 mt-1.5 leading-relaxed">You have unsaved edits in your blueprint session. Leaving now will wipe these temporary modifications.</p>
            </div>
