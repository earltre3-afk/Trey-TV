...
const SHOW_TYPES: BroadcastShowType[] = [
  {
    ...
    description: 'Host your own official broadcast lane. Blend tracks, share exclusive commentary, and talk directly to listeners.',
  ...
  },
  {
    ...
    description: 'Coordinate a high-status first-listen drop. Invite fan chats, queue stems, and bundle pre-saves in real-time.',
    allowedRole: 'Artist / Admin',
    setupTime: '5-10 mins',
  ...
  },
  {
    id: 'dj-mix',
    title: 'DJ Live Mix',
    description: 'Stream structured DJ sets, beat blends, and live request blocks with animated interactive VU level feedback.',
    allowedRole: 'DJ / Admin',
    setupTime: '15-20 mins',
    tags: ['Live', 'Replayable'],
    icon: <Disc className="h-5 w-5 text-cyan-300 animate-spin" style={{ animationDuration: '4s' }} />,
  },
  {
    id: 'producer-spotlight',
    title: 'Producer Spotlight',
    description: 'Upload loops or full beats. Invite artists to pitch vocals or save stems in a themed beat showcase session.',
    allowedRole: 'Producer / Admin',
    setupTime: '10 mins',
    tags: ['Scheduled', 'Replayable'],
    icon: <Sparkles className="h-5 w-5 text-purple-300 animate-bounce" style={{ animationDuration: '3s' }} />,
  },
  {
    id: 'fan-request',
    title: 'Fan Request Hour',
    description: 'Unlock crowdsourced radio segments. Fans submit requests and vote on live chat queues dynamically.',
    allowedRole: 'DJ / Admin',
    setupTime: '5 mins',
    tags: ['Live Only'],
  ...
  },
  {
    ...
    description: 'Set round constraints, introduce competing artists, preview battle tracks, and trigger fan voting.',
...
const INITIAL_DRAFTS: BroadcastDraft[] = [
  { id: 'dr-1', title: 'Late Night Chill with Jordan', type: 'Artist Radio Show', lastEdited: '2 hours ago', completion: 75 },
  { id: 'dr-2', title: '6AM Thoughts Premiere Block', type: 'Release Premiere', lastEdited: 'Yesterday', completion: 40 },
];

const INITIAL_SCHEDULED: ScheduledBroadcast[] = [
  { id: 'sch-1', dateTime: 'Tonight at 9:00 PM', station: 'Trey Trizzy Radio', type: 'Artist Radio Show', host: 'Trey Trizzy', status: 'scheduled' },
  { id: 'sch-2', dateTime: 'Tomorrow at 4:00 PM', station: 'After Dark R&B', type: 'DJ Live Mix', host: 'Mila Rain', status: 'scheduled' },
  { id: 'sch-3', dateTime: 'Live Now', station: 'Trey TV Main Layer', type: 'Song Wars Pre-Show', host: 'Jordan Host', status: 'live' },
];

const inputClass = 'w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/35 focus:border-cyan-400/40';
...
export const BroadcastStudioGateway: React.FC<Props> = ({ onBack, initialTab }) => {
  ...
  // Structures List
  const [seqSegments, setSeqSegments] = useState<string[]>(['intro', 'music-block', 'host-talk', 'outro']);
  // Selected Music
  const [addedMusic, setAddedMusic] = useState<string[]>(['Midnight Velvet', '6AM Thoughts']);
  // Selected Interactions
  const [addedInteractions, setAddedInteractions] = useState<string[]>(['Live Chat', 'Emoji Reactions']);
  ...
  const [broadcastLegalValues, setBroadcastLegalValues] = useState<LegalAcceptanceValues>(() => createLegalAcceptanceValues('dj_broadcast_schedule'));
  const [broadcastLegalStatus, setBroadcastLegalStatus] = useState<'idle' | 'saving' | 'saved' | 'fallback' | 'error'>('idle');
  const [broadcastLegalMessage, setBroadcastLegalMessage] = useState<string | null>(null);
  const [accessLegalValues, setAccessLegalValues] = useState<LegalAcceptanceValues>(() => createLegalAcceptanceValues('role_access_request'));
  const [accessLegalStatus, setAccessLegalStatus] = useState<'idle' | 'saving' | 'saved' | 'fallback' | 'error'>('idle');
  ...
  const availableSegments = [
    { id: 'intro', label: 'Opening Intro Script', duration: 180, icon: <Mic2 className="h-4 w-4" /> },
    { id: 'music-block', label: 'Premium Music Block', duration: 480, icon: <Music className="h-4 w-4" /> },
    { id: 'host-talk', label: 'Host Commentary Talk', duration: 120, icon: <Mic2 className="h-4 w-4" /> },
    { id: 'producer-spotlight', label: 'Producer Spotlight Show', duration: 240, icon: <Sparkles className="h-4 w-4" /> },
    { id: 'artist-premiere', label: 'Artist Premiere Hour', duration: 300, icon: <Radio className="h-4 w-4" /> },
    { id: 'fan-request', label: 'Fan Request Segment', duration: 360, icon: <Users className="h-4 w-4" /> },
    { id: 'commercial', label: 'Ad/Commercial Break', duration: 60, icon: <Megaphone className="h-4 w-4" /> },
    { id: 'outro', label: 'Closing Message Outro', duration: 180, icon: <Calendar className="h-4 w-4" /> }
  ];

  const availableMusic = ['Midnight Velvet', '6AM Thoughts', 'Persuasion', 'City Lights', 'After Hours', 'Neon Heartbreak', 'Producer Beat #1', 'Memphis Drum Loop'];
  const availableInteractions = ['Live Chat', 'Fan Audio Requests', 'Live Audience Polls', 'Real-time Emoji Reactions', 'GIF Chat via Giphy', 'Top Fan Spotlight', 'Tracklist Voting'];
  ...
  const handleSubmitApplication = async (e: React.FormEvent) => {
    ...
    setAccessLegalMessage(result.source === 'supabase' ? 'Access acknowledgement saved.' : result.warning);
  ...
  const handleCompleteShowBuild = async () => {
    ...
    setBroadcastLegalMessage(result.source === 'supabase' ? 'Broadcast acknowledgement saved.' : result.warning);
    ...
    <div className="space-y-8 pb-12 lg:space-y-10">
      ...
      {activeLiveShow ? (
        ...
        <div className="px-4 sm:px-6 lg:px-10 space-y-8">
          ...
          <GlassCard glow className="relative rounded-[2.5rem] border-[0.5px] border-purple-500/15 bg-gradient-to-br from-[#1A0C2B]/55 via-[#060412]/80 to-black/95 p-8 md:p-12 overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.8)]">
            ...
            <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-2xl">
                ...
                <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight leading-none">
                  Tradio <span className="bg-gradient-to-r from-fuchsia-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent">Broadcast Suite</span>
                </h1>
                <p className="mt-4 text-sm md:text-base text-white/60 leading-relaxed font-medium">
                  "You are not just creating a post. You are programming a music experience." Step into our private, high-status broadcast gateway and control room.
                </p>
              ...
              <GlassCard className="w-full lg:w-[320px] p-5 border-white/10 bg-black/40 backdrop-blur-2xl relative shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_15px_30px_rgba(0,0,0,0.5)] flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <img src={ACTIVE_USER.avatar} alt="" className="h-10 w-10 rounded-xl object-cover border border-white/15" />
                    <div>
                  ...
                  {role !== 'fan' && <span className="h-2 w-2 rounded-full bg-emerald-400 animate-ping" />}
                </div>
                ...
                <div className="pt-3 border-t border-white/5 space-y-2">
                  ...
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className={`font-semibold rounded-full px-2 py-0.5 text-[9px] font-mono tracking-wider ${
                      accessStatus === 'Cleared' ? 'bg-emerald-400/10 text-emerald-300 border border-emerald-400/20' : 'bg-yellow-400/10 text-yellow-300 border border-yellow-400/20'
                    }`}>
                  ...
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-white/80 font-bold">{role === 'fan' ? 'Guest Only' : 'Official Tradio Host'}</span>
                  </div>
                </div>

                {role !== 'fan' && (
                  <div className="text-[10px] bg-cyan-400/10 border border-cyan-400/20 p-2 rounded-xl text-cyan-200 leading-snug">
                    <span className="font-bold">Cleared Categories:</span> {role === 'artist' ? 'Shows, Premieres, Listening Parties' : role === 'producer' ? 'Spotlights, Beat Battles' : 'Mixes, Replays, Request Hours'}
                  </div>
                )}
              </GlassCard>
          ...
          <div>
            <div className="mb-4">
              ...
              <p className="text-xs text-white/50">Each type has customized segment structures, timers, and widget presets.</p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              {SHOW_TYPES.map((t) => {
                const isLocked = role === 'fan' || (role === 'artist' && t.id === 'dj-mix') || (role === 'producer' && t.id === 'release-premiere');
                return (
                  <GlassCard key={t.id} className={`p-4 flex flex-col justify-between hover:-translate-y-1 hover:border-white/15 hover:bg-white/[0.02] transition-all duration-300 relative group/card ${isLocked ? 'opacity-50' : ''}`}>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-white/60 leading-relaxed min-h-[48px]">{t.description}</p>
                    </div>
                    ...
                    <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-[10px] font-mono font-bold text-purple-300">
                      ...
                      <span className="rounded-full bg-white/[0.04] border border-white/5 px-2 py-0.5 text-white/45">{t.allowedRole}</span>
                    </div>
          ...
          <GlassCard glow className="p-5 border border-cyan-500/10 bg-gradient-to-r from-[#0C1A2D]/40 to-transparent">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-base font-bold text-white flex items-center gap-1.5">Build With Tradio AI <Chip label="Premium" selected /></h4>
                  <p className="text-xs text-white/60 mt-1 max-w-xl leading-relaxed">
                    Let Tradio's generative music engine organize your segment timeline, compile recommended music tracks based on your mood, and write a ready-to-read AI speech teleprompter.
                  </p>
                </div>
              </div>
              <button
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-black text-white/45 uppercase tracking-widest font-mono">My Active Drafts</h3>
                {role !== 'fan' && (
                  <button onClick={handleCreateNewShow} className="text-xs text-purple-300 font-bold hover:text-white transition-all flex items-center gap-1">
              ...
              <div className="space-y-2.5">
                {drafts.map((d) => (
                  <GlassCard key={d.id} className="p-3.5 flex items-center justify-between gap-3 hover:border-white/10 transition-all">
                    <div className="min-w-0">
                      <div className="font-bold text-white text-sm truncate">{d.title}</div>
                      <div className="text-[10px] text-white/45 mt-0.5">{d.type} • Edited {d.lastEdited}</div>
                      
                      {/* Percent progress bar */}
                      <div className="flex items-center gap-2 mt-2">
                        <div className="h-1.5 w-24 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-purple-500" style={{ width: `${d.completion}%` }} />
                        </div>
                        <span className="text-[9px] font-mono text-purple-300 font-bold">{d.completion}%</span>
                      </div>
                    </div>
                    
                    <button
                      onClick={() => {
                        const matchedType = d.type === 'Artist Radio Show' ? 'artist-show' : 'release-premiere';
            ...
            {/* Scheduled Block */}
            <div className="space-y-3">
              <h3 className="text-sm font-black text-white/45 uppercase tracking-widest font-mono">My Upcoming Schedule</h3>
              <div className="space-y-2.5">
                {scheduled.map((s) => (
                  <GlassCard key={s.id} className="p-3.5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <div className="font-bold text-white text-sm truncate">{s.title || s.type}</div>
                        {s.status === 'live' && (
                      ...
                      <div className="text-[10px] text-white/45 mt-0.5">{s.dateTime} • {s.station}</div>
                      <div className="text-[10px] text-cyan-300 font-bold mt-1.5">Host: @{s.host}</div>
                    </div>
          ...
          {role === 'fan' && (
            <div id="apply-upgrade" className="pt-4 scroll-mt-24">
              <div className="mb-4">
                ...
                <p className="text-xs text-white/50">Apply for cleared broadcast rights inside the Trey TV music universe.</p>
              </div>
              <GlassCard className="p-5 border-white/10 bg-black/30">
                {applied ? (
                  <div className="py-8 text-center space-y-3">
                    ...
                    <h4 className="text-base font-black text-white">Cleared Application Submitted</h4>
                    <p className="text-xs text-white/50 max-w-sm mx-auto leading-relaxed">
                      Your request is currently being prioritized by station directors. Watch your notifications tab for official host credentials.
                    </p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmitApplication} className="space-y-4">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <Field label="Choose Broadcast Role">
                        <select className={inputClass} value={applyRole} onChange={(e) => setApplyRole(e.target.value as 'Artist' | 'Producer' | 'DJ' | 'Host')}>
                      ...
                      <Field label="Station / Artist Profile Link">
                        <input className={inputClass} value={applyLink} onChange={(e) => setApplyLink(e.target.value)} placeholder="e.g. tradio.fm/station/jordan" required />
                      </Field>
                    </div>
                    <Field label="Why do you want to host on Tradio?">
                      <textarea className={`${inputClass} h-24 resize-none`} value={applyMotivation} onChange={(e) => setApplyMotivation(e.target.value)} placeholder="Explain your show concept, genre focus, and release lineup plans..." required />
                    </Field>
                    ...
                    <div className="flex justify-end pt-2">
                      <button
                        ...
                        className={`rounded-full px-6 py-2.5 text-xs font-black uppercase tracking-widest transition-all active:scale-95 shadow-[0_0_20px_rgba(168,85,247,0.3)] ${
                          accessLegalAccepted ? 'bg-purple-500 hover:bg-purple-600 text-white' : 'border border-white/10 bg-white/5 text-white/25'
                        }`}
                      >
