...
export const DJStudio: React.FC<{
          ...
          <GlassCard glow className="overflow-hidden">
            <div className="relative min-h-[260px]">
              <img
                ...
                alt=""
                className="absolute right-0 top-0 h-full w-1/2 object-cover opacity-75"
              />
              ...
              <div className="relative p-5 sm:p-6">
                ...
                <div className="mt-5 flex max-w-[640px] gap-4">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-bold ${isLive ? "border-red-400/50 bg-red-500/15 text-red-200" : "border-white/10 bg-white/[0.04] text-white/55"}`}
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${isLive ? "animate-pulse bg-red-400" : "bg-white/30"}`}
                        />
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  ...
                  <SecondaryButton onClick={() => onOpenBroadcastStudio?.("builder")}>
                    <Radio className="h-4 w-4 animate-pulse text-purple-300" /> Open Broadcast
                    Studio
                  </SecondaryButton>
                  <SecondaryButton onClick={() => onOpenBroadcastStudio?.("builder")}>
                    <Sparkles className="h-4 w-4 text-cyan-300" /> Build Show With AI
                  </SecondaryButton>
                  <SecondaryButton
                    disabled={!legalAccepted || legalStatus === "saving"}
                    onClick={() => recordBroadcastLegal("schedule_mix")}
                    className={legalAccepted ? "" : "pointer-events-none opacity-40"}
                  >
                    <CalendarDays className="h-4 w-4" /> Schedule Mix
                  </SecondaryButton>
                </div>
              </div>
            </div>
                ...
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-white/50">
                      Music blocks, talk breaks, ads, and live controls
                    </div>
                  </div>
        ...
        {tab === "shows" && (
          <div className="px-4 sm:px-6 lg:px-10">
            <ContentFeelAnalysisPanel
              profile={showContentFeel.profile}
              status={showContentFeel.status}
              onRun={showContentFeel.run}
            />
          </div>
        )}
            ...
            {(myShows ?? RADIO_SHOWS).map((show) => (
              <GlassCard key={show.id} className="p-4">
                ...
                <p className="mt-3 text-sm leading-relaxed text-white/70">
                  For {show.targetAudience}, hosted in a {show.hostTone} tone with{" "}
                  {show.commercialBreaks} ad breaks.
                </p>
                <div className="mt-4 flex gap-2">
                  <SecondaryButton className="px-3 py-2 text-xs">
                    <ListMusic className="h-3.5 w-3.5" /> Edit Blocks
                  </SecondaryButton>
                  <SecondaryButton
                    className="px-3 py-2 text-xs"
                    onClick={() => onOpenBroadcastStudio?.("builder")}
                  >
                    <Sparkles className="h-3.5 w-3.5" /> Rebuild With AI
                  </SecondaryButton>
                </div>
              </GlassCard>
            ))}
                ...
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 text-xs text-white/50">
                      {(mix.duration / 60).toFixed(0)} min - {mix.genre} -{" "}
                    ...
                    <div className="mt-3 flex gap-2">
                      <SecondaryButton
                        className="px-3 py-2 text-xs"
                        onClick={() =>
                          playItem(djMixToPlaybackItem(mix), {
                            source: {
                              id: mix.id,
                              type: "dj_mix",
                              label: "DJ Mix",
                      ...
                      <SecondaryButton
                        ...
                        onClick={() =>
                          playQueue(
                            mix.tracklist.map((entry) => entry.track),
                            0,
                            {
                              id: `${mix.id}-tracklist`,
                              type: "dj_mix",
