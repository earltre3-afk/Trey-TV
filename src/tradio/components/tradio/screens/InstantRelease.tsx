...
const Step: React.FC<{ n: string; title: string; sub?: string; right?: React.ReactNode; children?: React.ReactNode }> = ({ n, title, sub, right, children }) => (
  <div>
    <div className="mb-2 flex items-start justify-between">
      <div>
        <div className="text-sm font-semibold text-white"><span className="text-white/50">{n}.</span> {title}</div>
...
const DATE_OPTIONS: { k: ReleaseDate; icon: React.ReactNode; title: string; sub: string }[] = [
  { k: 'tomorrow', icon: <Calendar className="h-4 w-4" />, title: 'Tomorrow', sub: 'Fri, May 24, 2025' },
...
const SCOPE_OPTIONS: { k: ReleaseScope; icon: React.ReactNode; title: string; sub: string }[] = [
  { k: 'first', icon: <Radio className="h-4 w-4" />, title: 'Tradio First', sub: 'Exclusive on Tradio. Build momentum' },
  { k: 'external', icon: <Globe className="h-4 w-4" />, title: 'External Later', sub: 'Release on other platforms in the future' },
];
  ...
  return (
    ...
    <div className="space-y-8 pb-4 lg:space-y-10">
      ...
      <div className="px-4 sm:px-6 lg:px-10">
        ...
        <p className="mt-1 text-sm text-white/60">Publish your music directly to Tradio in minutes.</p>
      </div>
        ...
        <GlassCard className="space-y-5 p-4">
          ...
          <Step n="2" title="Track Title">
            <div className="mt-1 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
              <input defaultValue="Falling For You" className="flex-1 bg-transparent text-sm text-white focus:outline-none" />
          ...
          <Step n="3" title="Artist Name">
            <div className="mt-1 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
              <input defaultValue="Mila Rain" className="flex-1 bg-transparent text-sm text-white focus:outline-none" />
                ...
                return (
                  <button
                    ...
                    className={`flex items-start gap-2 rounded-xl border p-3 text-left transition ${
                      sel ? 'border-purple-400/60 bg-purple-500/10' : 'border-white/10 bg-white/[0.03]'
                    }`}
                  >
          ...
          {[
            { n: '6', title: 'Explicit Content', sub: 'Contains explicit lyrics', val: explicit, set: setExplicit },
            { n: '7', title: 'Station Premiere', sub: 'Feature on Tradio AI Radio & editorial', val: premiere, set: setPremiere },
            { n: '8', title: 'Fan Notifications', sub: 'Notify followers when it goes live', val: notify, set: setNotify },
          ].map((row) => (
          ...
          <ContentFeelAnalysisPanel profile={contentFeel.profile} status={contentFeel.status} onRun={contentFeel.run} />
        ...
        <GlassCard glow className="p-4">
          ...
          <GlassCard className="mt-3 overflow-hidden p-3">
            ...
            <div className="mt-3 text-center">
              ...
              <Waveform className="mx-auto mt-3 h-5 w-40" bars={32} color="from-cyan-300 to-fuchsia-400" />
          ...
          <GlassCard className="mt-3 space-y-2 p-3.5 text-xs text-white/75">
            {[
              { icon: <Radio className="h-3.5 w-3.5 text-purple-300" />, t: 'Releases to your artist station' },
              { icon: <Music className="h-3.5 w-3.5 text-purple-300" />, t: 'Available on Tradio AI Radio' },
              { icon: <Heart className="h-3.5 w-3.5 text-purple-300" />, t: 'Fans can listen, like & share' },
              { icon: <ListMusic className="h-3.5 w-3.5 text-purple-300" />, t: 'Leaderboard & charts eligible' },
            ].map((r) => (
              <div key={r.t} className="flex items-center gap-2">{r.icon}{r.t}</div>
            ))}
          </GlassCard>
          <GlassCard className="mt-3 overflow-hidden p-3.5">
            ...
            <p className="mt-1 text-xs text-white/75">
              Enable <span className="font-semibold text-white">Station Premiere</span> to increase your reach and get featured on Tradio AI Radio.
            </p>
          </GlassCard>
        </GlassCard>
      </div>
