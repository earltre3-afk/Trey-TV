...
import type { PostShowAsset } from '@/lib/trey-i/broadcastPostShowTypes';
...
export const AdminPostShowReviewDashboard: React.FC<AdminPostShowReviewDashboardProps> = () => {
  ...
  const loadQueue = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await listPendingPostShowAssetsForReviewServer();
      if (result.error) {
        ...
        setSelectedAssetId((current) => current ?? result.assets[0]?.id ?? null);
      }
    } catch {
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {assets.length} generated or pending post-show assets awaiting review.
        </p>
      </div>
