...
const MESSAGES = [
  {
    ...
    time: "9:41 PM",
    msg: "This new track hits different live 🔥🔥🔥",
    reactions: "24",
    heart: "pink",
    img: IMG.kianaLane,
  },
  {
    ...
    time: "9:42 PM",
    msg: "Been here since the SoundCloud days. Proud of how far you've come, Trey! 🙌",
    reactions: "18",
    heart: "orange",
    img: IMG.noahKade,
  },
  {
    ...
    time: "9:43 PM",
    msg: 'Y\'all already know "Falling For You" goes crazy at 2AM 🕺',
    reactions: "16",
    heart: "pink",
    img: IMG.dariusCole,
  },
  {
    ...
    time: "9:44 PM",
    msg: "Drop that unreleased one at the end 🙏🙏",
    reactions: "9",
    heart: "orange",
    img: IMG.milaRain,
  },
    ...
    <div className="space-y-8 pb-4 lg:space-y-10">
        ...
        <div className="flex gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1 text-lg font-bold text-white">
              Trey Trizzy Radio Community <VerifiedBadge />
            </div>
            <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-white/70">
              <span className="inline-flex items-center gap-1">
                <Users className="h-3.5 w-3.5" /> 8,745 Members
              </span>
              <span className="inline-flex items-center gap-1">
                <Headphones className="h-3.5 w-3.5" /> 236 Listening Now
              </span>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/70">
        ...
        <div className="flex gap-6">
          {["Chat", "Polls", "Top Fans", "About"].map((t) => (
            ...
            >
              {t}
              {tab === t && (
                <span className="absolute inset-x-0 -bottom-px h-0.5 rounded-full bg-gradient-to-r from-fuchsia-400 to-purple-500" />
              )}
            </button>
          ))}
        </div>
            ...
            <div key={i} className="flex items-start gap-2.5">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span
                    className={`inline-flex items-center gap-0.5 rounded-full border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] ${m.badgeColor}`}
                  >
                ...
                <div className="mt-1 inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] text-white/70">
                  <span className={m.heart === "pink" ? "text-pink-400" : "text-orange-400"}>
                    {m.heart === "pink" ? "❤" : "🔥"}
                  </span>
                  {m.reactions}
                </div>
        ...
        <div className="space-y-3">
          ...
          <GlassCard className="p-3">
            ...
            <div className="text-sm font-semibold text-white">
              What song should open tonight's show?
            </div>
            <div className="mt-2 space-y-1.5">
              {POLL.map((p) => (
                <div key={p.label} className="relative overflow-hidden rounded-lg">
                  <div
                    className={`absolute inset-y-0 left-0 ${p.lead ? "bg-gradient-to-r from-fuchsia-500 to-purple-500" : "bg-white/8"}`}
                    style={{ width: `${p.pct}%` }}
                  />
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[10px] text-white/50">
                      {request.listenerName} requested {request.requestedAt}
                    </div>
                  </div>
                </div>
              </div>
            ...
            {FAN_VOTES.map((vote) => (
              <div key={vote.id} className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
                ...
                <div className="mt-1 text-[10px] text-white/50">
                  {vote.voteType} vote - {vote.timestamp}
                </div>
              </div>
            ))}
          </div>
        ...
        <GlassCard className="flex items-center gap-3 p-2.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1 text-sm font-semibold text-white">
              Trey Trizzy Radio <VerifiedBadge />
            </div>
            ...
            <div className="flex items-center gap-1.5 text-[10px]">
              ...
              <span className="rounded-full border border-pink-400/40 bg-pink-500/10 px-1.5 text-[9px] font-bold text-pink-300">
                LIVE
              </span>
            </div>
          </div>
          <button className="flex h-10 w-10 items-center justify-center rounded-full border border-purple-400/40 bg-purple-500/10 text-white">
            ⏮
          </button>
          <button className="flex h-12 w-12 items-center justify-center rounded-full border border-purple-400/40 bg-purple-500/20 text-white">
            ⏸
          </button>
          <button className="flex h-10 w-10 items-center justify-center rounded-full border border-purple-400/40 bg-purple-500/10 text-white">
            ⏭
      ...
      <div className="flex items-center gap-2 px-4 sm:px-6 lg:px-10">
        ...
        <div className="flex flex-1 items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2.5">
          ...
          <button className="rounded-md border border-white/10 px-1.5 py-0.5 text-[10px] font-bold text-white/60">
            GIF
          </button>
