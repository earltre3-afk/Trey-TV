  ...
  return (
    <div className="relative min-h-screen bg-[#0A0A0F] text-white pb-16 overflow-y-auto">
      ...
      <div className="relative h-[280px] sm:h-[350px] w-full overflow-hidden">
        <img src={p.coverUrl} alt="" className="absolute inset-0 h-full w-full object-cover scale-105 filter brightness-[0.4] blur-[2px]" />
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-10 -mt-24 sm:-mt-28 flex flex-col md:flex-row items-start md:items-end justify-between gap-6 pb-6 border-b border-white/5">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-end gap-5 text-center sm:text-left">
          ...
          <div className="relative h-28 w-28 sm:h-32 sm:w-32 rounded-3xl p-[3px] bg-gradient-to-tr from-fuchsia-500 via-purple-600 to-cyan-400 shadow-[0_15px_35px_rgba(168,85,247,0.35)] shrink-0">
            <img src={p.avatar} alt={p.name} className="h-full w-full rounded-[21px] object-cover border border-black/20" />
          ...
          <div className="space-y-1 sm:mb-2">
            <div className="flex items-center justify-center sm:justify-start gap-2">
              <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-white uppercase">{p.name}</h2>
              <VerifiedBadge />
            </div>
            <p className="text-xs sm:text-sm font-medium text-fuchsia-300 tracking-wide filter drop-shadow-[0_0_8px_rgba(217,70,239,0.3)]">{p.tagline}</p>
            
            <div className="mt-2 flex flex-wrap items-center justify-center sm:justify-start gap-4 text-xs text-white/50 font-mono">
              <span className="flex items-center gap-1.5"><Users className="h-3.5 w-3.5 text-cyan-400" /> {p.monthlyListeners || p.listeners} Monthly Listeners</span>
              <span className="flex items-center gap-1.5"><Heart className="h-3.5 w-3.5 text-fuchsia-400" /> {p.followers} Followers</span>
            </div>
          </div>
        </div>
      ...
      <div className="px-4 sm:px-6 lg:px-10 py-5">
        <div className="flex gap-2 border-b border-white/5 pb-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {[
            ...
            { id: 'catalog', label: isArtist ? 'Discography' : isProducer ? 'Beat Store' : 'Mix Catalog' },
            { id: 'dna', label: isArtist ? 'Artistic Story' : isProducer ? 'Sonic DNA' : 'Airplay Matrix' },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-fade-in">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div>
                <div className="text-xs font-mono font-black uppercase tracking-widest text-purple-300 mb-3 flex items-center gap-1.5">
                  <Radio className="h-4 w-4 text-purple-400 animate-pulse" /> Platform Broadcast Station
                </div>
                <GlassCard glow className="p-5 border-fuchsia-500/20 bg-gradient-to-br from-[#1A0B2B]/20 via-[#0B0D1E]/10 to-transparent relative overflow-hidden group">
                    ...
                    <div className="min-w-0 flex-1 text-left">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="inline-flex items-center gap-1 rounded-full border border-fuchsia-400/40 bg-fuchsia-500/15 px-2 py-0.5 text-[9px] font-bold text-fuchsia-200">
                          <span className="h-1.5 w-1.5 rounded-full bg-rose-500 animate-ping" /> LIVE SIGNAL
                        </span>
                      </div>
                      <p className="text-[11px] text-fuchsia-300 font-mono font-bold uppercase mt-0.5">Tuned in & verified</p>
                      <p className="text-xs text-white/70 leading-snug mt-1.5">
                        Tuning 100% curated vibe loops designed specifically by {p.name}. Features live voice drop interventions.
                      </p>
                    </div>
                  </div>
                </GlassCard>
              ...
              <div>
                <div className="text-xs font-mono font-black uppercase tracking-widest text-purple-300 mb-3">Top Airplay Rotations</div>
                  ...
                  {p.recentReleases.map((track, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3.5 min-w-0 flex-1">
                        <span className="text-sm font-bold text-purple-300/60 font-mono w-4 text-center">{idx + 1}</span>
                        <img src={track.art} alt="" className="h-11 w-11 rounded-lg object-cover border border-white/10 shrink-0" />
                        <div className="min-w-0 flex-1 text-left">
                          <h5 className="text-sm font-bold text-white truncate">{track.title}</h5>
                          <p className="text-[10px] text-white/50 truncate mt-0.5">{track.station}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-4 text-xs font-mono">
                        <span className="text-cyan-400/80">{(Math.random() * 8 + 1).toFixed(1)}M Streams</span>
                        <PlayCircle size={28} />
                      </div>
                    </GlassCard>
                  ))}
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <GlassCard className="p-5 text-center border-zinc-400/20 bg-gradient-to-b from-white/[0.06] via-white/[0.01] to-transparent shadow-[inset_0_1.5px_2px_rgba(255,255,255,0.15)] relative overflow-hidden">
                ...
                <h4 className="text-[10px] font-mono font-black uppercase tracking-widest text-zinc-400">Tradio Verified Autograph Seal</h4>
                <div className="my-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4">
                {p.stats.map((stat, idx) => (
                  <GlassCard key={idx} className="p-4 border-white/5 flex justify-between items-center gap-3">
                    <span className="text-xs font-bold text-white/60 uppercase font-mono">{stat.label}</span>
                    <span className="text-sm font-black text-cyan-300 font-mono filter drop-shadow-[0_0_5px_rgba(34,211,238,0.2)]">{stat.value}</span>
                  </GlassCard>
                ))}
              </div>
              ...
              <GlassCard className="p-5 border-white/5 space-y-3 text-left">
                <h4 className="text-xs font-mono font-black uppercase tracking-widest text-purple-300">Curator Background Biography</h4>
        ...
        {activeTab === 'catalog' && (
          <div className="space-y-6 animate-fade-in text-left">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-white/50 uppercase">{isArtist ? 'Verified Discography' : isProducer ? 'Analog Beats' : 'Curated Recordings'}</span>
            </div>

            {/* If Artist Profile: Show Releases */}
            {isArtist && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.values(TRACKS).slice(0, 8).map((t) => (
                  <GlassCard key={t.id} className="p-3 border-white/5 hover:border-white/15 flex items-center gap-3.5 transition-all duration-300 cursor-pointer" onClick={() => play(t)}>
                    <img src={t.art || IMG.aiSphere} alt="" className="h-14 w-14 rounded-xl object-cover border border-white/10 shrink-0 shadow-md" />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-black text-white truncate">{t.title}</h4>
                        <span className="rounded bg-cyan-400/10 px-1.5 py-0.5 text-[8px] font-black text-cyan-300">VERIFIED</span>
                      </div>
            ...
            {isProducer && (
              <div className="space-y-3">
                {[
                  { id: 'b1', title: 'Velvet Highway Bounce', bpm: '92 BPM', key: 'C# Minor', mood: 'Melodic Trap', lease: '$49', exclusive: '$249' },
                  { id: 'b2', title: 'Tokyo Rainy Windows', bpm: '78 BPM', key: 'F Major', mood: 'Chill Lo-Fi', lease: '$39', exclusive: '$199' },
                  { id: 'b3', title: 'Southern Heatwave Stems', bpm: '142 BPM', key: 'A Minor', mood: 'Hard Memphis Rap', lease: '$59', exclusive: '$349' },
                  { id: 'b4', title: 'Celestial Gravity Chords', bpm: '85 BPM', key: 'E Minor', mood: 'Space Ambience', lease: '$49', exclusive: '$249' }
                ].map((beat, idx) => (
                  <GlassCard key={beat.id} className="p-4 border-white/5 hover:border-cyan-500/25 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all duration-300">
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto justify-end border-t sm:border-t-0 border-white/5 pt-3 sm:pt-0">
                      <div className="text-right font-mono shrink-0">
                        ...
                        <div className="text-xs font-extrabold text-white mt-0.5">{beat.lease} / <span className="text-cyan-300">{beat.exclusive}</span></div>
                      </div>
                      <button 
            ...
            {isHost && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { title: 'Velvet Afterhours Live Session', date: 'Recorded: Jun 12', listeners: '14.2K played', tags: 'Soul • R&B' },
                  { title: 'Mid-week Midnight Soul Ride', date: 'Recorded: Jun 08', listeners: '11.8K played', tags: 'Vibe Hop' },
                  { title: 'Memphis Roots Airplay Mix', date: 'Recorded: Jun 03', listeners: '9.4K played', tags: 'Southern Rap' },
                  { title: 'Chilly Sunday Morning Reset', date: 'Recorded: May 28', listeners: '15.6K played', tags: 'Neo Soul' }
                ].map((mix, idx) => (
                  <GlassCard key={idx} className="p-4 border-white/5 hover:border-purple-500/20 flex items-center justify-between gap-4 transition-all duration-300 cursor-pointer" onClick={() => play(TRACKS.spinning)}>
                    <div className="min-w-0 text-left">
                      <h4 className="text-sm font-black text-white truncate">{mix.title}</h4>
                      <p className="text-[10px] text-purple-300 font-mono mt-1 uppercase tracking-wide font-bold">{mix.tags}</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <div className="lg:col-span-8 space-y-5">
                  <GlassCard className="p-6 border-white/5 space-y-4">
                    <h3 className="text-base font-black text-white uppercase tracking-wider">The Story of {p.name}</h3>
                    <p className="text-sm text-white/70 leading-relaxed font-medium">
                      Building dynamic soundscapes is second nature to {p.name}. Fusing old school soul with tomorrow's high energy production style, {p.name} represents a next generation pioneer under our theme.
                    </p>
                    <p className="text-sm text-white/70 leading-relaxed font-medium">
                      Every single piece of audio content, release file, and live stream is curated completely inside our platform interface, providing a highly integrated community atmosphere.
                    </p>
                  </GlassCard>
                </div>
                ...
                <div className="lg:col-span-4 space-y-4">
                  <div className="text-xs font-mono font-black uppercase tracking-widest text-purple-300">Voice Drop Soundbites</div>
                  {p.voiceDrops.map((vd) => (
                    <GlassCard key={vd.id} className="p-4 border-white/5 flex items-center justify-between gap-3">
                      <div className="text-left">
                        <div className="text-sm font-bold text-white flex items-center gap-1.5">
                          <Mic className="h-3.5 w-3.5 text-fuchsia-400" /> {vd.title}
                        </div>
                        <span className="text-[10px] text-white/40 font-mono uppercase mt-0.5 block">Duration: {vd.duration}</span>
                      </div>
                      <button 
                        onClick={() => play(p.spotlight)}
            ...
            {isProducer && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <GlassCard className="p-6 border-white/5 space-y-6 text-center">
                  <h3 className="text-sm font-mono font-black uppercase tracking-widest text-purple-300">Producer Acoustic DNA Dials</h3>
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      return (
                        <div key={idx} className="flex flex-col items-center p-3 border border-white/5 rounded-2xl bg-black/20">
                          <span className="text-[10px] font-mono uppercase text-white/45">{knob.l}</span>
                          ...
                          <div className="relative my-2.5 h-[65px] w-[65px]">
                            <svg viewBox="0 0 100 100" className="h-full w-full -rotate-90">
                              ...
                              <circle cx="50" cy="50" r={radius} fill="none" stroke="currentColor" strokeWidth="6" strokeDasharray={c} strokeDashoffset={c - (knob.p / 100) * c} className={knob.c} />
                            </svg>
                          </div>
                        </div>
                      );
                ...
                <div className="space-y-4">
                  <div className="text-xs font-mono font-black uppercase tracking-widest text-purple-300">Signature Co-Productions</div>
                  {p.recentReleases.map((t) => (
                    <GlassCard key={t.id} className="p-3 border-white/5 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 text-left">
                        <img src={t.art} alt="" className="h-11 w-11 rounded-lg object-cover border border-white/10" />
                        <div>
                          <h4 className="text-sm font-black text-white">{t.title}</h4>
                          <p className="text-[10px] text-white/55">Singers on beat: Trey Trizzy, Mila Rain</p>
                        </div>
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <GlassCard className="p-6 border-white/5 space-y-4 text-left">
                  ...
                  {[
                    ...
                    <div key={idx} className="p-3 border border-white/5 rounded-xl bg-white/[0.01] flex items-center justify-between gap-4">
                      <div>
                        <div className="text-[9px] font-mono text-cyan-300 font-bold tracking-widest">{sl.day}</div>
                ...
                <GlassCard className="p-6 border-white/5 space-y-4 text-left">
                  ...
                  {[
                    ...
                    { user: 'kiana_soul_10', song: 'Falling For You (Acoustic)', progress: 'Live Scheduled' },
                    { user: 'onyx_vibe_99', song: 'Memphis After Dark', progress: 'Fulfilled' }
                  ].map((req, idx) => (
                    <div key={idx} className="p-2.5 border border-white/5 rounded-xl bg-black/20 flex items-center justify-between gap-3">
                      <div>
                        <div className="text-[10px] text-white/55 font-mono">{req.user} requested:</div>
                        <div className="text-xs font-extrabold text-white mt-0.5">{req.song}</div>
                      </div>
                      <span className={`rounded-full px-2 py-0.5 text-[8px] font-mono font-bold tracking-wider ${
                        req.progress === 'Fulfilled' ? 'bg-emerald-400/10 text-emerald-300 border border-emerald-400/20' : 'bg-cyan-400/10 text-cyan-300 border border-cyan-400/20'
                      }`}>
        ...
        {activeTab === 'community' && (
          <div className="space-y-6 animate-fade-in text-left max-w-4xl mx-auto">
            ...
            <GlassCard className="p-5 border-white/5 space-y-4 bg-black/20 min-h-[300px]">
              {[
                { author: 'Jordan_Trizzy_Fan', text: 'This station curated by Trey is literally the best late night vibe, the transition on Velvet Dream is insane!', time: '2m ago', avatar: IMG.jordan },
                { author: 'luna_vibe_seeker', text: 'Does Jaye have a new beat pack dropping soon? The backing chords on cityLights are beautiful.', time: '5m ago', avatar: IMG.flowers },
                { author: 'bren_faiy_R&B', text: 'Velvet sessions weekly show is literally my go-to. Jordan always fulfills SZA requests instantly, love the stream!', time: '12m ago', avatar: IMG.midnightVelvet }
              ].map((comment, idx) => (
                <div key={idx} className="flex gap-3.5 p-3 rounded-2xl border border-white/5 bg-white/[0.01]">
                  <img src={comment.avatar} alt="" className="h-9 w-9 rounded-xl border border-white/10 shrink-0 object-cover" />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-mono font-black text-purple-300">{comment.author}</span>
                      <span className="text-[10px] text-white/35 font-mono">{comment.time}</span>
                    </div>
                    <p className="text-xs text-white/85 leading-snug mt-1.5 font-medium">{comment.text}</p>
                  </div>
                </div>
              ))}
