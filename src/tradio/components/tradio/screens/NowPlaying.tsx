...
const sourceCopy: Record<string, { eyebrow: string; detail: string; action: string }> = {
  ...
  artist_station: {
    eyebrow: "Artist Station",
    detail: "Artist-owned radio lane",
    action: "Follow Artist",
  },
  instant_release: {
    eyebrow: "Instant Release",
    detail: "Fresh drop on Tradio",
    action: "Save Release",
  },
  producer_beat: {
    eyebrow: "Producer Beat",
    detail: "Beat preview workspace",
    action: "Pitch to Artist",
  },
...
const LYRIC_EXPLANATIONS: Record<string, string> = {
  "midnight-velvet":
    "A warm invitation to find comfort in late-night sounds. The velvet metaphor represents a soft, secure auditory space designed to make you feel completely grounded and creatively connected.",
  "after-hours":
    "This melody reflects the quiet, contemplative moments of R&B after midnight. Deep, cozy synths provide a secure and comforting space for creative thoughts.",
  default:
    "Tuned using our local audio filters to optimize pleasant soundwaves, acoustic warmth, and soft, comforting spatial playback.",
};
...
export const NowPlayingScreen: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [activeConsoleTab, setActiveConsoleTab] = useState<
    "morph" | "dsp" | "lyrics" | "queue" | "context" | "cast"
  >("morph");
  ...
  const [eqPreset, setEqPreset] = useState<"neutral" | "neon" | "lofi" | "bass" | "acoustic">(
    "neutral",
  );
  ...
  const applyQuickRemix = (
    preset: string,
    prompt: string,
    associatedEq: "lofi" | "neon" | "bass" | "acoustic",
  ) => {
    setIsProcessingAi(true);
    setAiFeedback(null);
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#080711] text-white flex flex-col justify-between overflow-hidden relative font-sans animate-slide-up-modal">
      ...
      <div
        className="pointer-events-none absolute inset-0 z-50 opacity-[0.022] mix-blend-overlay"
        style={{
      ...
      <div className="absolute inset-0 pointer-events-none opacity-40 overflow-hidden z-0 select-none">
        <div
          className="absolute top-[-10%] left-[20%] h-[50%] w-[45%] rounded-full bg-[#3d2b75]/25 blur-[120px] animate-pulse"
          style={{ animationDuration: "9s" }}
        />
        <div
          className="absolute bottom-[-15%] right-[25%] h-[55%] w-[40%] rounded-full bg-[#1e4854]/25 blur-[110px] animate-pulse"
      ...
      <div className="relative z-10 flex h-16 items-center justify-between px-6 sm:px-8 border-b border-white/[0.06] bg-[#07060e]/50 backdrop-blur-2xl select-none">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`hidden sm:inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1 text-[9px] font-mono font-bold tracking-wider transition-all duration-300 ${
              spatialAudio
        ...
        <div className="lg:col-span-5 flex flex-col justify-between gap-4 overflow-hidden h-full">
          <GlassCard className="overflow-hidden p-0 border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.65),inset_0_1px_0_rgba(255,255,255,0.08)] flex-1 flex flex-col justify-center bg-[#07060f]/40 backdrop-blur-3xl relative rounded-[32px]">
            <div className="relative aspect-square w-full max-w-[400px] mx-auto flex items-center justify-center">
              ...
              {isAiSphere ? (
                ...
                <div className="absolute inset-0 z-10 flex items-center justify-center">
                  <div className="relative w-[68%] h-[68%] flex items-center justify-center">
                    {/* Orbiting streaks */}
                    <div
                      className="absolute inset-[-8%] rounded-full border border-t-cyan-400/30 border-r-purple-500/0 border-b-fuchsia-500/0 border-l-transparent animate-spin pointer-events-none opacity-40"
                      style={{ animationDuration: "6s" }}
                    />
                    <div
                      className="absolute inset-[-3%] rounded-full border border-b-fuchsia-400/35 border-l-transparent border-t-purple-500/0 border-r-cyan-400/0 animate-spin-reverse pointer-events-none opacity-40"
                    ...
                    <div className="absolute inset-0 z-20 rounded-full overflow-hidden pointer-events-none mix-blend-color-dodge">
                      <div
                        ...
                        style={{
                          background:
                            "linear-gradient(135deg, transparent 35%, rgba(139,92,246,0.15) 45%, rgba(255,255,255,0.6) 50%, rgba(34,211,238,0.15) 55%, transparent 65%)",
                        }}
                      />
                    </div>
                  </div>
              ...
              >
                <Heart
                  className={`h-4 w-4 ${isLiked ? "fill-pink-500 text-pink-500" : "text-white/60"}`}
                />
              </button>
              ...
              <div className="absolute inset-x-0 bottom-0 p-5 z-30 select-none text-left">
                ...
                <p className="text-xs sm:text-sm text-white/70 mt-0.5 font-bold truncate">
                  {display.artist}
                </p>
              ...
              {[
                {
                  ...
                  label: "Add Queue",
                  action: () => addToQueue(display),
                },
                {
                  icon: <Sliders className="h-3.5 w-3.5 text-white/60" />,
                  label: "Sound Deck",
                  action: () => setActiveConsoleTab("dsp"),
                },
                {
                  icon: <Cast className="h-3.5 w-3.5 text-cyan-300" />,
                  label: "AirWave Cast",
                  action: () => setActiveConsoleTab("cast"),
                },
                {
                  icon: <Info className="h-3.5 w-3.5 text-white/60" />,
                  label: "About",
                  action: () => setActiveConsoleTab("context"),
                },
              ].map((q, idx) => (
                <button
                  key={idx}
              ...
              <button
                ...
                className={`flex h-8 w-8 items-center justify-center rounded-xl border border-white/5 ${
                  repeatMode === "repeat_one"
                    ? "text-purple-300 bg-purple-950/15"
              ...
              {[
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      setActiveConsoleTab(
                        tab.id as "morph" | "dsp" | "lyrics" | "queue" | "context" | "cast",
                      )
                    }
                ...
                <div className="space-y-4 animate-fade-in flex flex-col h-full justify-between">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      {isCasting ? (
                        <span className="text-[8px] font-mono text-cyan-300 bg-cyan-950/30 border border-cyan-500/30 px-2 py-0.5 rounded animate-pulse">
                          CASTING ACTIVE
                        </span>
                      ) : (
                    ...
                    <p className="text-[11px] text-white/55 leading-relaxed font-mono">
                      Stream losslessly to any local AirPlay, Chromecast, Bluetooth device, or
                      browser receiver.
                    </p>
                    ...
                    {isScanning ? (
                      ...
                      <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1">
                        {[
                          {
                            id: "jordan-monitors",
                            name: "Jordan's Studio Monitor",
                            sub: "AirPlay • Lossless 192kHz enabled",
                            icon: <Laptop className="h-4 w-4" />,
                          },
                          {
                            id: "living-room-tv",
                            name: "Living Room Apple TV 4K",
                            sub: "AirPlay • Dolby Atmos 5.1 ready",
                            icon: <Tv className="h-4 w-4" />,
                          },
                          {
                            id: "trey-tv-control",
                            name: "Trey TV Master Control Deck",
                            sub: "Chromecast • 24-bit HD Stream active",
                            icon: <Tv className="h-4 w-4" />,
                          },
                          {
                            id: "soundbar-bluetooth",
                            name: "Dolby Soundbar System",
                            sub: "Bluetooth • aptX HD receiver",
                            icon: <Volume2 className="h-4 w-4" />,
                          },
                          {
                            id: "web-receiver",
                            name: "Tradio Web Cast Receiver",
                            sub: "Web Audio • Direct sync node ready",
                  ...
                  <div className="pt-3 border-t border-white/5 space-y-3 shrink-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-mono font-black uppercase tracking-widest text-white/45">
                        Local File Direct Beam
                      </span>
                      <span className="text-[8px] font-mono text-purple-300 bg-purple-500/15 border border-purple-500/25 px-1.5 py-0.5 rounded">
                        ANY RECEIVER
                      </span>
                    </div>
                    ...
                    {castFileUploading ? (
                      <div className="p-3 rounded-xl border border-cyan-500/15 bg-cyan-950/5 text-[10px] leading-relaxed text-cyan-300 font-mono shadow-inner flex flex-col gap-2">
                        ...
                        <div className="relative h-1 w-full bg-white/10 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${castUploadProgress}%` }}
                          />
                        </div>
                        <div className="text-[8px] text-white/40 text-right">
                          Uploading stream cue • {castUploadProgress}%
                        </div>
                      </div>
                    ) : (
                      <div className="relative rounded-xl border border-dashed border-white/10 bg-white/[0.01] hover:bg-white/[0.03] transition-all p-3 flex flex-col items-center justify-center text-center cursor-pointer group">
                        ...
                        <span className="text-[10px] font-mono text-white/55 mt-1">
                          Beam any local audio file to Cast receivers
                        </span>
                        <span className="text-[8px] text-white/30 font-mono mt-0.5 uppercase tracking-wider">
                    ...
                    <div className="flex items-center justify-between p-2.5 rounded-xl border border-white/5 bg-black/20 text-[10px] font-mono">
                      <div className="space-y-0.5 text-left">
                        ...
                        <div className="text-white/40 text-[9px]">
                          Open{" "}
                          <strong className="text-cyan-300 font-bold">tradio.tv/receive</strong> and
                          enter:
                        </div>
                        <div className="text-xs text-white font-black tracking-wider uppercase mt-1">
                          TRADIO-8855
                        </div>
                      </div>
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(
                            "https://tradio.tv/receive?code=TRADIO-8855",
                          );
                ...
                <div className="space-y-3.5 animate-fade-in flex flex-col h-full justify-between">
                  <div className="space-y-2.5">
                    ...
                    <p className="text-[11px] text-white/50 leading-relaxed font-mono">
                      Softly shape the dynamic soundwave. Choose a balanced preset shortcut below or
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        {
                          label: "Dusty Lo-Fi",
                          prompt: "Comforting vinyl dust, warm tape lowpass",
                          eq: "lofi" as const,
                        },
                        {
                          label: "Balanced Space",
                          prompt: "Deep stereophonic chorus, center vocal clarity",
                          eq: "neon" as const,
                        },
                        {
                          label: "Ambient Cavern",
                          prompt: "Spacious soft echoes, 10% slow decompress",
                          eq: "bass" as const,
                        },
                        {
                          label: "Acoustic Cozy",
                          prompt: "Harmonize dynamic range, warm vocal filters",
                          eq: "acoustic" as const,
                        },
                      ].map((preset) => (
                        <button
                          key={preset.label}
                        ...
                        >
                          ...
                          <div className="text-[8px] text-white/30 truncate mt-0.5">
                            {preset.prompt}
                          </div>
                        </button>
                      ))}
                ...
                <div className="space-y-3.5 animate-fade-in flex flex-col h-full justify-between">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <span className="text-[10px] font-mono font-black uppercase tracking-widest text-purple-300 block mb-1">
                        Signal Filters
                      </span>
                      {[
                        {
                          id: "spatial",
                          title: "Atmos Spatial Stage",
                          state: spatialAudio,
                          toggle: () => setSpatialAudio(!spatialAudio),
                        },
                        {
                          id: "upscale",
                          title: "Harmonic Warmth Filter",
                          state: harmonicUpscale,
                          toggle: () => setHarmonicUpscale(!harmonicUpscale),
                        },
                        {
                          id: "vocal",
                          title: "Vocal Presence Focus",
                          state: vocalIsolation,
                          toggle: () => {
                            setVocalIsolation(!vocalIsolation);
                            if (instrumentalMode) setInstrumentalMode(false);
                          },
                        },
                        {
                          id: "instrumental",
                          title: "Acoustic Beat Focus",
                          state: instrumentalMode,
                          toggle: () => {
                            setInstrumentalMode(!instrumentalMode);
                            if (vocalIsolation) setVocalIsolation(false);
                          },
                        },
                      ].map((filter) => (
                        <button
                        ...
                        >
                          ...
                          <div
                            className={`h-3 w-6 rounded-full p-0.5 shrink-0 ${filter.state ? "bg-purple-400" : "bg-white/10"}`}
                          >
                            <div
                              className={`h-2 w-2 rounded-full bg-white transition-transform ${filter.state ? "translate-x-3" : "translate-x-0"}`}
                            />
                          </div>
                        </button>
                      ))}
                    ...
                    <div className="flex flex-col h-full justify-between">
                      <div className="flex items-center justify-between border-b border-white/5 pb-1">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setEqPreset(
                              e.target.value as "neutral" | "neon" | "lofi" | "bass" | "acoustic",
                            )
                          }
                      ...
                      <div className="flex gap-1.5 items-end justify-between h-[95px] pt-1.5">
                        {eqGains.map((gain, idx) => {
                          ...
                          return (
                            <div
                              key={idx}
                              className="flex-1 h-full flex flex-col justify-between items-center group"
                            >
                              ...
                              <div className="relative w-1 h-[65px] bg-white/5 rounded-full flex justify-center items-end cursor-ns-resize">
                                <div
                                  className="absolute w-full rounded-full bg-gradient-to-t from-purple-400 to-cyan-300"
                                  style={{ height: `${pct}%` }}
                                />
                                <input
                                  ...
                                  value={gain}
                                  onChange={(e) =>
                                    handleEqSliderChange(idx, Number(e.target.value))
                                  }
                                  className="absolute inset-0 opacity-0 cursor-ns-resize h-full w-full"
                                />
                              </div>
                              <span className="text-[7px] font-mono text-white/20 uppercase mt-1">
                                {EQ_FREQS[idx].replace("Hz", "").replace("kHz", "k")}
                              </span>
                            </div>
                          );
                        })}
                  ...
                  <div className="border-t border-white/5 pt-3 flex justify-center shrink-0">
                    <span className="text-[9px] font-mono text-white/30 flex items-center gap-1">
                      <Activity className="h-3 w-3 text-purple-400 animate-pulse" /> CALM ANALOG
                      EMBEDDED DSP COCKPIT
                    </span>
                  </div>
                </div>
              )}
                    ...
                    <div className="space-y-2 overflow-y-auto max-h-[110px] pr-1 scrollbar-thin flex-1 min-h-0">
                      {songLyrics.map((lyric, idx) => {
                        ...
                        return (
                          ...
                          >
                            {isActive && (
                              <Activity className="h-3.5 w-3.5 text-purple-400 animate-pulse shrink-0 mt-0.5" />
                            )}
                    ...
                    <div className="pt-3 border-t border-white/5 shrink-0">
                      {isAnalyzingLyrics ? (
                        <div className="flex items-center gap-2 text-[9px] font-mono text-white/45">
                          ...
                          Connecting semantic translation nodes...
                        </div>
                      ) : (
                  ...
                  <div className="space-y-2.5 flex-1 flex flex-col overflow-hidden">
                    <div className="flex items-center justify-between border-b border-white/5 pb-1 shrink-0">
                      ...
                      {queue.length > 0 && (
                        <button
                          onClick={clearQueue}
                          className="text-[9px] font-mono font-bold text-rose-400 hover:text-rose-300 underline uppercase tracking-wider"
                        >
                          Clear all
                        </button>
                      )}
                      ...
                      {upNextList.map((item) => (
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="truncate text-[9px] text-white/40 mt-0.5">
                              {item.artist}
                            </div>
                          </button>
                ...
                <div className="space-y-3.5 animate-fade-in flex flex-col h-full justify-between">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl border border-white/5 bg-white/[0.01] p-3 text-left space-y-1">
                      <span className="text-[9px] font-mono font-black text-white/30 uppercase block">
                        Bitrate Profile
                      </span>
                    ...
                    <div className="rounded-xl border border-white/5 bg-white/[0.01] p-3 text-left space-y-1">
                      ...
                      <span className="text-xs font-mono font-bold text-white truncate block">
                        {currentSource?.title || display.station || "Core System"}
                      </span>
                    </div>

                    <div className="rounded-xl border border-white/5 bg-white/[0.01] p-3 text-left space-y-1 col-span-2">
                      <span className="text-[9px] font-mono font-black text-white/30 uppercase block">
                        Spatial Engine & Codec
                      </span>
                      <span
                        className={`text-xs font-mono font-bold transition-colors duration-300 ${spatialAudio ? "text-purple-300" : "text-white/40"}`}
                      >
                        {spatialAudio
                          ? "● Dolby Atmos Digital Plus (Wired)"
                          : "○ Discrete L/R Stereo (Bypass)"}
                      </span>
                    </div>

                    {isLive && (
                      <div className="rounded-xl border border-purple-500/10 bg-purple-500/5 p-3 text-left space-y-1 col-span-2">
                        <span className="text-[9px] font-mono font-black text-purple-300/60 uppercase block">
                          Active Listeners
                        </span>
                        <span className="text-xs font-mono font-bold text-white block">
                          {(
                            currentSource?.listenerCount ||
                            stationContext?.listenerCount ||
                            liveContext?.listenerCount ||
                            18420
                          ).toLocaleString()}{" "}
                          receivers active
                        </span>
                      ...
                      <>
                        <div className="rounded-xl border border-white/5 bg-white/[0.01] p-3 text-left space-y-1">
                          ...
                          <span className="text-xs font-mono font-bold text-cyan-300">
                            {beatContext.bpm} BPM • {beatContext.musicalKey}
                          </span>
                        </div>
                        <div className="rounded-xl border border-white/5 bg-white/[0.01] p-3 text-left space-y-1">
                          <span className="text-[9px] font-mono font-black text-white/30 uppercase block">
                          ...
                          <span className="text-xs font-mono font-bold text-white truncate block">
                            {beatContext.licenseLabel || "Exclusive"}
                          </span>
                        </div>
                      </>
                    )}
                    ...
                    {songWarsContext && (
                      <>
                        <div className="rounded-xl border border-white/5 bg-white/[0.01] p-3 text-left space-y-1">
                          ...
                          <span className="text-xs font-mono font-bold text-white truncate block">
                            {songWarsContext.artistA} vs {songWarsContext.artistB}
                          </span>
                        </div>
                        <div className="rounded-xl border border-white/5 bg-white/[0.01] p-3 text-left space-y-1">
                          <span className="text-[9px] font-mono font-black text-white/30 uppercase block">
                          ...
                          <span className="text-xs font-mono font-bold text-white">
                            Round {songWarsContext.roundNumber}
                          </span>
                        </div>
                      </>
                    )}
                  ...
                  <div className="p-3 rounded-xl border border-white/5 bg-black/10 text-[10px] leading-relaxed text-white/55 font-mono">
                    ...
                    Tuned with Dolby Atmos filters. Local signal analysis identifies this as a
                    secure, harmonic {copy.eyebrow} stream environment.
                  </div>
                </div>
              )}
            </div>
