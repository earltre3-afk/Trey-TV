...
const RoleProfilePage = lazy(() =>
  import("./screens/RoleProfilePage").then((m) => ({ default: m.RoleProfilePage })),
);
...
const BattleSetupForm = lazy(() =>
  import("./songwars/views/BattleSetupForm").then((m) => ({ default: m.BattleSetupForm })),
);
const BattleStage = lazy(() =>
  import("./songwars/views/BattleStage").then((m) => ({ default: m.BattleStage })),
);
const BattleResults = lazy(() =>
  import("./songwars/views/BattleResults").then((m) => ({ default: m.BattleResults })),
);
const BattleReplayScreen = lazy(() =>
  import("./songwars/views/BattleReplayScreen").then((m) => ({ default: m.BattleReplayScreen })),
);
...
const FEATURE_SHORTCUTS: {
  ...
  { key: "release", label: "Instant Release", sub: "Drop directly into Tradio", Icon: UploadCloud },
  {
    ...
    sub: "Build a full live radio plan",
    Icon: Sparkles,
  },
  {
    key: "routeMe",
    label: "Route Me",
    sub: "Prescribe a path through the Trey TV universe",
    Icon: Route,
  },
  { key: "community", label: "Community", sub: "Station rooms and fan reactions", Icon: Users },
  {
    key: "schedule",
    label: "Live Schedule",
    sub: "Premieres and radio blocks",
    Icon: CalendarDays,
  },
];
...
export const TradioShellContent: React.FC = () => {
  ...
  const [tab, setTab] = useState<TabKey>(() => {
    if (typeof window !== "undefined") {
      ...
      if (
        params.get("artistUid") ||
        params.get("uid") ||
        params.get("producerUid") ||
        params.get("djUid")
      ) {
        return "studio";
      }
    ...
    if (typeof window !== "undefined") {
      ...
      if (artistUid)
        return {
          ...
          role: "artist",
          ownerView: false,
          lookup: { publicProfileUid: artistUid },
        };
      if (producerUid)
        return {
          kind: "roleProfile",
          role: "producer",
          ownerView: false,
          lookup: { publicProfileUid: producerUid },
        };
      if (djUid)
        return {
          kind: "roleProfile",
  ...
  const setScreen = (key: ScreenKey) => {
    const map: Record<ScreenKey, () => void> = {
      ...
      release: () => {
        setTab("studio");
        setView({ kind: "release" });
      },
      search: () => {
        setTab("search");
        setView({ kind: "tab", tab: "search" });
      },
      library: () => {
        setTab("library");
      ...
      artistHub: () => {
        setTab("studio");
        setView({ kind: "artistHub" });
      },
      producerHub: () => {
        setTab("studio");
        setView({ kind: "producerHub" });
      },
      djStudio: () => {
        setTab("studio");
        setView({ kind: "djStudio" });
      },
      showBuilder: () => {
        setTab("studio");
        setView({ kind: "showBuilder" });
      },
      analytics: () => {
        setTab("studio");
        setView({ kind: "tab", tab: "studio" });
  ...
  const handleTab = (t: TabKey) => {
    ...
    if (t === "profile" && !hasProfileAccess) {
      toast.error(
        "Only approved artists, DJs, and producers have access to a Tradio profile page.",
      );
      return;
    }
    setTab(t);
  ...
  const openStudioDestination = (destination: StudioDestination) => {
    if (destination === "songwars") {
      ...
      if (
        destination === "artistProfile" ||
        destination === "producerProfile" ||
        destination === "djProfile"
      ) {
        const role: RoleProfileType =
          destination === "artistProfile"
            ? "artist"
            : destination === "producerProfile"
              ? "producer"
  ...
  const renderScreen = () => {
    if (view.kind === "broadcastStudio")
      return (
        <BroadcastStudioGateway onBack={() => setScreen("studio")} initialTab={view.initialTab} />
      );
        ...
        <RoleProfilePage
          ...
          onOpenPublicShell={(r) =>
            setView({
              kind: "profile",
              role: r === "dj" ? "host" : r,
              name: r === "producer" ? "JAYE." : r === "dj" ? "Jordan" : "Trey Trizzy",
            })
          }
    ...
    if (view.kind === "legal")
      return <LegalCenter initialTarget={view.target} onExit={() => setScreen("home")} />;
    if (view.kind === "legalAdmin")
      return <LegalAdminDashboard onBack={() => setScreen("studio")} />;
    ...
    if (view.kind === "artistHub")
      return (
        <ArtistHubScreen
          onOpenRelease={() => setScreen("release")}
          onOpenSchedule={() => setScreen("schedule")}
          onOpenBroadcastStudio={(initialTab) => setView({ kind: "broadcastStudio", initialTab })}
          onViewPublicProfile={() => openRoleProfile("artist", false)}
          onEditProfile={() => openRoleProfile("artist", true)}
        />
      );
    if (view.kind === "producerHub")
      return (
        <ProducerHubScreen
          onOpenBroadcastStudio={(initialTab) => setView({ kind: "broadcastStudio", initialTab })}
          onViewPublicProfile={() => openRoleProfile("producer", false)}
          onEditProfile={() => openRoleProfile("producer", true)}
        />
      );
    if (view.kind === "djStudio")
      return (
        <DJStudioScreen
          onOpenBroadcastStudio={(initialTab) => setView({ kind: "broadcastStudio", initialTab })}
          onViewPublicProfile={() => openRoleProfile("dj", false)}
          onEditProfile={() => openRoleProfile("dj", true)}
        />
      );
    ...
    switch (view.tab) {
      ...
      case "stations":
        return (
          <StationsHub
            onOpen={openStationsDestination}
            onOpenBroadcastStudio={(initialTab) => setView({ kind: "broadcastStudio", initialTab })}
          />
        );
      case "search":
      ...
      case "studio":
        return (
          <StudioScreen
            onOpenRelease={() => setScreen("release")}
            onOpenDestination={openStudioDestination}
          />
        );
      case "profile": {
        const isApprovedCreator = hasAnyRole(identity, [
          "artist",
          "producer",
          "dj",
          "admin",
        ...
        if (!isApprovedCreator) {
          return (
            <div className="min-h-screen px-4 py-8 sm:px-6 lg:px-10 flex flex-col items-center justify-center text-center animate-fade-in">
              <GlassCard
                glow
                className="max-w-md w-full p-8 space-y-6 relative border border-white/10 bg-black/60"
              >
                ...
                <div className="flex flex-col items-center gap-3 relative z-10">
                  ...
                  <p className="text-xs text-white/55 max-w-sm leading-relaxed">
                    Tradio profile pages are exclusive to approved artists, beat producers, and
                    radio hosts. Request access to unlock your professional creator channel,
                    customized release portfolio, and live broadcast features.
                  </p>
                </div>
  ...
  return (
    <div className="min-h-dvh w-full overflow-x-hidden bg-gradient-to-b from-[#06050A] via-[#0E071C] to-[#06050A] text-white selection:bg-purple-500/30 selection:text-white">
      ...
      <div
        className="pointer-events-none fixed inset-0 z-50 opacity-[0.022] mix-blend-overlay"
        style={{
      ...
      <main className="relative flex min-h-dvh w-full flex-col lg:pl-[300px] xl:pr-[360px]">
        ...
        <header className="sticky top-0 z-40 flex h-14 w-full shrink-0 items-center justify-between border-b border-white/[0.06] bg-[#06050a]/80 px-4 backdrop-blur-xl lg:hidden">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-black uppercase tracking-wider bg-gradient-to-r from-fuchsia-400 to-cyan-400 bg-clip-text text-transparent">
              Tradio
            </span>
          </div>
              ...
              <div className="grid min-h-0 flex-1 grid-cols-1 gap-2 overflow-y-auto overscroll-contain px-6 pb-4 pr-5 sm:grid-cols-2">
                {SCREEN_LABELS.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`size-10 rounded-xl overflow-hidden grid place-items-center bg-white/5 transition-transform group-hover:scale-110 shrink-0 ${
