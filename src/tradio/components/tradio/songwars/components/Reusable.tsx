...
//  VS EMBLEM
export const VSEmblem: React.FC<{ active?: boolean; className?: string }> = ({
  active = true,
  className = "",
}) => {
  return (
    <div className={`relative flex items-center justify-center ${className}`}>
      ...
      <div
        className={`absolute h-24 w-24 rounded-full bg-gradient-to-tr from-fuchsia-500/20 via-purple-600/30 to-cyan-500/20 blur-xl transition-all duration-1000 ${active ? "scale-150 animate-pulse" : "scale-100"}`}
      />
      ...
      {/* Main emblem ring */}
      <div
        className={`relative flex h-20 w-16 items-center justify-center rounded-3xl border bg-gradient-to-b from-black/80 to-[#10061F]/90 shadow-premium transition-transform duration-700 ${
          active
...
export const ArtistEntrance: React.FC<ArtistEntranceProps> = ({ artistA, artistB, onComplete }) => {
  const [step, setStep] = useState<
    "slide" | "reveal" | "countdown3" | "countdown2" | "countdown1" | "ready" | "done"
  >("slide");
    ...
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#050508]/95 p-6 backdrop-blur-xl transition-all duration-500">
      ...
      <div className="relative w-full max-w-4xl text-center">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {/* Artist A Panel */}
          <div
            className={`flex flex-col items-center rounded-3xl border border-fuchsia-500/20 bg-gradient-to-br from-fuchsia-950/20 to-transparent p-8 backdrop-blur-2xl transition-all duration-1000 transform ${
          ...
          >
            <div className="relative">
              <img
                src={artistA.avatar}
                alt={artistA.name}
                className="h-40 w-40 rounded-full border-4 border-fuchsia-500 object-cover shadow-[0_0_40px_rgba(244,63,94,0.4)]"
              />
              {artistA.verified && (
            ...
            <div className="mt-5">
              <span className="rounded-full bg-fuchsia-500/15 px-3 py-1 text-xs font-bold tracking-wider text-fuchsia-300 uppercase">
                Artist Participant
              </span>
          ...
          {/* Artist B Panel */}
          <div
            className={`flex flex-col items-center rounded-3xl border border-cyan-500/20 bg-gradient-to-br from-cyan-950/20 to-transparent p-8 backdrop-blur-2xl transition-all duration-1000 transform ${
              step === "slide" ? "translate-x-full opacity-0" : "translate-x-0 opacity-100"
            }`}
          >
            <div className="relative">
              <img
                src={artistB.avatar}
                alt={artistB.name}
                className="h-40 w-40 rounded-full border-4 border-cyan-500 object-cover shadow-[0_0_40px_rgba(6,182,212,0.4)]"
              />
              {artistB.verified && (
            ...
            <div className="mt-5">
              <span className="rounded-full bg-cyan-500/15 px-3 py-1 text-xs font-bold tracking-wider text-cyan-300 uppercase">
                Artist Participant
              </span>
        ...
        {/* Intro Banner */}
        <div
          className={`mt-12 text-center transition-all duration-1000 ${step !== "slide" ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
        >
          <div className="inline-flex items-center gap-2 rounded-full border border-purple-400/20 bg-purple-500/10 px-5 py-2.5 backdrop-blur-xl text-purple-200 text-sm font-semibold">
            <Sparkles className="h-4 w-4 text-purple-300 animate-spin" /> Entering the battle
            stage...
          </div>
        </div>
      </div>
...
export const VoteMeter: React.FC<VoteMeterProps> = ({
  votesA,
  votesB,
  labelA,
  labelB,
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="text-left w-[40%]">
          <div className="flex items-center gap-2 text-[10px] uppercase font-bold tracking-widest text-white/40">
            <span className="h-1.5 w-1.5 rounded-full bg-gradient-to-r from-pink-500 to-fuchsia-500 shadow-[0_0_8px_rgba(244,63,94,0.6)]" />{" "}
            {labelA}
          </div>
          <div className="text-3xl font-black bg-gradient-to-r from-pink-400 to-fuchsia-300 bg-clip-text text-transparent mt-1 leading-none">
            {hasVoted ? `${pctA}%` : "??%"}
          </div>
          {userVote === "A" && (
            <span className="inline-block text-[8px] bg-fuchsia-500/25 text-fuchsia-300 border border-fuchsia-500/30 px-2 py-0.5 rounded-full font-extrabold uppercase tracking-wider mt-1.5 shadow-[0_0_10px_rgba(244,63,94,0.15)]">
              Your Choice
            </span>
          )}
        </div>
        ...
        <div className="text-center bg-white/[0.03] border border-white/5 rounded-2xl px-4 py-2 flex flex-col justify-center shadow-inner">
          <div className="text-[8px] text-white/30 uppercase tracking-[0.25em] font-black">
            LEDGER TALLIES
          </div>
          <div className="text-sm font-black text-purple-200 mt-1">
            {hasVoted ? total.toLocaleString() : "LOCKED"}
          </div>
        </div>
        ...
        <div className="text-right w-[40%]">
          <div className="flex items-center gap-2 justify-end text-[10px] uppercase font-bold tracking-widest text-white/40">
            {labelB}{" "}
            <span className="h-1.5 w-1.5 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 shadow-[0_0_8px_rgba(6,182,212,0.6)]" />
          </div>
          <div className="text-3xl font-black bg-gradient-to-r from-cyan-300 to-teal-200 bg-clip-text text-transparent mt-1 leading-none">
            {hasVoted ? `${pctB}%` : "??%"}
          </div>
          {userVote === "B" && (
            <span className="inline-block text-[8px] bg-cyan-500/25 text-cyan-300 border border-cyan-500/30 px-2 py-0.5 rounded-full font-extrabold uppercase tracking-wider mt-1.5 shadow-[0_0_10px_rgba(6,182,212,0.15)]">
              Your Choice
            </span>
          )}
        </div>
      ...
      <div className="relative h-5 w-full overflow-hidden rounded-full bg-black/60 border border-white/10 p-[2px] shadow-inner">
        {hasVoted ? (
          ...
          <div className="h-full w-full bg-gradient-to-r from-fuchsia-500/5 via-purple-500/10 to-cyan-500/5 animate-pulse flex items-center justify-center">
            <span className="text-[9px] text-purple-300/40 uppercase tracking-[0.25em] font-black animate-pulse">
              CAST VOTE TO UNLOCK REALTIME DISPLAY
            </span>
          </div>
        )}
      </div>
...
//  CROWD ENERGY METER
export const CrowdEnergyMeter: React.FC<{ energy?: number; className?: string }> = ({
  energy = 78,
  className = "",
}) => {
  const bars = 18;
  ...
  return (
    <div
      className={`rounded-2xl bg-black/40 border border-white/5 p-4 backdrop-blur-md ${className}`}
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          <Flame className="h-4 w-4 text-fuchsia-400 animate-pulse" />
          <span className="text-[10px] text-white/50 uppercase tracking-wider font-extrabold">
            Crowd Energy
          </span>
        </div>
        <span className="text-xs font-bold text-fuchsia-300 bg-fuchsia-500/10 px-2 py-0.5 rounded-full">
          {energy}%
        </span>
      </div>
      ...
      <div className="flex items-end justify-between h-8 gap-0.5">
        {Array.from({ length: bars }).map((_, i) => {
          ...
          if (isActive) {
            if (i < bars / 3)
              colorClass =
                "bg-gradient-to-t from-cyan-500 to-purple-400 shadow-[0_0_5px_rgba(6,182,212,0.4)]";
            else if (i < (2 * bars) / 3)
              colorClass =
                "bg-gradient-to-t from-purple-400 to-fuchsia-500 shadow-[0_0_5px_rgba(176,38,255,0.4)]";
            else
              colorClass =
                "bg-gradient-to-t from-fuchsia-500 to-pink-500 shadow-[0_0_8px_rgba(244,63,94,0.6)]";
          }

          // Varied height factors for visual flavor
          const heightPercent = [
            30, 45, 60, 75, 90, 80, 70, 55, 40, 60, 75, 90, 95, 80, 65, 50, 40, 30,
          ][i];
      ...
      {EMOJI_LIST.map((item) => (
        <button
          ...
          onClick={() =>
            onSelect(
              item.emoji,
              item.label as "fire" | "crown" | "lightning" | "heart" | "speaker" | "roar",
            )
          }
...
export const AnimatedReactionTray: React.FC<AnimatedReactionTrayProps> = ({
  floatingReactions,
}) => {
  return (
...
export const GIFPickerPlaceholder: React.FC<GIFPickerPlaceholderProps> = ({
  onSelect,
  onClose,
}) => {
  ...
  const mockGIFs = [
    {
      id: "gif-1",
      term: "FWD",
      name: "Trey DJing Live",
      url: IMG.treyTrizzy,
      desc: "Trey on the CDJs",
    },
    {
      id: "gif-2",
      term: "FWD",
      name: "Bass Drop Shake",
      url: IMG.aiSphere,
      desc: "Subwoofer vibrating",
    },
    {
      id: "gif-3",
      term: "Trending",
      name: "Crowd Roar Glow",
      url: IMG.midnightVelvet,
      desc: "Neon concert",
    },
    {
      id: "gif-4",
      term: "Trending",
      name: "Winner Flame Burst",
      url: IMG.neonHeartbreak,
      desc: "Fire sparks",
    },
    {
      id: "gif-5",
      term: "Tradio",
      name: "Spinning Gold Plate",
      url: IMG.lateNightSoul,
      desc: "Rotating premium vinyl",
    },
    {
      id: "gif-6",
      term: "Tradio",
      name: "Equalizer Clashing",
      url: IMG.instantDrop,
      desc: "Digital waveforms",
    },
  ];
        ...
        {filteredGIFs.map((gif) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-1.5">
              ...
              <span className="text-[7px] text-purple-300 bg-purple-500/20 px-1 py-0.2 rounded self-start mt-0.5 font-bold uppercase tracking-widest">
                GIF
              </span>
            </div>
          </button>
        ...
        return (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mt-2 border-t border-white/5 pt-2">
              <div
                className={`min-w-0 ${isAWinner ? "text-fuchsia-300 font-bold" : "text-white/60"}`}
              >
              ...
              <div
                className={`min-w-0 text-right ${isBWinner ? "text-cyan-300 font-bold" : "text-white/60"}`}
              >
            ...
            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden mt-3 flex">
              <div
                style={{ width: `${pctA}%` }}
                className={`h-full ${isAWinner ? "bg-fuchsia-500 shadow-[0_0_10px_rgba(244,63,94,0.5)]" : "bg-white/20"}`}
              />
              <div
                style={{ width: `${pctB}%` }}
                className={`h-full ${isBWinner ? "bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]" : "bg-white/20"}`}
              />
            </div>
          </div>
        );
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#07030C]/98 p-6 backdrop-blur-3xl">
      ...
      <div className="relative max-w-lg w-full text-center scale-95 animate-[zoomIn_0.6s_ease-out_forwards]">
        ...
        <p className="text-white/50 text-sm mt-1 uppercase tracking-[0.2em]">
          Song Wars Session Winner
        </p>

        {/* Winner Main Card */}
        <GlassCard
          glow
          className="mt-8 overflow-hidden border-yellow-500/20 bg-gradient-to-br from-[#1C1405] via-[#0E071A] to-[#0A0A0F]"
        >
          <div className="p-8">
            ...
            <p className="mt-1 text-xs font-bold text-yellow-300 uppercase tracking-widest">
              {winner.station}
            </p>
            ...
            <div className="mt-6 space-y-3 text-left border-t border-white/5 pt-5">
              ...
              <div className="flex justify-between items-center bg-yellow-500/5 p-3 rounded-2xl border border-yellow-500/10">
                ...
                <span className="text-xs font-extrabold text-yellow-300 truncate max-w-[200px]">
                  {prize}
                </span>
              </div>
            </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-6 backdrop-blur-md">
      <div className="relative w-full max-w-sm rounded-3xl border border-white/10 bg-gradient-to-b from-[#1C0D26] to-[#0A0A0F] p-6 shadow-premium animate-fade-in">
        ...
        <p className="mt-2 text-center text-xs text-white/60 leading-relaxed">
          The battle session is currently live! If you exit, you will miss out on the round-by-round
          voting and real-time chat with the community.
        </p>
