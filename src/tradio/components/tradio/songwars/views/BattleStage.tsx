...
export const BattleStage: React.FC<BattleStageProps> = ({
  ...
  const [roundStatus, setRoundStatus] = useState<"playing" | "voting" | "completed">(
    currentRound.status as "playing" | "voting" | "completed",
  );
  ...
  const [chatTab, setChatTab] = useState<"Live Chat" | "Polls" | "Requests" | "Top Fans">(
    "Live Chat",
  );
  ...
  const playSongWarCorner = useCallback(
    (corner: "A" | "B") => {
  ...
  const handleSendMessage = () => {
    ...
    const newMsg: SongWarChatMessage = {
      id: `chat-${Date.now()}`,
      author:
        role === "admin" ? "Trizzy Trey" : role === "dj" ? "DJ Midnight Spin" : "You (Jordan)",
      avatar:
        role === "admin"
          ? MOCK_SONG_WAR_ARTISTS.treyTrizzy.avatar
          : MOCK_SONG_WAR_ARTISTS.djSparkles.avatar,
      message: chatInput,
      timestamp: "Just now",
      role: role,
    ...
    if (action === "open_voting") {
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          ...
          avatar: MOCK_SONG_WAR_ARTISTS.treyTrizzy.avatar,
          message:
            'Commercial break placeholder: "Support the Tradio creator network. Double your fan votes with Trey TV pass."',
          timestamp: "Just now",
          role: "admin",
          badge: "Sponsor",
        },
    ...
    <div className="relative min-h-dvh flex flex-col">
      ...
      <div className="sticky top-0 z-20 border-b border-white/5 bg-black/40 px-4 py-4 backdrop-blur-md">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-white/8 bg-white/[0.03] px-3 py-2">
          ...
          <div className="flex flex-wrap gap-1.5 text-[9px] font-black uppercase tracking-wider">
            <span
              className={`rounded-full border px-2 py-1 ${chatLocked ? "border-rose-400/40 bg-rose-500/10 text-rose-200" : "border-emerald-400/30 bg-emerald-500/10 text-emerald-200"}`}
            >
              {chatLocked ? "Chat locked" : "Chat open"}
            </span>
            {slowMode && (
            ...
            <span
              className={`rounded-full border px-2 py-1 ${reactionsDisabled ? "border-white/10 bg-white/5 text-white/40" : "border-cyan-400/30 bg-cyan-500/10 text-cyan-200"}`}
            >
              {reactionsDisabled ? "Reactions off" : "Reactions live"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-4 sm:p-6">
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col justify-between">
          ...
          <GlassCard
            glow
            className="p-6 overflow-hidden bg-gradient-to-br from-[#100720]/90 via-[#04020a]/95 to-[#060A18]/90 border-purple-500/20 flex-1 flex flex-col justify-between relative min-h-[420px]"
          >
            ...
            <div className="flex justify-between items-center z-10 border-b border-white/5 pb-4">
              <div className="text-left">
                <div className="text-[10px] font-black uppercase tracking-[0.25em] bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
                  ROUND {currentRound.roundNumber} OF {battle.roundsCount}
                </div>
                <div className="text-xs text-white/50 mt-1 font-semibold tracking-wide">
                  {battle.title}
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 bg-white/[0.03] border border-white/8 rounded-2xl px-5 py-2 shadow-inner">
                <div className="text-right">
                  <div className="text-[8px] uppercase text-white/40 tracking-[0.18em] font-extrabold">
                    Session Score
                  </div>
              ...
              <div className="flex flex-col items-center md:items-start text-center md:text-left z-10">
                <div className="relative p-1.5 rounded-3xl bg-gradient-to-tr from-white/10 via-transparent to-white/5 border border-white/10 shadow-[0_15px_35px_rgba(0,0,0,0.6)]">
                  <img
                    ...
                    alt=""
                    className={`h-28 w-28 rounded-2xl object-cover transition-transform duration-700 ${isPlayingA && globalPlaybackPlaying ? "scale-105 shadow-[0_0_40px_rgba(244,63,94,0.6)] border border-fuchsia-400/40" : "opacity-65"}`}
                  />
                  {isPlayingA && (
                ...
                <div className="mt-5 min-w-0 w-full">
                  <span className="inline-block rounded-full bg-fuchsia-500/15 border border-fuchsia-500/30 px-3 py-1 text-[9px] font-black text-fuchsia-300 uppercase tracking-widest">
                    Artist A
                  </span>
                  ...
                  <div className="mt-5 bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/10 p-4 rounded-2xl flex items-center gap-4 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.05)] backdrop-blur-xl hover:border-white/20 transition-all duration-300">
                    <TradioImage
                      ...
                      isPlaying={
                        isPlayingA &&
                        currentItem?.id === currentRound.trackA.id &&
                        globalPlaybackPlaying
                      }
                      isLoading={false}
                      fallbackSize="mini"
                    ...
                    <div className="min-w-0 flex-1 text-left">
                      <div className="text-xs font-bold text-white tracking-wide truncate">
                        {currentRound.trackA.title}
                      </div>
                      <div className="text-[10px] text-fuchsia-300 font-bold mt-0.5 tracking-wider uppercase">
                        {battle.artistA.station}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center md:items-end text-center md:text-right mt-10 md:mt-0 z-10">
                <div className="relative p-1.5 rounded-3xl bg-gradient-to-tr from-white/10 via-transparent to-white/5 border border-white/10 shadow-[0_15px_35px_rgba(0,0,0,0.6)]">
                  <img
                    ...
                    alt=""
                    className={`h-28 w-28 rounded-2xl object-cover transition-transform duration-700 ${isPlayingB && globalPlaybackPlaying ? "scale-105 shadow-[0_0_40px_rgba(6,182,212,0.6)] border border-cyan-400/40" : "opacity-55"}`}
                  />
                  {isPlayingB && (
                ...
                <div className="mt-5 min-w-0 w-full">
                  <span className="inline-block rounded-full bg-cyan-500/15 border border-cyan-500/30 px-3 py-1 text-[9px] font-black text-cyan-300 uppercase tracking-widest">
                    Artist B
                  </span>
                  <h3 className="text-xl font-black text-white mt-2 tracking-tight">
                    {battle.artistB.name}
                  ...
                  <div className="mt-5 bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/10 p-4 rounded-2xl flex items-center gap-4 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.05)] backdrop-blur-xl hover:border-white/20 transition-all duration-300 flex-row-reverse">
                    <TradioImage
                      ...
                      isPlaying={
                        isPlayingB &&
                        currentItem?.id === currentRound.trackB.id &&
                        globalPlaybackPlaying
                      }
                      isLoading={false}
                      fallbackSize="mini"
                    ...
                    <div className="min-w-0 flex-1 text-right">
                      <div className="text-xs font-bold text-white tracking-wide truncate">
                        {currentRound.trackB.title}
                      </div>
                      <div className="text-[10px] text-cyan-300 font-bold mt-0.5 tracking-wider uppercase">
                        {battle.artistB.station}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="z-10 bg-black/20 p-4 border border-white/5 rounded-2xl">
              <div className="flex justify-between items-center text-[10px] text-white/40 uppercase tracking-widest mb-1 font-extrabold">
                ...
                <span className="text-purple-300 animate-pulse">
                  {roundStatus === "playing"
                    ? `00:${activePlaySec.toString().padStart(2, "0")} / 00:${currentRound.duration}`
                    : "Waiting on voting"}
                </span>
              </div>
          ...
          {roundStatus === "voting" ? (
              ...
              {!isVoteLocked ? (
                ...
                <div className="bg-emerald-500/10 border border-emerald-500/25 rounded-2xl p-4 text-center">
                  <span className="text-xs font-extrabold text-emerald-300 flex items-center justify-center gap-2">
                    <CheckCircle className="h-4 w-4" /> Your vote is securely locked on the Tradio
                    ledger!
                  </span>
                  <p className="text-[10px] text-white/40 mt-1">
                    Real-time tally counts update with listener blocks.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white/5 border border-white/8 rounded-2xl p-5 text-center">
              <span className="text-xs font-bold text-white/80 animate-pulse">
                {" "}
                Song is playing in live rotation...
              </span>
              <p className="text-[10px] text-white/40 mt-1">
                Voting panel opens automatically in {currentRound.duration - activePlaySec}s once
                track play ends.
              </p>
            </div>
          ) : (
            <div className="bg-purple-950/20 border border-purple-500/25 rounded-2xl p-4 text-center flex justify-between items-center">
              <span className="text-xs font-bold text-purple-200">
                Round {currentRound.roundNumber} is fully concluded.
              </span>
          ...
          <GlassCard className="flex-1 flex flex-col justify-between overflow-hidden bg-black/60 min-h-[450px] lg:max-h-[calc(100dvh-10rem)]">
            ...
            <div>
              <div className="flex border-b border-white/5 bg-black/40">
                {(
                  ["Live Chat", "Polls", "Requests", "Top Fans"] as (
                    | "Live Chat"
                    | "Polls"
                    | "Requests"
                    | "Top Fans"
              ...
              {pinnedMessage && (
                <div className="bg-purple-500/10 border-b border-purple-400/20 px-4 py-2.5 flex items-center justify-between text-[11px] text-purple-200">
                  <div className="flex items-center gap-2 truncate">
                    <span className="font-extrabold text-[10px] bg-purple-500/25 border border-purple-400/30 px-1.5 py-0.5 rounded uppercase tracking-wider">
                      PINNED
                    </span>
              ...
              {chatTab === "Live Chat" ? (
                    ...
                    <div key={msg.id} className="flex gap-2.5 items-start">
                      ...
                      <div className="min-w-0 flex-1 bg-white/[0.02] border border-white/5 rounded-2xl px-3 py-2">
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-[11px] font-black flex items-center gap-1 ${msg.isArtist ? "text-purple-300" : "text-white/70"}`}
                          >
                            {msg.author}
                            {msg.badge && (
                              <span
                ...
                <div className="space-y-4 py-4 text-center">
                  ...
                  <h4 className="text-xs font-black text-white uppercase tracking-widest">
                    Interactive Polls Room
                  </h4>
                  <p className="text-[11px] text-white/40 max-w-[200px] mx-auto">
                    No mini polls are running in this session. Host can launch a poll from the
                    control drawer.
                  </p>
                </div>
              ) : chatTab === "Requests" ? (
                <div className="space-y-3">
                  <h4 className="text-xs font-black text-white/50 uppercase tracking-widest mb-3">
                    Live Fan Requests
                  </h4>
                        ...
                        onClick={() => {
                          setChatMessages((prev) => [
                            ...prev,
                            {
                              ...
                              avatar: MOCK_SONG_WAR_ARTISTS.djSparkles.avatar,
                              message:
                                "Host accepted run-it-back request. Replaying Round 1 songs...",
                              timestamp: "Just now",
                              role: "dj",
                              badge: "Host",
                            },
                ...
                <div className="space-y-3">
                  ...
                  {[
                    ...
                    },
                    {
                      ...
                      pts: "15,110 pts",
                      avatar: MOCK_SONG_WAR_ARTISTS.milaRain.avatar,
                    },
                    {
                      name: "Liv",
                      badge: "Premiere Star",
                    ...
                    <div
                      key={vip.name}
                      className="flex items-center justify-between bg-white/[0.02] border border-white/5 p-3 rounded-2xl"
                    >
            ...
            <div className="p-4 border-t border-white/5 bg-black/40 relative">
              ...
              <div className="flex gap-2 mb-3">
                <span className="hidden items-center rounded-xl border border-white/8 bg-white/5 px-2 text-[9px] font-black uppercase tracking-wider text-white/40 sm:flex">
                  Animated reactions
                </span>
                <EmojiTray
          ...
          {role === "artist" && (
            <div className="bg-purple-950/20 border border-purple-500/25 rounded-2xl p-4">
              <h4 className="text-xs font-black text-purple-300 uppercase tracking-widest mb-1 flex items-center gap-1.5">
                <Radio className="h-4 w-4 text-purple-400 animate-pulse" /> Artist Participant
                Controls
              </h4>
              <p className="text-[10px] text-white/50 leading-relaxed mb-3">
                As an active competitor, you can record voice drops and post live notices directly
                to fan chat queues.
              </p>
              <button
