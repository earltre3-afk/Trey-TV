...
export const BattleResults: React.FC<BattleResultsProps> = ({ battleId = 'battle-1', onNavigate, onReturnHome }) => {
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-10">
      ...
      <div className="mb-6 flex justify-between items-center flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-white/40 uppercase tracking-widest font-black">Official Results Ledger</span>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="relative rounded-3xl border border-yellow-500/20 bg-gradient-to-b from-[#1E1103] via-[#0D081F] to-[#0A0A0F] p-8 md:p-12 overflow-hidden shadow-[0_0_50px_rgba(234,179,8,0.3)] text-center">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-col items-center justify-center">
              <div className="relative">
                <img src={winner.avatar} alt={winner.name} className="h-32 w-32 rounded-full object-cover border-4 border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.4)]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-2xl border border-white/5 bg-black/40 p-5 text-center">
            <div className="text-[10px] uppercase text-white/40 tracking-wider">Total Tally Votes</div>
            <div className="text-2xl font-black text-white mt-1.5">{totalVotesCount.toLocaleString()}</div>
            <p className="text-[9px] text-emerald-400 mt-1 font-bold">+14% over baseline</p>
          </div>
          <div className="rounded-2xl border border-white/5 bg-black/40 p-5 text-center">
            <div className="text-[10px] uppercase text-white/40 tracking-wider">Peak Live Audience</div>
            <div className="text-2xl font-black text-white mt-1.5">{peakListenersCount.toLocaleString()}</div>
            <p className="text-[9px] text-purple-300 mt-1 font-bold">14.8K live at final round</p>
          </div>
          <div className="rounded-2xl border border-white/5 bg-black/40 p-5 text-center">
            <div className="text-[10px] uppercase text-white/40 tracking-wider">Trophy Prize Pool</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Winning Playlist */}
            <GlassCard className="p-6">
              <h3 className="text-base font-extrabold text-white mb-3">Winning Champion Showcase</h3>
              <p className="text-xs text-white/50 mb-4">The championship tracks that conquered the session rounds.</p>
              
              <div className="space-y-3">
                {battle.rounds.map((round) => {
                  const song = round.winner === 'A' ? round.trackA : round.trackB;
                  return (
                    <div key={round.roundNumber} className="flex items-center justify-between bg-white/[0.02] border border-white/5 p-3.5 rounded-2xl hover:border-white/10 transition-all">
          ...
          <div className="lg:col-span-1 space-y-6">
            <GlassCard className="p-5 space-y-4">
              <h3 className="text-sm font-black text-white uppercase tracking-wider">Post-Session Actions</h3>

              <button 
                onClick={() => alert('Sponsor action: Added winner tracks to your Station Rotation!')}
              ...
              <div className="border-t border-white/5 pt-4">
                <div className="text-[10px] uppercase text-white/40 font-bold mb-3">Follow Competitors</div>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-white/70 truncate max-w-[120px]">{battle.artistA.name}</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-white/70 truncate max-w-[120px]">{battle.artistB.name}</span>
                    <button className="text-[10px] font-bold border border-white/12 bg-white/5 px-2.5 py-1 rounded-full hover:bg-white/10 text-white">
                      Follow
                    </button>
                  </div>
            ...
            <GlassCard className="p-5 text-center">
              ...
              <p className="text-[10px] text-white/40 mt-1 leading-normal">Relive the battle, read fan chat replays, and check round chapters.</p>
              <button 
