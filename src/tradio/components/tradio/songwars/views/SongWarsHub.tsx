...
import { Sparkles, Play, Calendar, RotateCcw, Plus, Radio, Users, Flame, ChevronRight, BarChart3, Award, Star, Trophy, Activity } from 'lucide-react';
...
export const SongWarsHub: React.FC<SongWarsHubProps> = ({ onNavigate, role, onRoleChange, onBack }) => {
  const { identity } = useTradioIdentity();
  const [filter, setFilter] = useState<'Live' | 'Upcoming' | 'Replays' | 'My Battles' | 'Following'>('Live');
  ...
  const renderList = () => {
          ...
          {live.map(b => (
            ...
            >
              ...
              <div className="p-7 relative z-10">
                ...
                <p className="text-xs font-semibold text-purple-300/90 mt-1.5 uppercase tracking-wider">{b.type}</p>
                ...
                <div className="mt-6 relative flex items-center justify-between rounded-2xl bg-black/40 p-4 border border-white/5 shadow-[inset_0_1px_3px_rgba(0,0,0,0.4)]">
                  ...
                  <div className="flex items-center gap-3.5 w-[42%]">
                    <div className="relative shrink-0">
                      <img src={b.artistA.avatar} alt="" className="h-12 w-12 rounded-xl object-cover border border-white/10 group-hover:scale-105 transition-transform duration-500" />
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-fuchsia-400/80 font-bold uppercase tracking-wider truncate">{b.artistA.station}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3.5 w-[42%] flex-row-reverse text-right">
                    <div className="relative shrink-0">
                      <img src={b.artistB.avatar} alt="" className="h-12 w-12 rounded-xl object-cover border border-white/10 group-hover:scale-105 transition-transform duration-500" />
                      <div className="absolute inset-0 rounded-xl bg-gradient-to-t from-black/50 to-transparent" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-black text-white truncate">{b.artistB.name}</div>
                      <div className="text-[10px] text-cyan-400/80 font-bold uppercase tracking-wider truncate">{b.artistB.station}</div>
                    </div>
                  </div>
                </div>

                <div className="mt-6 flex items-center justify-between gap-4 border-t border-white/5 pt-4">
                  <span className="text-[11px] text-white/40">Hosted by <strong className="text-white/70 font-semibold">{b.hostName}</strong></span>
                  <button 
                  ...
                  >
                    <Play className="h-3.5 w-3.5 fill-current text-white animate-pulse" /> Enter Battle Stage
                  </button>
                </div>
              </div>
            </GlassCard>
            ...
            >
              ...
              <div className="p-7 relative z-10">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-extrabold text-white/50 tracking-wider uppercase">{b.scheduleDate}</span>
                </div>
                ...
                <p className="text-xs font-semibold text-cyan-400/90 mt-1.5 uppercase tracking-wider">{b.type}</p>
                ...
                <div className="mt-6 grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-3 bg-black/30 border border-white/5 p-3.5 rounded-2xl hover:border-white/10 transition-colors">
                    <img src={b.artistA.avatar} alt="" className="h-10 w-10 rounded-xl object-cover border border-white/10" />
                    <span className="text-xs font-black text-white truncate">{b.artistA.name}</span>
                  </div>
                  <div className="flex items-center gap-3 bg-black/30 border border-white/5 p-3.5 rounded-2xl hover:border-white/10 transition-colors">
                    <img src={b.artistB.avatar} alt="" className="h-10 w-10 rounded-xl object-cover border border-white/10" />
                ...
                <div className="mt-6 flex items-center justify-between border-t border-white/5 pt-4">
                  <span className="text-[11px] text-white/40 flex items-center gap-1">
                    Prize: <strong className="text-yellow-400 font-extrabold flex items-center gap-1 ml-1"><Trophy className="h-3.5 w-3.5" />{b.prize}</strong>
                  </span>
                  <button 
                    onClick={() => handleBattleClick(b)}
            ...
            >
              <div className="flex flex-col sm:flex-row">
                <div className="relative w-full sm:w-1/3 h-36 sm:h-auto overflow-hidden">
                  <img src={r.artwork} alt="" className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-700" />
                ...
                <div className="p-6 flex-1 flex flex-col justify-between relative z-10">
                  <div>
                    ...
                    <h3 className="mt-3 text-base font-black text-white leading-snug group-hover:text-purple-300 transition-colors">{r.title}</h3>
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-white/5 pt-4">
                    <span className="text-[11px] font-bold text-emerald-400 flex items-center gap-1">
                      <Star className="h-3.5 w-3.5 fill-current animate-pulse text-emerald-400" /> Winner: {r.winnerName}
                    </span>
                    <button 
    ...
    return (
      <div className="text-center py-20 border border-dashed border-white/10 rounded-3xl bg-white/[0.01] backdrop-blur-sm">
        ...
        <div className="mt-8">
          <SecondaryButton onClick={() => can(identity, 'access-studio') && onNavigate({ view: 'setup' })} className="py-2.5 px-5 text-xs font-black uppercase tracking-wider border-white/10">
            <Plus className="h-4 w-4 text-purple-400" /> {can(identity, 'access-studio') ? 'Setup Custom Battle' : 'Creator Access Required'}
          </SecondaryButton>
        </div>
      </div>
    );
    ...
    <div className="space-y-8 pb-6 lg:space-y-10">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-black/60 border-[0.5px] border-white/10 p-4 rounded-3xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_15px_30px_rgba(0,0,0,0.5)] backdrop-blur-2xl">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="block text-[10px] font-black tracking-widest text-emerald-400 uppercase">Mock Role View</span>
            <span className="hidden sm:block text-[10px] font-semibold text-white/35">Local-only permission preview</span>
          </div>
        </div>
      ...
      <div className="relative rounded-[2.5rem] border-[0.5px] border-white/10 bg-gradient-to-br from-[#180A2D]/55 via-[#070514]/75 to-[#05050A]/95 p-8 md:p-14 overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.8)]">
        ...
        <div className="relative max-w-2xl">
          ...
          <h1 className="mt-5 text-4xl md:text-6xl font-black text-white tracking-tight leading-tight">
            Song <span className="bg-gradient-to-r from-fuchsia-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent">Wars</span>
          </h1>
          <p className="mt-4 text-sm md:text-base text-white/60 leading-relaxed font-medium">
            Artist vs Artist. Song for song. Fans decide in real time. Live music battles with dynamic equalizers and interactive reaction clashing.
          </p>

          <div className="mt-8 flex flex-wrap gap-4">
            <PrimaryButton onClick={() => can(identity, 'access-studio') && onNavigate({ view: 'setup' })} className="py-3 px-6 text-xs font-black uppercase tracking-wider shadow-lg">
              <Plus className="h-4 w-4 text-white" /> {can(identity, 'access-studio') ? 'Create Custom Battle' : 'Creator Access Required'}
            </PrimaryButton>
            <SecondaryButton onClick={() => onNavigate({ view: 'stage', battleId: 'battle-1' })} className="py-3 px-6 text-xs font-black uppercase tracking-wider border-white/10 hover:border-white/20">
      ...
      <div>
        <div className="flex justify-between items-center mb-5">
          ...
          <span className="text-xs text-white/40 uppercase font-black tracking-wider">PvP Configurations</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          {SONG_WAR_CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                <div className={`rounded-xl bg-white/5 p-2.5 border border-white/5 text-purple-300 transition-all duration-300 ${isSelected ? 'bg-fuchsia-500/20 text-fuchsia-300 border-fuchsia-500/30 shadow-[0_0_10px_rgba(217,70,239,0.3)]' : 'group-hover:bg-purple-500/10'}`}>
                ...
                <div className="w-full">
                  <div className="text-xs font-black text-white truncate max-w-full group-hover:text-fuchsia-300 transition-colors">{cat.title}</div>
                  <div className="text-[10px] font-bold text-white/40 mt-1 uppercase tracking-wider">{cat.count} battles</div>
                </div>
              </button>
            );
          })}
