...
export const BattleReplayScreen: React.FC<BattleReplayScreenProps> = ({
  replayId = "replay-1",
  onNavigate,
  onReturnHome,
}) => {
        ...
        <div className="relative rounded-3xl border border-white/10 bg-[#0B0714] overflow-hidden shadow-premium">
          <div className="aspect-video md:aspect-[21/9] w-full relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
              ...
              <p className="text-xs text-purple-300 mt-1 uppercase tracking-widest font-extrabold">
                {replay.playedAt} - Completed Replay
              </p>
            </div>
          </div>
          ...
          <div className="p-6 border-t border-white/5 bg-black/60 flex flex-wrap gap-4 items-center justify-between">
            <div className="flex gap-6 text-xs text-white/50">
              <div>
                ...
                <strong className="text-white text-sm font-black mt-0.5">
                  {replay.totalVotes.toLocaleString()}
                </strong>
              </div>
              <div>
                <span className="block text-[10px] uppercase text-white/30 tracking-wider">
                ...
                <strong className="text-white text-sm font-black mt-0.5">
                  {replay.peakListeners.toLocaleString()}
                </strong>
              </div>
              <div>
                <span className="block text-[10px] uppercase text-white/30 tracking-wider">
                ...
                <strong className="text-yellow-300 text-sm font-black mt-0.5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3.5 min-w-0">
                        <div
                          className={`h-8 w-8 rounded-lg flex items-center justify-center font-bold text-xs ${isActive ? "bg-purple-500 text-white" : "bg-white/5 text-white/50"}`}
                        >
                          R{round.roundNumber}
                        </div>
                        <div className="min-w-0">
                          <div className="text-xs font-bold text-white truncate">
                            {round.trackA.title} vs {round.trackB.title}
                          </div>
                          <div className="text-[10px] text-white/40 truncate">
                            Matchup length: {round.duration}s
                          </div>
                        </div>
                      </div>
            ...
            <GlassCard className="p-6">
              ...
              <div className="space-y-2">
                {replay.rounds.map((round) => {
                  ...
                  return (
                    <div
                      key={round.roundNumber}
                      className="flex items-center justify-between bg-white/[0.01] p-3 rounded-xl border border-white/5"
                    >
              ...
              <PrimaryButton
                ...
                onClick={() => alert("Playing all winning songs in order.")}
              >
                Play Winning Songs
              </PrimaryButton>
            </GlassCard>
          ...
          <div className="lg:col-span-1 space-y-6">
                ...
                <div className="flex items-center justify-between mb-3 border-b border-white/5 pb-2">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer text-[10px] font-bold text-purple-300 select-none">
                    <input
                      type="checkbox"
                      checked={chatReplay}
                      onChange={() => setChatReplay(!chatReplay)}
                ...
                {chatReplay ? (
                  <div className="space-y-3 max-h-[200px] overflow-y-auto no-scrollbar">
                    <div className="text-[10px] text-white/50">
                      <strong className="text-purple-300 font-bold">[00:15] Casey:</strong> ATL
                      Sound is destroying this sub-bass loop!
                    </div>
                    <div className="text-[10px] text-white/50">
                      <strong className="text-cyan-300 font-bold">[00:42] Alex:</strong> LA Sunset
                      vibe is pure liquid glass though
                    </div>
                    <div className="text-[10px] text-white/50">
                      <strong className="text-purple-300 font-bold">[01:10] Moderator_D:</strong>{" "}
                      34K concurrent streams right now! Let\'s go!
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-10 text-[10px] text-white/30 italic">
                    Chat replay muted. Toggle sync to view historical chat blocks.
                  </div>
                )}
              </div>
            ...
            <GlassCard className="p-5">
              <h4 className="text-xs font-black uppercase tracking-widest text-white/60 mb-3">
                Related Replays
              </h4>
              <div className="space-y-3">
