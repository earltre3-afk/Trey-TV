...
): LegalServiceResult<T> => ({ source, data, warning });
...
const mapRow = (row: Record<string, unknown>): LegalRequestRow => ({
  ...
  payload:
    (row.payload as Record<string, unknown>) ?? (row.details as Record<string, unknown>) ?? {},
...
export const submitLegalRequest = async (
  ...
  if (!client) {
    return ok<LegalRequestRow>(
      "unavailable",
      null,
      "Supabase is not configured. The request is preserved as a UI placeholder.",
    );
  }
...
export const submitAccountDeletionRequest = async (
  ...
  if (!client) {
    return ok<LegalRequestRow>(
      "unavailable",
      null,
      "Supabase is not configured. Deletion request remains a frontend placeholder.",
    );
  }
...
export const recordLegalAcceptance = async (
  ...
  if (!client) {
    return ok<Record<string, unknown>>(
      "unavailable",
      null,
      "Supabase is not configured. Acceptance is not persisted.",
    );
  }
...
export const recordUploadRights = async (
  ...
  if (!client) {
    return ok<Record<string, unknown>>(
      "unavailable",
      null,
      "Supabase is not configured. Upload rights confirmation is not persisted.",
    );
  }
