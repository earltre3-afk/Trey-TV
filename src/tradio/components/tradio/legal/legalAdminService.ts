...
const serviceWarning = () => isSupabaseConfigured
...
const supabaseWarning = () => 'Loaded from protected legal operations RPC. Deletion, DMCA, and takedown actions remain review-only.';
const rpcWarning = (error: unknown) => `${handleMissingTradioTables(error).message} Showing mock legal operations records.`;

const maybeSupabase = () => (isSupabaseConfigured && supabase ? supabase : null);

const event = (requestId: string, action: LegalRequestEvent['action'], actor: string, note: string, createdAt: string): LegalRequestEvent => ({
...
let mockRequests: LegalAdminRequest[] = [
  {
    ...
    requester: { name: 'Jordan Miles', email: 'jordan@example.com', user_id: 'user_fan_001', public_profile_uid: 'treytv_fan_jordan_001' },
    status: 'new',
    priority: 'normal',
    submitted_at: '2026-05-29T09:15:00.000Z',
    summary: 'User requested an export of Trey TV and Tradio account data.',
    details: { requestType: 'Export my data', scope: 'Trey TV profile, Tradio listening history, saved stations' },
    related_policy: 'privacy',
    reviewer_note: 'Verify identity before export packaging.',
    events: baseEvents('leg-data-export-001', '2026-05-29T09:15:00.000Z', 'Data export request submitted.'),
  },
  {
    id: 'leg-delete-002',
    category: 'deletion',
    ...
    requester: { name: 'Mila Rain', email: 'mila@example.com', user_id: 'user_artist_002', public_profile_uid: 'treytv_artist_mila_rain' },
    status: 'pending_review',
    priority: 'high',
    submitted_at: '2026-05-29T10:22:00.000Z',
    summary: 'Artist requested account deletion and acknowledged data loss.',
    details: { confirmed: true, acknowledged: true, creatorContent: 'artist station, releases, profile draft' },
    related_policy: 'data-retention',
    reviewer_note: 'No irreversible deletion action is wired. Needs identity and retention review.',
    events: baseEvents('leg-delete-002', '2026-05-29T10:22:00.000Z', 'Deletion request entered queue.'),
    confirmation_status: { confirmed_delete: true, acknowledged_retention: true, identity_verified: false },
    retained_records_note: 'Retention exceptions may apply for audit, safety, copyright, and financial/legal records.',
  } as LegalDeletionRequest,
  {
    ...
    requester: { name: 'Rights Desk', email: 'rights@example-label.com', user_id: 'external-rights-contact' },
    status: 'open',
    priority: 'urgent',
    ...
    details: { claimant: 'Example Label', claimedWork: 'Midnight Demo Master', allegedlyInfringingUrl: '/tradio/release/falling-for-you' },
    related_policy: 'dmca',
  ...
  } as LegalDmcaRequest,
  {
    ...
    requester: { name: 'Trey Trizzy', email: 'trey@example.com', user_id: 'user_artist_004', public_profile_uid: 'treytv_artist_trey_trizzy' },
    ...
    details: { originalNoticeId: 'leg-dmca-003', statement: 'Artist claims ownership/control of the upload.' },
    related_policy: 'dmca',
  ...
  } as LegalDmcaRequest,
  {
    ...
    requester: { name: 'Darius Cole', email: 'darius@example.com', user_id: 'user_producer_005', public_profile_uid: 'treytv_producer_darius' },
    status: 'open',
    priority: 'high',
    ...
    events: baseEvents('leg-unauth-005', '2026-05-28T16:02:00.000Z', 'Unauthorized upload report submitted.'),
  },
  {
    id: 'leg-songwars-006',
    category: 'moderation',
    ...
    requester: { name: 'Kiana Lane', email: 'kiana@example.com', user_id: 'user_artist_006', public_profile_uid: 'treytv_artist_kiana_lane' },
    status: 'escalated',
    priority: 'normal',
    submitted_at: '2026-05-28T13:40:00.000Z',
    summary: 'Artist appeals a Song Wars disqualification for alleged vote manipulation.',
    details: { battleId: 'battle_late_night_trap_soul', round: 3, appealReason: 'Fan activity was organic.' },
    ...
    events: [
      ...
      event('leg-songwars-006', 'escalated', 'legal-admin-mock', 'Escalated for vote audit placeholder.', '2026-05-28T14:05:00.000Z'),
    ],
  ...
  } as LegalModerationAppeal,
  {
    ...
    requester: { name: 'Noel Beats', email: 'noel@example.com', user_id: 'user_producer_007', public_profile_uid: 'treytv_producer_noel' },
    status: 'new',
    priority: 'normal',
    submitted_at: '2026-05-29T08:30:00.000Z',
    summary: 'Producer appeals a rejected producer access request.',
    details: { requestedRole: 'producer', previousDecision: 'rejected', reason: 'Submitted additional catalog links.' },
  ...
  } as LegalModerationAppeal,
  {
    ...
    requester: { name: 'Ari Fan', email: 'ari@example.com', user_id: 'user_fan_008', public_profile_uid: 'treytv_fan_ari' },
    status: 'open',
    priority: 'urgent',
  ...
  },
  {
    ...
    requester: { name: 'Sam Listener', email: 'sam@example.com', user_id: 'user_fan_009', public_profile_uid: 'treytv_fan_sam' },
    status: 'new',
    priority: 'low',
    ...
    events: baseEvents('leg-prescribe-009', '2026-05-29T07:05:00.000Z', 'Privacy question submitted.'),
  },
];
...
const mapRequestRow = (row: Record<string, unknown>): LegalAdminRequest => {
  const details = (row.details as Record<string, unknown>) ?? (row.payload as Record<string, unknown>) ?? {};
  const requestType = String(row.request_type ?? 'contact_support') as LegalAdminRequest['request_type'];
  ...
  return {
    ...
    title: subject,
    requester: {
      name: (row.requester_name as string) || (row.requester_email as string) || (row.contact_email as string) || 'Requester',
    ...
    submitted_at: (row.submitted_at as string) ?? (row.created_at as string) ?? new Date().toISOString(),
...
const mapEventRow = (row: Record<string, unknown>): LegalRequestEvent => ({
  ...
  action: row.event_type === 'submitted' ? 'created' : (row.event_type as LegalRequestEvent['action']) ?? 'status_changed',
...
const mapAuditRow = (row: Record<string, unknown>): LegalAcceptanceAuditRecord => ({
  ...
  rights_confirmation_type: (row.rights_confirmation_type as LegalAcceptanceAuditRecord['rights_confirmation_type']) ?? undefined,
  related_reference_id: (row.related_reference_id as string) ?? undefined,
  summary: row.record_kind === 'upload_rights' ? 'Upload rights confirmation recorded.' : 'Legal policy acceptance recorded.',
});
...
const fetchSupabaseRequests = async (filters: LegalAdminQueueFilter = {}) => {
  ...
  return (Array.isArray(data) ? data : []).map((row) => mapRequestRow(row as Record<string, unknown>));
};

const applyFilters = (records: LegalAdminRequest[], filters: LegalAdminQueueFilter = {}) => records.filter((record) => {
  ...
  if (filters.requestType && !String(record.request_type).includes(filters.requestType)) return false;
  if (filters.query) {
    const haystack = `${record.id} ${record.title} ${record.summary} ${record.requester.name} ${record.requester.user_id ?? ''} ${record.requester.public_profile_uid ?? ''}`.toLowerCase();
    if (!haystack.includes(filters.query.toLowerCase())) return false;
  }
  if (filters.tab && filters.tab !== 'all') {
    const tabCategories: Partial<Record<NonNullable<LegalAdminQueueFilter['tab']>, LegalRequestCategory[]>> = {
...
const result = <T>(data: T): LegalAdminServiceResult<T> => ({ source: source(), data, warning: serviceWarning() });
const supabaseResult = <T>(data: T): LegalAdminServiceResult<T> => ({ source: 'supabase', data, warning: supabaseWarning() });
const mockResult = <T>(data: T, warning: string | null = serviceWarning()): LegalAdminServiceResult<T> => ({ source: source(), data, warning });

export const getLegalAdminDashboardStats = async (): Promise<LegalAdminServiceResult<LegalAdminDashboardStats>> => {
  try {
    ...
    if (supabaseRequests) {
      ...
      const stats: LegalAdminDashboardStats = {
        ...
        copyright_dmca_reports: supabaseRequests.filter((item) => ['copyright', 'dmca', 'unauthorized_upload'].includes(item.category)).length,
        unresolved_appeals: supabaseRequests.filter((item) => ['moderation_appeal', 'role_appeal', 'creator_profile_appeal'].includes(item.request_type) && !['resolved', 'rejected', 'archived'].includes(item.status)).length,
        acceptance_records_today: audit.data.filter((item) => item.accepted_at.startsWith(today)).length,
      };
  ...
  const stats: LegalAdminDashboardStats = {
    ...
    copyright_dmca_reports: mockRequests.filter((item) => ['copyright', 'dmca', 'unauthorized_upload'].includes(item.category)).length,
    unresolved_appeals: mockRequests.filter((item) => ['moderation_appeal', 'role_appeal', 'creator_profile_appeal'].includes(item.request_type) && !['resolved', 'rejected', 'archived'].includes(item.status)).length,
    acceptance_records_today: mockAuditRecords.filter((item) => item.accepted_at.startsWith(today)).length,
  };
  return result(stats);
};
...
export const getLegalRequestById = async (id: string) => {
  ...
  if (client) {
    try {
      ...
      if (error) throw error;
      const row = (Array.isArray(data) ? data : []).find((item) => String((item as Record<string, unknown>).id) === id);
      if (row) {
...
export const updateLegalRequestStatus = async (id: string, status: LegalAdminRequestStatus, note: string) => {
  const client = maybeSupabase();
  if (client) {
    try {
      ...
      if (error) throw error;
      const request = mapRequestRow((Array.isArray(data) ? data[0] : data) as Record<string, unknown>);
      const events = await getLegalRequestEvents(id);
      return supabaseResult({ ...request, events: events.data });
    } catch {
  ...
  mockRequests = mockRequests.map((item) => {
    ...
    const nextEvent = event(id, 'status_changed', 'legal-admin-mock', note || `Status changed to ${status}.`, new Date().toISOString());
    return { ...item, status, reviewer_note: note || item.reviewer_note, events: [nextEvent, ...item.events] };
  });
...
export const escalateLegalRequest = async (id: string, reason: string) => updateLegalRequestStatus(id, 'escalated', reason || 'Escalated for legal operations review.');
...
export const getLegalAcceptanceAuditRecords = async (filters: LegalAdminQueueFilter = {}) => {
  ...
  if (client) {
    try {
      ...
      if (error) throw error;
      return supabaseResult((Array.isArray(data) ? data : []).map((row) => mapAuditRow(row as Record<string, unknown>)));
    } catch (error) {
  ...
  const filtered = mockAuditRecords.filter((record) => {
    if (filters.query) {
      const haystack = `${record.user_id} ${record.public_profile_uid ?? ''} ${record.flow_id} ${record.policy_id} ${record.related_reference_id ?? ''}`.toLowerCase();
      if (!haystack.includes(filters.query.toLowerCase())) return false;
    }
...
export const getDeletionRequests = async (filters: LegalAdminQueueFilter = {}) => result(applyFilters(mockRequests, { ...filters, category: 'deletion' }) as LegalDeletionRequest[]);
export const getDmcaRequests = async (filters: LegalAdminQueueFilter = {}) => result(applyFilters(mockRequests, filters).filter((item) => ['dmca', 'copyright', 'unauthorized_upload'].includes(item.category)) as LegalDmcaRequest[]);
export const getModerationAppeals = async (filters: LegalAdminQueueFilter = {}) => result(applyFilters(mockRequests, filters).filter((item) => ['moderation', 'creator'].includes(item.category)) as LegalModerationAppeal[]);
