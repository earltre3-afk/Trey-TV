...
export const LegalStatusBadge: React.FC<{ status: LegalStatus }> = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${meta.tone}`}>
...
export const LegalCategoryCard: React.FC<{ group: LegalHomeGroup; onOpen: (target: string) => void }> = ({ group, onOpen }) => (
    ...
    <ul className="mt-3 space-y-1">
      {group.links.map((link) => (
        <li key={link.target}>
          ...
          >
            <span className="inline-flex items-center gap-2"><FileText className="h-3.5 w-3.5 text-white/40" /> {link.label}</span>
...
export const LegalLinkList: React.FC<{ title?: string; links: { label: string; target: string }[]; onOpen: (target: string) => void }> = ({ title = 'Related', links, onOpen }) => {
  if (links.length === 0) return null;
  return (
    <div>
      <div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-white/40">{title}</div>
...
export const LegalAcceptanceCheckbox: React.FC<{
  ...
  <label className="flex cursor-pointer items-start gap-2.5 text-[12px] leading-relaxed text-white/72">
    ...
    />
    <span>{children}{required && <span className="ml-1 text-fuchsia-300">*</span>}</span>
  </label>
);

export const PolicyLinkInline: React.FC<{ target: string; children: React.ReactNode; className?: string }> = ({ target, children, className = '' }) => {
...
export const LegalFlowNotice: React.FC<{ children?: React.ReactNode }> = ({ children }) => (
  <div className="rounded-2xl border border-amber-300/20 bg-amber-500/10 px-3 py-2.5 text-[11px] leading-relaxed text-amber-100/80">
    {children ?? 'Legal language is pending counsel review. These checkpoints prepare acceptance records without making final legal claims.'}
  </div>
);

export const LegalAcceptanceStatus: React.FC<{ status?: 'idle' | 'saving' | 'saved' | 'fallback' | 'error'; message?: string | null }> = ({ status = 'idle', message }) => {
  ...
  return (
    <div className={`flex items-start gap-2 rounded-2xl border px-3 py-2 text-[11px] leading-relaxed ${tone}`}>
      {success ? <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" /> : <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" />}
      ...
      <div className="mt-3 space-y-2.5">
        ...
        {config.requiredPolicies.map((item) => (
          <LegalAcceptanceCheckbox
            ...
            required={item.required}
          >
            I agree to the <PolicyLinkInline target={item.documentId}>{item.title}</PolicyLinkInline>
      ...
      {config.relatedLegalRoutes.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {config.relatedLegalRoutes.map((link) => (
            <PolicyLinkInline key={link.target} target={link.target}>{link.label}</PolicyLinkInline>
          ))}
        </div>
      )}
...
export const LegalVersionHistory: React.FC<{ entries?: LegalVersionEntry[]; version: string; lastUpdated: string }> = ({ entries, version, lastUpdated }) => (
  <div>
    <div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-white/40">Version history</div>
    <ul className="space-y-1.5">
      {(entries && entries.length > 0 ? entries : [{ version, date: lastUpdated, note: 'Initial placeholder draft.' }]).map((entry) => (
        <li key={entry.version} className="flex items-start gap-2 text-[11px] text-white/55">
          <span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 font-mono text-[10px] text-white/60">v{entry.version}</span>
...
export const LegalFooterLinks: React.FC<{ onOpen: (target: string) => void; className?: string }> = ({ onOpen, className = '' }) => {
  ...
  return (
    <div className={`flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[11px] text-white/45 ${className}`}>
      {links.map((link, index) => (
        <React.Fragment key={link.target}>
          {index > 0 && <span className="text-white/20">·</span>}
          <button onClick={() => onOpen(link.target)} className="transition hover:text-white/80">{link.label}</button>
        </React.Fragment>
      ))}
    </div>
  );
