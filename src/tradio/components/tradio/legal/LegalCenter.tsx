 ...
 */
export const LegalCenter: React.FC<{ initialTarget?: string; onExit: () => void }> = ({
  initialTarget,
  onExit,
}) => {
  ...
  const Header = (
    <div className="flex items-center justify-between px-4 pt-[max(2rem,env(safe-area-inset-top))] pb-3 sm:px-6 lg:px-10">
      <button
        onClick={target ? backToHome : onExit}
        aria-label="Back"
  ...
  if (!target) {
    ...
    body = (
      <GlassCard className="p-8 text-center text-white/60">Unknown legal page: {target}</GlassCard>
    );
  }
