...
const statusOptions: (LegalAdminRequestStatus | "all")[] = [
  "all",
  "new",
  "open",
  "pending_review",
...
const priorityOptions: (LegalRequestPriority | "all")[] = [
  "all",
  "low",
  "normal",
  "high",
...
const categoryOptions: (LegalRequestCategory | "all")[] = [
  "all",
  "privacy",
  "data_rights",
  "deletion",
...
const selectClass =
  "rounded-2xl border border-white/10 bg-black/35 px-3 py-2 text-xs font-bold text-white outline-none";

const StatusPill: React.FC<{ status: LegalAdminRequestStatus }> = ({ status }) => (
  <span
    className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${statusTone[status]}`}
  >
    {formatLabel(status)}
  </span>
);
...
const DetailRows: React.FC<{ details: Record<string, unknown> }> = ({ details }) => {
  ...
  if (entries.length === 0)
    return <div className="text-xs text-white/45">No submitted form data attached.</div>;
  return (
...
const RequestDetailPanel: React.FC<{
    ...
    return (
      <GlassCard className="p-5">
        ...
        <p className="mt-2 text-xs leading-relaxed text-white/55">
          Open a queue item to review its request metadata, timeline, notes, and safe prototype
          actions.
        </p>
      </GlassCard>
    );
  ...
  const deletion = isDeletion
    ? (request as LegalAdminRequest & {
        confirmation_status?: {
    ...
    <GlassCard glow className="p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <span
            className={`rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${priorityTone[request.priority]}`}
          >
            {request.priority}
          </span>
        </div>
      ...
      {(isDeletion || isDmca) && (
        <div className="mt-4 rounded-2xl border border-amber-300/20 bg-amber-500/10 p-3 text-xs leading-relaxed text-amber-100/85">
          <div className="flex items-center gap-2 font-black text-amber-100">
            ...
            {isDeletion ? "Deletion processing not wired" : "DMCA workflow placeholder"}
          </div>
          <div className="mt-1">
            {isDeletion
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          {Object.entries(deletion.confirmation_status).map(([key, value]) => (
            <div key={key} className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
              ...
              <div
                className={`mt-1 text-xs font-bold ${value ? "text-emerald-200" : "text-amber-200"}`}
              >
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-2">
        <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
          ...
          <div className="mt-2 break-all text-[11px] text-white/40">
            {request.requester.user_id ?? "No user id"} ·{" "}
            {request.requester.public_profile_uid ?? "No public profile uid"}
          </div>
        </div>
        <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
          <div className="text-xs font-black uppercase tracking-wider text-white/38">Related</div>
          <div className="mt-2 text-xs text-white/60">
            Policy: {request.related_policy ?? "Not attached"}
          </div>
          <div className="mt-1 break-all text-xs text-white/60">
            Content: {request.related_content_id ?? "Not attached"}
          </div>
      ...
      <div className="mt-5">
        <div className="mb-2 text-xs font-black uppercase tracking-wider text-white/38">
          Submitted form data
        </div>
          ...
          {request.events.map((item) => (
            <div key={item.id} className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
              <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] font-bold text-white/55">
                <span>
                  {formatLabel(item.action)} · {item.actor}
                </span>
      ...
      <div className="mt-5">
        <label className="text-xs font-black uppercase tracking-wider text-white/38">
          Reviewer note
        </label>
        <textarea
          value={note}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <SecondaryButton className="px-4 py-2 text-[10px]" onClick={() => onAction("open")}>
            Mark open
          </SecondaryButton>
          <SecondaryButton
          ...
          <SecondaryButton className="px-4 py-2 text-[10px]" onClick={() => onAction("escalated")}>
            Escalate
          </SecondaryButton>
          <SecondaryButton className="px-4 py-2 text-[10px]" onClick={() => onAction("resolved")}>
            Resolve
          </SecondaryButton>
          <SecondaryButton className="px-4 py-2 text-[10px]" onClick={() => onAction("rejected")}>
            Reject
          </SecondaryButton>
          <SecondaryButton className="px-4 py-2 text-[10px]" onClick={() => onAction("archive")}>
            <Archive className="h-3.5 w-3.5" /> Archive
          </SecondaryButton>
          <PrimaryButton className="px-4 py-2 text-[10px]" onClick={() => onAction("note")}>
  ...
  <GlassCard className="overflow-hidden p-4">
            ...
            <tr key={record.id}>
              <td className="py-3 pr-3 align-top">
                ...
                <div className="text-[11px] text-white/38">
                  {record.public_profile_uid ?? "No profile uid"}
                </div>
              </td>
              <td className="py-3 pr-3 align-top">{record.flow_id}</td>
              <td className="py-3 pr-3 align-top">
                {record.policy_id} v{record.policy_version}
              </td>
              ...
              <td className="py-3 pr-3 align-top">
                {record.rights_confirmation_type ?? "acknowledgement"}
              </td>
    ...
    {records.length === 0 && (
      <div className="py-8 text-center text-xs text-white/45">
        No acceptance records match the current filters.
      </div>
    )}
  </GlassCard>
);
...
export const LegalAdminDashboard: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const filters = useMemo<LegalAdminQueueFilter>(
    () => ({ tab, status, priority, category, query }),
    [tab, status, priority, category, query],
  );
  ...
  const reload = useCallback(async () => {
    ...
    if (
      ...
      !requestsResult.data.some((item) => item.id === selectedId)
    ) {
  ...
  const handleAction = async (action: LegalAdminRequestStatus | "note" | "archive") => {
    ...
    if (action === "note") {
      await addLegalReviewNote(
        selectedRequest.id,
        note || "Reviewer note added in mock dashboard.",
      );
    } else if (action === "archive") {
      await archiveLegalRequest(selectedRequest.id);
    } else {
      await updateLegalRequestStatus(
        selectedRequest.id,
        action,
        note || `Status changed to ${formatLabel(action)} in mock dashboard.`,
      );
    }
    await reload();
  };
      ...
      <div className="space-y-6 pb-8">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  />
                  <Chip
                    label={source === "mock" ? "Mock / Local" : "Supabase Ready / Mock Data"}
                ...
                <p className="mt-2 max-w-3xl text-sm leading-relaxed text-white/62">
                  Review intake, privacy/data rights, deletion queue, copyright reports, moderation
                  appeals, and acceptance activity. Prototype only. Not legal case management.
                </p>
              </div>
              <div className="rounded-2xl border border-amber-300/20 bg-amber-500/10 p-3 text-xs leading-relaxed text-amber-100/80 lg:max-w-xs">
                ...
                <div className="mt-1">
                  No legal email, takedown, reinstatement, or account deletion action is wired here.
                </div>
              </div>
            </div>
            {warning && (
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-xs text-white/55">
                {warning}
              </div>
            )}
            <div className="mt-3 text-[11px] text-white/40">
              Signed in as {identity.display_name} · {identity.public_profile_uid}
            </div>
          </GlassCard>
        </div>
        ...
        <div className="grid gap-3 px-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-6 lg:px-10">
          ...
          <StatCard
            ...
            value={stats.acceptance_records_today}
            Icon={CheckCircle2}
          />
        </div>
          ...
          <GlassCard className="p-4">
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-4">
              ...
              <select
                value={status}
                onChange={(event) =>
                  setStatus(event.target.value as LegalAdminRequestStatus | "all")
                }
              ...
              <select
                value={priority}
                onChange={(event) =>
                  setPriority(event.target.value as LegalRequestPriority | "all")
                }
                className={selectClass}
              >
              ...
              <select
                value={category}
                onChange={(event) =>
                  setCategory(event.target.value as LegalRequestCategory | "all")
                }
                className={selectClass}
              >
                {categoryOptions.map((item) => (
                  <option key={item} value={item}>
                    Category: {formatLabel(item)}
                  </option>
                ))}
              </select>
            </div>
                  ...
                  {requests.map((request) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 truncate text-xs text-white/50">
                            ...
                            {request.requester.public_profile_uid ??
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <StatusPill status={request.status} />
                        <span
                          className={`text-[10px] font-black uppercase tracking-wider ${priorityTone[request.priority]}`}
                        >
                          {request.priority}
                        </span>
