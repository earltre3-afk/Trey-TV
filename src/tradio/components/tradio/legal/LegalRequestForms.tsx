...
import { submitAccountDeletionRequest, submitLegalRequest, type LegalServiceSource } from './legalIntakeService';

const FieldInput: React.FC<{ field: LegalFormField; value: string; onChange: (v: string) => void }> = ({ field, value, onChange }) => {
  const base = 'w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30 focus:border-fuchsia-300/40';
  if (field.type === 'textarea') {
    return <textarea value={value} onChange={(e) => onChange(e.target.value)} placeholder={field.placeholder} rows={3} className={`${base} resize-none`} />;
  }
  ...
  return <input type={field.type === 'email' ? 'email' : field.type === 'url' ? 'url' : 'text'} value={value} onChange={(e) => onChange(e.target.value)} placeholder={field.placeholder} className={base} />;
};
...
export const LegalRequestForm: React.FC<{
  ...
  const canSubmit = required.every((n) => (values[n] || '').trim().length > 0) && (!config.acknowledgement || ack) && !submitting;
    ...
    return (
      <GlassCard glow className="p-6 sm:p-8">
        <div className="flex flex-col items-center gap-4 py-4 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-3xl border border-emerald-300/30 bg-emerald-500/10 text-emerald-200"><CheckCircle2 className="h-8 w-8" /></div>
          ...
          {onBack && <SecondaryButton className="mt-2 px-6 py-3 text-xs" onClick={onBack}>Back to Legal Center</SecondaryButton>}
        </div>
      </GlassCard>
    );
  }
  ...
  return (
    <GlassCard glow className="p-6 sm:p-8">
      ...
      {config.relatedDocument && onOpen && (
        <button onClick={() => onOpen(config.relatedDocument as string)} className="mt-2 text-[11px] font-semibold text-cyan-300 hover:text-cyan-200">
      ...
      <div className="mt-5 space-y-4">
        ...
        {config.acknowledgement && (
          <LegalAcceptanceCheckbox checked={ack} onChange={setAck}>{config.acknowledgement}</LegalAcceptanceCheckbox>
        )}
      </div>
      <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          {onBack && <SecondaryButton className="px-4 py-3 text-xs" onClick={onBack}>Cancel</SecondaryButton>}
          <PrimaryButton
...
const withConfig = (id: string): React.FC<{ onBack?: () => void; onOpen?: (target: string) => void }> => {
  const Component: React.FC<{ onBack?: () => void; onOpen?: (target: string) => void }> = ({ onBack, onOpen }) => {
...
export const DeleteAccountRequest: React.FC<{ onBack?: () => void; onOpen?: (target: string) => void }> = ({ onBack, onOpen }) => {
  ...
  if (requested) {
    return (
      <GlassCard glow className="p-6 sm:p-8">
        <div className="flex flex-col items-center gap-4 py-4 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-3xl border border-amber-300/30 bg-amber-500/10 text-amber-200"><CheckCircle2 className="h-8 w-8" /></div>
          ...
          {feedback && <p className="max-w-md text-xs leading-relaxed text-white/50">{feedback}</p>}
          {onBack && <SecondaryButton className="mt-2 px-6 py-3 text-xs" onClick={onBack}>Back</SecondaryButton>}
        </div>
      </GlassCard>
    );
  }
  ...
  return (
    <GlassCard glow className="p-6 sm:p-8">
      <div className="flex items-center gap-2 text-2xl font-black tracking-tight text-white"><Trash2 className="h-5 w-5 text-red-300" /> Delete Account</div>
      <p className="mt-1.5 text-sm leading-relaxed text-white/60">
        Deleting your account removes your profile and content from Trey TV and Tradio. Please review what happens before continuing.
      </p>
      <div className="mt-4 rounded-2xl border border-red-300/15 bg-red-500/[0.05] p-4">
        <div className="flex items-center gap-2 text-sm font-bold text-red-200"><ShieldAlert className="h-4 w-4" /> What gets deleted</div>
        <ul className="mt-2 space-y-1.5">
          {DELETE_CHECKLIST.map((item) => (
            <li key={item} className="flex items-start gap-2 text-[12px] leading-relaxed text-white/65"><span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-red-300/70" /> {item}</li>
          ))}
        </ul>
      </div>
      <div className="mt-4 space-y-3">
        <LegalAcceptanceCheckbox checked={confirmed} onChange={setConfirmed}>I understand what will be deleted and what may be retained for legal/security reasons.</LegalAcceptanceCheckbox>
        <LegalAcceptanceCheckbox checked={ack} onChange={setAck}>I want to request permanent deletion of my account. (Placeholder — pending legal review.)</LegalAcceptanceCheckbox>
        {onOpen && (
          <button onClick={() => onOpen('data-retention')} className="text-[11px] font-semibold text-cyan-300 hover:text-cyan-200">Read the Data Retention policy →</button>
        )}
      </div>
      <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-end">
        {onBack && <SecondaryButton className="px-4 py-3 text-xs" onClick={onBack}>Keep My Account</SecondaryButton>}
        <button
