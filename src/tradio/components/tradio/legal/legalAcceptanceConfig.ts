...
export const LEGAL_ACCEPTANCE_FLOWS: Record<LegalAcceptanceFlowId, LegalAcceptanceFlowConfig> = {
  instant_release_submit: {
    ...
    requiredConfirmations: [
      {
        id: "own_music_rights",
        label: "I confirm I own or control the rights to upload this music.",
        required: true,
      },
      {
        id: "unauthorized_removal",
        label: "I understand unauthorized uploads may be removed.",
        required: true,
      },
    ],
    relatedLegalRoutes: [
  ...
  producer_beat_publish: {
    ...
    requiredConfirmations: [
      {
        id: "own_beat_rights",
        label: "I confirm I own or control the rights to this beat.",
        required: true,
      },
      {
        id: "samples_cleared",
        label: "I understand samples/loops must be cleared where required.",
        required: true,
      },
    ],
    relatedLegalRoutes: [
    ...
    fallbackMessage:
      "Beat rights confirmation will be recorded when backend legal records are ready.",
  },
  dj_broadcast_schedule: {
    ...
    requiredConfirmations: [
      {
        id: "broadcast_moderation",
        label: "I understand broadcasts may be moderated, archived, or removed.",
        required: true,
      },
      {
        id: "authorized_show_content",
        label: "I confirm I am responsible for authorized content in my show.",
        required: true,
      },
    ],
    relatedLegalRoutes: [
    ...
    fallbackMessage:
      "Broadcast acknowledgement will be recorded when backend legal records are ready.",
  },
  song_wars_create: {
    id: "song_wars_create",
    ...
    requiredConfirmations: [
      {
        id: "authorized_battle_songs",
        label: "I confirm submitted songs are authorized.",
        required: true,
      },
      {
        id: "fair_play_acknowledged",
        label: "I understand vote manipulation or abusive behavior may disqualify a battle.",
        required: true,
      },
    ],
    relatedLegalRoutes: [
    ...
    uploadRightsType: "music",
    fallbackMessage:
      "Song Wars acknowledgement will be recorded when backend legal records are ready.",
  },
  role_access_request: {
    ...
    requiredConfirmations: [
      ...
      {
        id: "access_revocable",
        label: "I understand access can be denied, restricted, or revoked.",
        required: true,
      },
    ],
    relatedLegalRoutes: [
    ...
    fallbackMessage:
      "Access acknowledgement will be recorded when backend legal records are ready.",
  },
  creator_profile_publish: {
    ...
    requiredConfirmations: [
      {
        id: "profile_accuracy",
        label: "I agree that public profile information must be accurate.",
        required: true,
      },
      {
        id: "profile_asset_rights",
        label: "I confirm I have the right to display linked media/profile assets.",
        required: true,
      },
      {
        id: "profile_review",
        label: "I understand my creator profile may be reviewed or restricted.",
        required: true,
      },
    ],
    relatedLegalRoutes: [
    ...
    fallbackMessage:
      "Creator profile acknowledgement will be recorded when backend legal records are ready.",
  },
  prescribe_me_ai_notice: {
    id: "prescribe_me_ai_notice",
    title: "AI / Prescribe Me notice",
    description:
      "Informational AI transparency links for recommendation and prescription surfaces.",
...
export const createLegalAcceptanceValues = (
  flowId: LegalAcceptanceFlowId,
): LegalAcceptanceValues => {
  const flow = LEGAL_ACCEPTANCE_FLOWS[flowId];
  return [...flow.requiredPolicies, ...flow.requiredConfirmations].reduce<LegalAcceptanceValues>(
    (acc, item) => {
      acc[item.id] = false;
      return acc;
    },
...
export const isLegalFlowAccepted = (
  flowId: LegalAcceptanceFlowId,
...
export const recordLegalFlowAcceptance = async (
  ...
  if (!isLegalFlowAccepted(flowId, values)) {
    return {
      source: "unavailable",
      data: null,
      warning: "Required legal acknowledgements are incomplete.",
    };
  }

  const recordedAt = new Date().toISOString();
  const policyResults = await Promise.all(
    flow.requiredPolicies.map((item) =>
      recordLegalAcceptance(item.documentId, item.version, flow.id, {
        flow: flow.id,
        details: {
  ...
  const rightsResult = flow.uploadRightsType
    ? await recordUploadRights(
        flow.uploadRightsType,
        typeof context.referenceId === "string" ? context.referenceId : null,
        {
          flow: flow.id,
          acceptanceEventType: flow.acceptanceEventType,
          checked: values,
  ...
  const warning =
    [...policyResults, rightsResult]
      .filter(Boolean)
      .map((result) => result?.warning)
      .find(Boolean) ?? null;

  const source = [...policyResults, rightsResult].some((result) => result?.source === "supabase")
    ? "supabase"
    : "unavailable";
  return {
    source,
