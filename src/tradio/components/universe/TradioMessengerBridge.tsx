  ...
  <GlassCard glow className="w-[330px] max-w-[92vw] p-3.5">
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2">
          <ContextLabel notification={notification} />
        </div>
        <div className="mt-3 flex items-center gap-2">
          <button
            onClick={() => onOpen(notification)}
            className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-fuchsia-500 to-cyan-400 px-3 py-1.5 text-[11px] font-bold text-white"
          >
            {MESSENGER_COPY.openInMessenger} <ArrowUpRight className="h-3.5 w-3.5" />
          </button>
          <button
            onClick={() => onDismiss(notification.id)}
            className="inline-flex items-center gap-1 rounded-full border border-white/12 px-3 py-1.5 text-[11px] font-bold text-white/70 hover:text-white"
          >
...
export const TradioMessengerBell: React.FC<{ unreadCount: number; onClick?: () => void }> = ({
  unreadCount,
  onClick,
}) => (
  <button
  ...
  <GlassCard className="p-4">
    ...
    <div className="mt-3 space-y-2">
      {notifications.length === 0 && (
        <div className="rounded-xl border border-white/8 bg-white/[0.02] px-3 py-4 text-center text-[11px] text-white/45">
          No new messages.
        </div>
      )}
