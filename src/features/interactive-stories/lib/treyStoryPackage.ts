...
const BUNDLED_STORY_SOURCES = ["/interactive-stories/stories/the-god-ram-book-one.ttstory"];
  ...
  return {
    ...
    voiceName:
      typeof raw.voiceName === "string"
        ? raw.voiceName
        : typeof raw.voice_name === "string"
          ? raw.voice_name
          : null,
    audioStyle:
      typeof raw.audioStyle === "string"
        ? raw.audioStyle
        : typeof raw.audio_style === "string"
          ? raw.audio_style
          : null,
    settings:
...
function normalizeCharacterVoices(
  value: unknown,
  characters: TreyStoryCharacter[],
): StoryCharacterVoices | undefined {
...
function getSceneLines(scene: Partial<TreyStoryScene>): TreyStoryDialogueLine[] {
  ...
  return lines
    .filter(
      (line) =>
        line && typeof line === "object" && typeof line.text === "string" && line.text.trim(),
    )
    .map((line, index) => ({
      ...line,
...
function normalizeCharacters(characters: unknown): TreyStoryCharacter[] {
  ...
  return characters.map((raw, index) => {
    const item = raw as Partial<TreyStoryCharacter> & {
      id?: string;
      name?: string;
      image?: string;
      voice?: unknown;
    };
    const characterId = normalizeMachineId(
      item.character_id || item.id || item.name,
      `character_${index + 1}`,
    );
    if (seen.has(characterId))
      throw new TreyStoryPackageError(`Duplicate character_id: ${characterId}.`);
...
function normalizeChoice(
  ...
  const choice = rawChoice as Partial<TreyStoryChoice> & {
    next_scene_id?: string;
    next?: string;
    impact_summary?: string;
  };
  const id = normalizeMachineId(
    choice.id || choice.text || `choice_${index + 1}`,
    `${sceneId}_choice_${index + 1}`,
  );
  const nextSceneId = String(
    choice.nextSceneId || choice.next_scene_id || choice.next || "",
  ).trim();
  ...
  return {
    ...
    relationshipImpactType: choice.relationshipImpactType,
    impactSummary:
      choice.impactSummary || choice.impact_summary
...
function normalizeScenes(scenes: unknown, initialSceneId: string): TreyStoryScene[] {
  ...
  const firstPass = scenes.map((raw, index) => {
    const item = raw as Partial<TreyStoryScene> & {
      scene_id?: string;
      image_key?: string;
      image_fit?: string;
  ...
  return firstPass.map(({ item, id }, index) => {
    ...
    return {
      ...
      activeCharacterIds: Array.isArray(item.activeCharacterIds || item.active_characters)
        ? (item.activeCharacterIds || item.active_characters || [])
            .map((v) => normalizeMachineId(v))
            .filter(Boolean)
        : undefined,
      choices: Array.isArray(item.choices)
        ? item.choices.map((choice, choiceIndex) =>
            normalizeChoice(choice, id, choiceIndex, sceneIds),
          )
        : undefined,
...
export function parseTreyStoryPackage(raw: string): TreyStoryPackage {
  ...
  try {
    ...
    throw new TreyStoryPackageError(
      "This is not a valid .ttstory file. It must be valid JSON or a TTS1 encoded JSON package.",
    );
  }
  ...
  if (!pkg.story || typeof pkg.story !== "object")
    throw new TreyStoryPackageError("Missing story object.");
  ...
  const characterVoices = normalizeCharacterVoices(
    pkg.story.characterVoices || storyRecord.character_voices,
    characters,
  );
  ...
  return {
    ...
    story: {
      ...
      slug: normalizeMachineId(
        pkg.story.slug || pkg.story.title || pkg.story.id,
        pkg.story.id,
      ).replace(/_/g, "-"),
...
function continueChoiceForScene(
  pkg: TreyStoryPackage,
  scene: TreyStoryScene,
): Choice[] | undefined {
...
export function resolveInstalledStoryChoice(
  ...
  imagePosition?: string;
  sceneId?: string;
  voiceLines?: TreyStoryDialogueLine[];
  characterVoices?: StoryCharacterVoices;
  storyCharacters?: TreyStoryCharacter[];
  ...
  if (!pkg) return null;
  const targetSceneId =
    choice.nextSceneId || String(branch.flags.current_scene_id || pkg.story.initialSceneId);
  ...
  return {
    ...
    is_ending:
      !!scene.isEnding || (!scene.choices?.length && !continueChoiceForScene(pkg, scene)?.length),
