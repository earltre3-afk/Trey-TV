...
export const TONE_COLORS: Record<string, { bg: string; border: string; text: string; glow: string }> = {
...
// ---------------------------------------------------------------------------
// CHARACTER PHOTO MAP â€” STRICT, FIXED, SINGLE SOURCE OF TRUTH.
//
// Every Cast & Connections card (and any other profile circle anywhere in the
// app â€” choice cards, meters, ending posters, branch map avatars, archive)
// MUST render its image from CHARACTER_PHOTO_MAP[character_id].image.
//
// HARD RULES â€” do not violate:
//   â€¢ DO NOT generate new character faces.
//   â€¢ DO NOT replace these uploaded portraits.
//   â€¢ DO NOT use scene stills for profile circles.
//   â€¢ DO NOT auto-select images based on role labels.
//   â€¢ DO NOT crop from story scenes unless no portrait exists.
//   â€¢ DO NOT use the Reggie/Dante portrait for Malik (or vice versa).
//   â€¢ DO NOT use football gear for Micah's profile portrait.
//   â€¢ DO NOT use the twins' image for Denise.
//   â€¢ Malik & Micah are identical twins â€” separated ONLY by world:
//       Malik = football gear / stadium lighting
//       Micah = ballet dancewear / studio lighting
//   â€¢ Dante is "Dante Reeves" (Backup QB) â€” never "Dante Williams".
//   â€¢ "Jordan" is not canon; merged into Reggie.
//   â€¢ "Principal Harris" is not canon; replaced with Compliance Officer.
// ---------------------------------------------------------------------------

/**
 * CHARACTER_PHOTO_MAP â€” fixed character_id â†’ portrait asset.
...
> = {
  malik_carter: {
    ...
    // teammate photo â€” restored here so Malik gets the twin face that matches
  ...
  reggie: {
    ...
    // Teammate portrait â€” bigger build / locker-room energy, NOT the twin face.
    // Previously this slot held the twin photo (mixed up with Malik) â€” restored.
...
export const CHARACTERS = [
  ...
  },
  {
    ...
    // with the full name "Ariana 'Ari' Cole" â€” flagged as app-canon.
    name: 'Ariana "Ari" Cole',
    firstName: 'Ari',
    role: 'Lead Dancer',
    canon: 'app-expanded' as const,
    canonNote: 'Original story names her only "Ari" â€” full name is app canon.',
  ...
  },
  {
    ...
    emotionalIQ: 72,
    secrets: 1,
    // Reggie absorbs the old "Jordan" function â€” loud teammate who notices a
    // little too much when something's off with Malik.
    description: "Malik's football teammate. Loud, hilarious, loyal â€” but he watches a little too closely when something seems off.",
...
export const CHAPTER_1 = {
  title: 'Chapter 1 â€” The Carter Twin Agreement',
  image: '/interactive-stories/scenes/studio_b_request.png',
  sceneId: 'studio_b_request',
  paragraphs: [
    `The fluorescent lights in the east hallway buzzed like they had something to confess. Vending machines hummed. The smell of the school â€” floor wax, ancient pizza, low-grade ambition â€” hung in the air.`,
    ...
    `Malik did the thing he always did before a bad confession â€” he laughed once, like he was about to tell a joke. The joke never came.`,
    `"Okay. So. Last night. I may have beenâ€¦ recreationally unwise."`,
    ...
    `"I need a favor," he said. "A big one. A â€” okay, listen, before you say no â€” a switch-shaped one."`,
    ...
    `Micah looked at his brother â€” really looked at him. Under the cocky he could see the scared. The kid who used to climb into his bed during thunderstorms and pretend it was a strategic decision.`,
    `Malik saw the look. He pressed.`,
    `"One day. ONE day. You go to football, you keep your mouth shut, you let Reggie do most of the talking â€” which, let's be honest, Reggie's gonna do anyway â€” you sit in the back of practice, you pee in the cup, you come home. That's it. That's the whole movie."`,
    ...
    `Malik looked at the floor. Then back up. And the thing about Malik was â€” when he was actually scared, his whole face changed. The performance dropped. And what was underneath was so much younger than seventeen.`,
    `"Micah. If I fail this test, I lose the showcase. If I lose the showcase, I lose the offer. If I lose the offer â€”" He stopped. Swallowed. "Mom worked three jobs. Three. I'm not â€” I can't be the reason that was for nothing."`,
...
export const CHAPTER_1_CHOICES = [
  ...
  {
    label: 'C',
    text: 'Make a deal â€” only if Malik covers his adjudication.',
    tone: 'Bold' as const,
  ...
  {
    label: 'E',
    text: 'Walk awayâ€¦ or so he thinks.',
 ...
 * Uses the official character registry â€” never invents new characters.
...
// ---------------------------------------------------------------------------
// SCENE IMAGE MAP â€” STRICT, FIXED, SCENE-ID â†’ IMAGE METADATA.
//
...
//
// HARD RULES:
//   â€¢ Scene images are NEVER used inside profile circles.
//   â€¢ Faces of focal characters must remain visible after cropping.
//   â€¢ If a wide image would cut faces in a vertical container, prefer
//     recommended_fit: "contain" (zoom out) over a tight cover crop.
//   â€¢ Never randomly pick a scene image by mood or role â€” use scene_id.
//   â€¢ Tone-based fallback (pickChapterImage) is only for AI-generated
...
/**
 * SCENE_IMAGE_MAP â€” locked scene_id â†’ metadata. Renderers MUST pull framing
...
export const SCENE_IMAGE_MAP: Record<string, SceneImageMeta> = {
  ...
  football_dive: {
    ...
    scene_description: 'Football practice on the field â€” Malik\'s world, full intensity.',
  ...
  costume_room: {
    ...
    image: '/interactive-stories/scenes/truth_reveal.png',
    scene_description: 'Behind-the-scenes costume room â€” quick change, full tension.',
    focal_characters: ['malik_carter', 'micah_carter'],
    focal_point_x: 0.5,
    focal_point_y: 0.4,
    safe_crop: true,
  ...
  twins_cover: {
    ...
    scene_description: 'Cover image â€” the twins, identical, separated by their two worlds.',
    focal_characters: ['malik_carter', 'micah_carter'],
    focal_point_x: 0.5,
    focal_point_y: 0.35,
    safe_crop: true,
 ...
 *
 * Returns sane defaults if the URL isn't in SCENE_IMAGE_MAP â€” defaults
