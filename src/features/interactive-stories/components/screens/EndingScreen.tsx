...
import { Share2, RefreshCw, BookOpen, Sparkles, Download, Image as ImageIcon, Loader2, Layers } from 'lucide-react';
...
interface Props {
  ending: Ending;
  /** Optional â€” when provided, the poster will feature the cast that actually appeared on this branch. */
  branch?: Branch | null;
  onNewBranch: () => void;
  onLibrary: () => void;
  onReread?: () => void;
...
export const EndingScreen: React.FC<Props> = ({ ending, branch, onNewBranch, onLibrary, onReread }) => {
  ...
  const doShare = async () => {
    ...
    try {
      ...
      const shareData: ShareData = {
        title: 'Switch Kicks',
        text: `I just unlocked "${ending.name}" on Switch Kicks â€” ${ending.tagline}`,
        files: [file],
      };
      const canShareFiles =
      ...
      if (navigator.share && canShareFiles) {
        ...
        setFeedback('Image downloaded â€” share it anywhere.');
      }
    } catch (e: any) {
    ...
    <div className="min-h-screen px-4 pt-8 pb-24">
      <div className="relative mx-auto max-w-sm overflow-hidden rounded-3xl border border-amber-500/40 shadow-2xl shadow-amber-500/20">
        {previewUrl ? (
          ...
          <div className="relative aspect-[9/16] w-full">
            <img src={heroImage} alt={ending.name} className="absolute inset-0 h-full w-full object-cover" />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 text-center">
              ...
              <p className="mt-5 font-serif text-base italic leading-snug text-white/90">{ending.tagline}</p>

              {/* In-page cast preview (fallback while canvas is still rendering) */}
              {castIds.length > 0 && (
                <div className="mt-5 flex flex-col items-center gap-2">
                  <div className="text-[9px] font-bold uppercase tracking-[0.4em] text-white/50">Â· Starring Â·</div>
                  <div className="flex -space-x-3">
                    {castIds.slice(0, 5).map((id) => {
                      const c = CHARACTERS_BY_ID[id];
                      if (!c) return null;
                      return (
                        <div key={id} className="h-12 w-12 overflow-hidden rounded-full border-2 border-black ring-2 ring-amber-400/70">
      ...
      <div className="mx-auto mt-6 max-w-sm space-y-3">
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-center text-[11px] text-white/50">
          ...
          1080 Ã— 1920 â€” optimized for Instagram Stories
        </div>
