...
export const EndingsScreen: React.FC<Props> = ({ endings }) => {
  const [tab, setTab] = useState<"completed" | "locked">(
    endings.length > 0 ? "completed" : "locked",
  );
  ...
  return (
    <div className="min-h-screen px-5 pt-10 pb-24">
      ...
      {tab === "completed" ? (
            ...
            {endings.map((e, i) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3">
                  <div className="font-display text-base font-black leading-tight text-white">
                    {e.name}
                  </div>
                  <div className="mt-1 text-[11px] italic text-white/70 line-clamp-2">
                    {e.tagline}
                  </div>
                </div>
              </button>
        ...
        <div className="grid grid-cols-2 gap-3">
          {LOCKED_HINTS.map((h, i) => (
            <div
              key={i}
              className="relative aspect-[3/4] overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-zinc-900 to-black"
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-3">
                ...
                <div className="mt-0.5 text-[11px] italic text-white/30 line-clamp-2">
                  Hint: {h.hint}
                </div>
              </div>
            </div>
      ...
      {openEnding && <EndingPoster ending={openEnding} onClose={() => setOpenEnding(null)} />}
    </div>
  );
};
      ...
      >
        <div className="relative aspect-[9/16] w-full">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 text-center">
            ...
            <p className="mt-4 font-serif text-base italic leading-snug text-white/90">
              {ending.tagline}
            </p>
        ...
        <div className="flex gap-2 bg-zinc-950 p-3">
          ...
          <button
            onClick={handleShare}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-amber-500 py-2.5 text-sm font-bold text-black hover:bg-amber-400"
          >
