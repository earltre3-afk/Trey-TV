...
const IMPACT_META: Record<
  ...
  { Icon: React.ComponentType<{ className?: string }>; ring: string; chip: string; text: string }
> = {
  Trust: {
    Icon: Shield,
...
export const StopPointScreen: React.FC<Props> = ({
  branch,
  onBack,
  onSubmit,
            ...
            <div className="mt-4 flex items-center gap-2 rounded-2xl border border-white/10 bg-black/40 p-2.5 backdrop-blur-md">
              ...
              <div className="flex -space-x-2">
                {cast.map((ch) => (
                  <div
                    key={ch.id}
                    className="h-7 w-7 overflow-hidden rounded-full border-2 border-zinc-950 ring-1 ring-violet-500/30"
                  >
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl ${tone.bg} font-display text-lg font-black ${tone.text}`}
                  >
                    {c.label}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <div
                        className={`inline-block rounded-full ${tone.bg} px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest ${tone.text}`}
                      >
                        {c.tone}
                      </div>
                      {impactMeta && (
