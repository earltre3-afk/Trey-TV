...
import { Settings, Mail, Trash2, Info, ExternalLink, Heart, LogIn, LogOut, User, Layers } from 'lucide-react';
...
export const SettingsScreen: React.FC<Props> = ({ onResetAll, onOpenReread, hasActiveBranch }) => {
  ...
  return (
    <div className="min-h-screen px-5 pt-10 pb-24">
          ...
          <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="font-display text-sm font-bold text-white">{user.name || 'Trey TV reader'}</div>
      ...
      {onOpenReread && (
        <section className="mb-6">
          <h3 className="mb-2 text-xs font-bold uppercase tracking-widest text-white/50">Your Story</h3>
          <button
          ...
          >
            ...
            <span className="text-xs text-white/40">{hasActiveBranch ? 'View archive' : 'No branch yet'}</span>
          </button>
        </section>
      )}
      ...
      <section className="mb-6 rounded-2xl border border-white/10 bg-gradient-to-br from-violet-500/10 to-fuchsia-500/5 p-5">
        ...
        <form onSubmit={submit} className="mt-4 flex gap-2">
          ...
          />
          <button type="submit" className="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-violet-500/30">
            Subscribe
          </button>
        </form>
      ...
      <section className="mb-6 space-y-2">
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          ...
          <p className="mt-2 text-sm text-white/60">
            Story by Trey Trizzy. Built for Trey TV. Powered by AI â€” every chapter is generated based on your choices.
          </p>
        </div>
        <a
        ...
        >
          <span className="flex items-center gap-2"><Heart className="h-4 w-4 text-pink-400" /> Visit Trey TV</span>
          <ExternalLink className="h-4 w-4 text-white/40" />
        </a>
      </section>

      <section>
        <h3 className="mb-2 text-xs font-bold uppercase tracking-widest text-white/50">Danger Zone</h3>
        <button
          onClick={() => { if (confirm('Delete ALL branches and endings? This cannot be undone.')) onResetAll(); }}
      ...
      <p className="mt-8 text-center text-xs text-white/30">
        Made with intention. Stories you don't just read â€” you live.
      </p>
    </div>
  );
};
