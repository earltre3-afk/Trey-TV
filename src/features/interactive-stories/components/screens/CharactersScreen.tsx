...
const firstNameOf = (name: string) => name.split(/\s+/)[0]?.replace(/["â€œâ€]/g, '') || name;
...
const relationshipFor = (
  ...
  switch (key) {
    ...
    case 'reggie': {
      // Reggie absorbs the old "Jordan" tension â€” friendship dips as risk rises.
...
/** Find the most recent chapter that mentions this character (for "Last changed inâ€¦"). */
    ...
    <div className="min-h-screen px-5 pt-10 pb-24">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {enriched.map(({ character: c, rel, lastChapter }) => {
          ...
          // Trend: very rough â€” derived from value vs 50 baseline
            ...
            >
              <div className="flex gap-3">
                <div className={`h-20 w-20 flex-shrink-0 overflow-hidden rounded-full border border-white/10 ring-2 ${style.ring} shadow-inner`}>
                  {/* Profile circle: centered square crop pulled STRICTLY from CHARACTER_PHOTO_MAP[mapKey] â€” never a scene still. */}
              ...
              {lastChapter && (
                <div className="mt-2.5 flex items-center gap-1.5 rounded-lg border border-white/5 bg-white/5 px-2 py-1.5 text-[10px] text-white/55">
                  ...
                  <span className="truncate">Last changed: Ch. {lastChapter.number} â€” {lastChapter.title}</span>
                </div>
              )}
            </button>
          );
