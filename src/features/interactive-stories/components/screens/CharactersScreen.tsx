...
const getCharactersForBranch = (branch: Branch | null): Character[] => {
  const installed = branch?.flags?.installed_story
    ? getInstalledStoryPackage(branch.storyId)
    : null;
  if (installed?.characters?.length) {
    return installed.characters.map(
      (c): Character => ({
        ...
        secrets: 0,
        description:
          c.short_description || `${c.display_name} is part of this installed interactive story.`,
...
const STATUS_STYLE: Record<StatusKey, { ring: string; chip: string; text: string }> = {
  ...
  "Mentor Trust": {
    ...
    chip: "bg-emerald-500/15 border-emerald-500/40",
    text: "text-emerald-300",
  },
  "Family Strain": {
    ring: "ring-rose-400/60",
    chip: "bg-rose-500/15 border-rose-500/40",
    text: "text-rose-300",
  },
...
const relationshipFor = (key: string, branch: Branch | null): RelationshipState | null => {
  if (!branch) return null;
  const m = branch.meters;
  switch (key) {
    case "ari":
      return {
        label: "Trust Toward You",
        value: m.trust_ari,
        status: statusForCharacter("ari", m.trust_ari),
        meterColor: "pink",
      };
    case "dante":
      return {
        label: "Trust Toward You",
        value: m.trust_dante,
        status: statusForCharacter("dante", m.trust_dante),
        meterColor: "violet",
      };
    case "malik":
      return {
        label: "Trust in Micah",
        value: m.trust_malik_to_micah,
        status: statusForCharacter("malik", m.trust_malik_to_micah),
        meterColor: "emerald",
      };
    case "micah":
      return {
        label: "Trust in Malik",
        value: m.trust_micah_to_malik,
        status: statusForCharacter("micah", m.trust_micah_to_malik),
        meterColor: "emerald",
      };
    case "mom": {
      const v = 100 - m.suspicion_mom;
      return {
        label: "Calm (vs Suspicion)",
        value: v,
        status: statusForCharacter("mom", v),
        meterColor: "amber",
      };
    }
    case "coach": {
      ...
      return {
        label: "Coach Trust",
        value: v,
        status: statusForCharacter("coach", v),
        meterColor: "amber",
      };
    }
    case "valentina": {
      ...
      return {
        label: "Valentina Trust",
        value: v,
        status: statusForCharacter("valentina", v),
        meterColor: "amber",
      };
    }
    case "reggie": {
      ...
      return {
        label: "Friendship (vs Suspicion)",
        value: v,
        status: statusForCharacter("reggie", v),
        meterColor: "violet",
      };
    }
    case "compliance": {
      ...
      return {
        label: "Standing at School",
        value: v,
        status: statusForCharacter("compliance", v),
        meterColor: "amber",
      };
    }
    default:
          ...
          return (
            ...
            >
              <div className="flex gap-3">
                <div
                  className={`h-20 w-20 flex-shrink-0 overflow-hidden rounded-full border border-white/10 ring-2 ${style.ring} shadow-inner`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-display text-base font-bold leading-tight text-white truncate">
                    {c.name}
                  </div>
                  ...
                  <div
                    className={`mt-1.5 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${style.chip} ${style.text}`}
                  >
              ...
              {lastChapter && (
                <div className="mt-2.5 flex items-center gap-1.5 rounded-lg border border-white/5 bg-white/5 px-2 py-1.5 text-[10px] text-white/55">
                  ...
                  <span className="truncate">
                    Last changed: Ch. {lastChapter.number} — {lastChapter.title}
                  </span>
                </div>
              )}
            </button>
      ...
      >
        <div className="relative aspect-[4/5] w-full">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5">
            ...
            <div
              className={`mt-2 inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest ${style.chip} ${style.text}`}
            >
              ...
              <ol className="space-y-2">
                {history.map((ch) => (
                  ...
                  >
                    ...
                    {ch.summary && (
                      <div className="mt-1 text-xs italic text-white/60 line-clamp-3">
                        {ch.summary}
                      </div>
                    )}
                    {ch.choiceMade && (
