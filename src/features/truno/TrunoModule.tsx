          ...
          <div className="flex flex-col min-h-screen">
            ...
            <div className="flex-1 overflow-y-auto">
              {view === "home" && (
                <HomeScreen
                  onNavigate={navigate}
                  onQuickPlay={() => startLocalMatch("quick")}
                  onAiMatch={() => startLocalMatch("ai")}
                  onPlayFriends={createPrivateRoom}
                />
              )}
              {view === "play" && (
                <PlayScreen
            ...
            <TrunoBottomNav
              active={view as TrunoTab}
              onChange={(tab) => setView(tab as TrunoView)}
            />
          </div>
        ) : (
          <div className="min-h-screen">
            {view === "match" && (
              <MatchScreen
                onNavigate={navigate}
                identity={identity}
                roomId={matchParams?.roomId ?? roomId}
                mode={matchParams?.mode ?? "quick"}
              />
            )}
            {view === "room" && (
              <RoomScreen
                ...
                roomId={matchParams?.roomId ?? roomId}
                roomError={roomError}
                suppressActiveSession={!!matchParams?.suppressActiveSession}
                onJoinRoom={joinRoom}
                onRoomReady={(id) => setRoomId(id)}
              />
            )}
