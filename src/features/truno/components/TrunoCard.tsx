...
const EnergyIcon: React.FC<{ color: string; size?: number }> = ({ color, size = 24 }) => {
  if (color === "red")
    return <Flame size={size} className="text-red-400 animate-pulse" strokeWidth={2.5} />;
  if (color === "blue")
    return (
      <Zap
        size={size}
...
export const SpectrumSwirl: React.FC<{ size?: number }> = ({ size = 24 }) => (
  <svg
    ...
    height={size}
    viewBox="0 0 40 40"
    className="drop-shadow-[0_0_10px_rgba(255,0,128,0.9)] animate-pulse"
  >
    <defs>
...
const TrunoCard: React.FC<Props> = ({
  card,
  size = "md",
  playable,
  selected,
  ...
  if (faceDown) {
    return (
      ...
      >
        ...
        <div
          className="absolute top-0 left-0 w-[150%] h-[150%] bg-gradient-to-br from-white/0 via-white/5 to-white/0 pointer-events-none"
          style={{
  ...
  const renderSymbol = () => {
    ...
    if (card.symbol === "reverse")
      return (
        <RotateCw className="w-1/2 h-1/2 drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]" strokeWidth={3} />
      );
    if (card.symbol === "skip")
      return (
        <Ban className="w-1/2 h-1/2 drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]" strokeWidth={3} />
      );
  ...
  const isWild =
    card.color === "black" || (card.color === "purple" && card.symbol === "wild_draw_four");
  ...
  return (
    ...
    >
      ...
      {playable && (
        <div
          className="absolute top-0 left-0 w-[200%] h-[200%] bg-gradient-to-br from-white/0 via-white/12 to-white/0 pointer-events-none"
          style={{
      ...
      <div
        className={`self-start text-[0.6em] font-black tracking-tighter ${style.text} leading-none drop-shadow-[0_1px_4px_rgba(0,0,0,0.4)]`}
      >
        {card.symbol === "number"
          ? card.value
          : card.symbol === "draw_two"
            ? "+2"
            : card.symbol === "wild_draw_four"
              ? "+4"
              : ""}
      </div>
      ...
      {/* Main Center Symbol */}
      <div
        className={`flex-1 flex items-center justify-center font-black ${style.text} drop-shadow-[0_4px_12px_rgba(0,0,0,0.6)] text-center scale-100 group-hover:scale-105 transition-transform duration-300`}
      >
        {renderSymbol()}
      </div>
