...
const tabs: { key: TrunoTab; label: string; Icon: React.ComponentType<{ size: number; className?: string; strokeWidth?: number }> }[] = [
                ...
                >
                  <div className={`relative w-9 h-9 rounded-2xl flex items-center justify-center
                    ...
                    <Icon size={20} className={
                      isActive
                        ? (isPlay ? 'text-fuchsia-300' : 'text-amber-300')
                        : 'text-zinc-400'
                  ...
                  <span className={`text-[10px] font-semibold ${isActive ? (isPlay ? 'text-fuchsia-300' : 'text-amber-300') : 'text-zinc-500'}`}>
                    {label}
                  </span>
                </button>
              );
