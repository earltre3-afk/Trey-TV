  ...
  return (
    <div className="px-3 pb-24 space-y-4">
      <div className="flex flex-col items-center">
        ...
        <p className="text-xs tracking-[0.3em] font-bold text-fuchsia-300 mt-2">
          RANKED LEADERBOARD
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-fuchsia-900 to-purple-900 flex items-center justify-center text-xl">
            🃏
          </div>
          <div>
          ...
          {podium.map((p) => (
            ...
            >
              <div className="relative">
                ...
                <div
                  className={`${p.rank === 1 ? "w-20 h-20" : "w-14 h-14"} rounded-full bg-gradient-to-br ${p.color} ring-4 ${p.ringColor} relative`}
                >
                  {p.rank === 1 && (
                    <div
                ...
                <div
                  className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black ${p.rank === 1 ? "bg-amber-500 text-amber-950" : p.rank === 2 ? "bg-cyan-500 text-white" : "bg-red-500 text-white"}`}
                >
                  {p.rank}
                </div>
              </div>
              <div className="text-center mt-3">
                <div className="flex items-center gap-1 justify-center">
                  ...
                  {p.isYou && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded-md bg-fuchsia-500/20 border border-fuchsia-500/40 text-fuchsia-300">
                      You
                    </span>
                  )}
                </div>
                <div className={`text-[10px] font-black tracking-wider ${p.tierColor}`}>
                  {p.tier}
                </div>
                ...
                <div className="text-[10px] text-zinc-500 flex items-center justify-center gap-1">
                  Win Streak <Flame size={9} className="text-orange-400" />{" "}
                  <span className="text-amber-300 font-bold">{p.streak}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="space-y-1.5">
        {rankings.map((r, i) => (
          <div
            key={i}
            className="rounded-xl border border-zinc-800 bg-zinc-950/80 p-2.5 flex items-center gap-3"
          >
            <span className="w-6 text-center text-zinc-400 font-bold text-sm">{r.rank}</span>
            <div
              className={`w-9 h-9 rounded-full bg-gradient-to-br ${r.color} ring-1 ring-white/10`}
            />
            ...
            <span className="flex items-center gap-0.5 text-xs text-amber-300 font-bold">
              <Flame size={11} className="text-orange-400" /> {r.streak}
            </span>
