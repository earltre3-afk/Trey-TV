      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-black text-white truncate" title={currentUser.name}>
              {currentUser.name || 'Trey TV Member'}
            </h1>
            {currentUser.verified && (
              <div className="w-5 h-5 rounded-full bg-fuchsia-500 text-xs font-black text-white flex items-center justify-center flex-shrink-0">✓</div>
            )}
          </div>
          <div className="mt-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-purple-500/10 border border-purple-500/20">
            ...
            <span className="text-[11px] font-bold text-purple-300 truncate">@{currentUser.handle || 'member'}</span>
          </div>
          <div className="flex items-center gap-2 mt-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="h-2 rounded-full bg-zinc-800 overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-fuchsia-500 to-purple-500 transition-all duration-500" style={{ width: `${xpPercentage}%` }} />
              </div>
      ...
      <div className="rounded-2xl border border-zinc-850 bg-zinc-950/40 p-3 grid grid-cols-5 gap-2 text-center backdrop-blur-xl">
        {[
          ...
          <div key={i} className={i < 4 ? 'border-r border-zinc-900' : ''}>
            <div className="text-[9px] font-bold text-zinc-500 tracking-wider truncate">{s.label}</div>
        ...
        <div className="grid grid-cols-4 md:grid-cols-7 gap-2">
          {badges.map((b, i) => (
            <div key={i} className="flex flex-col items-center">
              <div className={`w-11 h-13 rounded-xl bg-gradient-to-b ${b.bg} border ${b.color.replace('text', 'border')}/30 flex items-center justify-center relative`}
                style={{ clipPath: 'polygon(50% 0, 100% 25%, 100% 75%, 50% 100%, 0 75%, 0 25%)' }}>
                <b.icon size={18} className={b.color} fill={b.icon === Star ? 'currentColor' : 'none'} />
      ...
      <div className="rounded-2xl border border-zinc-850 bg-zinc-950/40 overflow-hidden backdrop-blur-xl">
        ...
        <div className="p-3">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="text-xs font-bold text-fuchsia-300 px-3 py-1 rounded-lg border border-zinc-800 hover:border-fuchsia-500/40 transition-colors">View Collection</button>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-2">
            {decks.map((d, i) => (
              <div key={i} className={`flex-shrink-0 w-28 rounded-xl border ${d.equipped ? 'border-amber-500/40 shadow-[0_0_15px_rgba(255,215,0,0.2)]' : 'border-zinc-850'} bg-zinc-950/40 overflow-hidden hover:scale-[1.01] transition-transform`}>
      ...
      <div className="rounded-2xl border border-zinc-850 bg-zinc-950/40 p-3 backdrop-blur-xl">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <div key={i} className="rounded-xl border border-zinc-900/60 bg-zinc-900/30 p-2 text-center hover:scale-[1.02] transition-transform">
