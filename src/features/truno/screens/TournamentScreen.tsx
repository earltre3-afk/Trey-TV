    ...
    <div className="px-3 pb-32 space-y-4">
      <div className="flex flex-col items-center">
        ...
        <p className="text-xs text-zinc-300 mt-1">
          {selected?.description || "Compete. Connect. Conquer."}
        </p>
        ...
        <div className="grid grid-cols-3 gap-2 rounded-2xl border border-amber-500/30 bg-zinc-950/80 p-3">
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-xl font-black text-amber-300">
                {selected.prize_pool.toLocaleString()}
              </span>
            </div>
          ...
          <div className="flex flex-col items-center border-x border-zinc-800">
            ...
            <span className="text-xl font-black text-white mt-1">
              {selected.registered_players.toLocaleString()}
            </span>
            <span className="text-[9px] text-zinc-500">
              Registered - max {selected.max_players}
            </span>
          </div>
          <div className="flex flex-col items-center">
            <div className="flex items-center gap-1 text-[10px] text-zinc-400 font-bold">
            ...
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-base font-black text-amber-300 tabular-nums">
                {formatCountdown(selected.starts_at)}
              </span>
            </div>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 overflow-hidden">
        ...
        <div className="p-4">
            ...
            <div>
              ...
              <div className="rounded-xl border border-dashed border-zinc-800 bg-zinc-900/30 p-6 text-center">
                ...
                <p className="text-sm text-zinc-400">
                  {selected
                    ? "Bracket opens when this tournament is seeded."
                    : "No tournament bracket is available right now."}
                </p>
              </div>
            </div>
          )}
          {tab === "live" && (
            <div>
              <h3 className="text-[10px] text-zinc-400 font-bold tracking-wider mb-3">
                LIVE MATCHES
              </h3>
              <div className="rounded-xl border border-dashed border-zinc-800 bg-zinc-900/30 p-6 text-center">
                <p className="text-sm text-zinc-400">
                  No live Truno tournament matches are running right now.
                </p>
              </div>
            </div>
          )}
          {tab === "rules" && (
            <div className="space-y-2 text-xs text-zinc-400">
              ...
              <p>
                Match by color, number, or action. Wild cards set the current color. Empty your hand
                to win your table.
              </p>
            </div>
          )}
          {tab === "prizes" && (
            <div className="space-y-2 text-xs text-zinc-400">
              ...
              <p>
                {selected
                  ? "Prize details come from the tournament record shown above."
                  : "No prize pool is available until a tournament is scheduled."}
              </p>
            </div>
          )}
        </div>
