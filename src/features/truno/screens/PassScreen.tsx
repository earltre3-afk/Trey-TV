  ...
  return (
    <div className="px-3 pb-24 space-y-4">
      <div className="flex flex-col items-center">
        <div className="flex items-end gap-3 w-full">
          <div className="flex-1">
            ...
            <p className="text-[11px] tracking-wider font-bold text-fuchsia-300 mt-1 text-center">
              SEASON 1: NEON ASCENSION
            </p>
      ...
      <div className="rounded-2xl border border-fuchsia-500/30 bg-zinc-950/80 p-3 flex items-center gap-3">
        <div className="w-12 h-12 rounded-xl border-2 border-purple-500 bg-purple-500/10 flex items-center justify-center font-black text-purple-300 shadow-[0_0_15px_rgba(157,78,221,0.4)]">
          23
        </div>
        <div className="flex-1">
        ...
        <button className="text-xs font-bold px-3 py-2 rounded-xl border border-amber-500/50 text-amber-300 hover:bg-amber-500/10">
          Buy Levels
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={`py-2.5 rounded-xl text-xs font-bold ${tab === "rewards" ? "bg-fuchsia-500/20 border border-fuchsia-500/50 text-fuchsia-300" : "bg-zinc-900 border border-zinc-800 text-zinc-400"}`}
        >
          REWARDS
        </button>
        <button
          onClick={() => setTab("challenges")}
          className={`relative py-2.5 rounded-xl text-xs font-bold ${tab === "challenges" ? "bg-fuchsia-500/20 border border-fuchsia-500/50 text-fuchsia-300" : "bg-zinc-900 border border-zinc-800 text-zinc-400"}`}
        >
          CHALLENGES
          <span className="absolute top-1 right-2 w-4 h-4 rounded-full bg-amber-500 text-[9px] font-black text-amber-950 flex items-center justify-center">
            7
          </span>
        </button>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 overflow-hidden">
        <div className="flex">
          <div className="w-20 flex-shrink-0 bg-gradient-to-b from-amber-900/40 to-amber-950/20 border-r border-amber-500/30 flex flex-col items-center justify-center p-2">
            ...
            <button className="mt-2 text-[10px] font-bold text-amber-400 border border-amber-500/50 rounded-md px-2 py-0.5">
              UPGRADE
            </button>
          </div>
          <div className="flex-1 overflow-x-auto">
            <div className="flex gap-1 p-2 min-w-max">
              {premiumRow.map((r, i) => (
                <div
                  key={i}
                  className={`relative w-20 rounded-xl border ${r.locked ? "border-zinc-800 bg-zinc-900/60" : "border-amber-500/50 bg-amber-500/10"} p-2 flex flex-col items-center`}
                >
                  ...
                  {!r.locked && (
                    <button className="mt-1 text-[9px] font-black text-amber-300 border border-amber-500/50 rounded-md px-2 py-0.5">
                      CLAIM
                    </button>
                  )}
                </div>
        ...
        <div className="flex items-center justify-around py-2 bg-gradient-to-r from-transparent via-fuchsia-950/40 to-transparent border-y border-zinc-800">
          {[21, 22, 23, 24, 25].map((l, i) => (
            <div
              key={i}
              className={`w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-black ${l === 23 ? "border-2 border-amber-400 text-amber-300 bg-amber-500/10" : "border border-zinc-700 text-zinc-400"}`}
            >
              {l}
            </div>
          ))}
          ...
          <div className="flex-1 overflow-x-auto">
            <div className="flex gap-1 p-2 min-w-max">
              {freeRow.map((r, i) => (
                <div
                  key={i}
                  className={`relative w-20 rounded-xl border ${r.locked ? "border-zinc-800 bg-zinc-900/60" : r.claimed ? "border-emerald-500/40 bg-emerald-500/5" : "border-fuchsia-500/50 bg-fuchsia-500/10"} p-2 flex flex-col items-center`}
                >
                  {r.locked && <Lock size={11} className="absolute top-1 right-1 text-zinc-600" />}
                  <div className="text-2xl">{r.icon}</div>
                  <div className="text-[8px] text-zinc-500 mt-1">{r.type}</div>
                  ...
                  {r.claim && (
                    <button className="mt-1 text-[9px] font-black text-fuchsia-300 border border-fuchsia-500/50 rounded-md px-2 py-0.5">
                      CLAIM
                    </button>
                  )}
                </div>
      ...
      <div>
        <h3 className="text-sm font-bold text-fuchsia-300 tracking-wider text-center mb-3">
          FEATURED REWARDS
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {featured.map((f, i) => (
            <div
              key={i}
              className="rounded-2xl border border-purple-500/30 bg-zinc-950/80 p-3 text-center"
            >
              <div className="aspect-square rounded-xl bg-gradient-to-br from-purple-900/40 to-fuchsia-900/20 mb-2 flex items-center justify-center text-4xl">
                {f.icon}
              </div>
              <div className="text-[9px] text-zinc-500 font-bold">{f.type}</div>
              <div className="text-xs font-bold text-white">{f.name}</div>
              <div
                className={`text-[9px] mt-1.5 px-2 py-0.5 rounded-md inline-flex items-center gap-1 ${f.badge.includes("Season") ? "bg-fuchsia-500/20 text-fuchsia-300 border border-fuchsia-500/40" : "bg-amber-500/20 text-amber-300 border border-amber-500/40"}`}
              >
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-3">
        <h3 className="text-xs font-bold text-fuchsia-300 tracking-wider text-center mb-2">
          TRUNO PASS PERKS
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
          {[
            {
              icon: (
                <div className="w-7 h-7 rounded-lg rotate-45 bg-blue-500/20 border border-blue-500 flex items-center justify-center">
                  <Star size={12} className="text-blue-300 -rotate-45" fill="currentColor" />
                </div>
              ),
              title: "+50%",
