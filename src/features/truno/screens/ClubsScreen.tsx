  ...
  return (
    <div className="px-3 pb-24 space-y-4">
      <div className="flex flex-col items-center">
        ...
        <p className="text-xs tracking-[0.3em] font-bold text-fuchsia-300 mt-1">CLUBS & COMMUNITY</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          >
            ...
            {'badge' in t && (
              <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-fuchsia-500 text-[9px] font-black text-white flex items-center justify-center">{t.badge}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="text-xs text-zinc-400 flex items-center gap-1">See all <ChevronRight size={12} /></button>
      </div>

      <div className="space-y-3">
        {clubs.map((c, i) => (
          <div key={i} className="rounded-2xl border border-zinc-800 bg-zinc-950/80 backdrop-blur-xl overflow-hidden flex">
            <div className={`relative w-28 flex-shrink-0 bg-gradient-to-br ${c.gradient}`}>
              <div className="absolute inset-0" style={{ background: 'radial-gradient(circle at center, rgba(255,0,128,0.3), transparent 70%)' }} />
              {c.tag && (
            ...
            <div className="flex-1 p-3 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {c.verified && <div className="w-4 h-4 rounded-full bg-cyan-500 text-[9px] text-white font-black flex items-center justify-center flex-shrink-0">✓</div>}
                  </div>
              ...
              <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                {c.tags.map((t, j) => (
                  <span key={j} className="text-[9px] px-1.5 py-0.5 rounded-md bg-zinc-900 border border-zinc-800 text-zinc-400">{t}</span>
                ))}
              </div>
              <div className="flex items-center justify-between mt-2">
                <div className="flex items-center -space-x-1">
                  {[0,1,2,3,4].map(j => (
                    <div key={j} className={`w-5 h-5 rounded-full border border-zinc-900 bg-gradient-to-br ${['from-pink-500 to-rose-700', 'from-purple-500 to-indigo-700', 'from-fuchsia-500 to-purple-700', 'from-amber-500 to-red-700', 'from-cyan-500 to-blue-700'][j]}`} />
                  ))}
      ...
      <div className="rounded-2xl border border-amber-500/40 bg-gradient-to-r from-amber-950/40 via-fuchsia-950/40 to-purple-950/40 p-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-zinc-400 truncate">News, drops, events, & exclusive rewards.</div>
          </div>
        </div>
