...
test('describeMoveEvent includes action-card target details', () => {
  ...
  before.discardPile = [{ id: 'top-blue-5', color: 'blue', symbol: 'number', value: 5, label: '5' }];
  before.players[0].hand = [
    { id: 'skip-card', color: 'blue', symbol: 'skip', label: 'S' },
  ];
  ...
  const after = {
    ...
    players: before.players.map((player, index) => index === 0 ? { ...player, hand: [] } : player),
    turn: 2,
  };
