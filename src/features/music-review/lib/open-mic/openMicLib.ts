...
export async function incrementDailyCount(userId: string) {
  ...
  if (import.meta.env.PROD) {
    console.warn(
      "[Trey TV Music Review] increment_open_mic_daily_count RPC missing. Client daily-limit mutation disabled in production.",
    );
    return;
  }
...
export async function finalizeOpenMicItem(itemId: string) {
  ...
  if (import.meta.env.PROD) {
    console.warn(
      "[Trey TV Music Review] finalize_open_mic_item RPC missing. Client cleanup fallback disabled in production.",
    );
