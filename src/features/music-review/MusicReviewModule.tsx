...
export const MusicReviewModule: React.FC<MusicReviewModuleProps> = ({ initialRoute = "home" }) => {
  ...
  const go = (r: MusicReviewModuleRoute) => {
    ...
    <div className="min-h-screen text-[#F8FAFC]" style={{ background: "#05070D" }}>
      ...
      <Header
        ...
        rightSlot={
          user?.isAdmin ? (
            <button
              onClick={() => go(isAdminRoute ? "home" : "admin")}
              className="px-3 py-2 rounded-2xl border border-[#FFC857]/60 text-[#FFC857] text-[10px] font-bold tracking-wider flex items-center gap-1"
            >
        ...
        {isAdminRoute && !adminAllowed && (
          <div className="px-4 py-12 max-w-md mx-auto text-center">
            <div className="rounded-3xl border border-[#EF4444]/40 bg-[#0B1426] p-8">
              <div className="text-[#EF4444] text-xs tracking-[4px] font-bold">
                ADMIN ACCESS REQUIRED
              </div>
              <h2 className="text-2xl font-black text-[#F8FAFC] mt-2">
                Music Review Admin is protected
              </h2>
              <p className="text-[#94A3B8] text-sm mt-2">
                Connect this module to Trey TV owner/admin role checks before enabling production
                admin actions.
              </p>
              <button
                onClick={() => go("home")}
                className="mt-5 px-5 py-3 rounded-2xl border border-[#00B7FF]/50 text-[#00B7FF] text-sm font-bold"
              >
                Back to Review
              </button>
            </div>
