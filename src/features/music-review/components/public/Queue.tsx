...
const tierBadge = (tier: string | null | undefined, paid: boolean) => {
  ...
  if (!m) return null;
  return (
    <span className="px-2 py-0.5 rounded-lg text-[9px] font-bold flex items-center gap-1" style={{ background: `${m.color}20`, color: m.color, border: `1px solid ${m.color}40` }}>
      {m.icon} {m.label}
    </span>
  );
};
...
export const Queue: React.FC<QueueProps> = ({ onLive, highlightUserId }) => {
  ...
  return (
    <div className="px-4 pt-4 pb-32 max-w-3xl mx-auto space-y-4">
      ...
      {nowPlaying && (
        <button onClick={onLive} className="w-full text-left rounded-3xl border-2 border-[#FFC857]/60 bg-gradient-to-r from-[#FFC857]/10 to-transparent p-4 shadow-[0_0_25px_-8px_rgba(255,200,87,0.6)]">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[#FFC857] text-[10px] tracking-[3px] font-bold">NOW PLAYING — LIVE REVIEW</div>
              <div className="text-[#F8FAFC] font-bold text-lg truncate">{nowPlaying.song_title}</div>
              <div className="text-[#00B7FF] text-xs truncate">{nowPlaying.artist_name}</div>
            </div>
            <span className="flex items-center gap-1 text-[#EF4444] text-[10px] font-bold"><span className="w-2 h-2 rounded-full bg-[#EF4444] animate-pulse" /> LIVE</span>
          </div>
        </button>
      )}
      ...
      <div className="space-y-2">
        {items.map((s, i) => {
          ...
          return (
            <div key={s.id} className={`flex items-center gap-3 rounded-2xl border bg-[#0B1426]/80 p-3 ${isMe ? 'border-[#00B7FF] shadow-[0_0_20px_-5px_rgba(0,183,255,0.6)]' : 'border-[#1a2942]'}`}>
              <div className="w-7 h-7 rounded-full bg-[#101827] border border-[#1a2942] flex items-center justify-center text-[#00B7FF] text-xs font-bold">{i + 2}</div>
              {cover ? <img src={cover} alt="" className="w-12 h-12 rounded-xl object-cover border border-[#1a2942]" /> : <div className="w-12 h-12 rounded-xl bg-[#101827] border border-[#1a2942] flex items-center justify-center text-[#00B7FF]">♪</div>}
              <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="flex items-center gap-1 text-[#94A3B8] text-xs"><Clock size={10} /> ~{Math.max(2, (i + 1) * 4)} min</span>
              </div>
            </div>
          );
        })}
        {items.length === 0 && !nowPlaying && (
          <div className="text-center py-10 text-[#94A3B8]">Queue is empty. Submit your song to be first.</div>
        )}
      </div>
    </div>
  );
