...
import { Upload, Image as ImageIcon, AlertCircle, CheckCircle2, ChevronRight, Brain, Loader2 } from 'lucide-react';
...
export const Submit: React.FC<SubmitProps> = ({ onPrecheckReady }) => {
    ...
    return (
      <div className="px-4 py-10 max-w-md mx-auto text-center">
        <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426] p-8">
          ...
          <p className="text-[#94A3B8] text-sm mt-3">Your account email is used for your live review report.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 pt-4 pb-40 md:pb-32 max-w-2xl mx-auto space-y-4" style={{ paddingBottom: `calc(max(2.5rem, env(safe-area-inset-bottom)) + 120px)` }}>
      ...
      <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426]/80 p-4 md:p-5 space-y-4 backdrop-blur-xl">
        {/* Audio upload */}
        <div>
          <label className="text-[#00B7FF] text-[11px] tracking-[3px] font-bold">AUDIO FILE *</label>
          <label className={`mt-2 flex items-center gap-3 p-4 rounded-2xl border-2 border-dashed ${audioFile ? 'border-[#22C55E]/60 bg-[#22C55E]/5' : 'border-[#1a2942] bg-[#05070D]/40'} cursor-pointer hover:border-[#00B7FF]/60 transition`}>
            <input type="file" accept="audio/*,.mp3,.wav,.m4a,.aac" className="hidden" onChange={(e) => onAudioPick(e.target.files?.[0])} />
            <div className="w-12 h-12 rounded-2xl bg-[#0B1426] border border-[#1a2942] flex items-center justify-center text-[#00B7FF] shrink-0">
              {audioFile ? <CheckCircle2 size={22} className="text-[#22C55E]" /> : <Upload size={22} />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-[#F8FAFC] font-bold truncate">{audioFile ? audioFile.name : 'Tap to upload song'}</div>
              <div className="text-[#94A3B8] text-xs">MP3, WAV, M4A, AAC — max {MAX_AUDIO_MB}MB</div>
            </div>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="text-[#00B7FF] text-[11px] tracking-[3px] font-bold">SONG TITLE *</label>
            <input value={form.song_title} onChange={(e) => setForm({ ...form, song_title: e.target.value })}
              className="mt-2 w-full px-4 py-3 rounded-2xl bg-[#05070D] border border-[#1a2942] text-[#F8FAFC] focus:border-[#00B7FF] outline-none text-base" placeholder="e.g. Rising Dreams" />
          </div>
          <div>
            <label className="text-[#00B7FF] text-[11px] tracking-[3px] font-bold">ARTIST NAME *</label>
        ...
        {/* Cover art */}
        <div>
          <label className="text-[#00B7FF] text-[11px] tracking-[3px] font-bold">COVER ART (OPTIONAL)</label>
          <label className="mt-2 flex items-center gap-3 p-3 rounded-2xl border border-[#1a2942] bg-[#05070D]/40 cursor-pointer hover:border-[#00B7FF]/60 transition">
            ...
            {coverFile ? (
              <img src={URL.createObjectURL(coverFile)} className="w-12 h-12 rounded-xl object-cover border border-[#1a2942] shrink-0" alt="" />
            ) : (
              <div className="w-12 h-12 rounded-xl bg-[#0B1426] border border-[#1a2942] flex items-center justify-center text-[#94A3B8] shrink-0">
                <ImageIcon size={20} />
              </div>
            )}
            <div className="flex-1 min-w-0 text-[#94A3B8] text-sm truncate">{coverFile ? coverFile.name : 'Upload cover art'}</div>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-[#00B7FF] text-[11px] tracking-[3px] font-bold">MOOD / VIBE</label>
        ...
        {/* Note to reviewer */}
        <div>
          <label className="text-[#00B7FF] text-[11px] tracking-[3px] font-bold">NOTES TO REVIEWER (OPTIONAL)</label>
        ...
        <label className="flex items-center justify-between p-3 rounded-2xl border border-[#1a2942] bg-[#05070D]/40">
          ...
          <input type="checkbox" checked={form.explicit} onChange={(e) => setForm({ ...form, explicit: e.target.checked })} className="w-5 h-5 accent-[#00B7FF]" />
        </label>

        <label className="flex items-start gap-3 p-3 rounded-2xl border border-[#1a2942] cursor-pointer hover:bg-[#05070D]/40 transition">
          <input type="checkbox" checked={form.rights} onChange={(e) => setForm({ ...form, rights: e.target.checked })} className="mt-1 w-4 h-4 accent-[#00B7FF] shrink-0" />
          <span className="text-[#94A3B8] text-xs leading-relaxed">I confirm I own or have rights to submit this song for review. *</span>
        </label>
        <label className="flex items-start gap-3 p-3 rounded-2xl border border-[#1a2942] cursor-pointer hover:bg-[#05070D]/40 transition">
          <input type="checkbox" checked={form.terms} onChange={(e) => setForm({ ...form, terms: e.target.checked })} className="mt-1 w-4 h-4 accent-[#00B7FF] shrink-0" />
          <span className="text-[#94A3B8] text-xs leading-relaxed">I agree to the Trey TV Live Review terms. *</span>
        </label>
        ...
        <button onClick={submit} disabled={stage !== 'idle'}
          ...
          {stage === 'uploading' && <><Loader2 className="animate-spin" size={18} /> UPLOADING...</>}
          {stage === 'analyzing' && <><Brain className="animate-pulse" size={18} /> RUNNING AI PRE-CHECK...</>}
          {stage === 'idle' && <>RUN AI PRE-CHECK <ChevronRight size={18} /></>}
        </button>
        <p className="text-[10px] text-[#64748B] text-center">AI pre-check is a quality estimate. Trey's live review is the official score.</p>
      </div>
    </div>
  );
};
