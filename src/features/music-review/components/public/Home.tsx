...
import { Cloud, ChevronRight, Brain, Users, Mic, Trophy, Zap, Flame, Gem, Crown, BarChart3 } from 'lucide-react';
...
export const Home: React.FC<HomeProps> = ({ onSubmit, onOpenMic, onQueue, onLive, onProfile, onSkipLine }) => {
    ...
    <div className="px-4 pt-4 pb-32 max-w-5xl mx-auto space-y-5">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-[#1a2942] bg-gradient-to-br from-[#0B1426] via-[#0a1830] to-[#08111F] p-5 md:p-8 shadow-[0_0_60px_-15px_rgba(0,183,255,0.5)]">
        <div className="absolute -right-10 top-0 w-60 h-60 opacity-30 pointer-events-none">
          <div className="w-full h-full rounded-full" style={{ background: 'radial-gradient(circle, rgba(0,183,255,0.5), transparent 70%)', filter: 'blur(20px)' }} />
        </div>
        <div className="relative">
          ...
          <p className="mt-3 text-[#94A3B8] text-sm md:text-base max-w-md leading-relaxed">
            Real feedback. Live on air.<br />Get heard. Get scored. Get seen.
          </p>
      ...
      {/* NOW PLAYING MINI */}
      <button onClick={onLive} className="w-full text-left rounded-3xl border border-[#FFC857]/40 bg-gradient-to-r from-[#0B1426] to-[#101827] p-4 hover:border-[#FFC857] transition shadow-[0_0_25px_-10px_rgba(255,200,87,0.5)]">
      ...
      <section className="rounded-3xl border border-[#1a2942] bg-[#0B1426]/60 p-5 backdrop-blur-xl">
        <div className="text-[#00B7FF] text-[11px] tracking-[6px] text-center mb-4 font-bold">• HOW IT WORKS •</div>
        <div className="grid grid-cols-4 gap-2 md:gap-4">
          {[
            { n: 1, icon: <Brain size={20} />, title: 'AI PRE-CHECK', sub: 'Quality, vocals & content', color: '#00B7FF' },
            { n: 2, icon: <Users size={20} />, title: 'JOIN THE QUEUE', sub: 'Enter live queue', color: '#00B7FF' },
            { n: 3, icon: <Mic size={20} />, title: 'LIVE REVIEW', sub: 'Trey reviews it live', color: '#FFC857' },
            { n: 4, icon: <Trophy size={20} />, title: 'SCORE POSTED', sub: 'Saved to your profile', color: '#00B7FF' }
          ].map((s) => (
            <div key={s.n} className="flex flex-col items-center text-center">
              <div className="relative">
                ...
                <div className="absolute -top-1 -left-1 w-5 h-5 rounded-full bg-[#0B1426] border border-[#1a2942] text-[10px] flex items-center justify-center text-[#00B7FF] font-bold">{s.n}</div>
              </div>
              <div className="mt-2 text-[10px] md:text-xs font-bold text-[#F8FAFC] tracking-wide">{s.title}</div>
              <div className="text-[9px] md:text-[10px] text-[#94A3B8] mt-1 hidden md:block">{s.sub}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <button onClick={onQueue} className="text-left rounded-3xl border border-[#1a2942] bg-[#0B1426]/80 p-4 hover:border-[#00B7FF]/60 transition">
          ...
          {queue.length === 0 && !nowPlaying && (
            <div className="text-[#94A3B8] text-xs py-4 text-center">Queue is empty. Submit to be first.</div>
          )}
          <div className="space-y-1.5">
            {queue.slice(0, 4).map((q, i) => (
              <div key={q.id} className="flex items-center gap-3 py-1.5">
                <div className="w-6 h-6 rounded-full bg-[#101827] flex items-center justify-center text-[#00B7FF] text-xs font-bold">{i + 2}</div>
                <div className="flex-1 min-w-0">
                  <div className="text-[#F8FAFC] text-sm truncate">{q.song_title}</div>
                  <div className="text-[#94A3B8] text-xs truncate">{q.artist_name}</div>
                </div>
                <div className="text-[#00B7FF] text-[10px] font-bold">{i === 0 ? 'UP NEXT' : 'IN QUEUE'}</div>
              </div>
            ))}
          </div>
        ...
        <button onClick={onSkipLine} className="text-left rounded-3xl border border-[#FFC857]/40 bg-gradient-to-br from-[#0B1426] to-[#101827] p-4 hover:border-[#FFC857] transition">
          ...
          <div className="space-y-2">
            {[
              { icon: <Zap size={16} />, label: 'QUICK PASS', sub: 'Top of the queue', price: '$5', color: '#FFC857' },
              { icon: <Flame size={16} />, label: 'HOT SEAT', sub: 'Next up, guaranteed', price: '$10', color: '#A855F7' },
              { icon: <Gem size={16} />, label: 'FRONT OF LINE', sub: 'Priority + promo boost', price: '$15', color: '#00B7FF' }
            ].map((t) => (
              <div key={t.label} className="flex items-center gap-3 p-2.5 rounded-2xl border border-[#1a2942] bg-[#05070D]/40">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: `${t.color}20`, color: t.color, border: `1px solid ${t.color}40` }}>
                  {t.icon}
                </div>
                <div className="flex-1">
                  <div className="text-[#F8FAFC] font-bold text-sm" style={{ color: t.color }}>{t.label}</div>
      ...
      {/* SCORES TEASER */}
      <button onClick={onProfile} className="w-full text-left rounded-3xl border border-[#1a2942] bg-gradient-to-r from-[#0B1426] to-[#08111F] p-4 flex items-center gap-4 hover:border-[#A855F7]/60 transition">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#A855F7,#00B7FF)' }}>
          <Trophy size={22} className="text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-[#F8FAFC] font-bold">BUILD YOUR LEGACY.</div>
          <div className="text-[#94A3B8] text-xs">Climb the leaderboard, earn badges, become the next Featured Artist.</div>
        </div>
