...
export const OpenMic: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel("open-mic")
      .on("postgres_changes", { event: "*", schema: "public", table: "open_mic_queue" }, () =>
        load(),
      )
      .subscribe();
    return () => {
  ...
  const submit = async () => {
    ...
    if (count >= OPEN_MIC_MAX_PER_DAY)
      return setError(`Daily limit reached (${OPEN_MIC_MAX_PER_DAY}/day).`);
    const qCount = await getActiveQueueCount();
    if (qCount >= OPEN_MIC_MAX_QUEUE)
      return setError(
        `Open Mic queue is full (${OPEN_MIC_MAX_QUEUE}/${OPEN_MIC_MAX_QUEUE}). Try again soon.`,
      );
    ...
    <div className="px-4 pt-4 pb-32 max-w-6xl mx-auto space-y-4">
      <div className="text-center">
        ...
        <p className="text-[#94A3B8] text-sm mt-1">
          {OPEN_MIC_MAX_QUEUE} song queue. {OPEN_MIC_MAX_PER_DAY} submissions per artist per day.
        </p>
      </div>
        ...
        <div className="space-y-3">
          {playing ? (
            <div className="rounded-3xl border border-[#A855F7]/40 bg-[#0B1426] p-3 shadow-[0_0_30px_-10px_rgba(168,85,247,0.6)]">
              <div className="flex items-center justify-between mb-2">
                <span className="flex items-center gap-1 text-[#A855F7] text-[10px] font-bold">
                  <span className="w-2 h-2 rounded-full bg-[#A855F7] animate-pulse" /> PLAYING
                </span>
                <span className="text-[#94A3B8] text-[10px]">from {playing.user_name}</span>
              </div>
              <AudioPlayer
                src={audioUrl}
                title={playing.song_title}
          ...
          <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426] p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-[#A855F7] text-xs font-bold">
                {queue.length}/{OPEN_MIC_MAX_QUEUE}
              </div>
            </div>
            <div className="space-y-2">
              {queue
                ...
                .map((q, i) => (
                  ...
                  >
                    <div className="w-6 h-6 rounded-full bg-[#101827] text-[#A855F7] flex items-center justify-center text-xs font-bold">
                      {i + 1}
                    </div>
        ...
        <div className="rounded-3xl border border-[#A855F7]/40 bg-[#0B1426] p-4 space-y-3 h-fit">
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-[#94A3B8]">
              {dailyCount}/{OPEN_MIC_MAX_PER_DAY} today
            </div>
          </div>
          ...
          {user && dailyCount < OPEN_MIC_MAX_PER_DAY && (
            <>
              <input
                ...
                className="w-full px-3 py-2.5 rounded-xl bg-[#05070D] border border-[#1a2942] text-[#F8FAFC] outline-none focus:border-[#A855F7]"
              />
              <label className="block">
                ...
                />
                <div
                  className={`flex items-center gap-3 p-3 rounded-xl border-2 border-dashed cursor-pointer ${file ? "border-[#22C55E]/60 bg-[#22C55E]/5" : "border-[#1a2942]"}`}
                >
                  <Upload size={18} className="text-[#A855F7]" />
                  <span className="text-sm text-[#F8FAFC] truncate flex-1">
                    {file ? file.name : `Upload audio (max ${MAX_AUDIO_MB}MB)`}
                  </span>
                </div>
              </label>
              {error && (
                <div className="flex items-center gap-2 text-[#EF4444] text-xs bg-[#EF4444]/10 rounded-xl p-2">
              ...
              <button
                onClick={submit}
                disabled={submitting}
                className="w-full py-3 rounded-2xl bg-gradient-to-r from-[#A855F7] to-[#00B7FF] text-white font-black tracking-wider disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {submitting ? (
                  <>
          ...
          <div className="text-[10px] text-[#64748B] leading-relaxed border-t border-[#1a2942] pt-3">
            Open Mic audio is auto-deleted after playback. A lightweight play record stays for
