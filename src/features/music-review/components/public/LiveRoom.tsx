...
export const LiveRoom: React.FC = () => {
  ...
  const load = async () => {
    ...
    const { data: q } = await supabase.from('music_review_submissions').select('*')
      .in('status', ['in_queue', 'ai_prechecked'])
      .order('queue_position', { ascending: true, nullsFirst: false }).limit(8);
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'music_review_comments' }, () => setEnergy((e) => Math.min(100, e + 4)))
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'music_review_reactions' }, () => setEnergy((e) => Math.min(100, e + 6)))
      .subscribe();
    const decay = setInterval(() => setEnergy((e) => Math.max(20, e - 1)), 3000);
    return () => { supabase.removeChannel(ch); clearInterval(decay); };
  }, []);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-2xl border border-[#22C55E]/40 bg-[#22C55E]/5">
          ...
          <div className="w-20 h-1.5 rounded-full bg-[#101827] overflow-hidden">
            <div className="h-full bg-gradient-to-r from-[#22C55E] to-[#FFC857] transition-all" style={{ width: `${energy}%` }} />
          </div>
        </div>
          ...
          {np ? (
            <div className="rounded-3xl border border-[#FFC857]/40 bg-gradient-to-br from-[#0B1426] to-[#08111F] p-4 shadow-[0_0_40px_-15px_rgba(255,200,87,0.5)]">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[#FFC857] text-[10px] font-bold tracking-wider">REVIEWING LIVE</span>
              </div>
              <AudioPlayer src={audio} title={np.song_title} artist={np.artist_name} coverUrl={cover} />
            </div>
          ) : (
            ...
            <div className="space-y-2">
              {queue.slice(0, 5).map((q, i) => (
                <div key={q.id} className="flex items-center gap-3 p-2 rounded-2xl border border-[#1a2942] bg-[#05070D]/40">
                  <div className="w-6 h-6 rounded-full bg-[#101827] text-[#00B7FF] flex items-center justify-center text-xs font-bold">{i + 1}</div>
              ...
              {queue.length === 0 && <div className="text-[#94A3B8] text-sm text-center py-4">Queue is empty.</div>}
            </div>
          </div>
        </div>
