...
export const ChatPanel: React.FC<ChatPanelProps> = ({ roomType, refId }) => {
  ...
  const [reactions, setReactions] = useState<{ like: number; fire: number; replay: number }>({
    like: 0,
    fire: 0,
    replay: 0,
  });
    ...
    <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426]/80 p-4 flex flex-col h-[520px]">
      <div className="flex items-center justify-between mb-3">
        ...
        {user?.isAdmin && (
          <button
            onClick={aiSpark}
            className="flex items-center gap-1 text-[10px] text-[#A855F7] border border-[#A855F7]/40 rounded-lg px-2 py-1"
          >
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1">
        ...
        {comments.map((c) => (
          <div
            key={c.id}
            className={`rounded-2xl p-2.5 ${c.is_ai_labeled ? "border border-[#A855F7]/40 bg-[#A855F7]/5" : c.is_admin ? "border border-[#FFC857]/40 bg-[#FFC857]/5" : "bg-[#05070D]/60"}`}
          >
            <div className="flex items-center gap-2 text-xs">
              <span
                className="font-bold"
                style={{ color: c.is_ai_labeled ? "#A855F7" : c.is_admin ? "#FFC857" : "#00B7FF" }}
              >
                {c.user_name || "User"}
              </span>
              {c.is_admin && (
                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[#FFC857]/20 text-[#FFC857]">
                  HOST
                </span>
              )}
              {c.is_ai_labeled && (
                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[#A855F7]/20 text-[#A855F7] flex items-center gap-1">
                  <Shield size={8} /> AI
                </span>
              )}
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <button
          ...
          className="w-9 h-9 rounded-xl bg-[#00B7FF] text-[#05070D] flex items-center justify-center disabled:opacity-30"
        >
          <Send size={14} />
        </button>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        <button
          onClick={() => react("like")}
          disabled={!user}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#1a2942] text-[#F8FAFC] text-xs disabled:opacity-40"
        >
