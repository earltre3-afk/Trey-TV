...
const KEYS = [
  ...
  {
    key: "open_mic_max_per_day",
    label: "Max Open Mic submissions per user / day",
    type: "number",
    def: 2,
  },
  ...
  {
    key: "allow_anonymous_listen",
    label: "Allow signed-out users to listen",
    type: "bool",
    def: true,
  },
...
export const AdminSettings: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      KEYS.forEach((k) => {
        if (map[k.key] === undefined) map[k.key] = k.def;
      });
      setVals(map);
    })();
  }, []);
    ...
    <div className="px-4 pt-4 pb-32 max-w-2xl mx-auto space-y-4">
      ...
      <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426] p-4 space-y-3">
        {KEYS.map((k) => (
          <div
            key={k.key}
            className="flex items-center justify-between gap-3 py-2 border-b border-[#1a2942] last:border-0"
          >
            <div>
            ...
            {k.type === "bool" ? (
              <input
                type="checkbox"
                checked={!!vals[k.key]}
                onChange={(e) => setVals({ ...vals, [k.key]: e.target.checked })}
        ...
        <button
          onClick={save}
          className="w-full mt-3 py-3 rounded-2xl bg-gradient-to-r from-[#FFC857] to-[#FFB000] text-[#05070D] font-black tracking-wider flex items-center justify-center gap-2"
        >
          {saved ? (
            <>
