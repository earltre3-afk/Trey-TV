...
export const AdminQueue: React.FC<AdminQueueProps> = ({ onReview }) => {
  ...
  const filtered = subs.filter((s) => {
    ...
    if (search && !`${s.song_title} ${s.artist_name}`.toLowerCase().includes(search.toLowerCase()))
      return false;
    return true;
  });
  ...
  const confirmPayment = async (id: string) => {
    ...
    if (data)
      await supabase
        .from("music_review_payments")
        .update({ status: "confirmed", confirmed_by_admin: true })
        .eq("id", data.id);
    ...
    <div className="px-4 pt-4 pb-32 max-w-6xl mx-auto space-y-3">
      ...
      <div className="flex flex-col md:flex-row gap-2">
        ...
        <select
          ...
          className="px-3 py-2 rounded-xl bg-[#05070D] border border-[#1a2942] text-[#F8FAFC] outline-none"
        >
          {[
            "all",
            "pending",
      ...
      <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426] overflow-hidden">
        ...
        <div className="divide-y divide-[#1a2942]">
          {filtered.map((s) => (
            <div
              key={s.id}
              className="grid grid-cols-[40px_1fr_120px_120px_180px] gap-2 px-3 py-2 items-center text-sm hover:bg-[#101827]/40"
            >
              ...
              <div className="min-w-0">
                ...
                <div className="text-[#94A3B8] text-xs truncate">
                  {s.artist_name} • AI {s.ai_precheck_score ?? "—"}
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-1">
                <button
                  title="Set Now Playing"
                  onClick={() => markNowPlaying(s.id)}
                ...
                <button
                  ...
                  className="w-7 h-7 rounded-lg bg-[#00B7FF]/15 border border-[#00B7FF]/40 text-[#00B7FF] flex items-center justify-center"
                >
                  <Edit3 size={12} />
                </button>
                {s.priority_tier && !s.priority_paid && (
                  <button
                    ...
                    className="w-7 h-7 rounded-lg bg-[#22C55E]/15 border border-[#22C55E]/40 text-[#22C55E] flex items-center justify-center"
                  >
                    <DollarSign size={12} />
                  </button>
                )}
          ...
          {filtered.length === 0 && (
            <div className="py-10 text-center text-[#94A3B8] text-sm">No submissions match.</div>
          )}
        </div>
      </div>
    </div>
