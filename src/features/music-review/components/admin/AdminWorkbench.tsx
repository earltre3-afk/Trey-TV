...
export const AdminWorkbench: React.FC<AdminWorkbenchProps> = ({ submissionId, onDone }) => {
  ...
  const [scores, setScores] = useState<Record<string, number>>({
    vocals_score: 85, lyrics_score: 85, mix_score: 85, originality_score: 85,
    hit_potential_score: 85, replay_value_score: 85, marketability_score: 85
  });
    ...
    <div className="px-4 pt-4 pb-32 max-w-6xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="space-y-3">
          <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426] p-4">
            ...
            <div className="text-[#94A3B8] text-xs mt-1">Genre: {sub.genre || '—'} • Mood: {sub.mood || '—'} • AI Pre-Check: {sub.ai_precheck_score ?? '—'}/100</div>
            {sub.note_to_reviewer && <div className="mt-2 text-sm text-[#94A3B8] bg-[#05070D]/60 rounded-2xl p-2 border border-[#1a2942]">Note: {sub.note_to_reviewer}</div>}
          </div>
          <AudioPlayer src={audio} title={sub.song_title} artist={sub.artist_name} coverUrl={cover} />
          <textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} rows={4} placeholder="Private admin notes…"
        ...
        <div className="space-y-3">
          <div className="rounded-3xl border border-[#1a2942] bg-[#0B1426] p-4">
            ...
            {CATS.map((c) => (
              <div key={c.key} className="mb-3">
                ...
                <input type="range" min={0} max={100} value={scores[c.key]} onChange={(e) => setScores({ ...scores, [c.key]: parseInt(e.target.value) })}
                  className="w-full accent-[#00B7FF]" />
              </div>
            ))}
            <div className="rounded-2xl border border-[#FFC857]/40 bg-[#FFC857]/5 p-3 text-center mt-2">
              <div className="text-[#FFC857] text-[10px] tracking-wider font-bold">OVERALL SCORE</div>
              <div className="text-5xl font-black text-[#FFC857]">{overall}</div>
            </div>
          </div>

          <textarea value={summary} onChange={(e) => setSummary(e.target.value)} rows={3} placeholder="Written summary (will appear in email & profile)"
            className="w-full px-3 py-2 rounded-2xl bg-[#0B1426] border border-[#1a2942] text-[#F8FAFC] outline-none focus:border-[#00B7FF]" />
          <textarea value={strengths} onChange={(e) => setStrengths(e.target.value)} rows={3} placeholder="Strengths (one per line)"
            className="w-full px-3 py-2 rounded-2xl bg-[#0B1426] border border-[#22C55E]/30 text-[#F8FAFC] outline-none focus:border-[#22C55E]" />
          <textarea value={improvements} onChange={(e) => setImprovements(e.target.value)} rows={3} placeholder="Improvements (one per line)"
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={saveDraft} className="flex items-center justify-center gap-2 py-3 rounded-2xl border border-[#1a2942] text-[#94A3B8] hover:border-[#00B7FF]/60"><Save size={14} /> SAVE DRAFT</button>
            <button onClick={() => finalize(true)} className="flex items-center justify-center gap-2 py-3 rounded-2xl bg-gradient-to-r from-[#FFC857] to-[#FFB000] text-[#05070D] font-black"><CheckCircle2 size={14} /> COMPLETE + PUBLISH</button>
            <button onClick={() => finalize(false)} className="flex items-center justify-center gap-2 py-3 rounded-2xl border border-[#A855F7]/40 text-[#A855F7]"><Eye size={14} /> COMPLETE PRIVATE</button>
            <button onClick={sendEmail} disabled={!savedReview} className="flex items-center justify-center gap-2 py-3 rounded-2xl bg-[#00B7FF] text-[#05070D] font-black disabled:opacity-40"><Send size={14} /> SEND EMAIL</button>
          </div>

          <button onClick={onDone} className="w-full py-2 rounded-2xl border border-[#1a2942] text-[#94A3B8] text-sm">← Back to Queue</button>
        </div>
      </div>
      ...
      {emailHtml && (
        <div className="mt-6">
          ...
          <iframe srcDoc={emailHtml} className="w-full h-[800px] rounded-3xl border border-[#1a2942] bg-white" title="Email preview" />
        </div>
      )}
    </div>
  );
