  ...
  useEffect(() => {
    const load = async () => {
      ...
      const avg = scores.data?.length
        ? scores.data.reduce((s: number, x: any) => s + x.overall_score, 0) / scores.data.length
        : 0;
      setStats({
        total: tot.count || 0,
        pending: pen.count || 0,
  ...
  return (
    <div className="px-4 pt-4 pb-32 max-w-5xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onSettings}
          className="px-3 py-2 rounded-2xl border border-[#1a2942] text-[#94A3B8] text-xs hover:border-[#00B7FF]/60"
        >
          Settings
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <button
          onClick={onQueue}
          className="text-left rounded-3xl border border-[#1a2942] bg-[#0B1426] p-5 hover:border-[#00B7FF]/60 transition"
        >
          <div className="text-[#00B7FF] text-xs tracking-wider font-bold">
        ...
        <button
          onClick={onOpenMic}
          className="text-left rounded-3xl border border-[#1a2942] bg-[#0B1426] p-5 hover:border-[#A855F7]/60 transition"
        >
