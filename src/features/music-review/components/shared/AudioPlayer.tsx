...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  return (
    <div className={`relative rounded-3xl border border-[#1a2942] bg-gradient-to-b from-[#0B1426] to-[#08111F] ${compact ? 'p-3' : 'p-4 md:p-5'}`}>
      <audio
        ref={ref}
        src={src}
        preload="metadata"
      ...
      <div className="flex items-center gap-3 md:gap-4">
        {coverUrl ? (
          ...
          <div className={`${compact ? 'w-14 h-14' : 'w-20 h-20 md:w-24 md:h-24'} rounded-2xl bg-gradient-to-br from-[#00B7FF]/20 to-[#A855F7]/20 border border-[#1a2942] flex items-center justify-center`}>
            <div className="text-[#00B7FF] text-2xl">♪</div>
          </div>
        )}
        <div className="flex-1 min-w-0">
          {title && <div className="font-bold text-[#F8FAFC] truncate text-base md:text-lg">{title}</div>}
      ...
      {error && (
        <div className="mt-3 flex items-center gap-2 text-sm text-[#EF4444] bg-[#EF4444]/10 border border-[#EF4444]/30 rounded-xl px-3 py-2">
          ...
          <button onClick={retry} className="flex items-center gap-1 text-[#00B7FF] hover:underline">
