...
import {
  loadPrescriptions, savePrescriptions, scoreContent, generatePrescriptionTitle,
...
const PrescribeMeApp: React.FC = () => {
  ...
  const goStart = () => {
    setMoods([]); setEnergy(null); setContentTypes([]); setMomentNeeds([]);
  ...
  return (
    <div className="prescribe-me-root fixed inset-0 h-[100dvh] overflow-hidden text-white antialiased font-sans" style={{ fontFamily: '"Inter", system-ui, -apple-system, sans-serif' }}>
      ...
      <main className="relative z-0 h-full overflow-hidden">
        ...
        {step === 'content' && (
          <ContentTypeSelector
            value={contentTypes} onChange={setContentTypes}
        ...
        {step === 'moment' && (
          <MomentNeedSelector
            value={momentNeeds} onChange={setMomentNeeds}
        ...
        {step === 'analyzing' && (
          <AnalyzingScreen moods={moods} onDone={onAnalyzeDone} />
        )}
        {step === 'results' && (
          <ResultsScreen
            answers={answers}
