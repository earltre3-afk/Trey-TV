        ...
        <div className="grid grid-cols-[1fr_1fr] gap-2">
          ...
          {active && index < filtered.length - 1 ? (
            ...
            <NeonGlassButton
              ...
              className="w-full"
              ariaLabel="Start a new prescription"
            >
              <span className="inline-flex items-center gap-2 justify-center">
...
const EmptyState: React.FC<{ tab: Tab; onNewPrescription: () => void }> = ({
  tab,
  onNewPrescription,
}) => (
  <LiquidGlassCard accent="purple">
    <div className="p-6 text-center">
      ...
      <p className="text-white/60 text-sm mt-1">
        {tab === "favorites"
          ? "Star the ones you love to find them here."
          : "Start a new prescription to build your library."}
      </p>
        ...
        : primaryMood === "Reflective"
          ...
          : primaryMood === "Motivated"
            ...
            : primaryMood === "Wild"
              ...
              : "purple"
    : ("purple" as const);
  const thumbs = p.recIds
    .slice(0, 3)
    .map((id) => CONTENT_LIBRARY.find((x) => x.id === id)?.thumbnail)
      ...
      <div className="p-4">
        <div className="flex items-center gap-3">
          {primaryMood && (
            <div
              ...
              style={{
                borderColor: MOOD_META[primaryMood].color,
                boxShadow: `0 0 14px ${MOOD_META[primaryMood].color}aa`,
              }}
            >
            ...
            <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] mt-0.5">
              {p.answers.moods.slice(0, 3).map((m, i) => (
                <React.Fragment key={m}>
                  ...
                  {i < Math.min(p.answers.moods.length, 3) - 1 && (
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <div className="flex -space-x-2">
            {thumbs.map((src, i) => (
              <div
                key={i}
                className="w-12 h-12 rounded-full border-2 border-black/80 overflow-hidden"
              >
        ...
        <div className="mt-3 grid grid-cols-3 gap-1.5">
          ...
          <IconBtn
            onClick={onToggleFavorite}
            icon={<Heart className="w-4 h-4" />}
            label={p.isFavorite ? "Loved" : "Favorite"}
            color="#f472b6"
          />
        </div>
          ...
          <LiquidGlassCard accent="orange">
            <div className="p-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-white/65 mt-0.5 truncate">
                  Your vibe. Your recipe. One tap.
                </p>
              </div>
            </div>
          </LiquidGlassCard>
...
const TabBtn: React.FC<{
  ...
  <button
    ...
    className={`inline-flex items-center justify-center gap-1.5 py-2 rounded-full text-xs transition ${
      active
        ? "bg-amber-500/20 text-amber-200 border border-amber-300/60 shadow-[0_0_18px_-4px_rgba(251,191,36,0.6)]"
...
const IconBtn: React.FC<{
  ...
  <button
    onClick={onClick}
    className="flex flex-col items-center gap-1 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition"
  >
