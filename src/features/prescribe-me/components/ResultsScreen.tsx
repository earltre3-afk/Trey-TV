...
const ResultsScreen: React.FC<Props> = ({
  ...
  const pickMoods: Mood[] =
    answers.moods.length > 0 ? answers.moods : (["Reflective", "Happy", "Motivated"] as Mood[]);
    ...
    <section className="w-full max-w-md mx-auto h-[100dvh] overflow-hidden px-4 pt-[max(0.25rem,env(safe-area-inset-top))] pb-[max(0.65rem,env(safe-area-inset-bottom))] flex flex-col">
      ...
      <div className="flex-1 min-h-0 flex flex-col justify-center">
        {activePage === "top" && top && (
          <TopPickCard
            top={top}
            feedback={feedback[top.id]}
            setFeedback={(v) => setFb(top.id, v)}
        ...
        {activePage === "recommended" && (
          <PagedPickList
            title="More matches"
            subtitle="Because of the mood recipe you picked."
            items={secondary}
            attribFor={attribFor}
            startIndex={0}
          />
        ...
        {activePage === "more" && (
          <PagedPickList
            title="Deep cuts"
            subtitle="Extra options without making the page scroll."
            items={moreRest}
            attribFor={attribFor}
            startIndex={3}
          />
        ...
        <div className="grid grid-cols-[1fr_1.5fr] gap-2">
          ...
          {page < pages.length - 1 ? (
            ...
            <NeonGlassButton
              onClick={onGoWatch}
              className="w-full"
              ariaLabel="Watch my prescription"
            >
              <span className="inline-flex items-center gap-2 justify-center">
  ...
  <LiquidGlassCard accent="multi">
    <div className="p-3.5">
      ...
      <div className="mt-3 grid grid-cols-4 gap-1.5">
        <FbButton
          active={feedback === "perfect"}
          onClick={() => setFeedback("perfect")}
          icon={<ThumbsUp className="w-3.5 h-3.5" />}
          label="Perfect"
        />
        <FbButton
          active={feedback === "not-my-vibe"}
          onClick={() => setFeedback("not-my-vibe")}
          icon={<ThumbsDown className="w-3.5 h-3.5" />}
          label="Not me"
        />
        <FbButton
          active={feedback === "more-like"}
          onClick={() => setFeedback("more-like")}
          icon={<Sparkles className="w-3.5 h-3.5" />}
          label="More"
        />
        <FbButton
          active={feedback === "less-like"}
          onClick={() => setFeedback("less-like")}
      ...
      {items.map((it, i) => {
        ...
        return (
          <LiquidGlassCard
            key={it.id}
            accent="cyan"
            onClick={() => window.alert(`Now playing: ${it.title}`)}
          >
  ...
  <LiquidGlassCard accent="orange">
    <div className="p-5 text-center">
      ...
      <p className="text-white/65 text-sm mt-1">
        Average match score: <span className="text-cyan-300 font-semibold">{avgMatch}%</span>
      </p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2">
        ...
        <button
          onClick={onShare}
          className="py-3 rounded-2xl border border-fuchsia-400/40 bg-fuchsia-500/10 hover:bg-fuchsia-500/20 text-fuchsia-200 text-xs flex flex-col items-center justify-center gap-1"
        >
          <Share2 className="w-5 h-5" /> Share
        </button>
        <button
          onClick={onTryAgain}
          className="py-3 rounded-2xl border border-white/15 bg-white/5 hover:bg-white/10 text-white/80 hover:text-white text-xs flex flex-col items-center justify-center gap-1"
        >
          <RotateCcw className="w-5 h-5" /> Try Again
        </button>
        <button
          onClick={onAdjustMood}
          className="py-3 rounded-2xl border border-cyan-400/40 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-200 text-xs flex flex-col items-center justify-center gap-1"
        >
      ...
      <button
        ...
        className="mt-4 text-sm text-white/70 hover:text-white inline-flex items-center gap-2"
      >
  ...
  <div>
    ...
    <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
      <div
        ...
        style={{ width: `${value}%`, boxShadow: "0 0 10px rgba(236,72,153,0.5)" }}
      />
    </div>
  </div>
);
