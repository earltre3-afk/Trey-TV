...
export async function joinRoomByCode(code: string, identity: PlayerIdentity): Promise<{ room: RoomRow; player: PlayerRow }> {
  ...
  if (seat === -1) {
    ...
    await supabase.from('game_room_players').delete().eq('id', (bot as PlayerRow).id);
...
export async function startGameSession(roomId: string, gameType: GameType): Promise<SessionRow> {
  ...
  if (gameType === 'spades') {
    ...
    state = s; phase = s.phase; currentSeat = s.currentSeat; round = s.round;
...
export async function updateSessionState(sessionId: string, state: any, currentSeat: number | null, phase: string, round?: number) {
  const patch: any = {
    state_json: state,
    current_turn_seat: currentSeat,
    phase,
...
export async function endSession(sessionId: string, roomId: string) {
  ...
  await supabase.from('game_rooms').update({ status: 'ended', last_activity_at: new Date().toISOString() }).eq('id', roomId);
}
