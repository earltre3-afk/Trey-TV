...
export interface BSPlayer { seat: number; name: string; isBot: boolean; hand: string[]; }
export interface BSClaim { seat: number; rank: Rank; count: number; cardIds: string[]; }
...
export function newBullshitGame(playerNames: string[]): BSState {
  ...
  const players: BSPlayer[] = playerNames.map((name, seat) => ({ seat, name, isBot: seat !== 0, hand: hands[seat] }));
  return {
    phase: 'playing',
    players,
    pile: [],
...
export function makeClaim(state: BSState, seat: number, cardIds: string[], claimedRank: Rank): BSState {
