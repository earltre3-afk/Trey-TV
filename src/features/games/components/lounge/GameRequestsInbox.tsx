  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "rgba(5,7,13,0.78)", borderColor: "rgba(255,200,87,0.18)" }}
      >
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[10px] tracking-[0.3em] font-bold" style={{ color: "#FFC857" }}>
              INBOX
            </div>
      ...
      <div className="relative max-w-3xl mx-auto px-4 py-5 space-y-4">
        {/* Tabs */}
        <div
          className="flex gap-2 rounded-2xl p-1 border trey-glass-panel"
        ...
        {tab === "out" && (
          <div className="space-y-2">
            ...
            {outgoing.map((r) => (
              <RequestCard key={r.id} r={r} kind="out" onCancel={() => handleCancel(r)} />
            ))}
          </div>
        )}
      </div>
...
const RequestCard: React.FC<{
  ...
  const statusColor =
    r.status === "accepted"
      ? "#22C55E"
      : r.status === "declined" || r.status === "cancelled" || r.status === "expired"
        ? "#FCA5A5"
        : meta.color;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-sm font-black mt-1.5 truncate">
            {kind === "in"
              ? `${r.from_display_name} invited you`
              : `Invited ${r.to_display_name || r.to_user_id.slice(0, 8)}`}
          </div>
          {r.message && (
