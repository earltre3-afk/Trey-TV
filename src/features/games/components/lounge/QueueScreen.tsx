...
import {
  ...
  cancelQueue,
  getQueueSnapshot,
  resolveMatchesForGame,
  heartbeatQueue,
  reapStaleQueueEntries,
...
const GAME_META: Record<
  GameType,
  { name: string; tag: string; desc: string; color: string; icon: React.ReactNode }
> = {
  spades: {
    name: "Spades",
    tag: "TEAMS · 4 PLAYERS",
    desc: "Bidding, books, and bag pressure with a partner.",
    color: "#00B7FF",
    icon: <Spade size={16} />,
  },
  blackjack: {
    name: "Blackjack",
    tag: "SOLO TABLE · 1 PLAYER",
  ...
  truno: {
    name: "Truno",
    tag: "COLOR MATCH - 2-4 PLAYERS",
    desc: "Match colors, fire action cards, and call TRUNO.",
    color: "#D946EF",
...
export const QueueScreen: React.FC<Props> = ({
  gameType,
  identity,
  onBack,
  onMatched,
  ...
  useEffect(() => {
    if (!backendEnabled) {
      setError(
        "Public queue is waiting on the Trey TV game database migration. Solo tables are available now.",
      );
      return;
    }
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (
          s.myEntry?.status === "matched" &&
          s.myEntry.matched_room_id &&
          !matchedHandledRef.current
        ) {
  ...
  const handleCancel = async () => {
    if (backendEnabled) {
      try {
        await cancelQueue(identity.userId);
      } catch {
        /* noop */
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 opacity-70">
        <div
          className="absolute -top-40 -left-32 w-[520px] h-[520px] rounded-full blur-[140px]"
          style={{ background: `radial-gradient(circle, ${meta.color}33 0%, transparent 70%)` }}
        />
        <div
          className="absolute bottom-0 right-0 w-[480px] h-[480px] rounded-full blur-[140px]"
          style={{
      ...
      <div
        ...
        style={{ background: "rgba(5,7,13,0.78)", borderColor: "rgba(0,183,255,0.18)" }}
      >
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[10px] tracking-[0.3em] font-bold" style={{ color: meta.color }}>
              MATCHMAKING
            </div>
      ...
      <div className="relative max-w-3xl mx-auto px-4 py-6 md:py-10 space-y-5">
          ...
          <div className="flex items-center justify-center my-8 md:my-10">
            <div className="relative">
              <div
                ...
                style={{
                  background: `radial-gradient(circle, ${phaseColor}55 0%, transparent 70%)`,
                }}
              />
              <div
        ...
        >
          ...
          {snap && snap.formingRooms.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{ background: c + "22", color: c, border: "1px solid " + c + "60" }}
                    >
                      #{idx + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-black">
                        {urgent ? "Needs ONE more" : `Needs ${need}`}
                      </div>
                      <div className="text-[10px] text-slate-500">
                        Room {r.id.slice(0, 6).toUpperCase()}
                      </div>
                    </div>
            ...
            <div className="text-xs text-slate-500 px-1 py-2">
              No rooms forming yet — you'll seed the next one.
            </div>
          )}
        </div>
        ...
        {phase === "found" && (
          <div
            ...
            style={{ background: "rgba(5,7,13,0.85)", backdropFilter: "blur(20px)" }}
          >
            <div
            ...
            >
              <div className="absolute inset-0 rounded-[32px] overflow-hidden pointer-events-none">
                ...
                />
                <div
                  className="absolute -bottom-20 -right-20 w-60 h-60 rounded-full blur-[80px]"
                  style={{ background: "rgba(0,183,255,0.4)" }}
                />
              </div>
              <div className="relative">
                <TreyBrandMark size={56} glow className="mx-auto mb-4" />
                <div className="text-[10px] tracking-[0.3em] font-bold text-emerald-300">
                  MATCH FOUND
                </div>
                ...
                <div className="text-sm text-slate-300 mt-2">
                  Routing you to your seat at the {meta.name} table…
                </div>
...
const StatTile: React.FC<{
  ...
  <div
    className="rounded-2xl px-3 py-3 border backdrop-blur-md trey-glass-panel"
...
const SeatPips: React.FC<{ current: number; max: number; color: string }> = ({
  current,
  max,
  color,
}) => (
