...
export const RoomLobby: React.FC<Props> = ({ roomId, identity, onBack, onSessionStarted }) => {
  ...
  const filledSeats = Array.from({ length: seats }).map(
    (_, i) => players.find((p) => p.seat_index === i) || null,
  );
  ...
  return (
    <div className="min-h-screen w-full text-white pb-10" style={{ background: "#05070D" }}>
      <div
        ...
        style={{ background: "rgba(5,7,13,0.85)", borderColor: gameMeta.color + "30" }}
      >
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={handleLeave} className="p-2 rounded-xl hover:bg-white/5">
            <ArrowLeft size={20} />
          </button>
          <div className="flex-1">
            <div className="text-[10px] tracking-[0.2em]" style={{ color: gameMeta.color }}>
              ROOM LOBBY
            </div>
            <div className="text-lg font-bold">{gameMeta.name}</div>
          </div>
          <span
            className="text-[10px] px-2 py-1 rounded-md font-bold tracking-widest"
            style={{
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(135deg, rgba(8,17,31,0.95), rgba(5,7,13,0.95))",
            borderColor: gameMeta.color + "60",
            boxShadow: `0 0 40px ${gameMeta.color}20`,
          }}
        >
          <div className="text-xs text-slate-400 tracking-widest mb-2">ROOM CODE</div>
          <div className="flex items-center justify-center gap-3">
            <div
              className="text-4xl md:text-5xl font-black tracking-[0.4em] font-mono"
              style={{ color: gameMeta.color, textShadow: `0 0 20px ${gameMeta.color}60` }}
            >
              {room.room_code}
            </div>
            <button
              onClick={copyCode}
              className="p-2.5 rounded-xl border hover:bg-white/5"
              style={{ borderColor: gameMeta.color + "40" }}
            >
              {copied ? (
                <Check size={18} className="text-green-400" />
              ) : (
                <Copy size={18} style={{ color: gameMeta.color }} />
              )}
            </button>
          </div>
          <p className="text-xs text-slate-500 mt-3">
            Share this code with friends to invite them to this table.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {filledSeats.map((p, i) => (
            <SeatCard
              ...
              accent={gameMeta.color}
              teamLabel={room.game_type === "spades" ? (i % 2 === 0 ? "WE" : "THEM") : null}
            />
          ))}
        </div>
        ...
        {isHost ? (
          <div className="space-y-3">
            ...
            <button
              ...
              style={{
                ...
                color: "#fff",
                boxShadow: "0 0 24px rgba(0,183,255,0.4)",
              }}
            >
          ...
          <div
            ...
            style={{ background: "rgba(8,17,31,0.6)", borderColor: "rgba(255,255,255,0.08)" }}
          >
...
}> = ({ seat, player, isYou, accent, teamLabel }) => (
  <div
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="text-[10px] text-slate-400 flex items-center gap-2">
        {player ? (
          <>
            {player.is_bot && (
              <span className="px-1.5 py-0.5 rounded bg-purple-500/15 text-purple-300">BOT</span>
            )}
            {isYou && (
              <span className="px-1.5 py-0.5 rounded bg-cyan-500/15 text-cyan-300">YOU</span>
            )}
            {teamLabel && (
              <span
