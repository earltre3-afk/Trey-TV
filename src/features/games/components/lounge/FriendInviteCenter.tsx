...
export const FriendInviteCenter: React.FC<Props> = ({
  identity,
  onBack,
  defaultGame = "spades",
  roomId,
  ...
  const handleAddFriend = async () => {
    if (!backendEnabled) {
      setFeedback(
        "Friend search will be available after the Trey TV game database migration is enabled.",
      );
  ...
  const handleInvite = async (f: Friend) => {
    if (!backendEnabled) {
      setFeedback(
        "Invites will be available after the Trey TV game database migration is enabled.",
      );
      setTimeout(() => setFeedback(null), 2500);
      return;
    }
  ...
  const handleRemove = async (f: Friend) => {
    if (!backendEnabled) {
      setFeedback(
        "Friend management will be available after the Trey TV game database migration is enabled.",
      );
      setTimeout(() => setFeedback(null), 2500);
      return;
    }
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0 opacity-60">
        <div
          className="absolute -top-32 -right-32 w-[500px] h-[500px] rounded-full blur-[140px]"
          style={{
      ...
      <div className="relative max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <div
          ...
          style={{
            ...
            boxShadow: `0 0 40px ${gm.color}18`,
          }}
        >
          <div className="text-[10px] tracking-[0.3em] font-bold mb-3" style={{ color: gm.color }}>
            INVITE TO
          </div>
          <div className="grid grid-cols-3 gap-2">
            {(["spades", "blackjack", "bullshit", "truno"] as GameType[]).map((g) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5 text-[9px] tracking-[0.2em] font-bold">
                    {meta.icon} GAME
                  </div>
          ...
          <div className="mt-4">
            <div className="text-[10px] tracking-[0.3em] font-bold text-slate-500 mb-1.5">
              MESSAGE
            </div>
            <input
              value={message}
        ...
        >
          ...
          {feedback && (
            <div className="mt-2 text-xs text-cyan-300 bg-cyan-500/10 border border-cyan-500/30 rounded-xl px-3 py-2">
              {feedback}
            </div>
          )}
        </div>
          ...
          <div className="flex items-end justify-between mb-3">
            <div>
              ...
              <div className="text-base font-black tracking-tight flex items-center gap-2">
                <Users size={14} className="text-purple-300" /> Friends ({friends.length})
              </div>
            </div>
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="text-xs text-slate-500 py-6 text-center">
              {!backendEnabled
                ...
                : friends.length === 0
                  ? "No friends yet. Search a display name above to add someone you've played with."
                  : "No matches."}
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, rgba(0,183,255,0.25), rgba(168,85,247,0.25))",
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl hover:bg-rose-500/10 transition"
                      title="Remove friend"
                    >
