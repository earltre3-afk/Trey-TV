...
async function getBSDecisionWithFallback(
  state: BSState,
  seat: number,
  isClaim: boolean,
): Promise<{ cardIds?: string[]; rank?: string; callBullshit?: boolean }> {
  let timeoutId: any;
  const timeoutPromise = new Promise<{ cardIds?: string[]; rank?: string; callBullshit?: boolean }>(
    (resolve) => {
...
export const BullshitTable: React.FC<Props> = (props) => {
  if (props.roomId && props.identity)
    return <ServerBS {...props} roomId={props.roomId!} identity={props.identity!} />;
  return <LocalBS {...props} />;
};

const LocalBS: React.FC<Props> = ({ onBack, onLegend }) => {
  const [state, setState] = useState<BSState>(() =>
    newBullshitGame(["You", "Aaliyah", "Marcus", "Jamal"]),
  );
  ...
  useEffect(() => {
            ...
            setState((s) => {
              if (s.phase === "playing" && s.currentSeat === seat) {
                return makeClaim(
                  s,
                  seat,
                  decision.cardIds ?? botClaim(s, seat).cardIds,
                  (decision.rank ?? s.expectedRank) as Rank,
                );
              }
              return s;
            });
    ...
    if (state.phase === "awaiting-challenge") {
      const callerSeats = state.players
        .filter((p) => p.isBot && p.seat !== state.lastClaim?.seat)
        .map((p) => p.seat);
      const hasHumanCaller = state.players.some(
        (p) => !p.isBot && p.seat !== state.lastClaim?.seat,
      );
      const t = setTimeout(
        async () => {
...
const ServerBS: React.FC<Props & { roomId: string; identity: PlayerIdentity }> = ({
  roomId,
  identity,
  onBack,
  onLegend,
  ...
  useEffect(() => {
          ...
          if (isBot) {
            ...
            room.setHostState(
              makeClaim(
                activeState,
                seat,
                decision.cardIds ?? botClaim(activeState, seat).cardIds,
                (decision.rank ?? activeState.expectedRank) as Rank,
              ),
            );
          } else {
    ...
    if (activeState.phase === "awaiting-challenge") {
      const botCallers = room.players
        .filter((p) => p.is_bot && p.seat_index !== activeState.lastClaim?.seat)
        .map((p) => p.seat_index);
      const hasHumanCaller = room.players.some(
        (p) => !p.is_bot && p.seat_index !== activeState.lastClaim?.seat,
      );
      const t = setTimeout(
        async () => {
          const decisions = await Promise.all(
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () => room.setHostState({ ...state, reveal: null }),
      REVEAL_CLEAR_DELAY_MS,
    );
    return () => clearTimeout(t);
  ...
  return (
    <BSView
      ...
      onClaim={() => {
        if (selected.length) {
          room.sendMove({
            type: "claim",
            seat: mySeat,
            payload: { cardIds: selected, rank: state.expectedRank },
          });
          setSelected([]);
        }
      }}
      ...
      chatButton={
        <ChatHeaderButton unread={chat.unread} accent="#A855F7" onClick={() => setChatOpen(true)} />
      }
      chatDrawer={
        <GameChatDrawer
          open={chatOpen}
...
interface ViewProps {
  ...
  mySeat: number;
  selected: string[];
  setSelected: (s: string[] | ((p: string[]) => string[])) => void;
  onClaim: () => void;
  onCall: () => void;
  onPass: () => void;
  onRestart: () => void;
...
const BSView: React.FC<ViewProps> = ({
  state,
  mySeat,
  selected,
  setSelected,
  ...
  const canCall =
    state.phase === "awaiting-challenge" && state.lastClaim && state.lastClaim.seat !== mySeat;
  ...
  const toggleSelectedCard = useCallback(
    (cardId: string) => {
      ...
      setSelected((s) =>
        s.includes(cardId) ? s.filter((x) => x !== cardId) : s.length < 4 ? [...s, cardId] : s,
      );
    },
  ...
  useTvRemoteInput((action) => {
    ...
    if (remoteZone === "hand") {
      if (action === "LEFT" || action === "RIGHT") {
        ...
        setRemoteCardIndex(
          (index) => (index + delta + Math.max(you.hand.length, 1)) % Math.max(you.hand.length, 1),
        );
        return;
      }
      if (action === "SELECT") {
    ...
    if (action === "LEFT" || action === "RIGHT") {
      setRemoteActionIndex((index) =>
        actions.length
          ? (index + (action === "LEFT" ? -1 : 1) + actions.length) % actions.length
          : index,
      );
      return;
    }
  ...
  return (
    ...
    >
      ...
      <header
        ...
        style={{ background: "rgba(8,17,31,0.85)", borderColor: "rgba(168,85,247,0.3)" }}
      >
        <div className="px-3 py-2 flex items-center gap-2">
          <button
            onClick={onBack}
            className="w-10 h-10 inline-flex items-center justify-center rounded-lg hover:bg-white/5 border border-white/5"
            aria-label="Back"
            title="Back"
          >
            <ArrowLeft size={18} />
          ...
          <div className="flex-1 min-w-0 flex items-center gap-2">
            ...
            {roomCode && (
              <div className="text-[9px] font-mono text-cyan-300 tracking-wider truncate">
                · {roomCode}
              </div>
            )}
          </div>
      ...
      <main className="flex-1 min-h-0 px-3 py-2 flex items-stretch justify-center">
        <div
          ...
          style={{
            background:
              "radial-gradient(120% 90% at 50% 0%, oklch(0.30 0.14 295) 0%, oklch(0.20 0.10 290) 30%, oklch(0.13 0.07 285) 60%, oklch(0.08 0.03 280) 100%)",
            boxShadow:
              "0 0 70px oklch(0.72 0.24 300 / 0.22), 0 0 100px oklch(0.84 0.16 215 / 0.12)",
          }}
        >
          {/* Top gloss */}
          <div
            ...
            style={{
              background: "linear-gradient(180deg, oklch(1 0 0 / 0.06) 0%, transparent 55%)",
            }}
          />
          {/* Color wash */}
          <div
            ...
            style={{
              background:
                "radial-gradient(80% 60% at 15% 110%, oklch(0.84 0.14 82 / 0.16) 0%, transparent 60%), radial-gradient(70% 50% at 90% 5%, oklch(0.84 0.16 215 / 0.18) 0%, transparent 60%)",
            }}
          />
          {/* Grain */}
          <div
          ...
          <img
            ...
            style={{ opacity: 0.1, filter: "drop-shadow(0 0 10px oklch(0.72 0.26 300 / 0.3))" }}
          />
          {/* Double gold ring */}
          <div
            ...
            style={{
              border: "1px solid oklch(0.84 0.14 82 / 0.35)",
          ...
          <span
            ...
            style={{ top: "16%", left: "22%", animationDelay: "0.5s" }}
          />
          <span
            className="sparkle z-[2]"
            style={{ top: "28%", right: "18%", animationDelay: "2.0s" }}
          />
          <span
            className="sparkle z-[2]"
            style={{ bottom: "32%", right: "24%", animationDelay: "1.1s" }}
          />
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: isYourTurn
                    ? "0 0 26px rgba(255,200,87,0.20), inset 0 1px 0 rgba(255,255,255,0.08)"
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div
                    ...
                    style={{ color: isYourTurn ? "#FFC857" : "#C4A6FF" }}
                  >
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center text-center px-2 py-2 pointer-events-none z-10">
            {state.reveal ? (
              ...
              >
                <div className="text-[9px] text-purple-300 tracking-[0.3em] font-black">
                  CURRENT CLAIM
                </div>
                <div className="text-2xl font-black mt-0.5">
                  {state.lastClaim
                    ? `${state.lastClaim.count} x ${state.lastClaim.rank}`
                    : `Claim ${state.expectedRank}`}
                </div>
                {state.lastClaim && (
                  <div className="text-[10px] text-slate-400 mt-0.5">
                    by {state.players[state.lastClaim.seat].name}
                  </div>
                )}
              </div>
            )}
            <div className="text-[10px] text-slate-300 font-bold mt-2 bg-black/40 px-2 py-0.5 rounded">
              {isYourTurn
                ? `Your turn — claim ${state.expectedRank}s`
                : canCall
                  ? `Pass or call BS`
            ...
            {(() => {
              ...
              return you.hand.map((cardId, i) => {
                ...
                const remoteFocused =
                  tvRemoteMode &&
                  remoteZone === "hand" &&
                  remoteCardIndex % Math.max(you.hand.length, 1) === i;
                return (
                  <button
                    type="button"
                    key={cardId}
              ...
              <>
                <button
                  ...
                  style={{
                    background:
                      selected.length > 0
                        ? "linear-gradient(90deg,#A855F7,#00B7FF)"
            ...
            {canCall && (
              <>
                <button
                  ...
                  style={{
                    background: "linear-gradient(90deg,#EF4444,#FFB000)",
                    color: "#fff",
                    boxShadow: "0 0 22px rgba(239,68,68,0.5)",
                  }}
                >
                  Call BS
                </button>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div
            ...
            style={{
              ...
              borderColor: "#A855F770",
              boxShadow: "0 0 60px rgba(168,85,247,0.35)",
            }}
          >
            <div className="text-[10px] text-purple-300 tracking-[0.35em] font-black">
              GAME OVER
            </div>
            <div className="text-2xl sm:text-3xl font-black mt-2 mb-4">
...
function syncBSPlayersFromRoom(
  state: BSState,
