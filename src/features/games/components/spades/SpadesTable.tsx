...
function spadesExtract(s: SpadesState) {
  return {
    ...
    phase: s.phase,
    round: s.round,
    ended: s.phase === "game-over",
  };
}

async function getSpadesDecisionWithFallback(
  state: SpadesState,
  seat: number,
  isBid: boolean,
): Promise<{ bid?: number; cardId?: string }> {
  let timeoutId: any;
...
export const SpadesTable: React.FC<Props> = (props) => {
  if (props.roomId && props.identity)
    return <ServerSpades {...props} roomId={props.roomId!} identity={props.identity!} />;
...
const LocalSpades: React.FC<Props> = ({ onBack, onLegend, targetScore = 500 }) => {
  const [state, setState] = useState<SpadesState>(() =>
    newSpadesGame(["You", "Aaliyah", "Marcus", "Jamal"], targetScore),
  );
  ...
  useEffect(() => {
    ...
    if (state.phase === "playing") {
      ...
      const isBot = state.players[seat].isBot;
      const delay = isBot
        ? state.trick.length === 3
          ? BOT_TRICK_CLOSE_DELAY_MS
          : BOT_CARD_DELAY_MS
  ...
  return (
    <SpadesView
      ...
      onPlayAgain={() =>
        setState(newSpadesGame(["You", "Aaliyah", "Marcus", "Jamal"], targetScore))
      }
      onBack={onBack}
      onLegend={onLegend}
      myAvatarUrl={null}
...
// SERVER MODE
// ============================================
const ServerSpades: React.FC<Props & { roomId: string; identity: PlayerIdentity }> = ({
  roomId,
  identity,
  onBack,
  onLegend,
  ...
  const roomPlayersKey = room.players
    .map((p) => `${p.seat_index}:${p.display_name}:${p.is_bot}`)
    .join("|");
    ...
    if (activeState.phase === "playing") {
      ...
      const t = setTimeout(async () => {
        if (isBot) {
          ...
          room.setHostState(
            playCard(activeState, seat, decision.cardId ?? botPlay(activeState, seat)),
          );
        } else {
  ...
  if (room.loading || !state || mySeat === null) {
    return (
      <div
        className="h-[100dvh] flex items-center justify-center text-slate-400"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, oklch(1 0 0 / 0.08) 0%, transparent 30%, transparent 70%, oklch(0 0 0 / 0.18) 100%)",
        }}
      />
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow:
          "inset 0 0 0 1px oklch(0.84 0.14 82 / 0.55), inset 0 0 8px oklch(0.72 0.24 300 / 0.4), 0 3px 6px oklch(0 0 0 / 0.6)",
        ...style,
      }}
    >
      <div
        ...
        style={{
          border: "0.5px solid oklch(0.84 0.14 82 / 0.35)",
          boxShadow: "inset 0 0 6px oklch(0.72 0.24 300 / 0.35)",
        }}
      />
      <img
...
const SpadesView: React.FC<ViewProps> = ({
  ...
  const yourLegal = useMemo(
    () => (state.phase === "playing" && isMyTurn ? legalCards(state, mySeat) : []),
  ...
  const handleCardTap = useCallback(
    (cardId: string) => {
      ...
      const isDoubleTap =
        selected === cardId ||
        (lastTap?.cardId === cardId && now - lastTap.at <= DOUBLE_TAP_PLAY_MS);
      if (isDoubleTap) {
        onPlayCard(cardId);
        lastCardTap.current = null;
        return;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: "radial-gradient(closest-side, oklch(0.84 0.16 215 / 0.30), transparent 70%)",
          filter: "blur(20px)",
        }}
      />
      <div
      ...
      >
        <div className="px-3 py-2 flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0 flex items-center gap-2">
            ...
            {roomCode && (
              <div className="text-[9px] font-mono text-amber-300 tracking-wider ml-1 truncate">
                · {roomCode}
              </div>
            )}
          </div>
          ...
          <button
            onClick={onLegend}
            className="w-10 h-10 inline-flex items-center justify-center rounded-lg hover:bg-white/5 transition border border-white/5"
            aria-label="Suit legend"
            title="Suit legend"
          >
            <Info size={16} />
          </button>
        ...
        <div className="px-3 pb-2 grid grid-cols-2 gap-2">
          <ScoreCard
            label="WE"
            score={teamWeScore}
            bid={state.teamRoundBids[myTeam]}
            tricks={state.teamRoundTricks[myTeam]}
            bags={state.teamBags[myTeam]}
            color="#00B7FF"
          />
          <ScoreCard
            label="THEM"
            score={teamThemScore}
            bid={state.teamRoundBids[1 - myTeam]}
            tricks={state.teamRoundTricks[1 - myTeam]}
            bags={state.teamBags[1 - myTeam]}
            color="#A855F7"
          />
        </div>
      </header>
      ...
      <main className="flex-1 min-h-0 relative flex items-center justify-center px-2 py-1.5">
        <div
          ...
          style={{
            background:
              "radial-gradient(120% 90% at 50% 0%, oklch(0.32 0.13 280) 0%, oklch(0.20 0.10 270) 30%, oklch(0.14 0.08 255) 60%, oklch(0.10 0.06 240) 100%)",
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(180deg, oklch(1 0 0 / 0.06) 0%, oklch(1 0 0 / 0.01) 50%, transparent 100%)",
            }}
          />
          {/* Color wash */}
          <div
          ...
          <img
            src={treyTvLogo}
            alt=""
            className="pointer-events-none select-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[140px] h-auto"
            style={{
              opacity: 0.28,
              filter:
                "drop-shadow(0 2px 8px oklch(0 0 0 / 0.55)) drop-shadow(0 0 18px oklch(0.84 0.14 82 / 0.35))",
            }}
          />
          {/* Double gold ring */}
          <div
            className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[160px] h-[160px] rounded-full breathe"
            style={{
              border: "1px solid oklch(0.84 0.14 82 / 0.40)",
              boxShadow: "0 0 20px oklch(0.84 0.14 82 / 0.25)",
            }}
          />
          {/* Sparkles */}
          ...
          <span
            className="sparkle"
            style={{ bottom: "28%", left: "30%", animationDelay: "2.6s" }}
          />
          <span
            className="sparkle"
          ...
          <div className="absolute inset-x-2 bottom-2 pointer-events-none" style={{ zIndex: 14 }}>
            ...
            >
              <div
                ...
                style={{
                  background:
                    "linear-gradient(90deg, transparent, rgba(0,183,255,0.65), rgba(255,200,87,0.45), transparent)",
                }}
              />
              <GamePlayerSeat
              ...
              <div className="min-w-0 flex-1">
                <div
                  className="text-[9px] tracking-[0.30em] font-black"
                  style={{ color: isMyTurn ? "#FFC857" : "#00B7FF" }}
                >
                ...
                <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] font-bold tabular-nums">
                  <span
                    className="rounded-full px-2 py-0.5 border text-cyan-200"
                    style={{
                      background: "rgba(0,183,255,0.10)",
                      borderColor: "rgba(0,183,255,0.32)",
                    }}
                  >
                    Bid {you.bid ?? "-"}
                  </span>
                  <span
                    className="rounded-full px-2 py-0.5 border text-slate-200"
                    style={{
                  ...
                  <span
                    className="rounded-full px-2 py-0.5 border text-amber-200"
                    style={{
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      pointerEvents:
                        state.phase === "playing" && isMyTurn && isLegal ? "auto" : "none",
                      filter: remoteFocused
                  ...
                  >
                    ...
                    {remoteFocused && (
                      <span className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border border-amber-300/70 bg-amber-400/20 px-2 py-0.5 text-[9px] font-black tracking-wider text-amber-100">
                        TV FOCUS
                      </span>
                    )}
                  </button>
      ...
      {state.phase === "bidding" && isMyTurn && (
        ...
        >
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, transparent, rgba(0,183,255,0.7), rgba(168,85,247,0.7), transparent)",
            }}
          />
          <div className="flex items-center justify-between mb-2">
            <div className="min-w-0">
              <div className="text-[9px] tracking-[0.3em] font-bold" style={{ color: "#FFC857" }}>
                YOUR TURN
              </div>
            ...
            {Array.from({ length: 13 }).map((_, n) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg, rgba(0,183,255,0.22), rgba(0,183,255,0.05))",
                    border: "1px solid rgba(0,183,255,0.5)",
                    color: "#F8FAFC",
                    boxShadow:
                      "inset 0 1px 0 rgba(255,255,255,0.12), 0 2px 8px rgba(0,183,255,0.18)",
                  }}
                >
      ...
      {state.phase === "bidding" && !isMyTurn && (
        ...
        >
          ...
          />
          <div className="text-[9px] tracking-[0.3em] font-bold text-cyan-300 text-center">
            BIDDING PHASE
          </div>
          ...
          {(() => {
            ...
            return (
              <GlassChip
                ...
                style={{
                  boxShadow:
                    "var(--shadow-glass), 0 -6px 24px oklch(0 0 0 / 0.5), inset 0 0 0 1px oklch(1 0 0 / 0.08), 0 0 30px oklch(0.72 0.26 300 / 0.18)",
                }}
              >
                {/* Status row */}
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    {isMyTurn && (
                      <span
                        ...
                        style={{
                          color: "var(--night)",
                          boxShadow:
                            "0 0 16px var(--gold-glow), inset 0 1px 0 oklch(1 0 0 / 0.5), inset 0 -1px 0 oklch(0 0 0 / 0.3)",
                          border: "1px solid oklch(0.62 0.14 70 / 0.7)",
                        }}
                      >
                        YOUR TURN
                      </span>
                    ...
                    <span className="text-[11px]" style={{ color: "oklch(0.84 0.04 260)" }}>
                      {isMyTurn
                        ...
                        : `${state.players[state.currentSeat].name} to play`}
                    </span>
                  </div>
                  <span
                    className="text-[12px] breathe"
                    style={{ color: "var(--gold)", textShadow: "0 0 8px var(--gold-glow)" }}
                  >
                    ✦
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: canPlay
                        ? `0 0 28px ${selNeon}, 0 8px 22px oklch(0 0 0 / 0.65), inset 0 1px 0 oklch(1 0 0 / 0.30), inset 0 -1px 0 oklch(0 0 0 / 0.35)`
                        : "none",
                    }}
                  >
                    ...
                    {selected && (
                      <span
                        ...
                        style={{
                          color: "white",
                          textShadow: canPlay
                            ? `0 0 10px ${selNeon}, 0 0 18px ${selNeon}`
                            : undefined,
                        }}
                      >
                        {selected.slice(0, -1)}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          ...
          >
            <div
              ...
              style={{
                background: "radial-gradient(circle, rgba(255,200,87,0.35) 0%, transparent 70%)",
              }}
            />
            <div className="relative">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div
                  ...
                  style={{ background: "rgba(0,183,255,0.1)", borderColor: "rgba(0,183,255,0.45)" }}
                >
                  <div className="text-[10px] text-cyan-300 tracking-[0.25em] font-bold">
                    TEAM WE
                  </div>
                ...
                >
                  <div className="text-[10px] text-purple-300 tracking-[0.25em] font-bold">
                    TEAM THEM
                  </div>
...
function syncSpadesPlayersFromRoom(
  state: SpadesState,
  players: Array<{ seat_index: number; display_name: string; is_bot: boolean }>,
) {
  let changed = false;
      ...
      <div className="relative flex items-center justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-1">
            ...
            />
            <div className="text-[9px] tracking-[0.32em] font-black leading-none" style={{ color }}>
              {label}
            </div>
          </div>
