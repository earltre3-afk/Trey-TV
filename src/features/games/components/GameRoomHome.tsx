...
export const GameRoomHome: React.FC<Props> = ({
  ...
  return (
    <div className="min-h-screen w-full text-white pb-20 relative overflow-hidden" style={{ background: '#05070D' }}>
      {/* Ambient background glow */}
      <div className="pointer-events-none absolute inset-0 opacity-60">
        <div className="absolute -top-40 -left-40 w-[600px] h-[600px] rounded-full blur-[140px]" style={{ background: 'radial-gradient(circle, rgba(0,183,255,0.25) 0%, transparent 70%)' }} />
        <div className="absolute top-1/3 -right-40 w-[600px] h-[600px] rounded-full blur-[140px]" style={{ background: 'radial-gradient(circle, rgba(168,85,247,0.22) 0%, transparent 70%)' }} />
        <div className="absolute bottom-0 left-1/3 w-[500px] h-[500px] rounded-full blur-[140px]" style={{ background: 'radial-gradient(circle, rgba(255,200,87,0.10) 0%, transparent 70%)' }} />
      </div>

      {/* Header */}

      <div className="sticky top-0 z-30 backdrop-blur-2xl border-b" style={{ background: 'rgba(5,7,13,0.75)', borderColor: 'rgba(0,183,255,0.20)' }}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center gap-3">
          ...
          <div className="w-9 h-9 rounded-2xl flex items-center justify-center relative shrink-0"
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[10px] tracking-[0.3em] font-bold" style={{ color: '#00B7FF' }}>GAME ROOM</div>
            <div className="text-sm md:text-base font-black leading-tight tracking-tight">Private Lounge</div>
          </div>

          {displayName && (
            <button onClick={onEditName} className="text-xs px-3 py-1.5 rounded-xl border hover:bg-white/5 hidden sm:flex items-center gap-1.5 transition"
          ...
          {onOpenFriends && (
            <button onClick={onOpenFriends} className="p-2 rounded-xl hover:bg-white/5 transition" title="Friends">
              <UserPlus size={18} />
            </button>
          )}
          {onOpenInbox && (
            <button onClick={onOpenInbox} className="p-2 rounded-xl hover:bg-white/5 transition relative" title="Game Requests">
              <Inbox size={18} />
              {inboxCount > 0 && (
                <span className="absolute -top-1 -right-1 text-[9px] font-black px-1.5 min-w-[18px] h-[18px] flex items-center justify-center rounded-full"
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-8">
        ...
        <div className="relative rounded-[32px] overflow-hidden border trey-table-rim"
          ...
          <div className="relative p-6 md:p-12 min-h-[440px] md:min-h-[520px] flex flex-col justify-end">
            <div className="inline-flex self-start items-center gap-2 text-[10px] tracking-[0.3em] font-bold mb-4 px-3 py-1.5 rounded-full backdrop-blur-md"
            ...
            <h1 className="text-4xl md:text-7xl font-black leading-[0.95] tracking-tight">
              Trey TV
              <br />
              <span style={{ background: 'linear-gradient(90deg,#00B7FF 0%,#A855F7 60%,#FFC857 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                Game Room
              </span>
            </h1>
            <p className="text-sm md:text-lg text-slate-300 mt-4 max-w-xl leading-relaxed">
              Classic card games with a Trey TV twist. Real engines, real scoring, live multiplayer rooms with the custom Trey TV deck.
            </p>
            <div className="flex flex-wrap gap-3 mt-7">
              <button onClick={() => onCreateRoom('spades')}
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-6 mt-8 max-w-2xl">
              ...
              <Stat icon={<Wifi size={14} />} label="Realtime Sync" value="Multiplayer" color="#A855F7" />
              <Stat icon={<Trophy size={14} />} label="Flagship" value="Spades 500" color="#FFC857" />
            </div>
          </div>
        </div>
        ...
        {/* Featured */}
        <SectionTitle icon={<Crown size={18} className="text-amber-300" />} label="Featured" sub="Flagship Game" />
        <div onClick={() => onCreateRoom('spades')} className="relative rounded-[28px] overflow-hidden border cursor-pointer group transition hover:-translate-y-0.5"
          style={{ borderColor: 'rgba(0,183,255,0.5)', boxShadow: '0 0 60px rgba(0,183,255,0.22)' }}>
          <div className="absolute inset-0">
            <img src={SPADES_IMG} alt="" className="w-full h-full object-cover opacity-70 group-hover:opacity-85 group-hover:scale-105 transition-all duration-700" />
            <div className="absolute inset-0" style={{ background: 'linear-gradient(90deg, rgba(5,7,13,0.96) 0%, rgba(5,7,13,0.55) 55%, rgba(5,7,13,0.2) 100%)' }} />
          </div>
          <div className="relative p-6 md:p-10 min-h-[220px] md:min-h-[280px] flex flex-col justify-end">
            <div className="text-[10px] tracking-[0.3em] font-bold" style={{ color: '#00B7FF' }}>FLAGSHIP · 4 PLAYERS · TEAMS</div>
            ...
            <div className="text-sm md:text-base text-slate-300 mt-2 max-w-md leading-relaxed">
              Live 4-player tables, full bidding, real bag scoring. Trump suit: <span className="font-bold" style={{ color: '#00B7FF' }}>Blades</span>.
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-black tracking-wide"
              ...
              <button onClick={(e) => { e.stopPropagation(); onLaunchSolo('spades'); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <GameCard title="Spades" tag="TRICK-TAKING · 4P" desc="Team strategy. Bids, books, and pressure." color="#00B7FF" img={SPADES_IMG} icon={<Spade size={16} />}
            ...
            onCreate={() => onCreateRoom('spades')} onSolo={() => onLaunchSolo('spades')} />
          <GameCard title="Blackjack" tag="SOLO TABLE · 1P" desc="Beat the dealer in the neon lounge." color="#FFC857" img={BLACKJACK_IMG} icon={<Layers size={16} />}
            needs={1} waiting={queueCounts.blackjack}
            onJoinQueue={onJoinQueue ? () => onJoinQueue('blackjack') : undefined}
            onInvite={onOpenFriends}
            onCreate={() => onCreateRoom('blackjack')} onSolo={() => onLaunchSolo('blackjack')} />
          <GameCard title="Bullshit" tag="BLUFFING · 3-4P" desc="Call the bluff before the table flips." color="#A855F7" img={BULLSHIT_IMG} icon={<Sparkles size={16} />}
            needs={4} waiting={queueCounts.bullshit}
        ...
        {/* How it works */}
        <SectionTitle icon={<Zap size={18} className="text-purple-300" />} label="How It Works" sub="Three steps to the table" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {[
            ...
            <div key={s.n} className="rounded-3xl p-5 border backdrop-blur-md transition hover:-translate-y-0.5"
              ...
              <div className="flex items-center gap-3 mb-2">
                <div className="text-3xl font-black tracking-tight" style={{ color: s.c, textShadow: `0 0 16px ${s.c}80` }}>{s.n}</div>
                <div className="h-px flex-1" style={{ background: `linear-gradient(90deg, ${s.c}60, transparent)` }} />
              </div>
        ...
        />
        <div className="rounded-3xl border p-5 md:p-6 backdrop-blur-md"
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs md:text-sm text-slate-300 mt-1.5 leading-relaxed">
                Spades use the <span className="font-bold text-cyan-300">Blades</span> suit, Hearts use <span className="font-bold text-rose-300">Soul</span>, Diamonds use <span className="font-bold text-amber-300">Crowns</span>, and Clubs use <span className="font-bold text-emerald-300">Sparks</span>. Gameplay rules stay 100% accurate — just the artwork is cinematic.
              </p>
              ...
              <button onClick={onLegend} className="mt-5 text-xs px-4 py-2 rounded-full font-bold inline-flex items-center gap-1.5"
        ...
        <p className="text-[11px] text-slate-500 mt-10 text-center max-w-md mx-auto leading-relaxed">
          Free-play entertainment only. No real-money wagering, deposits, withdrawals, or cash prizes.
        </p>
      </div>
    </div>
  );
...
const SectionTitle: React.FC<{ icon: React.ReactNode; label: string; sub: string }> = ({ icon, label, sub }) => (
  <div className="mt-10 md:mt-14 mb-4 flex items-end justify-between gap-3">
    <div>
      <div className="text-[10px] tracking-[0.3em] font-bold text-slate-500">{sub.toUpperCase()}</div>
      <h2 className="text-2xl md:text-3xl font-black flex items-center gap-2 tracking-tight">{icon} {label}</h2>
    </div>
    <div className="h-px flex-1 max-w-xs" style={{ background: 'linear-gradient(90deg, transparent, rgba(0,183,255,0.4), transparent)' }} />
  </div>
);

const Pill: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span className="text-[10px] tracking-wider font-bold px-2.5 py-1 rounded-full backdrop-blur-md"
    style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#CBD5E1' }}>
    {children}
  </span>
);

const SuitChip: React.FC<{ name: string; plays: string; color: string }> = ({ name, plays, color }) => (
  <div className="rounded-xl p-2.5 border" style={{ background: 'rgba(5,7,13,0.6)', borderColor: color + '40' }}>
...
const GameCard: React.FC<{
  title: string; tag: string; desc: string; color: string; img: string; icon: React.ReactNode;
  needs: number; waiting: number;
  ...
  <div className="rounded-[26px] overflow-hidden border transition-all hover:-translate-y-1 group backdrop-blur-md trey-game-tile-surface"
    ...
    <div className="relative h-40 overflow-hidden">
      ...
      <div className="absolute top-3 left-3 text-[10px] tracking-[0.25em] font-black px-2.5 py-1 rounded-md backdrop-blur-md"
        style={{ background: color + '25', color, border: '1px solid ' + color + '60' }}>{tag}</div>
      <div className="absolute bottom-3 right-3 w-10 h-10 rounded-2xl flex items-center justify-center backdrop-blur-md"
        style={{ background: color + '20', border: '1px solid ' + color + '60', color, boxShadow: `0 0 18px ${color}40` }}>
        {icon}
      </div>
      {/* Live queue chip */}
      <div className="absolute top-3 right-3 inline-flex items-center gap-1.5 text-[10px] font-black px-2 py-1 rounded-full backdrop-blur-md"
