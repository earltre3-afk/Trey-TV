...
async function buildBJScene(
  ...
  const scene: BJScene = {
    table, dealerContainer, playerContainer, chipContainer,
    betZone, winOverlay,
    tweens: [], sparks: [],
    cardBack, cardFaces,
    layout,
...
function renderBJCards(scene: BJScene, props: PixiBJTableProps) {
  ...
  props.playerCards.forEach((id, i) => {
    ...
    if (isPlayerPhase) {
      ...
      glow.roundRect(-cardW / 2 - 3, -cardH / 2 - 3, cardW + 6, cardH + 6, cardW * 0.12)
    ...
    if (!scene.table.reducedMotion) {
      const tw = tween(
        ...
        { x: card.x, y: card.y, alpha: 1, scaleX: 1, scaleY: 1 },
        0.40,
        { ease: ease.outBack, delay: i * 0.10 + 0.05 },
      );
      scene.tweens.push(tw);
    }
  ...
  if (props.result && props.phase === 'settled') {
    ...
    scene.winOverlay
      .roundRect(0, 0, layout.w, layout.h, 0)
      .fill({ color: overlayColor, alpha: 0 });
