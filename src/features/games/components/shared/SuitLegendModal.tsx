...
export const SuitLegendModal: React.FC<{ open: boolean; onClose: () => void }> = ({
  open,
  onClose,
}) => {
  if (!open) return null;
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: "0 0 60px rgba(0,183,255,0.25)",
        }}
      ...
      >
        ...
        <p className="text-sm text-slate-400 mb-5">
          Trey TV custom suits map directly to classic gameplay.
        </p>
          ...
          {suits.map((s) => {
            ...
            return (
              <div
                ...
                style={{ background: "rgba(16,24,39,0.6)", borderColor: m.color + "40" }}
              >
                <span
                  className="text-4xl"
                  style={{ color: m.color, textShadow: `0 0 12px ${m.glow}` }}
                >
                  {m.symbol}
                </span>
        ...
        <p className="text-xs text-slate-500 mt-5">
          Blades is trump in Spades. All rules follow standard card-game logic.
        </p>
      </div>
    </div>
  );
