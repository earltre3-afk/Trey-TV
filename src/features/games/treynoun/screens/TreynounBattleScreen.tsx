...
const TeamPanel: React.FC<{
  ...
  >
    {active && (
      <span
        className="absolute -top-2 left-1/2 -translate-x-1/2 px-2.5 py-0.5 rounded-full text-[9px] font-black tracking-widest whitespace-nowrap"
    ...
    <div className={`flex items-center gap-2 ${side === "r" ? "flex-row-reverse text-right" : ""}`}>
      <div className="relative shrink-0">
        <div
          className="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl flex items-center justify-center font-black text-base sm:text-lg"
          style={{
      ...
      <div className={side === "r" ? "text-right" : ""}>
        <div className="text-[10px] sm:text-xs font-black tracking-wider" style={{ color: accent }}>
          {name}
        </div>
    ...
    <div className={`mt-2.5 flex items-center gap-1.5 ${side === "r" ? "flex-row-reverse" : ""}`}>
      <ShieldAlert
        ...
        style={{ color: chaos > 66 ? "#FF3D6E" : "rgba(255,255,255,.4)" }}
      />
      <div className="flex-1 h-1.5 rounded-full bg-black/60 overflow-hidden border border-white/10">
        <div
          ...
          style={{
            width: `${chaos}%`,
            background: "linear-gradient(90deg,#FFB300,#FF3D6E)",
            boxShadow: chaos > 0 ? "0 0 8px #FF3D6E88" : "none",
          }}
        />
      </div>
    </div>
...
const TreynounBattleScreen: React.FC<Props> = ({ onFinish, onExit }) => {
  ...
  const award = (team: "glow" | "vibe", pts: number) => {
    if (team === "glow") setGlow((s) => s + pts);
    else setVibe((s) => s + pts);
  };

  const handleGuess = (trap = false) => {
    if (lockout > 0) {
      toast.error("Team Glow locked out!");
      return;
    }
    const g = guess.trim();
    if (!g) return;
  ...
  const handleCatLock = (c: NounType) => {
    ...
    if (c === data.cat) toast.success("Category locked! +25 ready");
      ...
      setGlowChaos((v) => Math.min(100, v + 15));
        ...
        <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 pb-10">
          ...
          <div className="mt-4 flex items-stretch gap-2 sm:gap-4">
            ...
            <div className="flex flex-col items-center justify-center px-1 sm:px-3 shrink-0">
              <div className="text-[9px] sm:text-[10px] font-black tracking-widest text-white/40 mb-1">
                ROUND {roundNo}/3
              </div>
              <NeonRing
                value={time}
              ...
              <div className="hidden sm:flex items-center gap-1 mt-2">
                {[1, 2, 3].map((n) => (
                  <span
                    key={n}
                    className={`w-2 h-2 rounded-full ${n < roundNo ? "bg-cyan-400" : n === roundNo ? "bg-fuchsia-400 shadow-[0_0_8px_#FF00E5]" : "bg-white/15"}`}
                  />
                ))}
              </div>
            </div>
          ...
          {stealWindow > 0 ? (
            <GlassCard
              glow={TEAM_VIBE.accent}
              className="mt-4 p-3 sm:p-4 flex items-center justify-between gap-3 animate-[trey-pop_.3s_ease]"
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div
                    ...
                    style={{ color: TEAM_VIBE.accent }}
                  >
                    STEAL WINDOW
                  </div>
                  <div className="text-[11px] text-white/60">
                    Team Vibe can lock it in - {stealWindow}s
                  </div>
                </div>
              </div>
              <GlossyButton
                tone="primary"
                onClick={vibeSteal}
                className="px-4 sm:px-5 py-2.5 text-sm whitespace-nowrap"
              >
                STEAL +75
              </GlossyButton>
            </GlassCard>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[1.3fr_1fr] lg:items-start">
            ...
            <div>
              <div className="text-[10px] font-black tracking-widest text-white/40 mb-2 flex items-center gap-1.5">
                NOUN TRAIL
                <span
                  className="px-1.5 py-0.5 rounded-full text-[9px] capitalize"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {data.signals.map((s) => {
                  ...
                  return (
                    <GlassCard
                      key={s.id}
                      glow={open ? accent : undefined}
                      className={`p-3 sm:min-h-[140px] flex flex-row sm:flex-col gap-3 items-center sm:items-start transition-all ${open ? "" : "opacity-70"}`}
                    >
                      <div
                        className="w-7 h-7 rounded-xl border flex items-center justify-center text-xs font-black shrink-0"
                        style={{
                          borderColor: open ? accent : "rgba(255,255,255,.15)",
                          color: open ? accent : "rgba(255,255,255,.3)",
                          background: open ? `${accent}1a` : "transparent",
                        }}
                      >
                        {s.id}
                      </div>
                      {open ? (
                        ...
                        <div className="flex-1 flex items-center sm:justify-center gap-2 text-white/30">
                          ...
                          <span className="text-[11px] font-semibold">
                            {s.id === 2 ? "unlocks 15s" : "unlocks 30s"}
                          </span>
                        </div>
                      )}
                      {open && (
                        <Icon
                          className="hidden sm:block w-4 h-4 mt-auto"
            ...
            <div className="space-y-3">
              {/* Category lock */}
              <div>
                <div className="text-[10px] font-black tracking-widest text-white/40 mb-2">
                  CATEGORY LOCK
                </div>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          color: catColor[c],
                          background: locked ? `${catColor[c]}22` : "rgba(0,0,0,.3)",
                          boxShadow: locked ? `0 0 16px ${catColor[c]}55` : "none",
                        }}
                      >
              ...
              <div>
                ...
                <GlassCard
                  glow={glowTurn ? TEAM_GLOW.accent : undefined}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <Lock className="w-5 h-5 mr-1" /> LOCK IN
                </GlossyButton>
                <GlossyButton
                  tone="danger"
                  onClick={() => handleGuess(true)}
                  disabled={trapUsed || lockout > 0}
                  className="py-4 flex flex-col items-center justify-center leading-none"
                >
                  <span className="flex items-center gap-1">
                    <Zap className="w-4 h-4" /> NOUN TRAP
                  </span>
                  <span className="text-[7px] text-white/80 uppercase tracking-widest mt-0.5">
                    Miss = +35 Chaos & 5s lockout
                  </span>
                </GlossyButton>
              </div>
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <button
                  onClick={onExit}
                  className="text-xs text-white/40 hover:text-red-400 underline transition"
                >
                  Exit battle
                </button>
              </div>
