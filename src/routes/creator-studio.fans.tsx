...
type FanRow = {
  ...
  avatar: string;
  pts: number;
  segment: (typeof SEGMENTS)[number]["id"];
  joined: string;
  watched: number;
};
    ...
    <CreatorStudioLayout title="Your fans" subtitle="The people powering your channel.">
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <CreatorMetricCard
          label="Top Supporter"
          value="@nightowl"
          sub="2,400 pts gifted"
          icon={Gem}
          tone="gold"
        />
        <CreatorMetricCard
          label="Most Engaged"
          value="Late Night fans"
          icon={Heart}
          tone="magenta"
        />
      </section>
        ...
        {SEGMENTS.map((s) => {
          ...
          return (
            <button
              key={s.id}
              onClick={() => setSeg(s.id)}
              className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border whitespace-nowrap transition flex items-center gap-1.5 ${
                active
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground truncate">
                    @{f.handle} · {f.pts} pts gifted · joined {f.joined}
                  </div>
                </div>
              </button>
              {f.segment === "top" && (
                <span className="hidden md:inline text-[10px] tracking-[0.18em] px-2 py-1 rounded-full bg-primary/10 text-primary border border-primary/30 uppercase">
                  Top
                </span>
              )}
              <button
                onClick={() => toast.success(`Thanked @${f.handle}`)}
                className="px-3 py-1.5 rounded-lg text-xs border border-primary text-primary hover:bg-primary/10 inline-flex items-center gap-1"
              >
      ...
      <Sheet open={!!open} onOpenChange={(o) => !o && setOpen(null)}>
        <SheetContent
          side="right"
          className="bg-background/95 backdrop-blur-xl border-l border-white/10 w-full sm:max-w-md"
        >
          {open && (
            <div className="space-y-4 pt-2">
              <div className="flex items-center gap-3">
                <div className="size-14 rounded-2xl conic-ring">
                  <img src={open.avatar} className="size-14 rounded-2xl object-cover" alt="" />
                </div>
                ...
                <button
                  ...
                  className="ml-auto size-8 grid place-items-center rounded-lg hover:bg-white/5"
                >
                  <X className="size-4" />
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => toast.success("Gifted Glow Heart back")}
                  className="px-3 py-2 rounded-xl text-sm font-semibold glass border border-primary/40 text-primary"
                >
                  Gift back
                </button>
              </div>
      ...
      <p className="text-xs text-muted-foreground text-center">
        More fan insights coming as your channel grows.
      </p>
    </CreatorStudioLayout>
  );
}
