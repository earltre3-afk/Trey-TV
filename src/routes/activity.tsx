      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="rounded-2xl liquid-glass border border-white/10 p-3">
              <div
                className="size-8 rounded-lg grid place-items-center mb-2"
                style={{
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          ...
          {FILTERS.map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`shrink-0 px-3 h-8 rounded-full text-xs font-semibold transition border ${filter === f ? "bg-primary text-primary-foreground border-transparent glow-gold" : "liquid-glass border-white/10 text-muted-foreground hover:text-foreground"}`}
            >
              {f}
            </button>
          ))}
          ...
          <section className="rounded-3xl liquid-glass border border-white/10 p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-bold flex items-center gap-2">
                <Bookmark className="size-4 text-primary" /> Saved bookmarks
              </h2>
              <Link to="/collections" className="text-xs text-primary hover:underline">
                Open collections
              </Link>
            </div>
              ...
              {posts
                ...
                .map((p) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-1.5 left-1.5 right-1.5 text-[10px] truncate">
                      {p.creator.name}
                    </div>
                  </div>
                ))}
        ...
        <section className="rounded-3xl liquid-glass border border-white/10 p-5">
          <h2 className="font-bold flex items-center gap-2 mb-3">
            <Sparkles className="size-4 text-primary" /> Recent activity
          </h2>
          {filtered.length === 0 ? (
            <div className="py-10 text-center text-sm text-muted-foreground">
              ...
              <div className="mt-4">
                <Link
                  to="/"
                  className="px-4 h-9 inline-flex items-center rounded-full bg-primary text-primary-foreground text-xs font-semibold glow-gold"
                >
                  Open feed
                </Link>
              </div>
...
function Row({ a }: { a: ActivityItem }) {
  ...
  return (
    <li className="flex items-center gap-3 p-3 rounded-2xl glass border border-white/10">
      {a.thumb ? (
        <img src={a.thumb} className="size-12 rounded-xl object-cover" alt="" />
      ) : (
        <div className="size-12 rounded-xl bg-white/5 grid place-items-center">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-semibold truncate flex items-center gap-2">
          ...
          {a.type === "react"
            ? `Reacted ${r?.label ?? ""} on ${a.title}`
            : a.type === "save"
              ? `Saved ${a.title}`
              : `Shared ${a.title}`}
        </div>
        <div className="text-[11px] text-muted-foreground truncate">
          @{a.creator} · {timeAgo(a.ts)}
        </div>
      </div>
    </li>
  );
