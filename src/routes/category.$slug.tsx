...
export const Route = createFileRoute("/category/$slug")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      { name: "description", content: `Most popular ${params.slug} videos and creators trending right now on Trey TV.` },
      { property: "og:title", content: `${cap(params.slug)} — Popular on Trey TV` },
      { property: "og:description", content: `Discover the top ${params.slug} content on Trey TV.` },
    ],
  }),
});
...
const CATEGORY_META: Record<string, { label: string; tagline: string; tint: string }> = {
  music: { label: "Music", tagline: "Beats, drops and live sets dominating the charts.", tint: "oklch(0.7 0.25 340)" },
  shows: { label: "Shows", tagline: "Binge-worthy series and breakout episodes.", tint: "oklch(0.82 0.16 85)" },
  podcasts: { label: "Podcasts", tagline: "Voices, conversations and hot mic moments.", tint: "oklch(0.82 0.15 215)" },
  gaming: { label: "Gaming", tagline: "Top streams, plays and tournament highlights.", tint: "oklch(0.65 0.22 300)" },
  lifestyle: { label: "Lifestyle", tagline: "Fashion, food and the day-in-the-life drops.", tint: "oklch(0.7 0.25 340)" },
  trending: { label: "Trending", tagline: "Everything blowing up right now.", tint: "oklch(0.82 0.16 85)" },
};
...
function CategoryPage() {
  ...
  const pool = [
    ...prescribed.map((p) => ({ id: p.id, title: p.title, media: p.media, creator: p.creator, kind: p.kind, duration: p.duration ?? p.viewers ?? "—" })),
    ...posts.map((p) => ({ id: p.id, title: p.text.split("\n")[0], media: p.media, creator: p.creator.name, kind: "VIDEO", duration: p.duration })),
  ];
  const popular = pool
    ...
    <AppShell wide>
      <div className="space-y-6">
        ...
        <section
          ...
          style={{ background: `radial-gradient(circle at 20% 0%, color-mix(in oklab, ${meta.tint} 28%, transparent), transparent 60%), radial-gradient(circle at 90% 100%, color-mix(in oklab, ${meta.tint} 18%, transparent), transparent 60%)` }}
        >
          <div className="text-[11px] tracking-[0.3em] font-semibold flex items-center gap-1.5" style={{ color: meta.tint }}>
            <Flame className="size-3.5" /> POPULAR · {meta.label.toUpperCase()}
          </div>
          <h1 className="mt-3 text-3xl lg:text-5xl font-bold leading-tight">Top {meta.label} on Trey TV</h1>
        ...
        <section>
          <h2 className="text-sm lg:text-base font-semibold mb-3 flex items-center gap-2">
            <TrendingUp className="size-4" style={{ color: meta.tint }} /> Top {meta.label.toLowerCase()} creators
          </h2>
          <div className="flex gap-3 overflow-x-auto no-scrollbar -mx-3 px-3 pb-1">
            {topCreators.map((c) => (
              ...
              >
                ...
                <div className="text-[10px] text-muted-foreground truncate w-full text-center">@{c.handle}</div>
              </Link>
            ))}
          </div>
        </section>
            ...
            {popular.map((p, i) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-3">
                  <div className="text-[9px] tracking-widest" style={{ color: meta.tint }}>{p.kind}</div>
