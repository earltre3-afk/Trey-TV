...
import { approveFwdOAuthAuthorization, validateFwdOAuthAuthorizeRequest } from "@/lib/fwd/oauth.server";
...
function FwdOAuthAuthorizePage() {
  ...
  if (loading || !app || !session) {
    return <BridgeShell><div className="text-center text-sm text-muted-foreground">Preparing FWD authorization...</div></BridgeShell>;
  }
    ...
    <BridgeShell>
      <div className="rounded-[28px] liquid-glass neon-border p-6 sm:p-8">
        <div className="text-center">
          ...
          <h1 className="mt-3 text-3xl sm:text-4xl font-black">Allow FWD to use your Trey TV account?</h1>
          <p className="mt-3 text-sm text-muted-foreground">FWD will not receive your Trey TV password or raw session.</p>
        </div>
