...
function queueItemToSubmission(item: AdminQueueItem): Submission {
  return {
    ...
    quality: "",
    visibility:
      item.visibility === "private" || item.visibility === "scheduled" ? item.visibility : "public",
...
function AdminContentApproval() {
  ...
  const filtered = submissions.filter((s) => {
    ...
    if (
      q &&
      !`${s.title} ${s.creator_handle} ${s.show_title}`.toLowerCase().includes(q.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  return (
    <AdminShell
      ...
      subtitle="Review creator episodes before they go live on Trey TV."
    >
      {/* Stats */}
      ...
      <div className="flex flex-col md:flex-row gap-2">
        <div className="flex-1 flex items-center gap-2 px-3 py-2 rounded-2xl glass border border-white/10">
          ...
          <input
            ...
            placeholder="Search by title, creator, show…"
        ...
        <div className="flex flex-wrap gap-1.5">
          {FILTERS.map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-full text-[11px] font-semibold border ${filter === f ? "bg-primary/15 border-primary text-primary glow-gold" : "border-white/10 glass"}`}
            >
        ...
        {filtered.map((s) => (
          <article key={s.content_id} className="rounded-3xl glass neon-border overflow-hidden">
            <div className="grid grid-cols-[140px,1fr]">
              ...
              <div className="p-3 min-w-0">
                <div className="flex items-center gap-1.5 mb-1">
                  <span
                    className={`text-[9px] px-2 py-0.5 rounded-full border ${STATUS_TONE[s.status]}`}
                  >
                ...
                <div className="text-[11px] text-muted-foreground truncate">
                  @{s.creator_handle} · {s.show_title} · S{s.season_number} E{s.episode_number}
                </div>
                <div className="mt-1 flex flex-wrap gap-1">
                  {s.category.slice(0, 2).map((c) => (
                    <span key={c} className="text-[9px] px-1.5 py-0.5 rounded bg-white/5">
                      {c}
                    </span>
                  ))}
            ...
            <div className="px-3 pb-3 flex flex-wrap gap-1.5">
              <button
                onClick={() =>
                  navigate({ to: "/admin/content-approval/$id", params: { id: s.content_id } })
                }
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-xs font-semibold glass border border-[oklch(0.78_0.18_150_/_0.4)] text-[oklch(0.82_0.18_150)]"
              >
              ...
              <button
                onClick={() => {
                  setFeedback({ id: s.content_id, mode: "changes" });
                  setFeedbackText("");
                }}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold glass border border-[oklch(0.7_0.25_340_/_0.4)] text-[oklch(0.78_0.25_340)]"
              >
              ...
              <button
                onClick={() => {
                  setFeedback({ id: s.content_id, mode: "reject" });
                  setFeedbackText("");
                }}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold glass border border-[oklch(0.65_0.24_15_/_0.4)] text-[oklch(0.78_0.24_15)]"
              >
      ...
      {feedback && (
        ...
        >
          <div
            className="max-w-md w-full rounded-3xl glass-strong border border-white/10 p-5 space-y-3"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-bold">
              {feedback.mode === "changes" ? "Request changes" : "Reject submission"}
            </h3>
            <textarea
              value={feedbackText}
              ...
              rows={4}
              placeholder={
                feedback.mode === "changes" ? "What needs to change?" : "Reason for rejection"
              }
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                onClick={() => {
                  ...
                  review(
                    feedback.id,
                    feedback.mode === "changes" ? "needs_changes" : "rejected",
                    feedbackText,
                  );
                }}
