...
function WatchPage() {
      ...
      <AppShell>
        <div className="rounded-3xl glass neon-border p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            This episode is awaiting admin approval.
          </p>
        </div>
      </AppShell>
    );
  ...
  const related = store.submissions.filter(
    (x) => x.content_id !== s.content_id && (x.status === "approved" || x.status === "published"),
  );
        ...
        <div>
          ...
          <div className="mt-4 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full border ${STATUS_TONE[s.status]}`}
              >
            ...
            <Link
              to="/channel/$handle"
              params={{ handle: s.creator_handle }}
              className="inline-flex items-center gap-2 hover:opacity-90"
            >
              <img src={s.creator_avatar} className="size-9 rounded-full object-cover" alt="" />
              <div>
                <div className="text-sm font-semibold flex items-center gap-1">
                  {s.creator_name} <Crown className="size-3 text-primary" />
                </div>
                <div className="text-[11px] text-muted-foreground">
                  @{s.creator_handle} · View channel
                </div>
              </div>
            </Link>
            <div className="flex gap-2 pt-1">
              {[
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl glass border border-white/10 text-xs flex items-center gap-1.5"
                >
            ...
            {s.full_description && (
              <p className="text-sm text-muted-foreground">{s.full_description}</p>
            )}
            {s.viewer_context && (
        ...
        <div>
          ...
          <div className="mt-4 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] text-muted-foreground">
                {show?.title} · S{ep.season} E{ep.number} · {ep.duration}m
              </span>
            </div>
            <h1 className="text-2xl font-bold text-gradient-gold">{ep.title}</h1>
            <Link
              to="/channel/$handle"
              params={{ handle: channel?.handle ?? "trey" }}
              className="inline-flex items-center gap-2 hover:opacity-90"
            >
              <img src={channel?.avatar} className="size-9 rounded-full object-cover" alt="" />
              <div>
                <div className="text-sm font-semibold flex items-center gap-1">
                  {channel?.name ?? "Trey TV"} <Crown className="size-3 text-primary" />
                </div>
                <div className="text-[11px] text-muted-foreground">
                  @{channel?.handle ?? "trey"} · View channel
                </div>
              </div>
            </Link>
            <div className="flex flex-wrap gap-2 pt-1">
              <button
                ...
                className="px-3 py-2 rounded-xl glass border border-white/10 text-xs flex items-center gap-1.5"
              >
                <Heart className="size-3.5" /> Mark watched
              </button>
              <button
                onClick={() => {
              ...
              <button
                ...
                className="px-3 py-2 rounded-xl glass border border-white/10 text-xs flex items-center gap-1.5"
              >
                <Lock className="size-3.5" /> {later ? "Watch Later" : "Watch Later"}
              </button>
              <button
              ...
              {(show?.episodes ?? [])
                ...
                .map((item) => (
                  <Link
                    ...
                    to="/watch/$id"
                    params={{ id: item.id }}
                    className="flex gap-2 hover:bg-white/5 rounded-xl p-1 transition"
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-muted-foreground truncate">
                        S{item.season} E{item.number} · {item.duration}m
                      </div>
                    </div>
                  </Link>
                ))}
        ...
        {items.slice(0, 5).map((s) => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px] text-muted-foreground truncate">
                @{s.creator_handle} · S{s.season_number} E{s.episode_number}
              </div>
            </div>
          </Link>
        ))}
