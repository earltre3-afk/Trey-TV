...
import {
  ...
  type Clip,
  type TrackKind,
} from "@/lib/creator-studio/editRecipe";
      ...
      <div className="relative max-w-md w-full rounded-3xl glass neon-border p-8 text-center overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            The Creator Edit Studio is unlocked for approved Trey TV creators and admins. Apply to
            join the network and get access to the full cinematic editor.
          </p>
...
function Studio() {
  ...
  const addLayer = (kind: TrackKind) => {
    if (!recipe) {
      toast("Upload media to start a timeline");
      return;
    }
    if (kind === "text") editor.addText({ text: "Trey TV", start: time, length: 3 });
      ...
      editor.addAudio({ src: posts[0].media, start: 0, length: Math.min(10, duration || 10) });
  ...
  const AI_TEXT_TASKS: Partial<
    Record<AiActionId, Parameters<typeof treyIGenerate>[0]["data"]["task"]>
  > = {
    captions: "caption",
    title: "title",
    desc: "description",
  ...
  const handleAiAction = async (id: AiActionId, label: string) => {
    ...
    try {
      const result = await treyIGenerate({
        data: { task, prompt: `Generate ${label.toLowerCase()} for a Trey TV creator episode` },
      });
  ...
  const handleRender = async () => {
    if (!recipe) {
      toast("Upload media to render");
      return;
    }
    const token = session?.access_token;
    if (!token) {
      toast.error("Sign in to render");
      return;
    }
    setShowRender(true);
  ...
  return (
    <main className="min-h-dvh bg-background text-foreground">
      ...
      <header className="sticky top-0 z-30 px-3 md:px-5 pt-3 pb-2 backdrop-blur-xl bg-background/60 border-b border-white/5">
        <div className="flex items-center gap-2 flex-wrap md:flex-nowrap">
          ...
          <div className="order-3 md:order-none w-full md:w-auto md:flex-1 min-w-0 flex items-center md:justify-center gap-2">
            ...
            <div className="relative min-w-0 flex-1 md:flex-none">
              ...
              >
                ...
                <ChevronDown
                  className={`size-3.5 shrink-0 transition-transform ${showProjects ? "rotate-180" : ""}`}
                />
              </button>
              {showProjects && (
          ...
          <div className="ml-auto flex items-center gap-2 shrink-0">
            ...
            <button
              onClick={() =>
                navigate({
                  ...
                  search: { id: draftId ?? undefined } as any,
                })
              }
            ...
            >
              <Download className="size-4" /> <span className="hidden sm:inline">Next:</span>{" "}
              Details
            </button>
          </div>
        </div>
      ...
      <div className="px-3 md:px-5 py-4 space-y-4 pb-[260px]">
              ...
              >
                ...
                <div>
                  <div className="text-sm font-semibold">
                    Upload your episode, clip, or promo to start editing
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">
                    MP4 · MOV · HEVC · up to 4K
                  </div>
                </div>
              ...
              <div className="absolute inset-0 grid place-items-center text-center px-6">
                <div className="w-full max-w-xs">
                  ...
                  <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-primary via-[oklch(0.7_0.25_340)] to-[oklch(0.65_0.22_300)] transition-all"
            ...
            {uploadState === "processing" && (
              <div className="absolute inset-0 grid place-items-center">
                <div className="px-4 py-3 rounded-xl glass-strong border border-white/10 flex items-center gap-2 text-sm shimmer-sweep">
                  <Sparkles className="size-4 text-primary animate-spin [animation-duration:2s]" />{" "}
                  Processing media…
                </div>
              </div>
            )}
            {uploadState === "error" && (
              <div className="absolute inset-0 grid place-items-center px-6 text-center">
                <div className="rounded-2xl border border-destructive/40 bg-destructive/10 p-4 max-w-xs">
                  ...
                  <button
                    onClick={onPickFile}
                    className="mt-3 px-3 py-1.5 rounded-lg text-xs font-semibold bg-primary text-primary-foreground"
                  >
                    Retry
                  </button>
                </div>
            ...
            {uploadState === "ready" && (
              <>
                ...
                {preview?.texts.map((tx) => (
                  <div
                    ...
                    style={{
                      ...
                      color: tx.color,
                      opacity: tx.opacity,
                      fontSize: `clamp(14px, ${tx.size / 12}px, ${tx.size}px)`,
                      background: tx.background ?? "transparent",
                      padding: tx.background ? "2px 10px" : 0,
                      borderRadius: tx.background ? 8 : 0,
                    }}
                  >
                    {tx.text}
                  </div>
                ...
                <div className="absolute top-3 left-3 flex items-center gap-1.5">
                  <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-[oklch(0.65_0.22_300_/_0.3)] border border-[oklch(0.65_0.22_300_/_0.6)] text-white">
                    4K
                  </span>
                  <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-white/10 border border-white/20">
                    HDR
                  </span>
                </div>
        ...
        <section className="flex items-center justify-between gap-2 rounded-2xl glass border border-white/10 p-2">
          ...
          <div className="flex items-center gap-1.5">
            <CtrlBtn
              icon={SkipBack}
              onClick={() => seekTo((videoRef.current?.currentTime ?? time) - 1)}
            />
            <button
              onClick={togglePlay}
                ...
                return (
                  ...
                  >
                    {track?.clips.map((clip) => {
                      const w = duration
                        ? Math.max(40, (clipLength(clip) / duration) * 600 * zoom)
                        : 96;
                      const isSel = selectedClip === clip.id;
                      const toneClass =
                        kind === "overlay"
                          ? "bg-gradient-to-r from-[oklch(0.65_0.22_300_/_0.4)] to-[oklch(0.7_0.25_340_/_0.4)] text-purple-100"
                          : kind === "text"
                            ? "bg-primary/10 text-primary"
                            : kind === "audio"
                              ? "bg-[oklch(0.82_0.15_215_/_0.14)] text-[oklch(0.82_0.15_215)]"
                              : kind === "caption"
                                ? "bg-[oklch(0.7_0.25_340_/_0.2)] text-[oklch(0.7_0.25_340)]"
                                : "";
                      return (
                        <button
                          key={clip.id}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-20 px-3 pb-3 pt-2 bg-gradient-to-t from-background via-background/95 to-transparent">
        <div className="rounded-3xl glass-strong border border-white/10 p-2 shadow-[0_20px_60px_-20px_oklch(0_0_0_/_0.7)] flex items-center gap-2">
          ...
          >
            ...
            <span className="absolute -bottom-1 text-[8px] font-bold tracking-[0.15em] text-primary-foreground/0">
              AI
            </span>
          </button>
        </div>
      ...
      <Sheet open={showAI} onOpenChange={setShowAI}>
        <SheetContent
          side="bottom"
          className="glass-strong border-t border-white/10 max-h-[85vh] overflow-y-auto"
        >
          <SheetHeader className="text-left">
            ...
            <SheetDescription>
              Premium AI tools for your upload. Tap any action to run it.
            </SheetDescription>
          </SheetHeader>
      ...
      <Dialog open={showRender} onOpenChange={setShowRender}>
        <DialogContent className="glass-strong border-white/10 max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Your edit recipe is rendered in the cloud, then ready to publish.
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
      ...
      <Dialog open={showExport} onOpenChange={setShowExport}>
        <DialogContent className="glass-strong border-white/10 max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Configure your upload before publishing to Trey TV.
            </DialogDescription>
          </DialogHeader>
...
function Track({
  ...
  const toneColor =
    tone === "purple"
      ? "text-[oklch(0.65_0.22_300)]"
      : tone === "gold"
        ? "text-primary"
        : tone === "cyan"
          ? "text-[oklch(0.82_0.15_215)]"
          : tone === "magenta"
            ? "text-[oklch(0.7_0.25_340)]"
            : "text-foreground";
  return (
    <div className="flex items-center gap-2 mb-1.5">
      ...
      <div className="relative flex-1 h-12 rounded-lg bg-white/5 ring-1 ring-white/10 overflow-hidden flex items-center gap-1 px-1">
        ...
        {onAdd && (
          <button
            onClick={onAdd}
            className="size-8 shrink-0 grid place-items-center rounded-md bg-white/10 hover:bg-white/20"
          >
    ...
    <div className="space-y-4 mt-2">
      <Field label="Title">
        <input
          ...
          placeholder="Late Night with Trey — S2 E15"
      ...
      <Field label="Description">
        <textarea
          ...
          rows={3}
          placeholder="Tell your fans what's inside…"
          ...
          {(["draft", "private", "public"] as const).map((v) => (
            <button
              ...
              className={`px-3 py-2 rounded-xl text-xs font-semibold capitalize border transition ${
                visibility === v
                  ? "bg-primary/15 border-primary text-primary glow-gold"
      ...
      <div className="flex items-center gap-2 pt-2">
        <button
          onClick={onClose}
          className="flex-1 px-4 py-3 rounded-xl text-sm font-semibold glass border border-white/10 hover:bg-white/5"
        >
          Save Draft
        </button>
        <button
          onClick={submit}
          className="flex-1 px-4 py-3 rounded-xl text-sm font-bold bg-primary text-primary-foreground glow-gold tilt-press hover-lift flex items-center justify-center gap-2"
        >
