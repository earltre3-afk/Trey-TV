...
function SchedulePage() {
  ...
  const approved = useMemo(
    () =>
      store.submissions.filter(
        (s) => s.status === "approved" || s.status === "draft" || s.status === "needs_changes",
      ),
    [store.submissions],
  );
  ...
  const doSchedule = () => {
    ...
    store.approve(pickEp, {
      scheduleAt: new Date(
        `2026-06-${String(selected ?? 1).padStart(2, "0")}T${pickTime}`,
      ).toISOString(),
    });
  ...
  return (
    <CreatorStudioLayout
      ...
      actions={
        <button
          ...
          className="px-3.5 py-2 rounded-xl text-sm font-bold bg-primary text-primary-foreground glow-gold tilt-press inline-flex items-center gap-1.5"
        >
    ...
    >
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <ul className="divide-y divide-white/5">
            {UPCOMING.map((u, i) => (
              <li key={i} className="flex items-center gap-3 py-3">
                ...
                <button
                  onClick={() => toast.success("Notified your fans")}
                  className="px-2.5 py-1.5 rounded-lg text-[11px] glass border border-white/10 inline-flex items-center gap-1"
                >
                  <Bell className="size-3" /> Notify fans
                </button>
                <span className="text-[10px] tracking-[0.18em] px-2 py-1 rounded-full bg-primary/10 text-primary border border-primary/30 uppercase">
                  {u.kind}
                </span>
              </li>
            ))}
        ...
        <div className="rounded-3xl glass neon-border p-4 md:p-5">
          ...
          <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4 text-center">
            ...
            <Link
              to="/creator-studio/channel"
              className="mt-3 inline-block px-3 py-1.5 rounded-lg text-xs border border-primary text-primary hover:bg-primary/10"
            >
              Change
            </Link>
          </div>
      ...
      <section className="rounded-3xl glass neon-border p-4 md:p-5">
        <SectionHeader
          icon={Calendar}
          title="Programming planner"
          action={<span className="text-[11px] text-muted-foreground">Tap a day to schedule</span>}
        />
        <div className="grid grid-cols-7 gap-1.5 text-center">
          {["S", "M", "T", "W", "T", "F", "S"].map((d) => (
            <div key={d} className="text-[10px] tracking-widest text-muted-foreground py-1">
              {d}
            </div>
          ))}
            ...
            return (
              <button
                ...
                className={`relative aspect-square rounded-lg text-xs grid place-items-center transition ${
                  active
                    ? "bg-primary text-primary-foreground glow-gold ring-1 ring-primary"
                    : has
                      ? "bg-primary/15 text-primary ring-1 ring-primary/40 font-semibold hover:bg-primary/25"
          ...
          <div className="mt-3 rounded-2xl bg-white/5 ring-1 ring-white/10 p-3">
            ...
            {dayHas[selected]?.length ? (
              ...
              <div className="text-xs text-muted-foreground">
                Nothing scheduled. Use "Schedule episode" to add one.
              </div>
            )}
          </div>
        )}
      ...
      <Dialog open={dialog} onOpenChange={setDialog}>
        <DialogContent className="bg-background border-white/10">
          <DialogHeader>
            ...
            <DialogDescription>
              Pick an approved episode and a release time. Fans get notified automatically.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <label className="block">
              <div className="text-[11px] tracking-[0.18em] text-muted-foreground uppercase mb-1">
                Episode
              </div>
              <select
                value={pickEp}
                onChange={(e) => setPickEp(e.target.value)}
                className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2 text-sm"
              >
                <option value="">Select…</option>
                {approved.map((s) => (
                  <option key={s.content_id} value={s.content_id}>
                    {s.title || "Untitled"} — S{s.season_number}E{s.episode_number}
                  </option>
                ))}
              </select>
            </label>
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                className="flex-1 px-3 py-2 rounded-xl text-sm font-semibold glass border border-white/10"
              >
                Cancel
              </button>
              <button
                onClick={doSchedule}
                className="flex-1 px-3 py-2 rounded-xl text-sm font-bold bg-primary text-primary-foreground glow-gold"
              >
                Schedule
              </button>
            </div>
