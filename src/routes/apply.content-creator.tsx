  ...
  return (
    <ApplicationWizardChrome
      ...
      sectionSubtitle={
        [
          "Tell us who you are.",
          "What channel are you building?",
          "What type of content will you create?",
          "Define your first show, schedule, and promotion plan.",
          "Agree to our guidelines.",
    ...
    >
      ...
      <WizardNav
        ...
        nextLabel={step === STEPS.length ? "Submit Creator Application" : "Next Step"}
      />
    </ApplicationWizardChrome>
  );
}
...
function StepIdentity({
  ...
  data: CreatorDraft;
  update: (p: Partial<CreatorDraft>) => void;
}) {
  return (
    <div className="space-y-5">
      ...
      <Field label="I am applying as" required>
        <TileChoice
          ...
          onChange={(v) => update({ applicantType: v as ApplicantType })}
          options={[
            {
              value: "individual",
            ...
            { value: "duo", label: "Duo / Group", icon: <Users className="h-5 w-5" /> },
            { value: "brand", label: "Brand / Business", icon: <Briefcase className="h-5 w-5" /> },
            { value: "org", label: "Organization", icon: <Building2 className="h-5 w-5" /> },
          ]}
        />
      </Field>
      <Field label="Short Bio" required>
        <NeonTextarea
          ...
          trailingIcon={<PenLine className="h-4 w-4" />}
        />
      </Field>
      <Field label="Social Links">
        <div className="grid gap-2 sm:grid-cols-2">
          <NeonInput
            ...
            onChange={(e) => update({ social: { ...data.social, tiktok: e.target.value } })}
            trailingIcon={<span className="text-xs">♪</span>}
          />
          <NeonInput
            placeholder="Instagram"
            value={data.social.instagram}
            onChange={(e) => update({ social: { ...data.social, instagram: e.target.value } })}
            trailingIcon={<Instagram className="h-4 w-4" />}
          />
          <NeonInput
            placeholder="YouTube"
            value={data.social.youtube}
            onChange={(e) => update({ social: { ...data.social, youtube: e.target.value } })}
            trailingIcon={<Youtube className="h-4 w-4" />}
          />
          <NeonInput
            placeholder="Website"
            value={data.social.website}
            onChange={(e) => update({ social: { ...data.social, website: e.target.value } })}
            trailingIcon={<Globe className="h-4 w-4" />}
          />
          <NeonInput
            placeholder="X/Twitter"
            value={data.social.x}
            onChange={(e) => update({ social: { ...data.social, x: e.target.value } })}
            trailingIcon={<Twitter className="h-4 w-4" />}
          />
        </div>
      </Field>
    </div>
...
function StepChannel({
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="Proposed Channel Name" required>
          <NeonInput
            value={data.channelName}
            onChange={(e) => update({ channelName: e.target.value })}
          />
        </Field>
        <Field label="Channel Category" required>
          <NeonSelect value={data.category} onChange={(e) => update({ category: e.target.value })}>
      ...
      <Field label="One-sentence channel pitch" required>
        <NeonInput value={data.pitch} onChange={(e) => update({ pitch: e.target.value })} />
      </Field>
      <Field label="Full Channel Description" required>
        <NeonTextarea
          rows={4}
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <Field label="Why does this belong on Trey TV?" required>
          <NeonTextarea
            rows={3}
            value={data.whyTreyTV}
            onChange={(e) => update({ whyTreyTV: e.target.value })}
          />
        </Field>
      </div>
...
function StepContent({
  ...
  return (
    <div className="space-y-5">
      <Field label="Content formats" required>
        <ChipPicker
          multi
          value={data.formats}
          onChange={(v) => update({ formats: v as string[] })}
          options={[
            "Episodes",
            "Shorts / Clips",
            "Interviews",
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="Average video length">
          ...
          >
            ...
            {["Under 5 min", "5-10 min", "10-20 min", "20-40 min", "40+ min"].map((x) => (
              <option key={x} className="bg-background">
                {x}
              </option>
            ))}
          </NeonSelect>
        </Field>
        <Field label="How often can you post?" required>
          <NeonSelect
            value={data.frequency}
            onChange={(e) => update({ frequency: e.target.value })}
          >
            <option value="">Select frequency</option>
            {["Daily", "Multiple per week", "Weekly", "Bi-weekly", "Monthly"].map((x) => (
              <option key={x} className="bg-background">
                {x}
              </option>
            ))}
      ...
      <Field label="Upload sample work (links accepted)">
        <div className="space-y-2">
          {data.sampleLinks.map((l, i) => (
            <NeonInput
              key={i}
      ...
      <Field label="Tools you use">
        <ChipPicker
          ...
          options={["Phone", "Camera", "CapCut", "Final Cut", "Premiere", "Canva", "Other"].map(
            (label) => ({ value: label, label }),
          )}
        />
      </Field>
      <Field
        label="Do you own or have permission for your content, music, footage, and images?"
        required
      >
        <ChipPicker
          value={data.rights}
          onChange={(v) => update({ rights: v as Rights })}
          options={[
            { value: "yes", label: "Yes, I do" },
            { value: "some", label: "I have some permissions" },
...
function StepLaunch({
  ...
  return (
    <div className="space-y-5">
      <Field label="First show name" required>
        <NeonInput value={data.firstShow} onChange={(e) => update({ firstShow: e.target.value })} />
      </Field>
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="Episode 1 title">
          <NeonInput value={data.episode1} onChange={(e) => update({ episode1: e.target.value })} />
        </Field>
        <Field label="Episode 2 title">
          <NeonInput value={data.episode2} onChange={(e) => update({ episode2: e.target.value })} />
        </Field>
      </div>
      <Field label="Publishing schedule" required>
        <NeonInput value={data.schedule} onChange={(e) => update({ schedule: e.target.value })} />
      </Field>
      <Field label="How will you promote your channel?" required>
        <NeonTextarea
          rows={4}
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-3">
        {sections.map((s) => (
          ...
          >
            ...
            <button
              onClick={() => jumpTo(s.n)}
              className="btn-ghost-glass inline-flex items-center gap-2 px-3 py-2 text-xs"
            >
      ...
      <div className="glass p-5">
        <p className="mb-3 text-sm font-semibold">Application Summary</p>
        <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
          <Row k="Channel Name" v={data.channelName || "-"} />
          <Row k="Category" v={data.category || "-"} />
          <Row k="Upload Frequency" v={data.frequency || "-"} />
          <Row k="First Show" v={data.firstShow || "-"} />
          <Row k="Sample Links" v={`${data.sampleLinks.filter(Boolean).length} links added`} />
          <Row
            k="Rights Confirmation"
            v={data.rights === "yes" ? "Yes" : data.rights === "some" ? "Partial" : "No"}
          />
        </dl>
      </div>
    </div>
        ...
        {PIPELINE_STAGES.map((s, i) => (
          <div key={s.key} className="flex flex-1 flex-col items-center">
            <div className="flex w-full items-center">
              ...
              <span className={`pipeline-icon mx-auto ${s.key === current ? "is-current" : ""}`}>
                {s.icon}
              </span>
              {i < PIPELINE_STAGES.length - 1 && <span className="pipeline-line" />}
            </div>
            <span
              className={`mt-2 text-center text-[10px] leading-tight sm:text-xs ${s.key === current ? "font-semibold text-[oklch(0.92_0.18_88)]" : "text-muted-foreground"}`}
            >
              {s.key}
            </span>
          </div>
        ...
        <div className="relative neon-blue p-6 md:p-10">
          ...
          <div className="relative flex flex-col items-center text-center">
            ...
            <div
              ...
              style={{
                boxShadow:
                  "inset 0 0 0 2px oklch(0.85 0.2 240 / 0.95), 0 0 60px oklch(0.65 0.3 245 / 0.6)",
              }}
            >
      ...
      <div className="hidden min-h-screen lg:flex">
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, oklch(0.14 0.06 252 / 0.9), oklch(0.08 0.03 262 / 0.9))",
              boxShadow:
            ...
            {[
              ...
              <div key={txt} className="flex items-start gap-3">
                <span
                  ...
                  style={{
                    boxShadow:
                      "inset 0 0 0 1px oklch(0.65 0.22 245 / 0.6), 0 0 10px oklch(0.6 0.3 245 / 0.2)",
                  }}
                >
                  {i + 1}
                </span>
...
function validate(step: number, data: CreatorDraft) {
  if (step === 1 && (!data.displayName.trim() || !data.handle.trim() || !data.bio.trim()))
    return "Display name, handle, and bio are required.";
  if (
    step === 2 &&
    ...
    return "Complete the required channel details.";
  if (step === 3 && (data.formats.length === 0 || !data.frequency || data.rights === "no"))
    return "Choose formats, posting frequency, and confirm rights.";
  if (step === 4 && (!data.firstShow.trim() || !data.schedule.trim() || !data.promotion.trim()))
    return "First show, schedule, and promotion plan are required.";
