              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <div className="text-[10px] tracking-[0.25em] text-primary">{a.status.toUpperCase()} - {category}</div>
                    <div className="text-[10px] text-muted-foreground">{new Date(a.created_at).toLocaleDateString()}</div>
                  </div>
                  <div className="text-sm font-bold mt-1 truncate">{v.display_name || `User ${a.user_id.slice(0, 8)}...`}</div>
                  {v.recognition_description && <div className="text-xs text-foreground/80 mt-1">{v.recognition_description}</div>}
                  {v.why_gold_badge && <div className="text-xs text-muted-foreground mt-1 italic">"{v.why_gold_badge}"</div>}
                  {links.length > 0 && (
                ...
                {a.status === "pending" && (
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => review(a.id, a.user_id, category, "approved")} className="size-9 rounded-xl bg-primary/15 text-primary border border-primary/40 grid place-items-center hover:bg-primary/25"><Check className="size-4" /></button>
                    <button onClick={() => review(a.id, a.user_id, category, "rejected")} className="size-9 rounded-xl bg-[oklch(0.65_0.24_15_/_0.15)] text-[oklch(0.65_0.24_15)] border border-[oklch(0.65_0.24_15_/_0.4)] grid place-items-center hover:bg-[oklch(0.65_0.24_15_/_0.25)]"><X className="size-4" /></button>
                  </div>
                )}
              </div>
            </div>
