  ...
  head: () => ({
    meta: [
      { title: "Creator Studio — Trey TV" },
      {
        name: "description",
        content:
          "Your premium creator command center: upload, edit, analyze, and grow your channel on Trey TV.",
      },
    ],
  }),
});
    ...
    >
      ...
      <section>
        <SectionHeader
          ...
          action={
            <Link
              to={"/creator-studio/analytics" as any}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mt-3">
          ...
          />
          <CreatorMetricCard
            label="Approved Episodes"
            value={String(approved)}
            sub="Live on the network"
            icon={CheckCircle2}
            tone="green"
          />
          <CreatorMetricCard
            label="Best Performer"
            value={top?.title?.slice(0, 18) ?? "—"}
            sub="Last 30 days"
            icon={Trophy}
            tone="cyan"
          />
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-3xl glass neon-border p-4 md:p-5">
          <SectionHeader
            icon={Sparkles}
            title="Today on your channel"
            action={
              <Link
                to={"/creator-studio/interactions" as any}
                className="text-sm text-primary inline-flex items-center gap-1"
              >
          ...
          <ul className="divide-y divide-white/5">
            <TodayRow
              icon={MessageSquare}
              title="14 new comments"
              desc="Across 3 episodes — 4 mention you directly."
              tone="magenta"
            />
            <TodayRow
              icon={Users}
              title="+128 new followers"
              desc="Up 22% vs yesterday's pace."
              tone="purple"
            />
            <TodayRow
              icon={Gem}
              title="3 new gifts received"
              desc="Top supporter: @nightowl — 500 pts."
              tone="gold"
            />
            {needsChanges > 0 && (
              <TodayRow
            ...
            <TodayRow
              ...
              desc='"Studio Sessions E8" hit Top 10 in Music.'
              tone="cyan"
            />
            <TodayRow
              icon={Wand2}
              title="Trey-I suggestion"
              desc="Your behind-the-scenes clips are saving 2.3× higher — make a series."
              tone="gold"
            />
          </ul>
        </div>
        ...
        <div className="rounded-3xl glass neon-border p-4 md:p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <CreatorActionButton
              icon={Film}
              label="New Show"
              desc="Set up a series"
              to={"/creator-studio/schedule"}
            />
            <CreatorActionButton
              icon={Tv}
              label="Edit Channel"
              desc="Banner, bio, links"
        ...
        <ul className="space-y-2">
          {submissions.slice(0, 4).map((s) => (
            ...
            >
              <div className="relative size-14 rounded-xl overflow-hidden shrink-0 bg-white/5">
                {s.thumbnail_url && (
                  <img src={s.thumbnail_url} className="size-full object-cover" alt="" />
                )}
              </div>
          ...
          {submissions.length === 0 && (
            <li className="text-sm text-muted-foreground text-center py-6">
              No submissions yet.{" "}
              <Link to="/creator-studio/edit" className="text-primary font-semibold">
...
function TodayRow({
  ...
  const toneCls = {
    ...
    purple:
      "text-[oklch(0.78_0.22_300)] bg-[oklch(0.65_0.22_300_/_0.1)] ring-[oklch(0.65_0.22_300_/_0.3)]",
    magenta:
      "text-[oklch(0.78_0.25_340)] bg-[oklch(0.7_0.25_340_/_0.1)] ring-[oklch(0.7_0.25_340_/_0.3)]",
  }[tone];
  const content = (
    <>
  ...
  if (href) {
    return (
      <li>
        <Link
          to={href}
          className="flex items-center gap-3 py-3 px-1 hover:bg-white/5 rounded-xl transition"
        >
          {content}
        </Link>
      </li>
