...
function ManualOnboarding() {
  ...
  const zodiacPreview = useMemo(
    ...
    [
      form.date_of_birth,
      form.birth_location_label,
      form.location,
      form.birth_time_precision,
  ...
  const handleFinish = async () => {
    ...
    try {
      await saveOnboardingProfile({
        data: {
          ...
          fields: {
            ...
            favorite_categories: form.favorite_categories.length
              ? form.favorite_categories
              : undefined,
      ...
      const { publicProfileUid } = await finalizeOnboarding({
        data: { accessToken, method: "manual" },
      });
...
function IdentityStep({
  ...
  const hintColor =
    usernameHint === "available"
      ? "text-green-400"
      : usernameHint === "taken" || usernameHint === "invalid"
        ? "text-red-400"
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="DISPLAY NAME">
        <TextInput value={form.display_name} onChange={setDisplayName} placeholder="Your name" />
      </Field>
      <Field label="USERNAME">
        <input
          ...
          className="w-full h-10 rounded-xl bg-white/5 border border-white/10 px-3 text-sm focus:outline-none focus:border-primary/50 transition"
        />
        {hintText && <p className={`text-[11px] mt-1 ${hintColor}`}>{hintText}</p>}
      </Field>
    </div>
  );
}
...
function ZodiacLockStep({
    ...
    return (
      <div className="space-y-4">
        <StepHeading
          ...
          sub="Add a birthday on the previous step to unlock your zodiac identity."
        />
        <div className="rounded-2xl bg-white/[0.04] border border-white/10 p-4 text-sm text-muted-foreground">
          Zodiac identity is optional here, but it needs a date of birth before Trey TV can
          calculate it.
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      <StepHeading
        title="Lock your zodiac identity"
        sub="Review the details before this becomes part of your permanent Trey TV profile."
      />
      <Field label="BIRTH LOCATION">
        <TextInput
          value={form.birth_location_label}
          onChange={setBirthLocation}
          placeholder="Memphis, TN (optional)"
        />
      </Field>
      <Field label="BIRTH TIMEZONE">
        <TextInput
          ...
          placeholder="America/Chicago (optional)"
        />
      </Field>
      ...
      {identity && (
        <div className="rounded-3xl liquid-glass neon-border p-4 space-y-4">
          <div className="flex items-center justify-between gap-3">
            <ZodiacBadge
              sign={identity.sunSign}
              isCusp={identity.isCusp}
              cuspLabel={identity.cuspLabel}
            />
            <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
              {identity.confidence.replace("_", " ")}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <ReviewMini
              ...
              value={
                form.birth_time_precision === "exact"
                  ? form.birth_time_local || "Exact"
                  : form.birth_time_precision.replace("_", " ")
              }
            />
            <ReviewMini
              label="Cusp"
              value={identity.isCusp ? (identity.cuspLabel ?? "Cusp Soul") : "No"}
            />
          ...
          <div className="rounded-2xl bg-primary/10 border border-primary/25 p-3 text-xs text-muted-foreground">
            Your zodiac identity is about to be added to your Trey TV profile. Once confirmed, this
            becomes part of your permanent profile identity unless support corrects an error.
          </div>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={confirm}
              className="flex-1 h-10 rounded-xl bg-primary text-primary-foreground text-xs font-black glow-gold"
            >
            ...
            {form.zodiac_confirmed && (
              <button
                type="button"
                onClick={edit}
                className="h-10 rounded-xl border border-white/10 px-3 text-xs font-semibold"
              >
                Edit Birth Details
              </button>
            )}
        ...
        <div className="flex gap-2 flex-wrap">
          {FREQUENCY_OPTIONS.map((o) => (
            <Chip
              ...
              onClick={() => setFrequency(frequency === o.value ? "" : o.value)}
            >
              {o.label}
            </Chip>
          ))}
      ...
      <div>
        ...
        <div className="flex gap-2 flex-wrap">
          {FAN_TYPE_OPTIONS.map((o) => (
            <Chip
              key={o.value}
              active={fanType === o.value}
              onClick={() => setFanType(fanType === o.value ? "" : o.value)}
            >
              {o.label}
            </Chip>
          ))}
