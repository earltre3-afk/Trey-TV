...
function GoVerificationApplication() {
  ...
  const save = async (status: "draft" | "pending") => {
    ...
    const payload = {
      ...
      status,
      verification_data: { ...data, public_profile_uid: user?.uid },
  ...
  return (
    <ApplicationWizardChrome
      ...
      sectionTitle={
        ["Badge Identity", "Notability Proof", "Official Links", "Safety Check", "Review & Submit"][
          step - 1
        ]
      }
      sectionSubtitle={
        [
          "Tell us who you are.",
          "Tell us about your public recognition.",
          "Share links that confirm your identity and notability.",
    ...
    >
      ...
      <WizardNav
        ...
        nextLabel={step === STEPS.length ? "Submit Verification Request" : "Next Step"}
      />
    </ApplicationWizardChrome>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="neon-gold p-5">
        <p className="text-xs uppercase tracking-[0.2em] text-[oklch(0.88_0.18_92)]">
          Gold Badge Preview
        </p>
        <div className="mt-4 rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.06] to-white/[0.02] p-5">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            {data.title || "Profile title (e.g., Recording Artist)"}
          </p>
        </div>
      </div>
      <CreatorPassport
...
function StepIdentity({
  ...
  data: VerifDraft;
  update: (p: Partial<VerifDraft>) => void;
}) {
  return (
    <div className="space-y-5">
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="Display name to verify" required>
          <NeonInput
            value={data.displayName}
            onChange={(e) => update({ displayName: e.target.value })}
            trailingIcon={<User className="h-4 w-4 text-[oklch(0.92_0.18_88)]" />}
          />
        </Field>
        <Field label="Username" required>
          <NeonInput
            ...
            trailingIcon={<AtSign className="h-4 w-4 text-[oklch(0.92_0.18_88)]" />}
          />
        </Field>
        <Field label="Email">
          <NeonInput
            ...
            trailingIcon={<Mail className="h-4 w-4 text-[oklch(0.92_0.18_88)]" />}
          />
        </Field>
      ...
      <Field label="What title should appear near your profile?" required>
        <NeonInput
          value={data.title}
          onChange={(e) => update({ title: e.target.value })}
          trailingIcon={<BadgeCheck className="h-4 w-4 text-[oklch(0.92_0.18_88)]" />}
        />
      </Field>
      <Field label="Short public bio" required>
        <NeonTextarea
          rows={3}
          value={data.bio}
          onChange={(e) => update({ bio: e.target.value })}
          trailingIcon={<FileText className="h-4 w-4 text-[oklch(0.92_0.18_88)]" />}
        />
      </Field>
      <Field label="Why should this profile receive a Go badge?" required>
        <NeonTextarea
          rows={4}
          value={data.reason}
          onChange={(e) => update({ reason: e.target.value })}
          trailingIcon={<Sparkles className="h-4 w-4 text-[oklch(0.92_0.18_88)]" />}
        />
      </Field>
    </div>
  );
...
function StepNotability({
  ...
  return (
    <div className="space-y-5">
      <Field label="Notability categories" required>
        <ChipPicker
          variant="gold"
          multi
          value={data.notabilityTypes}
          onChange={(v) => update({ notabilityTypes: v as string[] })}
          options={[
            "Press Coverage",
            "Music Releases",
      ...
      <Field label="Describe your public recognition (3-6 sentences)" required>
        <NeonTextarea
          rows={4}
          value={data.recognitionDescription}
          onChange={(e) => update({ recognitionDescription: e.target.value })}
        />
      </Field>
      <Field label="Any major achievements?">
        <NeonTextarea
          rows={3}
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="Any press mentions?">
          <NeonTextarea
            rows={3}
            value={data.pressMentions}
            onChange={(e) => update({ pressMentions: e.target.value })}
          />
        </Field>
        <Field label="Any official releases / projects?">
          <NeonTextarea
            rows={3}
            value={data.releases}
            onChange={(e) => update({ releases: e.target.value })}
          />
        </Field>
      </div>
    </div>
    ...
    <div className="space-y-6">
      <Field label="Official Links">
        <div className="space-y-2">
          {data.officialLinks.map((l, i) => (
            <NeonInput
              key={i}
              placeholder={`Official link ${i + 1}`}
              value={l}
              onChange={(e) => upd("officialLinks", i, e.target.value)}
            />
          ))}
        </div>
      </Field>
      <Field label="Proof Links">
        <div className="space-y-2">
          {data.proofLinks.map((l, i) => (
            <NeonInput
              key={i}
              placeholder={`Proof link ${i + 1}`}
              value={l}
              onChange={(e) => upd("proofLinks", i, e.target.value)}
            />
          ))}
        </div>
      </Field>
...
function StepSafety({
  ...
  return (
    <div className="space-y-5">
      ...
      />
      <Field label="Is there any impersonation risk or confusion we should know about? (optional)">
        <NeonTextarea
          rows={4}
          value={data.impersonationNote}
          onChange={(e) => update({ impersonationNote: e.target.value })}
        />
      </Field>
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        {sections.map((title, i) => (
          ...
          >
            <span className="step-circle is-done">{i + 1}</span>
      ...
      <div className="glass p-5">
        <p className="mb-3 text-sm font-semibold">Request Summary</p>
        <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
          <Row k="Display Name" v={data.displayName || "-"} />
          <Row k="Category" v={data.category || "-"} />
          <Row k="Profile Title" v={data.title || "-"} />
          <Row k="Notability Types" v={String(data.notabilityTypes.length)} />
          <Row k="Official Links" v={`${data.officialLinks.filter(Boolean).length} added`} />
          <Row k="Proof Links" v={`${data.proofLinks.filter(Boolean).length} added`} />
        </dl>
      </div>
    </div>
  );
        ...
        <div className="relative neon-gold p-6 md:p-10">
          ...
          <div className="relative flex flex-col items-center text-center">
            ...
            <div
              ...
              style={{
                boxShadow:
                  "inset 0 0 0 2px oklch(0.95 0.2 88 / 0.95), 0 0 60px oklch(0.85 0.2 85 / 0.6)",
              }}
            >
      ...
      <div className="hidden min-h-screen lg:flex">
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, oklch(0.14 0.06 80 / 0.9), oklch(0.08 0.03 70 / 0.9))",
              boxShadow:
            ...
            {[
              ...
              <div key={txt} className="flex items-start gap-3">
                <span
                  ...
                  style={{
                    boxShadow:
                      "inset 0 0 0 1px oklch(0.85 0.2 85 / 0.6), 0 0 10px oklch(0.85 0.2 85 / 0.2)",
                  }}
                >
                  {i + 1}
                </span>
...
function validate(step: number, data: VerifDraft) {
  if (
    ...
    return "Complete the required identity fields.";
  if (step === 2 && (data.notabilityTypes.length === 0 || !data.recognitionDescription.trim()))
    return "Select at least one notability category and describe your recognition.";
