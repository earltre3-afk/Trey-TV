...
function ConnectedAppsPage() {
  ...
  useEffect(() => {
    if (loading) return;
    if (!session) {
      try { sessionStorage.setItem("treytv_post_auth_redirect", "/settings/connected-apps"); } catch {}
      nav({ to: "/login" });
    }
  }, [loading, session, nav]);
  ...
  if (loading || !session) {
    return <div className="min-h-screen grid place-items-center text-sm text-muted-foreground">Loading connected apps...</div>;
  }
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-8 py-8">
        <header className="flex items-center justify-between">
          <Link to="/settings" className="size-10 rounded-full liquid-glass border border-white/10 grid place-items-center"><ArrowLeft className="size-4" /></Link>
          <Logo className="h-12" />
        </header>
        ...
        <section className="mt-8 rounded-[28px] liquid-glass neon-border p-6 sm:p-8">
          ...
          <p className="mt-3 text-muted-foreground">Review apps that can use your Trey TV identity and revoke access at any time.</p>
        </section>
        ...
        <section className="mt-6 space-y-3">
          {consents.length === 0 ? (
            ...
            return (
              <div key={consent.id} className="rounded-3xl liquid-glass border border-white/10 p-5 flex flex-wrap items-center gap-4">
                <div className="size-11 rounded-2xl bg-primary/10 text-primary grid place-items-center"><ShieldCheck className="size-5" /></div>
                <div className="flex-1 min-w-0">
                  <div className="font-bold truncate">{app?.app_name ?? "Developer App"}</div>
                  <div className="text-xs text-muted-foreground truncate">{app?.website_url ?? "No website"} · Connected {new Date(consent.granted_at).toLocaleDateString()}</div>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {(Array.isArray(consent.scopes) ? consent.scopes : []).map((scope: string) => (
                      <span key={scope} className="text-[10px] px-2 py-0.5 rounded-full border border-white/10 text-muted-foreground">{scope}</span>
                    ))}
                  </div>
                </div>
                <button
