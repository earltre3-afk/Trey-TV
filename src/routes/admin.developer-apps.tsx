...
function AdminDeveloperAppsPage() {
  ...
  return (
    <AdminShell
      ...
      subtitle="Review OAuth clients, app owners, scopes, and redirect URLs."
    >
      <div className="rounded-3xl liquid-glass border border-primary/30 p-4 mb-5 flex items-start gap-3">
        ...
        <p className="text-sm text-muted-foreground">
          Admin controls are staged here for app review. Suspend/revoke workflows should write audit
          events before public launch.
        </p>
      </div>
      <div className="space-y-3">
        {apps.length === 0 ? (
          <div className="rounded-2xl liquid-glass border border-white/10 p-8 text-center text-sm text-muted-foreground">
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {(Array.isArray(app.allowed_scopes) ? app.allowed_scopes : []).map(
                      (scope: string) => (
                        <span
                          key={scope}
                ...
                <span className="text-[10px] px-2 py-1 rounded-full border border-primary/30 text-primary capitalize">
                  {app.status}
                </span>
              </div>
