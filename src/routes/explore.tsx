...
import {
  searchTmdbContent,
  captureTmdbContent,
  type TmdbContentItem,
} from "@/lib/tmdb-scout.server";
...
function Explore() {
  ...
  const hasLocalResults =
    localResults && (localResults.shows.length > 0 || localResults.channels.length > 0);
  ...
  const showScout =
    isSearchActive && !hasLocalResults && (scoutResults.length > 0 || isScoutLoading);
        ...
        <section className="hidden lg:block relative overflow-hidden rounded-[2rem] border border-white/10 glass">
          ...
          <div className="relative grid grid-cols-2 gap-8 p-10 min-h-[340px]">
            <div className="flex flex-col justify-center">
              <div className="text-[11px] tracking-[0.3em] text-primary font-semibold">
                FEATURED · TONIGHT
              </div>
              <h1 className="mt-3 text-4xl xl:text-5xl font-bold leading-tight">
                Discover the next wave of creators
              </h1>
              <p className="mt-4 text-base text-muted-foreground max-w-md">
                Trending shows, live channels and editor-picked drops — refreshed every hour.
              </p>
              <div className="mt-6 flex items-center gap-3">
                <button
                  onClick={() =>
                    nav({ to: "/channel/$handle", params: { handle: hero.creator.handle } })
                  }
                  className="px-5 py-2.5 rounded-full bg-primary text-primary-foreground font-semibold glow-gold tilt-press flex items-center gap-2"
                >
                  <Play className="size-4 fill-current" /> Watch trailer
                </button>
                <Link
                  to="/prescribe-me"
                  className="px-5 py-2.5 rounded-full glass border border-white/15 font-semibold hover:bg-white/5"
                >
                  Prescribe me
                </Link>
              </div>
            ...
            {hasLocalResults && (
              <>
                {localResults.shows.length > 0 && (
                  <div>
                    <h2 className="text-sm font-semibold mb-3 flex items-center gap-2">
                      <Film className="size-4 text-primary" /> Shows
                    </h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                      {localResults.shows.map((s) => (
                        <div
                          key={s.id}
                          className="group relative aspect-[2/3] rounded-2xl overflow-hidden border border-white/10 cursor-pointer hover:ring-2 hover:ring-primary transition"
                        >
                          <img
                            src={s.poster}
                            alt=""
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-3">
                            <div className="text-[10px] tracking-widest text-primary mb-0.5">
                              {s.category}
                            </div>
                ...
                {localResults.channels.length > 0 && (
                  <div>
                    <h2 className="text-sm font-semibold mb-3 flex items-center gap-2">
                      <Radio className="size-4 text-primary" /> Channels
                    </h2>
          ...
          <>
            <section>
              <h2 className="text-sm lg:text-base font-semibold mb-3 flex items-center gap-2">
                <TrendingUp className="size-4 text-primary" /> Trending categories
              </h2>
              <div className="grid grid-cols-3 lg:grid-cols-6 gap-2 lg:gap-3">
                {categoryChips.map((c) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`size-10 lg:size-12 rounded-xl grid place-items-center ${c.bg} ${c.color} transition group-hover:scale-110`}
                      >
                ...
                {creators.map((c) => (
                  ...
                  >
                    ...
                    <div className="relative size-16 lg:size-20 rounded-full conic-ring">
                      ...
                      {c.live && (
                        <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 px-1.5 py-0.5 text-[9px] font-bold rounded-md bg-[oklch(0.65_0.24_15)] text-white animate-glow-pulse">
                          LIVE
                        </span>
                      )}
                    </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-sm lg:text-base font-semibold flex items-center gap-2">
                    <Flame className="size-4 text-[oklch(0.7_0.25_340)]" /> Hot right now
                  </h2>
              ...
              <aside className="hidden lg:flex flex-col gap-4 sticky top-6 h-fit">
                <div className="rounded-3xl glass neon-border p-4">
                  <h3 className="text-sm font-bold flex items-center gap-2 mb-3">
                    <Radio className="size-4 text-[oklch(0.65_0.24_15)]" /> Live channels
                  </h3>
