        ...
        <div className="relative">
          ...
          <div className="mt-6 flex flex-col gap-2">
            <button
              ...
              className="px-4 py-3 rounded-xl text-sm font-bold bg-primary text-primary-foreground glow-gold"
            >
              Apply to become a Creator
            </button>
            <button
              onClick={() => navigate({ to: "/creator-hub" })}
              className="px-4 py-3 rounded-xl text-sm font-semibold glass border border-white/10"
            >
              Back to Creator Hub
            </button>
          </div>
    ...
    <main className="min-h-dvh bg-background text-foreground pb-32">
      ...
      <header className="sticky top-0 z-30 px-3 md:px-5 pt-3 pb-3 backdrop-blur-xl bg-background/70 border-b border-white/5">
        <div className="flex items-center gap-2">
          <button
            onClick={goBack}
            className="size-10 grid place-items-center rounded-xl glass tilt-press"
          >
            <ArrowLeft className="size-5" />
          </button>
          <div className="flex-1 min-w-0">
            <div className="text-[10px] tracking-[0.3em] text-primary">CONTENT DETAILS</div>
            <h1 className="text-base md:text-lg font-bold text-gradient-gold truncate">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Organize your content for Trey TV before it goes to admin review.
        </p>
      </header>
      ...
      <div className="px-3 md:px-5 pt-4 space-y-5">
        ...
        <Section title="Content Preview" icon={Film}>
          <div className="grid md:grid-cols-[1.4fr,1fr] gap-3">
            <div className="relative rounded-2xl overflow-hidden glass neon-border aspect-video">
              <img
                src={d.thumbnail_url || posts[0].media}
                className="absolute inset-0 size-full object-cover"
                alt=""
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-black/30" />
              <span className="absolute top-2 left-2 text-[10px] px-2 py-0.5 rounded bg-black/60 border border-white/15">
                {d.quality}
              </span>
              <span className="absolute top-2 right-2 text-[10px] px-2 py-0.5 rounded bg-primary/20 text-primary border border-primary/40 capitalize">
                {d.status.replace("_", " ")}
              </span>
              <span className="absolute bottom-2 right-2 text-[10px] px-2 py-0.5 rounded bg-black/60 border border-white/15 tabular-nums">
                {d.duration}
              </span>
              <button
              ...
              >
                <span className="size-12 rounded-full bg-white text-black grid place-items-center">
            ...
            <div className="flex flex-col gap-2 text-sm">
              <button
                ...
                className="px-3 py-2 rounded-xl glass border border-white/10 text-left"
              >
                ← Back to Edit Studio
              </button>
              <button
                ...
                className="px-3 py-2 rounded-xl glass border border-white/10 text-left flex items-center gap-2"
              >
                <ImageIcon className="size-4 text-primary" /> Replace thumbnail
              </button>
              <button
                onClick={() => setPreviewOpen(true)}
                className="px-3 py-2 rounded-xl glass border border-white/10 text-left flex items-center gap-2"
              >
                <Eye className="size-4 text-primary" /> Preview episode
              </button>
            </div>
          </div>
        </Section>
        ...
        <Section title="Episode Identity" icon={Sparkles}>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Season #">
              <Input
                value={String(d.season_number)}
                onChange={(v) => set("season_number", Number(v) || 1)}
                type="number"
              />
            </Field>
            <Field label="Episode #">
              <Input
                value={String(d.episode_number)}
                onChange={(v) => set("episode_number", Number(v) || 1)}
                type="number"
              />
            </Field>
          </div>
          <div className="text-xs text-muted-foreground">
            ...
            <span className="text-foreground font-semibold">
              Season {d.season_number}, Episode {d.episode_number}: "{d.title || "Untitled"}"
            </span>
          </div>
        </Section>
        ...
        <Section title="Viewer Context" icon={Eye}>
          <Field label="Short description">
            <Input
              ...
              placeholder="One-line hook for viewers"
            />
          </Field>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <Field label="What viewers should know">
              <TextArea value={d.what_to_know} onChange={(v) => set("what_to_know", v)} rows={2} />
            </Field>
            <Field label="Why this episode matters">
              <TextArea
          ...
          <Field label="Creator note (optional)">
            <TextArea value={d.creator_note} onChange={(v) => set("creator_note", v)} rows={2} />
          </Field>
          <Field label="Categories">
            <ChipGroup
              options={CATEGORIES}
              selected={d.category}
              onToggle={(v) => toggleArr("category", v)}
            />
          </Field>
          <Field label="Mood tags">
            <ChipGroup
              options={MOOD_TAGS}
              selected={d.mood_tags}
              onToggle={(v) => toggleArr("mood_tags", v)}
            />
          </Field>
        ...
        <Section title="Series Organization" icon={Film}>
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-xl text-xs font-semibold border ${seriesMode === "existing" ? "bg-primary/15 border-primary text-primary glow-gold" : "border-white/10 glass"}`}
            >
              Use existing show
            </button>
            <button
              onClick={() => setSeriesMode("new")}
              className={`px-3 py-2 rounded-xl text-xs font-semibold border ${seriesMode === "new" ? "bg-primary/15 border-primary text-primary glow-gold" : "border-white/10 glass"}`}
            >
              Create new show
            </button>
          </div>
          {seriesMode === "new" && (
            <div className="space-y-3 mt-3">
              <Field label="Show title">
                <Input
                  value={newShow.title}
                  onChange={(v) => setNewShow({ ...newShow, title: v })}
                />
              </Field>
              <Field label="Show description">
                <TextArea
                  value={newShow.description}
                  onChange={(v) => setNewShow({ ...newShow, description: v })}
                  rows={2}
                />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Show category">
                  <Select
                    ...
                    onChange={(v) => setNewShow({ ...newShow, category: v })}
                    options={CATEGORIES}
                  />
                </Field>
                <Field label="Visibility after approval">
                  <Select
                    value={newShow.visibility}
                    onChange={(v) => setNewShow({ ...newShow, visibility: v })}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-2">
            ...
            <Toggle
              label="Season finale"
              value={d.is_finale}
              onChange={(v) => set("is_finale", v)}
            />
            <Toggle
              label="Premiere"
              value={d.is_premiere}
              onChange={(v) => set("is_premiere", v)}
            />
          </div>
        </Section>
        ...
        <Section title="Thumbnail & Cover" icon={ImageIcon}>
          <Field label="Episode thumbnail">
            <div className="grid grid-cols-3 gap-2">
              {posts.map((p) => (
                <button
                  key={p.id}
                  onClick={() => set("thumbnail_url", p.media)}
                  className={`relative aspect-video rounded-lg overflow-hidden ring-1 ${d.thumbnail_url === p.media ? "ring-primary glow-gold" : "ring-white/10"}`}
                >
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl text-xs font-semibold glass border border-white/10"
          >
        ...
        <Section title="Audience & Visibility" icon={Users}>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Content rating">
              <Select
                value={d.content_rating}
                onChange={(v) => set("content_rating", v)}
                options={["G", "PG", "PG-13", "TV-14", "TV-MA"]}
              />
            </Field>
          ...
          <Toggle
            ...
            value={d.explicit_content}
            onChange={(v) => set("explicit_content", v)}
          />
          <Field label="Visibility after approval">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {(["public", "subscribers", "private", "scheduled"] as const).map((v) => (
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-xs font-semibold capitalize border ${d.visibility === v ? "bg-primary/15 border-primary text-primary glow-gold" : "border-white/10 glass"}`}
                >
                  {v}
                </button>
              ))}
        ...
        <Section title="Monetization / Access" icon={DollarSign}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {(
              ...
              ] as { v: Submission["access_type"]; label: string; soon: boolean }[]
            ).map((o) => (
              <button
                key={o.v}
                disabled={o.soon}
        ...
        <Section title="Final Review Checklist" icon={CheckCircle2}>
          <ul className="space-y-2">
            {checklist.map((c) => (
              <li key={c.label} className="flex items-center gap-2 text-sm">
                {c.ok ? (
                  <CheckCircle2 className="size-4 text-[oklch(0.78_0.18_150)]" />
                ) : (
          ...
          <label className="flex items-start gap-2 mt-3 text-xs cursor-pointer">
            <input
              type="checkbox"
              checked={d.policy_ack}
              onChange={(e) => set("policy_ack", e.target.checked)}
              className="mt-0.5"
            />
            <span>
              I confirm this content follows the Trey TV community guidelines, copyright, and
              creator policies.
            </span>
          </label>
        </Section>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-3 pb-4 pt-3 backdrop-blur-xl bg-background/80 border-t border-white/10">
        <div className="max-w-[1400px] mx-auto flex items-center gap-2">
          <Link
            ...
            className="hidden md:inline-flex px-3 py-3 rounded-xl text-xs font-semibold glass border border-white/10"
          >
            My submissions
          </Link>
          <button
            ...
            className="flex-1 md:flex-none px-3 py-3 rounded-xl text-xs font-semibold glass border border-white/10"
          >
            Save draft
          </button>
          <button
      ...
      {previewOpen && (
        ...
        >
          <div
            className="relative max-w-3xl w-full rounded-3xl glass-strong border border-white/10 overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setPreviewOpen(false)}
              className="absolute top-3 right-3 z-10 size-9 grid place-items-center rounded-full glass"
            >
              <X className="size-4" />
            </button>
            ...
            <div className="p-4">
              ...
              <div className="text-xs text-muted-foreground">
                {d.show_title} · S{d.season_number} E{d.episode_number}
              </div>
            </div>
          </div>
        </div>
...
function TextArea({
  ...
  return (
    <textarea
      ...
      onChange={(e) => onChange(e.target.value)}
      rows={rows}
      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm resize-none focus:outline-none focus:border-primary/60"
    />
  );
}
function Select({
      ...
      {options.map((o) => {
        ...
        return (
          <button
            key={o}
            onClick={() => onToggle(o)}
            className={`px-3 py-1.5 rounded-full text-[11px] font-semibold border transition ${on ? "bg-primary/15 border-primary text-primary glow-gold" : "border-white/10 glass hover:bg-white/5"}`}
          >
  ...
  return (
    <div className="rounded-xl bg-white/5 border border-white/10 px-2 py-2 flex flex-wrap gap-1.5">
      {tags.map((t) => (
        <span
          key={t}
          className="px-2 py-0.5 rounded-full text-[11px] bg-primary/15 text-primary border border-primary/30 flex items-center gap-1"
        >
          #{t}{" "}
...
function Toggle({
  ...
  return (
    <button
      ...
      className={`px-3 py-2 rounded-xl text-xs font-semibold border text-left flex items-center justify-between ${value ? "bg-primary/15 border-primary text-primary glow-gold" : "border-white/10 glass"}`}
    >
      {label}
      <span
        className={`size-4 rounded-full border ${value ? "bg-primary border-primary" : "border-white/30"}`}
      />
    </button>
  );
}
