...
import {
  ...
  polishBio,
  polishAllFields,
} from "@/lib/trey-i/profile-polish";
...
async function clientTurn(
  ...
  const canFinish = !["confirm_display_name", "confirm_username", "review", "complete"].includes(
    stage,
  );
  ...
  if (stage === "ask_display_name") {
    ...
    return {
      ...
      fields,
      pending: { field: "display_name", value: name },
    };
  }
  ...
  if (stage === "confirm_display_name") {
    if (yesRe.test(t) && pending.value)
      return {
        ...
        stage: "ask_username",
        fields: { ...fields, display_name: String(pending.value) },
        pending: {},
      };
    ...
    if (rep.length >= 2)
      return {
        ...
        stage: "confirm_display_name",
        fields,
        pending: { field: "display_name", value: rep },
      };
    return {
  ...
  if (stage === "ask_username") {
    ...
    return {
      ...
      fields,
      pending: { field: "username", value: check.username },
    };
  }
  ...
  if (stage === "confirm_username") {
    if (yesRe.test(t) && pending.value)
      return {
        ...
        stage: "ask_bio",
        fields: { ...fields, username: String(pending.value) },
        pending: {},
      };
  ...
  if (stage === "confirm_bio") {
    if (yesRe.test(t) && pending.value) {
      ...
      const cleanedBio =
        rawBio.charAt(0).toUpperCase() + rawBio.slice(1) + (!/[.!?]$/.test(rawBio) ? "." : "");
      return {
        message: "Bio saved. Want to add your location? Say it now, or say skip.",
        stage: "ask_location",
        fields: { ...fields, bio: cleanedBio },
        pending: {},
      };
    }
    if (skipRe.test(t))
  ...
  if (stage === "ask_location") {
    ...
    return {
      ...
      fields,
      pending: { field: "location", value: loc },
    };
  }
  ...
  if (stage === "ask_birthday") {
    ...
    return {
      ...
      fields,
      pending: { field: "date_of_birth", value: dob },
    };
  }
  ...
  if (stage === "ask_show_birthday") {
    ...
    return {
      ...
      stage: "ask_content_choice",
      fields: { ...fields, show_birthday: show },
      pending: {},
    };
  }
  ...
  if (stage === "ask_categories") {
    ...
    return {
      ...
      fields,
      pending: { field: "favorite_categories", value: cats },
    };
  }
  ...
  if (stage === "ask_moods") {
    if (skipRe.test(t))
      return {
        message:
          "Moods skipped. How often do you watch or post? Daily, weekly, or only major drops?",
        stage: "ask_frequency",
        fields,
        pending: {},
      };
    ...
    return {
      ...
      stage: "confirm_moods",
      fields,
      pending: { field: "favorite_moods", value: moods },
    };
  }
  ...
  if (stage === "ask_frequency") {
    if (skipRe.test(t))
      return {
        message:
          "Frequency skipped. Are you mostly a viewer, creator, supporter, superfan, or Trizfit?",
        stage: "ask_fan_type",
        fields,
        pending: {},
      };
    ...
    return {
      ...
      fields,
      pending: { field: "content_frequency", value: freq },
    };
  }
  ...
  if (stage === "confirm_frequency") {
    if (yesRe.test(t) && pending.value) {
      ...
      return {
        message:
          "Frequency saved. Are you mostly a viewer, creator, supporter, superfan, or Trizfit?",
        stage: "ask_fan_type",
        fields: f,
        pending: {},
      };
  ...
  if (stage === "ask_fan_type") {
    ...
    return {
      ...
      fields,
      pending: { field: "fan_type", value: fanType },
    };
  }
  ...
  const socialAskMap: Partial<
    Record<Stage, { field: keyof Fields; next: Stage; nextPrompt: string }>
  > = {
    ask_instagram: {
      field: "instagram",
      next: "ask_tiktok",
  ...
  const socialConfirmMap: Partial<
    Record<Stage, { field: keyof Fields; ask: Stage; next: Stage; nextPrompt: string }>
  > = {
    confirm_instagram: {
      field: "instagram",
      ask: "ask_instagram",
  ...
  if (askSocial) {
    ...
    return {
      ...
      stage: confirmStage,
      fields,
      pending: { field: String(askSocial.field), value: val },
    };
  }
  ...
  if (stage === "ask_visibility") {
    ...
    return {
      ...
      fields,
      pending: { field: "profile_visibility", value: vis },
    };
  }
  ...
  if (stage === "confirm_visibility") {
    if (yesRe.test(t) && pending.value) {
      ...
      return {
        message:
          "Visibility saved. Should your location and birthday show on your profile? Say both, location only, birthday only, or neither.",
  ...
  if (stage === "review") {
    ...
    return {
      message:
        "What do you want to change? You can say display name, username, bio, location, birthday, categories, moods, frequency, profile type, socials, or privacy.",
      stage: "edit_field",
      fields,
      pending: {},
    };
...
const RINGS = [
  ...
  [0.78, 0.2, 17, 5.2, "orb-ring-cw"],
  [0.65, 0.18, -9, 9.0, "orb-ring-ccw"],
  [0.52, 0.16, 33, 4.0, "orb-ring-cw"],
  [0.38, 0.13, -43, 3.4, "orb-ring-ccw"],
] as const;
      ...
      >
        ...
        <g clipPath="url(#pOrbClip)">
          ...
          {RINGS.map(([rxR, ryR, tilt, durBase, animName], i) => {
            const gradId =
              i % 3 === 0 ? "url(#pGold)" : i % 3 === 1 ? "url(#pBlue)" : "url(#pPurple)";
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 50% 22%, oklch(0.2 0.07 290 / 0.78) 0%, oklch(0.1 0.03 270 / 0.88) 50%, #05060B 100%)",
        }}
      />
      <div
...
function ReviewPanel({
  ...
  return (
    <div
      className="relative mx-4 mb-3 rounded-2xl liquid-glass border border-primary/25 overflow-hidden"
      style={{ zIndex: 1 }}
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
        <div className="text-[9px] tracking-[0.35em] text-primary font-bold">
      ...
      <div className="p-4 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[9px] tracking-[0.2em] text-muted-foreground/50 mb-1 block">
              NAME
            </label>
            <input
              value={name}
...
function VoiceOnboardingInner() {
  ...
  const coreConfirmed = CORE_FIELDS.filter(
    ...
    const loadProgress = async () => {
      try {
        ...
        if (
          data &&
          !data.completed &&
          (data.selected_path === "voice" || data.selected_path === "trey_i")
        ) {
        ...
        if (token) {
          try {
            ...
            const { publicProfileUid } = await finalizeOnboarding({
              data: { accessToken: token, method: "voice" },
            });
          ...
          <div className="liquid-glass neon-border rounded-3xl p-8 space-y-6">
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Trey-I uses AI voice technology to guide you through your profile setup. Please
                review what you're agreeing to before we begin.
              </p>
            </div>
            ...
            <p className="text-[11px] text-muted-foreground/60 text-center leading-relaxed">
              By tapping <strong className="text-muted-foreground">Accept &amp; Start</strong> you
              consent to the above. This session is governed by our{" "}
  ...
  return (
    ...
    >
      ...
      {/* Top bar */}
      <div
        className="relative flex items-center justify-between px-4 pt-6 pb-2 shrink-0"
        style={{ zIndex: 1 }}
      >
        <Link
          to="/onboarding"
      ...
      <header className="relative text-center px-4 pt-1 pb-2 shrink-0" style={{ zIndex: 1 }}>
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-[oklch(0.82_0.16_85_/_0.35)] bg-[oklch(0.82_0.16_85_/_0.08)] mb-2">
          ...
          <span className="text-[9px] tracking-[0.3em] text-primary font-bold">
            PROFILE CONCIERGE
          </span>
        </div>
      ...
      {/* Voice / Manual toggle */}
      <div
        className="relative flex items-center justify-center gap-2 px-4 py-2 shrink-0"
        style={{ zIndex: 1 }}
      >
      ...
      {/* Galaxy Orb hero */}
      <div
        className="relative flex justify-center items-center py-5 shrink-0"
        style={{ zIndex: 1 }}
      >
      ...
      {/* TREY-I SAYS card */}
      <div
        className="relative mx-4 mb-3 rounded-2xl liquid-glass border border-white/10 p-4 shrink-0"
        style={{ zIndex: 1 }}
      >
        <div className="text-[9px] tracking-[0.35em] text-primary font-bold mb-1.5">
          TREY-I SAYS
        </div>
      ...
      <div className="relative mx-4 pb-8 shrink-0" style={{ zIndex: 1 }}>
        <div className="flex items-center justify-between mb-2.5">
          <span className="text-[9px] tracking-[0.22em] text-muted-foreground/50 font-medium">
            SETUP PATH
          </span>
            ...
            return (
              <Fragment key={step.key}>
                <div className="flex flex-col items-center gap-1.5">
                  <div
                    className={`size-6 rounded-full grid place-items-center border-2 transition-all duration-500 ${
                      done
                  ...
                  <span
                    className={`text-[9px] tracking-wider font-medium transition-colors duration-500 ${done ? "text-primary" : "text-muted-foreground/35"}`}
                  >
                    {step.label}
                  </span>
                </div>
                {i < SETUP_STEPS.length - 1 && (
                  <div
                    className={`flex-1 h-px mx-1 mb-4 transition-all duration-500 ${done ? "bg-primary/50" : "bg-white/10"}`}
                  />
                )}
              </Fragment>
            );
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <span className="text-[9px] text-muted-foreground/50 shrink-0">
            {coreConfirmed}/{CORE_FIELDS.length} required
          </span>
        </div>
      </div>
    </div>
