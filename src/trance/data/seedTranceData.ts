...
export async function seedDevelopmentFixtures(
  ...
  try {
    ...
    const { error: dancerErr } = await supabaseClient
    ...
    for (const c of devChoreographers) {
      const { error: pErr } = await supabaseClient
        ...
        .upsert({
          ...
          verified: c.verified,
          roles: ['choreographer'],
          permissions: ['browse_public_routines', 'practice_routines', 'view_own_scores', 'create_sessions', 'manage_own_channel']
        });
    ...
    for (const r of fixtureRoutines) {
      const { error: rtErr } = await supabaseClient
      ...
      if (r.moveHints && r.moveHints.length > 0) {
        for (const mh of r.moveHints) {
          const { error: mhErr } = await supabaseClient
            .from('trance_move_hints')
            .upsert({
              id: toUuid(mh.id),
              routine_id: toUuid(r.id),
      ...
      if (r.directionCues && r.directionCues.length > 0) {
        for (const dc of r.directionCues) {
          const { error: dcErr } = await supabaseClient
            .from('trance_direction_cues')
            .upsert({
              id: toUuid(dc.id),
              routine_id: toUuid(r.id),
    ...
    for (const entry of devLeaderboard) {
      ...
      // Leaderboard entry
      const { error: leErr } = await supabaseClient
