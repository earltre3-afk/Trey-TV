...
export const rowToDanceRoutine = (row: any): DanceRoutine => {
  ...
  return {
    ...
    scoring: scoringRow
      ...
      : { timing: 30, execution: 30, energy: 20, precision: 10, creativity: 10, scale: '0 - 100 Points' },
  };
};
...
export const tranceRoutineService = {
    ...
    if (shouldUseFixtures()) {
      ...
      if (filters) {
        if (filters.query) {
          ...
          filtered = filtered.filter((r) => r.title.toLowerCase().includes(q) || r.tagline.toLowerCase().includes(q));
        }
        if (filters.style) {
  ...
  createRoutineDraft: async (routine: Partial<DanceRoutine>): Promise<DanceRoutine> => {
    ...
    const defaultDraft: DanceRoutine = {
      ...
      cover: routine.cover || 'https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?w=800&auto=format&fit=crop',
      ...
      moveHints: [],
      directionCues: [],
      scoring: { timing: 30, execution: 30, energy: 20, precision: 10, creativity: 10, scale: '0 - 100 Points' }
    };
    ...
    await supabase.from('trance_count_sections').insert([
      ...
      { routine_id: defaultDraft.id, index: 2, label: 'Chest Hit & Travel', counts: '8 Counts x 2' }
    ]);

    await supabase.from('trance_move_hints').insert([
      { routine_id: defaultDraft.id, timestamp: '00:00', label: 'Groove In', description: 'Relax into the beat. Feel the bounce.' },
      { routine_id: defaultDraft.id, timestamp: '00:16', label: 'Chest Hit', description: 'Hit chest forward on 5, clean & sharp.' }
    ]);
