...
export const BuilderStepper: React.FC<{ active: number; onSelect: (n: number) => void }> = ({
  active,
  onSelect,
}) => (
  <div className="flex md:flex-col overflow-x-auto no-scrollbar gap-2 md:space-y-2 md:gap-0 pb-2 md:pb-0 scroll-smooth px-1 md:px-0">
    {builderSteps.map((s) => (
      <button
        ...
        className={cn(
          "shrink-0 md:shrink w-[150px] md:w-full flex items-center gap-2 md:gap-3 rounded-2xl border bg-white/[0.03] p-2.5 md:p-3 text-left transition",
          active === s.n ? colorMap[s.color] : "border-white/10 text-white/50",
        )}
      >
        <span
          className={cn(
            "w-5 h-5 md:w-6 md:h-6 rounded-full grid place-items-center text-[10px] md:text-xs font-black shrink-0",
            active === s.n ? "bg-current text-black" : "bg-white/10",
          )}
        >
          {s.n}
        </span>
        ...
        <div className="min-w-0">
          <div className="text-[10px] md:text-xs font-black uppercase truncate">{s.label}</div>
          <div className="text-[8px] md:text-[9px] text-white/40 truncate">{s.sub}</div>
        </div>
      </button>
    ))}
  </div>
