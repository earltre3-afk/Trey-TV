...
export const TranceGlassCard: React.FC<{
  children: React.ReactNode;
  className?: string;
  glow?: "magenta" | "cyan" | "purple" | "gold" | "none";
  onClick?: () => void;
  ...
  return (
    <div
      ...
      className={cn(
        ...
        glows[glow],
        onClick && "cursor-pointer transition-transform active:scale-[0.98] hover:border-white/20",
        className,
      )}
    >
      {children}
...
/* TranceBadge */
export const TranceBadge: React.FC<{ badge: Badge; size?: "sm" | "md" }> = ({
  badge,
  size = "md",
}) => {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 w-20">
      <div
        className={cn(
          "rounded-2xl border bg-gradient-to-b grid place-items-center",
          dim,
          tiers[badge.tier],
        )}
      >
      ...
      <span className="text-[10px] font-semibold text-center leading-tight text-white/70 uppercase">
        {badge.name}
      </span>
    </div>
  );
...
export const ErrorState: React.FC<{ label?: string; onRetry?: () => void }> = ({
  ...
  <div className="flex flex-col items-center justify-center py-12 text-center">
    ...
    {onRetry && (
      <button onClick={onRetry} className="text-xs font-bold text-cyan-300 underline">
        Try again
      </button>
    )}
  </div>
...
export const VerifiedTick: React.FC<{ className?: string }> = ({ className }) => (
  <span
    className={cn(
      "inline-grid place-items-center rounded-full bg-cyan-400 text-black w-4 h-4",
      className,
    )}
  >
...
export const GradientButton: React.FC<{
  ...
  onClick?: () => void;
  className?: string;
  variant?: "magenta" | "cyan" | "gold" | "outline";
}> = ({ children, onClick, className, variant = "magenta" }) => {
  const v: Record<string, string> = {
    magenta:
      "bg-gradient-to-r from-fuchsia-500 via-purple-500 to-cyan-500 text-white shadow-[0_0_24px_-4px_rgba(217,70,239,0.6)]",
