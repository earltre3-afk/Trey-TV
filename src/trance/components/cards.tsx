...
/* TranceRoutineCard */
export const TranceRoutineCard: React.FC<{
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        {rank && (
          <div className="absolute top-2 left-2 w-7 h-7 rounded-lg bg-black/75 border border-white/20 grid place-items-center text-xs font-black text-white z-10 transition-colors group-hover:border-fuchsia-400">
            {rank}
          </div>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 z-10">
          <h4 className="font-black text-white leading-tight transition-colors group-hover:text-fuchsia-200">
            {routine.title}
          </h4>
          ...
          <div className="flex items-center justify-between text-[10px] text-white/60">
            <span className="flex items-center gap-1">
              ...
              {fmtNum(routine.saves)}
            </span>
            <span className="flex items-center gap-1">
              <Users className="w-3 h-3" />
              {fmtNum(routine.students)}
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="relative aspect-square rounded-2xl overflow-hidden border border-white/10 transition-all duration-300 group-hover:border-cyan-400/50 group-hover:shadow-[0_0_15px_rgba(34,211,238,0.25)]">
        ...
        <span className="absolute top-2 left-2 text-xs font-black bg-black/75 px-2 py-0.5 rounded-md border border-white/20 transition-colors group-hover:border-cyan-400">
          #{rank}
        </span>
      ...
      <div className="mt-1.5">
        <h5 className="text-sm font-bold text-white truncate transition-colors group-hover:text-cyan-200">
          {routine.title}
        </h5>
      ...
      <div className="relative aspect-[4/5] overflow-hidden">
        ...
        <div className="absolute bottom-0 p-3">
          <div className="flex items-center gap-1">
            <h4 className="font-black text-fuchsia-300 uppercase text-sm transition-colors group-hover:text-fuchsia-200">
              {c.displayName}
            </h4>
...
export const StudioRoomCard: React.FC<{ s: StudioRoom }> = ({ s }) => {
  ...
  return (
    <div
      onClick={() => navigate(TRANCE_ROUTES.studio(s.id))}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h5 className="font-bold text-white text-sm truncate transition-colors group-hover:text-fuchsia-300">
          {s.name}
        </h5>
        <p className="text-[11px] text-white/50">
          {s.members} / {s.capacity} Members
        </p>
      </div>
      <span
        className={cn(
          "text-[10px] font-black uppercase flex items-center gap-1 transition-all duration-300",
          s.status === "LIVE" ? "text-emerald-400 group-hover:scale-105" : "text-white/40",
        )}
      >
        {s.status === "LIVE" && <Flame className="w-3 h-3 animate-pulse" />}
        {s.status}
      </span>
    </div>
  );
...
export const RehearsalAssignmentCard: React.FC<{ a: RehearsalAssignment }> = ({ a }) => (
  <div className="flex items-center gap-3 p-3 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] cursor-pointer">
    ...
    <div className="flex flex-col items-end gap-1">
      <span className="text-[9px] font-bold text-cyan-300 bg-cyan-500/10 border border-cyan-400/30 px-2 py-0.5 rounded-full">
        {a.due}
      </span>
  ...
  return (
    <div className={cn("flex items-center gap-3 p-3 rounded-2xl border", tier[e.badgeTier])}>
      <div className={cn("w-7 text-center font-black text-lg", rankColor)}>
        {e.rank === 1 ? (
          <Crown className="w-6 h-6 mx-auto text-yellow-300 fill-yellow-300/30" />
        ) : (
          e.rank
        )}
      </div>
      <div
        className={cn(
          ...
          e.rank === 1 ? "border-yellow-400" : "border-white/20",
        )}
      >
...
export const SessionModeCard: React.FC<{
  ...
  const cfg: Record<
    string,
    { icon: React.ComponentType<{ className?: string }>; glow: string; ring: string }
  > = {
    Learn: {
      icon: BookOpen,
      glow: "from-fuchsia-500/20 border-fuchsia-400/40",
      ring: "text-fuchsia-300",
    },
    Practice: { icon: Target, glow: "from-cyan-500/20 border-cyan-400/40", ring: "text-cyan-300" },
    Performance: {
      icon: Zap,
      glow: "from-yellow-500/20 border-yellow-400/40",
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex-1 text-left rounded-2xl border bg-gradient-to-b to-transparent p-3.5 active:scale-95 transition",
        C.glow,
      )}
    >
...
/* CountStructureCard */
export const CountStructureCard: React.FC<{ sections: CountSection[]; bpm: number }> = ({
  sections,
  bpm,
}) => (
...
export const DirectionCueCard: React.FC<{ cue: DirectionCue }> = ({ cue }) => {
  ...
  return (
    <div className="shrink-0 w-20 rounded-xl border border-cyan-400/30 bg-cyan-500/5 p-3 text-center">
      <Icon
        className="w-7 h-7 mx-auto text-cyan-300 mb-1"
