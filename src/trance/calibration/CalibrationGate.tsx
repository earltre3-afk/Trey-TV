...
export function CalibrationGate({ children }: CalibrationGateProps) {
  // Track device type synchronously to avoid flashing the desktop QR gate on mobile
  ...
  // 1. Device type not yet determined — show spinner to avoid flashing wrong gate
  ...
  // 2. Desktop → QR handoff
  if (!isMobile) {
    return <QRMobileHandoffGate routineId={routineId} />;
  }

  // 3. Handoff token present but expired
  if (tokenExpired) {
    return (
      <TranceShell hideNav>
  ...
  // 4. Auth loading or not authed → spinner (redirect effect handles navigation)
  ...
  // 7. All gates passed — render children
  return <>{children}</>;
}
