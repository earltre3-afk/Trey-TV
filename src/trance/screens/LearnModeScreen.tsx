...
const LearnModeScreen: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    return () => {
      void trancePoseTrackingService.stopPoseSession(null);
    };
  }, []);
    ...
    <TranceShell hideNav pad={false}>
      ...
      <div className="flex items-center justify-between px-4 pt-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs font-black text-cyan-300 uppercase">Learn Mode</div>
          </div>
        </div>
        ...
        <div className="text-right">
          <div className="text-[9px] text-yellow-300 uppercase">Session Score</div>
          <div className="text-lg font-black text-yellow-300">12,350</div>
        </div>
      </div>
      ...
      <div className="mx-4 mt-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="text-right">
            <div className="text-lg font-black text-white">01:28</div>
            <div className="text-[8px] text-white/50 uppercase">Remaining</div>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <line x1="42" y1="35" x2="38" y2="50" />
            <line x1="58" y1="35" x2="62" y2="50" />
            <line x1="45" y1="55" x2="42" y2="72" />
            <line x1="55" y1="55" x2="58" y2="72" />
            <line x1="42" y1="72" x2="40" y2="90" />
            <line x1="58" y1="72" x2="60" y2="90" />
          </g>
        </svg>
        <span className="absolute top-2 left-2 text-[8px] bg-black/60 px-2 py-1 rounded text-cyan-300 uppercase">
          AI Pose Overlay · Placeholder
        </span>

        {/* Stat stack left */}
        <div className="absolute top-3 left-3 space-y-2 hidden sm:block">
          <div className="rounded-2xl border border-cyan-400/30 bg-black/50 backdrop-blur p-2 text-center">
            <div className="text-[9px] text-white/60 uppercase">Timing</div>
            <div className="text-[10px] font-black text-cyan-300 uppercase mb-1">Perfect</div>
            <TranceStatRing
              value={98}
              size={64}
              color="#22d3ee"
              big={<span className="text-base font-black text-white">98%</span>}
            />
          </div>
          <div className="rounded-2xl border border-teal-400/30 bg-black/50 backdrop-blur p-2 text-center">
            <div className="text-[9px] text-white/60 uppercase flex items-center justify-center gap-1">
              Sync <LinkIcon className="w-2.5 h-2.5" />
            </div>
            <TranceStatRing
              value={94}
              size={56}
              color="#2dd4bf"
              big={<span className="text-sm font-black text-white">94%</span>}
            />
          </div>
          <div className="rounded-2xl border border-purple-400/30 bg-black/50 backdrop-blur p-2 text-center">
            <div className="text-[9px] text-white/60 uppercase">Combo</div>
            <div className="text-2xl font-black text-purple-300">x23</div>
            <div className="text-[8px] text-white/40">Best: x45</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className={cn(
                "shrink-0 w-14 rounded-xl border p-1 text-center",
                n === 1 ? "border-cyan-400/60 bg-cyan-500/10" : "border-white/10 bg-white/[0.03]",
              )}
            >
              <div
                className={cn(
                  "w-10 h-10 mx-auto rounded-lg grid place-items-center text-xs font-black",
                  n === 1 ? "text-cyan-300" : "text-white/40",
                )}
              >
                P{n}
              </div>
      ...
      <div className="mx-4 mt-3 mb-6 flex items-center justify-between gap-2">
        ...
        <button className="flex-1 rounded-2xl border border-white/10 bg-white/[0.03] py-3 flex flex-col items-center gap-1 text-white/70">
          <Pause className="w-5 h-5" />
          <span className="text-[9px] uppercase">Pause</span>
        </button>
        <button
          onClick={handleComplete}
          className="w-16 h-16 rounded-full grid place-items-center bg-gradient-to-br from-fuchsia-500 via-purple-600 to-cyan-500 border-2 border-white/20 shadow-[0_0_24px_-2px_rgba(217,70,239,0.7)]"
        >
          <span className="text-white font-black text-xl">T</span>
        </button>
        <button className="flex-1 rounded-2xl border border-white/10 bg-white/[0.03] py-3 flex flex-col items-center gap-1 text-white/70">
          <RotateCcw className="w-5 h-5" />
          <span className="text-[9px] uppercase">Replay</span>
        </button>
        <button className="flex-1 rounded-2xl border border-white/10 bg-white/[0.03] py-3 flex flex-col items-center gap-1 text-white/70">
