...
export const AdminScreen: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"moderation" | "verification" | "cheats">(
    "moderation",
  );
  ...
  const reports: ReportItem[] = [
    {
      id: "rep1",
      type: "routine",
      target: "Euphoria Remix",
      reporter: "@dancer_x",
      reason: "Unsafe movement suggestion on count 8",
      status: "pending",
    },
    {
      id: "rep2",
      type: "copyright",
      target: "Slow Burn Heels Routine",
      reporter: "Warner Music Group",
      reason: "Unlicensed background audio track",
      status: "pending",
    },
    {
      id: "rep3",
      type: "cheat",
      target: "@speedy_stepper (rt002)",
      reporter: "AntiCheat AutoFlag",
      reason: "Pose match score is mathematical outlier (100% precision with 0.1ms latency)",
      status: "pending",
    },
    {
      id: "rep4",
      type: "routine",
      target: "Code Red Solo Run",
      reporter: "@mod_ops",
      reason: "Low lighting makes pose estimation dangerous",
      status: "resolved",
    },
  ];
  ...
  const choreoRequests = [
    {
      ...
      name: "Devon Miles",
      handle: "@devonmoves",
      bio: "Former backup dancer for TRUNO. Specializes in Popping.",
      status: "pending",
    },
    {
      id: "req2",
      name: "Sarah Vance",
      handle: "@sarahvance",
      bio: "Instructor at LA Movement Centre. Afrobeats choreographer.",
      status: "pending",
    },
  ];
      ...
      <TranceTopBar
        title={
          <div className="flex items-center gap-2 text-yellow-400">
            ...
            <span className="font-black text-sm">TRANCE CONTROL</span>
          </div>
        }
        ...
        <div className="space-y-3">
          ...
          {reports.map((r) => (
            <TranceGlassCard
              ...
              className={cn(
                "p-4 border-l-4",
                r.status === "resolved" ? "border-l-emerald-500" : "border-l-fuchsia-500",
              )}
            >
          ...
          {choreoRequests.map((req) => (
            <TranceGlassCard key={req.id} className="p-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex gap-2">
                  <GradientButton variant="gold" className="text-xs px-3 py-1.5">
                    Verify
                  </GradientButton>
                  <button className="rounded-xl border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/70 hover:bg-white/10">
                    Decline
                  </button>
                </div>
              </div>
              ...
              <tbody className="divide-y divide-white/5 text-white/80">
                <tr>
                  ...
                  <td className="py-2 text-right">
                    <button className="text-fuchsia-400 font-bold hover:underline">
                      Restrict Profile
                    </button>
                  </td>
                </tr>
                <tr>
                  ...
                  <td className="py-2 text-right">
                    <button className="text-white/40 font-bold hover:underline mr-2">
                      Dismiss
                    </button>
