...
trey-i/tts.server.ts          — Trey-I TTS server function (REAL — createServerFn; `treyITts` calls Gemini TTS API; API key from GOOGLE_GENAI_API_KEY → GEMINI_API_KEY → GOOGLE_API_KEY (server-only, no VITE_ prefix); returns { audioBase64, mimeType: "audio/wav" } or { audioBase64: null } on any failure — never throws; @google/genai ^1.50.1; tsc ✅ build ✅)
trey-i/elevenlabs-session.server.ts — Trey-I ElevenLabs session server function (REAL — createServerFn; `treyIElevenLabsSession` verifies auth via `verifyTreyIUser` before reading env vars; ELEVENLABS_API_KEY + ELEVENLABS_AGENT_ID server-only, no VITE_ prefix; returns typed { ok: true, signedUrl, provider, expiresInSeconds } or { ok: false } — raw key never returned; endpoint fallback get_signed_url → get-signed-url on 404; @elevenlabs/react not added; mic wiring out of scope; tsc ✅ build ✅)
utils.ts                      — cn() utility
