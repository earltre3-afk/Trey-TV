...
import {
  createRecipe, splitClip, trimClip, setSpeed, setTransition, setFilter,
  toggleEffect, addTextClip, addAudioClip, deleteClip, moveClip,
  clipLength, recipeDuration, findClip, type EditRecipe, type VideoClip,
...
const source = { streamUid: "uid1", srcUrl: "https://x/v.mp4", width: 1920, height: 1080, fps: 30, duration: 10 };
...
assert(near(clipLength(r.tracks[0].clips[0]), 4) && near(clipLength(r.tracks[0].clips[1]), 6), "halves are 4s and 6s");
assert(near((r.tracks[0].clips[1] as VideoClip).trimIn, 4) && near(r.tracks[0].clips[1].start, 4), "second half trimIn/start == 4");
...
assert(c.transitionIn === "fade" && c.filter === "boost" && c.effects.includes("zoomIn"), "transition/filter/effect set");
r = toggleEffect(r, firstId, "zoomIn");
assert(!findClip(r, firstId)!.clip.effects.includes("zoomIn"), "toggleEffect removes on second call");
...
assert(r.tracks.some((t) => t.kind === "text") && r.tracks.some((t) => t.kind === "audio"), "text + audio tracks created");

console.log("immutability");
const snapshot = JSON.stringify(r);
deleteClip(r, firstId); moveClip(r, firstId, 5);
